...
if wait_for_pattern "$SERVER_LOG" "\\[bong\\]\\[world\\] creating overworld test area" 180; then
  pass "server world bootstrap"
else
  finalize_failure "server" "missing world bootstrap anchor in $SERVER_LOG"
fi

if wait_for_pattern "$SERVER_LOG" "\\[bong\\]\\[redis\\] subscribed to bong:agent_command and bong:agent_narrate" 180; then
...
(
  cd "$ROOT/agent/packages/tiandao"
  PATH="$NODE_BIN:$PATH" REDIS_URL="$REDIS_URL" npx tsx <<'TS'
import { runRuntime, DEFAULT_MODEL } from "./src/runtime.ts";
...
const deterministicChat = async (_model: string, messages: Array<{ role?: string; content?: string }>) => {
