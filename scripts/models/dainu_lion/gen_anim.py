...
输出:
  local_models/dainu_lion/DainuLionRig.bbmodel   带动画的模型（可直接拖进 Blockbench）
  local_models/dainu_lion/dainu_lion.animation.json  GeckoLib 动画（参考/兜底用）
...
def _kf(channel: str, time: float, vec, idx: int, aname: str) -> dict:
    """关键帧。字段照 local_models/JianPlayerAnim.bbmodel（用户手上能正常打开的
...
def write_geckolib(rig: Rig, names: list[str], out: Path) -> None:
    ...
    有可对拍的实例：仓库里现有的 geo.json 骨骼旋转全是 0，local_models 里也没有
    bbmodel ↔ animation.json 的同源对。所以正经路径是把 DainuLionRig.bbmodel 交给
    scripts/models/bbmodel_to_geckolib.py（驱动 Blockbench 官方 codec 导出），由
