...
python3 scripts/models/kekeda_goose/gen_anim.py           # 生成九条动画
python3 scripts/models/kekeda_goose/check_anim.py         # 动画后验（九项，见下）
python3 scripts/models/kekeda_goose/render_anim.py --gif  # 连拍图 + GIF
```
...
`scripts/models/anim_rig.py` 是与物种无关的底座（骨树 / 正解 / 逆解 / 关键帧导出），
...
`scripts/models/bbmodel_to_geckolib.py`（Blockbench 官方 codec），别直接用
