...
走过两条弯路，都记在这里：
  ...
  这一版按 MC 原版生物的做法：整只三十来块。躯干五块（主体 + 上下前后四块倒角）、
  头一块、颈一块、喙三块、翼各三片平板、尾两片、腿脚各五块。台阶少而大，是**设计
  出来的面**，不是逼近曲面时漏出来的锯齿 —— 后者才是"块状堆叠"难看的原因。
...
用法:
  ...
  python3 scripts/models/kekeda_goose/gen_plume.py --with-anatomy   # 半剖，看外皮多厚
...
def part_neck(rig: Rig) -> None:
    """颈：一块。真颈 17 节折成 S 整条埋在身体里，露出来就这一小截。"""
    # 颈要比头**窄一圈**、且身体顶面到头底之间得留出 1.4 单位以上，
    # 否则头直接坐在身体上，侧看是一整团白，看不出这是脖子
    rig.cube("neck_12", "neck", (-1.62, 10.20, -5.05), (1.62, 13.15, -2.55), mat="down")
...
def part_bill(rig: Rig) -> None:
    ...
    rig.cube("jaw", "bill_lower", (-1.05, 13.12, -8.20), (1.05, 13.55, -5.90), mat="bill_dark")
