...
用法:
  python3 scripts/models/kekeda_goose/gen_skeleton.py               # 全骨架
  python3 scripts/models/kekeda_goose/gen_skeleton.py --part skull  # 单件预览
  python3 scripts/models/kekeda_goose/gen_skeleton.py --list
  python3 scripts/models/kekeda_goose/gen_skeleton.py --check
"""
...
REPO = HERE.parents[2]
OUT_DIR = REPO / "local_models" / "kekeda_goose"
...
SKULL_C: Vec = (0.0, 14.30, -5.15)   # 脑颅中心
BILL_ROOT_Z = -6.15                  # 蜡膜/喙根
BILL_TIP_Z = -9.30                   # 喙尖。首版取 -8.4（喙长 2.25 ≈ 0.9 倍头长，
...
def part_bill(rig: Rig) -> None:
    ...
    for i, ((z0, w0, yl0, yh0), (z1, w1, yl1, yh1)) in enumerate(zip(seg, seg[1:])):
        ...
        rig.cube("bill_upper", f"maxilla_{i}", (-w, cy + yl, z1), (w, cy + yh, z0), mat="keratin")
