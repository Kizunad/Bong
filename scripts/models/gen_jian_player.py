...
（UV 整体 +64）。武器几何与 UV 直接搬自 local_models/BambooJianSingle.bbmodel，
不重新推导——那份才是单一真相源，改了锏只要重跑本脚本。

用法:
    python3 scripts/models/gen_jian_player.py                  # 垂持站立
    python3 scripts/models/gen_jian_player.py --pose cross     # 交叉护胸
    python3 scripts/models/gen_jian_player.py --pose ready     # 战斗预备
    python3 scripts/models/gen_jian_player.py --no-render      # 只出 bbmodel
"""
    ...
    def walk(nodes, chain):
        for n in nodes:
            if isinstance(n, str):
                ...
                src = n if "rotation" in n or "origin" in n else groups.get(n.get("uuid"), n)
...
def main():
    ...
    ap.add_argument("--out", default=None, help="另存路径（默认 local_models/JianPlayer.bbmodel）")
