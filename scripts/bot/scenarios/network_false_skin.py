...
def _wait_move_rejected(bot, anchor_t: float, timeout: float = 10.0) -> dict:
    event = bot.wait_for(
        lambda e: (
            e.kind == "server_data"
...
def run(env) -> None:
    with env.new_bot("FalseSkin") as bot:
        ...
        # 真元走 zone ledger 扣除：player_state.spirit_qi 应低于 forge 前的 5.0。
        bot.wait_for(
            lambda e: (
                e.kind == "server_data"
            ...
            timeout=10.0,
            description="forge 扣真元后 spirit_qi < 5.0",
        )
        ...
        rejected = _wait_move_rejected(bot, reject_anchor)
