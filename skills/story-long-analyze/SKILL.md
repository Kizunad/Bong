...
description: 长篇网文拆文分析，支持快速模式（黄金三章+整体结构）与深度模式（逐章系统拆解并产出结构化结果），用于学习爆款的人设、爽点与节奏设计。触发方式：/story-long-analyze、/长篇拆文、「帮我拆这本书」「分析黄金三章」；深度模式触发：「深度拆解」「完整拆解」「系统拆解」或提供小说文本文件路径。注意：本 skill 适用于通用网文创作，非末法残土世界观叙事（末法残土请用 /write-tale）。
argument-hint: [书名] [平台]
