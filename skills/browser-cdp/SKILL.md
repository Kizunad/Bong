---
name: browser-cdp
description: 浏览器 CDP 操作工具，通过 Chrome DevTools Protocol 复用登录态并执行自动化操作（打开页面、等待加载、执行 JS、快照、提取 token）。触发方式：/browser-cdp、browser automation、CDP、agent-browser、浏览器操作、操作浏览器、Chrome CDP、复用登录态、extract token from browser。
argument-hint: <url或操作>
