...
窗口或 GUI 缩放变化时原地重新排版，保留键盘焦点和可用范围内的滚动位置。
...
真实客户端验证了长中文说明的末尾可达、滚轮、键盘、滚动条拖动、短说明、返回和重试。
通过注入原版 framebuffer 尺寸回调和调整 GUI scale，覆盖 320×240、480×270、640×360、960×540、1280×720 逻辑视口，
验证文本重排、按钮点击区、焦点保留与滚动位置保留/限幅；截图位于 `build/menu-resize-review/`。
主菜单预览额外调用原版 `Screen.resize`，回归检查缩小和放大后的居中、窗口边界与正文/按钮分离。
验证使用 OpenAL null 驱动避开 WSL 音频设备重连；未覆盖 WSL 系统窗口的实际拖动缩放。
