 ...
 *   <li>soul_bond 金色徽章(每级一个金点,最多 3)</li>
 * </ul>
 *
...
public final class WeaponHotbarHudPlanner {
    ...
    static final int SOUL_BOND_COLOR = 0xFFFFCC40;           // 金:绑定徽章
    ...
    static final int SOUL_BOND_DOT_SIZE = 2;
    static final int SOUL_BOND_DOT_GAP = 1;
    ...
    private static void drawWeaponSlot(
        ...
        if (w.hasSoulBond() && w.soulBondLevel() > 0) {
            int dotsY = durY - SOUL_BOND_DOT_SIZE - 2;
            int dotsX = x + 2;
            int stride = SOUL_BOND_DOT_SIZE + SOUL_BOND_DOT_GAP;
            int level = Math.min(w.soulBondLevel(), 3);
            for (int i = 0; i < level; i++) {
                out.add(HudRenderCommand.rect(
                    HudRenderLayer.QUICK_BAR,
                    dotsX + i * stride, dotsY,
                    SOUL_BOND_DOT_SIZE, SOUL_BOND_DOT_SIZE,
                    SOUL_BOND_COLOR
                ));
            }
        }
