...
public enum BongEntityModelKind {
    ...
    // PR 接入 opening 裂纹叠层贴图后再扩 state count。raw_id 146-148 紧跟 fauna
    // (126..=145, 含 HEIWUSHI=145) 之后，与 server 端 entity_model.rs
    // COFFIN_*_ENTITY_KIND 1:1 对应（plan-supply-coffin-v1 P1.1）。
    COFFIN_COMMON(
        "coffin_common",
    ...
    // plan-entity-model-v1 / tsy-container：raw_id 149..=159，紧随 COFFIN_*。
    SPIRIT_NICHE(
        "spirit_niche",
        "SpiritNiche.bbmodel",
        149,
