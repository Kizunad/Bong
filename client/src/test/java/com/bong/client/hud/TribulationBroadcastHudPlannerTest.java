...
class TribulationBroadcastHudPlannerTest {
    ...
    @Test void jueBiBroadcastMatchesStateByPublicCoordinateBucket() {
        TribulationBroadcastStore.replace(new TribulationBroadcastStore.State(
            true, "绝壁劫", "jue_bi", 400, -400, 10_000L, false, 150
        ));
        TribulationStateStore.upsert(new TribulationStateStore.State(
            true, "offline:JueBiSource", "HiddenVoidActor", "jue_bi", "wave", 301, -301,
            2, 4, 100, 200, 300, false, false, List.of("offline:JueBiSource"), ""
        ));

        List<HudRenderCommand> cmds = TribulationBroadcastHudPlanner.buildCommands(800, 600, 1_000L);

        assertTrue(cmds.stream().anyMatch(c -> c.isText()
            && c.text().contains("绝壁劫")
            && c.text().contains("坐标 (400, -400)")
            && c.text().contains("劫波 2/4")));
        assertFalse(cmds.stream().anyMatch(c -> c.isText() && c.text().contains("301")));
    }

    @Test void jueBiPublicCoordinateBucketMatchesRustNegativeHalfRound() {
        TribulationBroadcastStore.replace(new TribulationBroadcastStore.State(
            true, "绝壁劫", "jue_bi", -400, 400, 10_000L, false, 150
        ));
        TribulationStateStore.upsert(new TribulationStateStore.State(
            true, "offline:JueBiSource", "HiddenVoidActor", "jue_bi", "wave", -300, 300,
            3, 4, 100, 200, 300, false, false, List.of("offline:JueBiSource"), ""
        ));
        ...
        assertTrue(cmds.stream().anyMatch(c -> c.isText()
            && c.text().contains("坐标 (-400, 400)")
            && c.text().contains("劫波 3/4")));
        assertFalse(cmds.stream().anyMatch(c -> c.isText() && c.text().contains("-300")));
    }
