...
class DeathCinematicTest {
    ...
    void localClockAdvancesPhaseSequenceAfterPayloadReceipt() {
        ...
        DeathCinematicState advanced = state.advancedTo(6_000L);
    ...
    @Test
    void repeatedNonFinalDeathUsesShortRoll() {
        DeathCinematicState state = baseState(
            DeathCinematicState.Phase.ROLL,
            0L,
            40L,
    ...
    void rendererDispatchesByAdvancedPhase() {
        ...
        assertTrue(commands.stream().anyMatch(command -> command.text().equals("运数")));
    }
    ...
    void insightAndRebirthRenderExpectedNarration() {
        ...
        assertEquals(1, InsightOverlayRenderer.visibleLineCount(
            baseState(DeathCinematicState.Phase.INSIGHT_OVERLAY, 0L, 120L, 200L, 300L, false, 1, 1_000L)
        ));
        assertEquals(2, InsightOverlayRenderer.visibleLineCount(
            baseState(DeathCinematicState.Phase.INSIGHT_OVERLAY, 79L, 120L, 200L, 300L, false, 1, 1_000L)
        ));
        assertEquals(3, InsightOverlayRenderer.visibleLineCount(
            baseState(DeathCinematicState.Phase.INSIGHT_OVERLAY, 120L, 120L, 200L, 300L, false, 1, 1_000L)
        ));
        ...
        assertTrue(RebirthCinematicRenderer.buildCommands(
                baseState(DeathCinematicState.Phase.REBIRTH, 20L, 60L, 340L, 300L, false, 1, 1_000L),
                320,
                180
