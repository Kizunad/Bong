...
时序（精度标准 #1/#2/#3）：
  anticipation 0→5   手刀引到左肩、躯干左拧蓄势（easeOut 族 OUTSINE）
  strike       5→11  横斩过身（tick 8 = cast 完成 = 斩断点）→ 顺势 overshoot
                     （t10）→ 定劲（t11，strike 段内）
...
POSE = {
    ...
    7: dict(
        ...
        rightArm=dict(pitch=-78, yaw=0, roll=-4, bend=40, axis=180),
    ...
    # 斩断点 = cast 完成（tick 8）：手刀全伸横切到右侧（bend 4）、torso.yaw
    # -24（52° 总扭矩）、body 前冲右移，左臂反向收紧（counter-pull）。
    8: dict(
        easing="INQUAD",
        body=dict(x=-0.06, y=+0.02, z=+0.16),
        head=dict(yaw=+10),
        torso=dict(pitch=+5, yaw=-24),
        rightArm=dict(pitch=-74, yaw=-34, roll=+10, bend=4, axis=180),
    ...
    10: dict(
        ...
        rightArm=dict(pitch=-72, yaw=-40, roll=+12, bend=2, axis=180),
    ...
    11: dict(
        ...
        rightArm=dict(pitch=-70, yaw=-36, roll=+10, bend=12, axis=180),
    ...
    12: dict(
        ...
        rightArm=dict(pitch=-58, yaw=-20, roll=+9, bend=35, axis=180),
...
def main() -> int:
    emit_json(
        ...
        description=(
            "P1 重制手刀斩脉：anticipation 0→5 手刀引左肩（yaw +32 / torso.yaw "
            "+28），strike 5→8 横斩过身全伸（yaw -34 / bend 4 / torso.yaw -24）"
            "+8→11 顺势 overshoot 定劲，recovery 11→14 回 guard。"
        ),
        end_tick=14,
        stop_tick=16,
        is_loop=False,
