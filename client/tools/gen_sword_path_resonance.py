...
时序（精度标准 #1/#2/#3）：
  anticipation 0→14  举剑入中线 + 初级微颤（roll ±5° 往复，easeOut 族 OUTSINE）
  strike       14→30 颤鸣渐强（roll ±8°~±11° 往复）→ t28 极限压缩 → t30 振荡外放
...
POSE = {
    ...
    # 微颤 +1（round 2 全档加幅：roll 摆幅 ±3→±5，游戏内更可读）。
    11: dict(
        easing="OUTSINE",
        body=dict(y=-0.025, z=-0.035),
        head=dict(pitch=-7),
        torso=dict(pitch=0, yaw=+2),
        rightArm=dict(pitch=-84, yaw=+7, roll=+35, bend=86, axis=180),
        leftArm=dict(pitch=-81, yaw=-7, roll=-25, bend=89, axis=180),
        leftLeg=dict(pitch=-8, bend=14, z=-0.05),
        rightLeg=dict(pitch=+8, bend=14, z=+0.05),
    ),
    # 微颤 -1。
    14: dict(
        ...
        head=dict(pitch=-9),
        torso=dict(pitch=0, yaw=-2),
        rightArm=dict(pitch=-86, yaw=+1, roll=+25, bend=87, axis=180),
        leftArm=dict(pitch=-79, yaw=-1, roll=-35, bend=91, axis=180),
    ...
    17: dict(
        ...
        head=dict(pitch=-8),
        torso=dict(pitch=+1, yaw=+3.5),
        rightArm=dict(pitch=-83, yaw=+9, roll=+38, bend=88, axis=180),
        leftArm=dict(pitch=-82, yaw=-9, roll=-22, bend=92, axis=180),
    ...
    20: dict(
        ...
        head=dict(pitch=-10),
        torso=dict(pitch=+1, yaw=-3.5),
        rightArm=dict(pitch=-87, yaw=-1, roll=+22, bend=90, axis=180),
        leftArm=dict(pitch=-78, yaw=+1, roll=-38, bend=94, axis=180),
    ...
    23: dict(
        ...
        head=dict(pitch=-9),
        torso=dict(pitch=+2, yaw=+4.5),
        rightArm=dict(pitch=-82, yaw=+10, roll=+41, bend=90, axis=180),
        leftArm=dict(pitch=-83, yaw=-10, roll=-19, bend=95, axis=180),
    ...
    26: dict(
        ...
        head=dict(pitch=-10),
        torso=dict(pitch=+2, yaw=-4.5),
        rightArm=dict(pitch=-88, yaw=-2, roll=+19, bend=92, axis=180),
        leftArm=dict(pitch=-77, yaw=+2, roll=-41, bend=97, axis=180),
...
def main() -> int:
    emit_json(
        ...
        description=(
            "P2 共鸣专属：anticipation 0→14 举剑入中线+初级微颤（roll ±5° 往复），"
            "strike 14→30 颤鸣渐强（roll ±8°~±11° 交替帧）→ t28 极限压缩"
            "（bend 96/100）→ t30 振荡外放（双臂推剑 bend→22 / torso +10 / "
            "body.z +0.16），recovery 30→36 经 t33 中段帧回 guard。"
        ),
        end_tick=36,
        stop_tick=38,
        is_loop=False,
