...
玩家每次普攻都看到同一记过顶抡砸，读感很快就磨平了。横抡补的正是另一档：10 tick、贴着
胸腹高扫过身前，快、平。
...
## 中段那一帧（tick 5）是为副手加的，不是为姿态

只卡 LOAD 和 IMPACT 两帧时，两条手臂在关节空间各走各的路：棍由右臂带着，左臂走另一条
弧，关键帧上严丝合缝，**中间照样甩脱**——8 tick 草稿实测两帧正中间副手离棍身 4.31px。
整 tick 取样只报 2.05px，根本看不见，所以 `track --grip` 和测试都改成 1/4 tick 取样。
钉住中段后全程 ≤1.38px。
...
## 分段（docs/player-animation-conventions.md §1）
     ...
     0  guard    棍横在身体左侧、双手握持；右肩前倾（用户手摆的那一帧）
     3  腿先动    后腿蹬地 bend 15→40（kinetic chain 起点）
     4  LOAD     腰扭到 −40°，棍沉进抡击平面（棍头肩左 17.8px、肩下 2.4px、放平）
     5  中段      棍过身体中线（棍头肩左 9.6px）—— 见上节，这一帧是为副手加的
     6  IMPACT   腰猛转到 +30°（总转矩 70°），棍扫到肩右 6.4px、身前 11.8px
     7  overshoot 棍再扫右 + 腕再拧（末端关节滞后 1 tick）
     9  收势      棍**贴着低位**抽回左侧，过了头的轮廓才抬起来
    10  == tick 0

峰值错开：腿 → 腰 → 肩 → 肘/腕。impact 落在 6/10 = 60%（标配 60%）。

## 这些 tick 是"拉长 1.2×"解出来的：整数网格上只能给到 1.25×

初稿是 8 tick 骨架 `(0, 2, 3, 4, 5, 6, 7, 8)`。tick 是**整数**——PlayerAnimator 读 JSON
时 `getAsInt()`（AnimationJson.java:123），存储层也是 `findAtTick(int)`。8 × 1.2 = 9.6
落不到网格上，写小数会被截断、和相邻帧撞成一帧，静默丢关键帧。最近的整数是 10，也就是
**1.25×**（比要的多 4%，20ms，肉眼无从分辨）。

多出来的 2 tick 落在哪儿是**解出来的**，不是挑的：`anim_common.integer_retime` 对累计
位置取整，保证任何一帧的时间误差 ≤ 0.5 tick。唯一的人工约束是 `KEEP_GAP`——overshoot
必须贴着 impact 后一 tick（conventions §2.6），被拉成 2 tick 就不再是弹性过冲，而是
"到位之后又慢慢挪了一下"。解出来是 guard→腿 3 tick（原 2）、overshoot→收势 2 tick（原 1）。

**落位解完就写进下面的 POSE 键了，不在出料时现搬。** 全仓每个动画生成器都满足
「POSE 的键 == 出料 JSON 的 tick」（`PoseTickContractTest` 逐个扫过去钉住），
`bbmodel_to_pose`（把 Blockbench 里手改的姿态读回成 POSE 表）正是靠这条等式才能把读到
的帧号原样当作 POSE 键用；在出料一步搬帧会单方面废掉这条回程。设计骨架和倍率留成
`DESIGN_TICKS` / `TIME_SCALE` / `KEEP_GAP` 三个常量，`ClubSweepRetimingTest` 拿它们
反算一遍核验落位——所以"这条动画的设计节奏是什么"依然说得清，下次改倍率不用凭猜。

**拉长是搬帧，不是重采样。** 姿态一个数都没改，所以贴棍距离、挡不挡脸、棍头包围盒这些
几何判据在 8 → 10 tick 之后逐字成立，变的只有速度。重采样（在新网格上按原曲线取值）
会把 LOAD / IMPACT 削掉：1.25 倍下设计的极值帧落在两个新整数 tick 之间，插值过去峰值
就没了，而峰值恰恰是这条动画最要紧的东西。
...
每帧的 easing 管「本帧 → 下一帧」。t0/t3 蓄势用 OUT 族，**t4 发力用 IN 族**（从静止
加速），t6 余势 OUTQUAD 卸力。把 OUTQUAD 写在撞击帧 t6 上是最容易犯的错——那管的是
撞击**之后**。

加了中段帧之后多出一条：**IN 族结束时是快的，开始时是慢的**，所以 t4 和 t5 都用 IN
族会在 t5 这个接缝上把速度掐回近乎 0（8 tick 草稿实测接缝处只剩 0.5px/tick，棍在抡击
中途卡住一下）。t5 因此用 LINEAR 接住 t4 末端的速度。

收势两帧（t7/t9）也都用 LINEAR，理由不同：收势要走的 3D 距离和抡击本来就接近（回程
21px / 抡击 24px），任何带峰的 easing 都会让**回程峰速反超打击峰速**，读成"抽回来比打
出去还快"。8 tick 草稿实测 INOUTSINE 回程峰 24.4 vs 打击峰 17.2，LINEAR 之后是回程
16.2 / 打击 17.2；拉长到 10 tick 后回程峰 16.4，仍然低于打击峰。
...
# 键就是出料 tick，和 `player_animation/club_sweep.json` 里的帧号逐一对应（见 §这些
# tick 是"拉长 1.2×"解出来的）。
#
...
POSE = {
    ...
    3: dict(  # 腿先动 —— 后腿蹬地，链条从下往上启动；棍同时**沉进抡击平面**
        easing="OUTQUAD",
    ...
    4: dict(  # LOAD —— 腰到极限，棍横在身体左侧、**齐胸放平**
        # （棍头肩左 17.8px、肩下 2.4px、身前 7.2px、仰角 +0.7°）
        easing="INSINE",           # 缓加速起步；猛加速留给 t5→撞击那一 tick
    ...
    5: dict(  # 抡击中段 —— 棍正过身体中线，**这一帧是为副手加的**
        # 两条手臂各自在关节空间插值：棍由右臂带着走，左臂走的是另一条路。t4/t6 上都
        # 严丝合缝，中间照样甩脱——8 tick 草稿实测两帧正中间副手离棍身 4.31px（整 tick
        # 取样只报 2.05px，完全看不见）。钉住中段之后全程 ≤0.6px。
        easing="LINEAR",           # 见 §easing：IN 接 IN 会在 t5 上撞出一个静止点
    ...
    6: dict(  # IMPACT —— 腰猛转 70°，棍横扫过身前（棍头肩右 6.4px、身前 11.8px）
        easing="OUTQUAD",
    ...
    7: dict(  # overshoot —— 末端关节滞后 1 tick：棍再扫右、腕再拧
        easing="LINEAR",
    ...
    9: dict(  # 收势 —— 棍**贴着低位**抽回左侧，过了头的轮廓才抬起来
        # 没有这一帧，overshoot 直接插值回 guard 会让棍从脸正前方 15px 处横过去
        # （8 tick 草稿实测 `track --dump` 报 -1.4px「挡脸」）。棍高要到末帧才回到
        # guard 的肩上高度。
        easing="LINEAR",
    ...
    10: dict(  # 回 guard（与 tick 0 完全一致，连击友好）
        easing="INOUTSINE",
        body=dict(x=-0.03, y=0.0, z=0.0, yaw=+24),
        head=dict(pitch=+1, yaw=-26),
...
# 上面那些 tick 是怎么来的（见 §这些 tick 是"拉长 1.2×"解出来的）。这三个常量是那次
# 求解的输入，`ClubSweepRetimingTest` 拿它们跑一遍 `integer_retime` 反算落位，对不上
# 就撞红——POSE 的键改了却没重新解，或者倍率改了却没重跑，都逃不掉。
DESIGN_TICKS = (0, 2, 3, 4, 5, 6, 7, 8)
TIME_SCALE = 1.25
KEEP_GAP = frozenset({6})   # 设计 tick 6 = overshoot，必须贴着 impact 后一 tick
END_TICK = max(POSE)
