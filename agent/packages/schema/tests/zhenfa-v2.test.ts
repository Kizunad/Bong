...
describe("zhenfa-v2 schema", () => {
  ...
  it("accepts new array kinds in zhenfa_place requests", () => {
    ...
    for (const [kind, itemInstanceId, targetFace] of [
      ...
      const invalidFaceResult = validate(ClientRequestV1, {
        v: 1,
        type: "zhenfa_place",
        x: 4,
        y: 64,
      ...
      expect(
        invalidFaceResult.ok,
        `${kind} with an invalid target_face must be rejected by the shared schema`,
      ).toBe(false);

      const nullFaceResult = validate(ClientRequestV1, {
        v: 1,
        type: "zhenfa_place",
        x: 4,
        y: 64,
      ...
      expect(
        nullFaceResult.ok,
        `${kind} with null target_face must be rejected by the shared schema`,
      ).toBe(false);
      ...
      expect(
        extraFieldResult.ok,
        `${kind} with unexpected extra_prop must be rejected by the shared schema`,
      ).toBe(false);
    }
