...
- [x] ✅ migration 框架（在 `apply_migrations` 内顺序执行 12 个版本块；尚未拆 `migrations/vN_to_vN+1.rs` 单独文件）
- [x] ✅ 启动时顺序执行所有未应用的 migration
- [x] ✅ SQLite `user_version` PRAGMA 记录当前 schema version（CURRENT_USER_VERSION = 12）
...
- [x] ✅ migration 框架（`user_version` + 按序执行，目前 user_version = 12）
...
**Phase 5 — Agent WorldModel** ✅（reconcile 待补）
