...
- [ ] **每个 bug 独立 PR**（不打包）：11 个 fix 互不依赖，分别走 fastlane。打包 = reviewer 上下文切换 + regress 风险翻倍 + 回归测试 churn
