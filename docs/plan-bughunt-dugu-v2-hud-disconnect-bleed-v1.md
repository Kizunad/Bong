# plan-bughunt-dugu-v2-hud-disconnect-bleed-v1（骨架）

> **骨架（草案）**。一句话主题：`dugu_v2_*` / `permanent_qi_max_decay_applied` 这条 server bridge 已经打通，但 client runtime wiring 漏了 disconnect reset，导致上一局的毒蛊 v2 HUD 状态会跨 session 残留到下一局；其中 `revealRisk` / `selfCurePercent` / `selfRevealed` 甚至可无限续命，直到再次收到新的毒蛊 v2 payload。
...
| 阶段 | 主题 | 路由 | 状态 |
|------|------|------|------|
| P0 | 毒蛊 v2 HUD 断线跨 session 残留 | fix_pr | ⬜ |
