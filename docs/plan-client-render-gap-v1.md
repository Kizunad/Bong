...
- **错槽、穿戴、破损、卸下**：`ArmorFeatureRendererTest.collectRenderableCoversFourMaterialsFourSlotsAndWearRemoveBrokenStates` 覆盖 hide 四槽的穿戴/空槽/耐久归零；`collectRenderableRejectsHideArmorInEveryWrongSlot` 覆盖 hide 四件放入其余三个错误槽均为空结果；`ArmorModelRegistryTest.registeredMaterialsStillHaveSlotMatchedEmergencyFallbackData` 保留 fallback 槽位对拍。
