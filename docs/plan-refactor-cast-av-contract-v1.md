...
- P1 同 PR 新增并执行 `scripts/bot/scenarios/cast_wire_identity.py` 作为生产接线主验收门：走真实施法入口与 live `bong:server_data` envelope，先观察 `CastSessionBegin`，再断言同 identity 的 cast payload、`CastPlayAnim` 与 `CastStopAnim` 可达；覆盖未 BEGIN/旧 session 拒绝、A→B 后迟到 A STOP 不停 B、B STOP 停层及重复 STOP 幂等，并覆盖 terminal-before-PLAY：先投递 STOP/终态，再投递同 identity PLAY，必须保持 tombstone、不得重新出现动画或 juice。该场景必须与 producer/wire/PLAY+STOP registration 同 PR 常绿，不得推迟到 P4；P4 只原样复跑并扩展其余矩阵。
