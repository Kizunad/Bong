...
5. `CastPhaseV1` 已存在，不增加重复 phase 字段。STOP 是同一 `CastIdentity` 的权威终态副作用：移动、污染、控制、用户取消、死亡、逃劫与换维度均在 owner 仍连接时发 `INTERRUPT + outcome`；断线在 server 内先结束 cast 并向旁观者广播 STOP，owner 侧由 R2 disconnect teardown 清 store（不伪称能给已断开的连接回包）。client 收到终态后停止 binding 的 looping start animation；VFX schema 新增施法专用 `CastPlayAnim { identity, target_player, anim_id, priority, fade_in_ticks }` / `CastStopAnim { identity, target_player, anim_id, fade_out_ticks }` 判别式，client `AnimationLayerManager` 的 cast ownership 记录 `(player_uuid, channel) → { anim_id, identity }`。`CastStopAnim` 只有复合 identity 与当前 owner 完全相等时才停层；旧 A 的延迟 STOP 遇到同招新 B 必须 no-op。STOP 或任一 COMPLETE/INTERRUPT 终态先于 PLAY 到达时必须写入同 identity 的有界 terminal tombstone；后续同 identity `CastPlayAnim` 必须先查 tombstone 并 no-op，不能重新武装 animation 或 juice。既有通用 `PlayAnim`/`PlayAnimInline`/`StopAnim` 保持非 cast API，不与 cast ownership 互停。VFX cast STOP 仍只是终态派生的 transport 副作用，不能成为独立状态真相源。
...
7. `target` 不承载 FPV 手臂姿态；R9 只迁移 `plan-fpv-cast-av-v1` 当前临时 `(slot, startedAtMs)` identity 到完整 `CastIdentity`，保留其 accepted-only juice 与 teardown 语义，FPV 动画资产仍归独立 plan。R9 冻结 `VfxEventRouter` 所需的 `CastPlayAnim`/`CastStopAnim` payload 与 `CastFovController.onAnimPlayed(identity, target_player, anim_id)` 接缝，并通过 R6 P2 的通用 registration API 成对注册 PLAY/STOP 分发：PLAY 在 animation bridge 成功后武装 token，STOP 仅按匹配 identity 停层并消费 token。controller 的 pending/anim token/terminal map 全部以复合 identity 为 key，`CastStopAnim`/终态消费同一 token。
...
- 前置：R5 P1、R6 P2、R2 P1 已 merge；R6 P2 必须先落地可扩展的 VFX channel/router registration API，并以 contract tests 固定注册/分发语义，且不依赖 R9 cast payload 或 consumer 类型。随后 R9 P1 通过该 API 成对注册 PLAY/STOP cast consumer，不修改 `VfxEventRouter` 独占文件或区段；若任一前置未满足，只更新本 plan 的依赖状态，不提前复制其职责。
...
7. `cast_juice_identity_bridge`：R6 的通用 router registration API 与 R9 的 consumer API/cast-specific PLAY+STOP 注册都合入后，在 P4 e2e 重跑完整生产链；真实 `CastPlayAnim` 经 R6 所有的 `VfxEventRouter` 调用 R9 注册的 PLAY 分发并成功播放后，把同一复合 identity 交给 R9 所有的 `CastFovController`；真实 `CastStopAnim` 经成对注册的 STOP 分发仅在 identity 匹配时停止层并消费 token，覆盖 COMPLETE/INTERRUPT、迟到与重复 STOP。断线旧 tombstone 后重连首个 `(new_session, 1)` 必须正常武装，旧 session 迟到 PLAY/CASTING/STOP 必须 no-op。
