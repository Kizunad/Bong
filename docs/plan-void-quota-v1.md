# Bong · plan-void-quota-v1 · Active
...
- [ ] **不再按人数定名额**：删除 / 替换 `ascension_quota_limit(player_count)` 的行为入口；人数只可作为 telemetry，不再参与 quota 上限。
- [ ] **世界灵气预算是公式输入**：v1 以 `WorldQiBudget.current_total` 为权威预算；`summarize_world_qi(...).total_observed()` 先用于 debug / drift 日志，避免把 `Zone.spirit_qi` 浓度和玩家 `qi_current` 生硬混单位。
- [ ] **K 值为 server config**：新增 `BONG_VOID_QUOTA_K`（默认按当前 `DEFAULT_SPIRIT_QI_TOTAL = 100.0` 校准为 50.0，使开服初期 quota≈2）；后续 telemetry 再调。
- [ ] **允许 quota=0**：`total_world_qi < K` 时无人能新化虚；既有化虚者不强制掉境，但新起劫必须走绝壁劫。
- [ ] **化虚者死亡名额回流**：复用现有 `release_ascension_quota_slot` hook；不凭空 `+K` 造灵气，数值回流由 qi_physics 释放路径承接。
- [ ] **超额惩罚直接**：超额玩家试图渡虚劫 → "绝壁劫" → `DuXuOutcomeV1::Killed` 或等价死亡结算，**不再半步化虚**。
- [ ] **兼容优先**：保留 `ascension_quota` payload 名称与 client handler；只扩字段 / 增 reason，避免平行 `void_quota` IPC 双轨。
...
| 阶段 | 内容 | 验收 |
|---|---|---|
| **P0** ⬜ | quota 公式切换：`quota = floor(WorldQiBudget.current_total / K)` + `check_void_quota` 替换 player-count 入口 | 起劫和 snapshot 广播都不再调用 `ascension_quota_limit(player_count)`；P0 单测覆盖 quota=0/1/2/边界 |
| **P1** ⬜ | 超额起劫改为"绝壁劫"死亡结算 + schema/agent reason | 超额玩家试图起劫时被打回并真死；agent 输出"天地装不下你了。"语义 |
| **P2** ◐ | 化虚者死亡 / 降境 / 死透 → 名额立刻再开 | 现有 release hook 已在重生、死透、降境路径接入；需补与新 `check_void_quota` 的回归测试 |
| **P3** ◐ | client inspect UI 显示当前世界化虚名额: X / Y + quota 来源 | handler/store 已有；inspect UI 与扩展字段展示未落 |
...
- [ ] `server/src/cultivation/tribulation.rs` 或拆 `server/src/cultivation/tribulation/void_quota.rs`：`VoidQuotaConfig` / `compute_void_quota_limit` / `check_void_quota`
- [ ] `server/src/network/ascension_quota_emit.rs`：snapshot 广播改读同一个 quota 计算函数，不再按 joined client count 算上限
- [ ] `server/src/schema/server_data.rs` + `agent/packages/schema/src/server-data.ts` + generated schema：扩展 `ascension_quota` payload（建议字段：`total_world_qi` / `quota_k` / `quota_basis`）
- [ ] `server/src/schema/tribulation.rs` + `agent/packages/schema/src/tribulation.ts`：为绝壁劫添加 reason / cause（建议 `void_quota_exceeded`），避免 agent 从普通 `killed` 猜语义
- [ ] `server/src/cultivation/death_hooks.rs` / `server/src/cultivation/mod.rs`：保留现有 release hook，并补新公式下的回归测试
- [ ] `agent/packages/tiandao/src/tribulation-runtime.ts`："天地装不下你了。" narration 分支 + fallback 测试
- [ ] `client/src/main/java/com/bong/client/combat/store/AscensionQuotaStore.java`：保存扩展字段
- [ ] `client/src/main/java/com/bong/client/combat/inspect/StatusPanelExtension.java` 或同级 inspect panel：通灵期显示"当前世界化虚名额：X / Y"
...
- [ ] **同 tick 多人同时起劫如何抢名额？** 默认 FCFS（系统读事件顺序），但 P0 需要单测锁定。
- [ ] **绝壁劫死亡是否需要新增 death cause？** 默认新增 `CultivationDeathCause::VoidQuotaExceeded`，若现有 death arbiter 不适合再退回普通 `Killed` + `reason`。
- [ ] **client 展示门槛**：仅通灵期可见，还是所有境界都能看到"天道座次"？默认通灵期及以上可见。
