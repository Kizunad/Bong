...
- **出料**：修正后的 `qi_collision` 公式 + 新增 `rejection_rate()` trait 方法 + 30 对验证测试 + PVP telemetry 日志
- **共享类型**：`StyleAttack` trait 扩展 `rejection_rate()` / telemetry schema 加 `attacker_style` / `defender_style` 字段
...
| **P1** ⬜ | 30 对攻防验证测试（4 攻×3 防 + 互克）+ 模拟器 HTML 对比报告 | 30 对测试全绿,无 0.00 行（除声学阈值 fail）,克制方向符合 worldview |
| **P2** ⬜ | PVP telemetry 增强：加 attacker_style / defender_style 字段 | cross-system-patch schema 对齐 + Redis 推送含 style tag |
| **P3** ⬜ | telemetry 聚合 + 校准：偏差 >30% 的调底层物理参数（rejection_rate / resistance 系数）,不改公式结构 | 跑 200+ 真实对战后,30 对组合偏差 ≤30% |
