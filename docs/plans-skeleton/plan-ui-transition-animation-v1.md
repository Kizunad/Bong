...
| 阶段 | 内容 | 状态 |
|----|------|----|
| P0 | ScreenTransition 引擎 + ScreenTransitionRegistry + hook `setScreen()` | ⬜ |
| P1 | 8 screen 过渡配置落实 | ⬜ |
| P2 | LoadingOverlay + 异步 preload | ⬜ |
| P3 | ConnectionStatusIndicator + 断线重连 UX | ⬜ |
| P4 | NPC 交互 screen 过渡 + 特殊死亡/重生过渡 | ⬜ |
| P5 | 性能 + 快速切换压测 + 低配 fallback | ⬜ |

---

## P0 — ScreenTransition 引擎 + Registry ⬜
...
- **落地清单**：`ScreenTransition` 引擎 / `TransitionConfig` / `ScreenTransitionRegistry` / `ScreenSetMixin` hook / 8+ screen 过渡配置 / `LoadingOverlay` / `ConnectionStatusIndicator` / NPC screen 链过渡 / 低配 fallback
- **关键 commit**：P0-P5 各自 hash
- **遗留 / 后续**：未来新 screen 只需一行 `registry.register(MyScreen.class, config)`——零框架改动
