...
- ⬜ P4 契约 pin 全量化：从当时 authoritative registries 派生 C2S/S2C type sets，对每个实际变体至少一条正反 sample；禁止手写 116/144 充当 freshness 证据。emit 迁移长尾。dropped-loot 分页必须覆盖 0/1/255、256（恰一满页且无空尾页）、257（256+1 两页）、缺末页与混 revision，client 只在同 revision 全页齐备后替换。accepted move/rotate/pack receipt 固定含 request identity、result revision、instance identity、from、to、带 post-operation footprint/orientation 的 authoritative item view；rejected receipt 固定含 request correlation、reason、instance、from、to。两类 receipt 必须逐 Rust domain→proto→TypeBox/sample→converter/emit→Java router/store（及适用 bot decoder）对拍。
...
4. `wire_inventory_receipt_and_pages`：accepted 2×1→1×2 rotate receipt 的 request/revision/instance/from/to 与 authoritative footprint/orientation 全链一致，rejected correlation/reason 仍完整；recipient projection 分别执行 0/1/255/256/257 entries，256 不产生空尾页，257 恰为 256+1，缺页/混 revision 不替换旧 store。
