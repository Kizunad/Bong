...
3. `Running`/`Suspended` hydration 的 Resume producer negative test 枚举本轮不扩展；延期到总纲 `plan-refactor-master-v1.md §3 Wave 2` 的 R7 producer 验收窗口，理由是需与完整 phase enum 及 R6 store generation-CAS 一次性对拍。
