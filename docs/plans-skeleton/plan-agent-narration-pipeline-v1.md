...
- **Master matrix note**：已在 `docs/plans-skeleton/plan-refactor-master-v1.md §6.11` 登记本 plan 的 Agent 轨归属；该 matrix row 与本 plan 同一 docs-only 修订提交。
- **实现形态**：先作为共享 agent 基础设施 plan，再由各 feature narration runtime 逐个迁移；不是把 26 个 issue 分成 26 个互相复制的修复出口。
