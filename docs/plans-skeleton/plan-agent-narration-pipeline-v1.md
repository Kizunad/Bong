...
4. 把 publisher resolve 与业务状态提交明确分开排序：resolve 只将 envelope 置为 `publish_pending_commit`；cooldown、cursor、last status、pending 删除等业务状态提交成功后，才能将 envelope 持久化为 terminal `published`。该 terminal 仍不代表 subscriber delivery 或可见副作用只发生一次；未来若引入带 receipt/dedupe 的 server contract，再扩展可选 `consumed`。
...
| #1590 | `halfstep-rechallenge-narration.ts:167-169` forwards the documented `offline:<name>` player representation; `server/src/network/agent_bridge.rs:285-334` normalizes the alias and matches normalized usernames/char IDs. | This producer/consumer contract is already compatible; retain the documented representation and add a positive compatibility pin rather than rewriting it as a raw-ID bug. |
...
### 4.2 narration-routing-and-delivery（13 个 confirmed source ID）

`#1481 #1486 #1495 #1510 #1518 #1525 #1528 #1530 #1532 #1567 #1581 #1587 #1702`

`#1551` 不在此清单：shard-2 判定其为 falsified，仅保留在 §3.2 的 regression note。

`#1590` 不在此清单：本轮复核确认其 `offline:<name>` producer representation 与 server selector 的 documented normalization 相容，保留为 compatibility pin，不进入 P4 raw-ID 修复范围。

### 4.3 agent-publish-idempotency（8 个 confirmed source ID）

`#1616 #1619 #1654 #1659 #1673 #1675 #1679 #1738`

**正式 source ledger 计数：cluster 3 有 30 个原始 issue ID，其中 #1551 已 falsified、#1590 经复核为 documented-compatible，另有 #1607、#1674、#1746 三个独立 world-model/query-tool defects 移出本 plan；余 25 个 narration-cluster confirmed source ID；另纳入与 bounded concurrency 交付物直接重合的跨簇 #1702，因此本 plan 共 26 个 confirmed source ID。26 个中 4 个是 #1470 的重复证据，22 个是非重复 confirmed source。** 不计入本 plan 的其余 8 个跨簇或独立域 follow-up 见 §4.4；不在 skeleton 阶段关闭或宣称任何 source 已修复。

### 4.4 跨簇与独立域 follow-up（8 个；不属于本 plan source ledger，也不进入 P 阶段锁）

以下编号是真实的 agent/narration 邻接 finding，但不属于本次 plan 正式 source 清单。它们只作为后续 owner 的输入，不计入本 plan 的 26 个 confirmed source 或 acceptance 的“全部覆盖”统计：

| Issue | shard cluster / domain | follow-up 边界 |
|---:|---|---|
| #1607 | query-tool correctness | 由 query-zone-history owner 修正时间窗口语义；本 plan 不锁定其修复。 |
| #1674 | query-tool correctness | 由 query-player owner 增加 relationships 结果边界；本 plan 不锁定其修复。 |
...
| #1746 | world-model correctness | 由 world-model snapshot owner 保留 `ZoneSnapshot.status`；本 plan 不锁定其修复。 |
...
- `attempt` / `state`：`pending`、`validated`、`routed`、`publish_pending_commit`、`published`、`rejected`、`dead_letter` 等本 plan 必选状态；`published` 只有在关联 business-state commit 成功后才能成为 terminal in-scope state。Redis client resolve 先产生 `publish_pending_commit`，绝不能直接把 envelope 标为 terminal `published`；该状态仍不表示 subscriber delivery 或 visible-side-effect confirmation。未来 server receipt contract 落地后才可增加可选 `consumed`。
...
**本阶段锁定**：#1510、#1518、#1673，并为 #1486/#1619 提供顺序入口；#1746 属于独立 world-model follow-up，见 §4.4。
...
3. 将 feature cooldown、`lastStatusByFaction`、`lastNarrationTickByKey` 等业务状态放在 `publish_pending_commit` 之后提交；只有业务状态提交成功，envelope 才能进入 terminal `published`。业务提交失败或进程在该边界崩溃时，保持 `publish_pending_commit` 并按同一 `event_id`/`correlation_id` 恢复，不得把 Redis resolve 当作业务成功。
...
**本阶段锁定**：#1530、#1616、#1654、#1673。#1674 与 #1746 属于独立 query-tool/world-model follow-up，见 §4.4。
...
3. publish 前以现有 server selector 兼容矩阵对拍：必须有合法 `player`、`zone`、`broadcast` 三条正向集成案例；`player` 案例同时 pin documented `offline:<name>` producer representation 经 server normalization 后可命中 canonical identity。缺失或非法 target 的负向案例必须 fail-closed，不能隐式 broadcast。
...
**本阶段锁定**：#1525、#1528、#1532、#1587；#1590 作为 documented `offline:<name>` → normalized player identity 的 compatibility pin 验证，不是 raw-ID 修复项。#1607 属于独立 query-tool follow-up，见 §4.4；#1738 仅登记 contract gap，不进入 P 阶段锁。
...
2. Redis client-side `PUBLISH` resolve 只将 envelope 置为 `publish_pending_commit`；它表示 publisher 已完成 transport call，不表示 subscriber 收到、server 已消费、recipient 已选择或客户端已展示。随后必须先提交关联的 cooldown、cursor、last status、pending 删除等业务状态；只有该 business-state commit 成功后，才能把 envelope 持久化为 terminal `published`。业务提交失败或 resolve 后崩溃时，恢复必须重试同一 `event_id`/`dedupe_key`/`correlation_id` 的 `publish_pending_commit`，不能把 resolve 记录为完整业务成功。现有 Pub/Sub 返回的 subscriber count 不能作为 delivery receipt，零 subscriber 时 resolve 也不构成端到端成功。只有未来 correlated receipt + consumer dedupe contract 落地后，才可增加可选 `consumed`，本 plan 不要求、也不等待不存在的 producer。
...
- [ ] pending store 使用持久、可跨进程恢复的实现，支持 pending 写入后的内部重启恢复、失败 requeue 和 bounded backpressure；普通 Pub/Sub ingress 是 at-most-once，pre-ingest crash、subscriber delivery 和 publisher resolve 后的重放不在保证内。
...
- [ ] 锁定 style/text/metadata/nullable/numeric/context bounds；禁止任意 style、无界文本进入发布。query-player 的 relationships 结果上限属于 §4.4 独立 follow-up，不纳入 pipeline acceptance。
- [ ] 对 malformed batch 做逐条 fallback/reject；对 server producer 与 agent consumer 做 event-name/schema pin。
- [ ] 覆盖 #1530、#1616、#1654、#1673。#1674 与 #1746 属于 §4.4 独立 query-tool/world-model follow-up，不计入 P3。

### P4：canonical scope/target route

- [ ] 统一生成 player/zone/broadcast target，删除 runtime 自拼 prefix/raw id 的路径；保留 documented `offline:<name>` producer representation，并由 server selector 的 normalization 兼容消费。
- [ ] route 前置 scope policy 与匿名化；缺 zone/target fail-closed。
- [ ] 加现有 server selector compatibility tests：至少一个 canonical player 正向案例、一个 canonical zone 正向案例、一个合法 broadcast 正向案例，以及 missing/invalid target fail-closed 负向案例；`offline:<name>` player 案例必须验证 normalization 后命中消费者。
- [ ] 覆盖 #1525、#1528、#1532、#1587；#1590 仅作为 documented-compatible compatibility pin 验证，不作为 raw-ID 修复交付物；#1607 属于 §4.4 独立 query-tool follow-up；#1738 等待 §8 authorization contract 决议，不得以本阶段完成为由关闭。
...
- [ ] 验证状态顺序：Redis client resolve 只写入 `publish_pending_commit`；先提交 cooldown/cursor/last status/pending deletion 等业务状态，成功后才持久化 terminal `published`。业务提交失败、断线或 resolve 后 crash 必须按稳定 key 恢复；不得把 resolve 当作 subscriber delivery ack、server consumed 或 single visible side effect 的证明。超过上限进入 bounded dead-letter。未来可选 `consumed` 不得成为当前状态提交前置。
...
- 任意已注册 narration event 都能在 telemetry 中看到完整状态链：`ingest → dedupe → validate payload → render → validate NarrationV1 → route → publish_pending_commit → business-state commit → published`（或明确的 rejected/dead-letter）；不存在只在单测可达、生产无 consumer 的路径。
- Redis publish 被人为拒绝、超时或断线时：调用不会产生未处理 promise rejection；事件保持 pending/retry/dead-letter 之一；不会提前推进 cooldown、cursor、last status 或删除 pending。Redis resolve 先产生 `publish_pending_commit`，只有业务状态提交成功后才转为 terminal `published`；resolve 只验证 publisher/client 侧调用完成，不验证 subscriber 收到或 server/client 可见。
- 同一 `event_id` 在 pipeline 内重试时复用原 key、不得生成新 batch id，并对重复 claim 产生审计记录；不把当前 Redis Pub/Sub 或 `NarrationV1`/server consumer 解释为可证明的 exactly-once 或 single visible side effect。若 publisher 在 resolve 后、业务状态提交或 terminal `published` 落盘前崩溃，恢复必须重新处理 `publish_pending_commit`，可能再次 publish；若 subscriber 未连接，resolve 也可能没有任何消费者收到。需要 single visible side effect 时，必须由未来的 correlated receipt + receiver dedupe contract 提供证据。
...
- `scope=zone` 的 target 必须是 server selector 可命中的 canonical zone；`scope=player` 必须是 normalized player identity，并以 documented `offline:<name>` producer representation 的正向兼容案例锁住 normalization；`scope=broadcast` 必须有合法 broadcast 正向集成案例；缺失或非法 target、隐藏事件不能隐式变成 broadcast。
...
- Redis 集成测试至少覆盖：publish failure/retry、同 key pipeline dedupe、批量 malformed item isolation、tick epoch restart、canonical zone/player target、合法 broadcast target、missing/invalid target fail-closed、`offline:<name>` player normalization、零 subscriber 的 resolve 语义、resolve 后 crash 的重复边界，以及 pending 写入后的内部重启恢复；测试明确不把 Pub/Sub pre-ingest replay、subscriber delivery ack、single visible side effect、dimension routing、SpiritNiche recipient authz 或 server consumed receipt 伪装成现有能力。
...
- **伪 exactly-once**：Redis publish resolve 只产生 `publish_pending_commit`，业务状态提交成功后才进入 publisher-side `published` terminal state；即便如此也不等于 subscriber delivery、server 已选择 recipient 或客户端已展示。未来 `consumed` receipt 只有在 §8.2 的完整生产链落地后才可增加。若需要 single visible side effect，必须同时提供 receiver dedupe 与 correlated receipt，不能由当前 Pub/Sub client resolve 推导。
...
本 plan 是纯 agent/schema 逻辑基础设施，不含 NBT、layout、模型或视觉资产，因此不引入视觉资产打磨阶段。实现期间不得顺手扩展 server dimension selector、SpiritNiche authorization 或 consumed receipt；三项只能按 §8 另立跨栈 contract owner。
...
5. **P4 / route**：只实现现有 scope/target canonical route 与匿名化；不伪造 dimension 或 SpiritNiche recipient contract。
6. **P5 / publish/migration**：publisher-side `publish_pending_commit → business-state commit → published` state sequence、retry/dead-letter/metrics、旧直发迁移与 agent/Redis 集成验收；不把 resolve 当作 subscriber ack 或 single visible side effect 证明；#1738 保持 open。
...
主线不亲自实施 PR；每个 PR 启动独立实现上下文，prompt 必须限定该 PR 范围、必读本计划 §10、门禁与禁止扩展的 §8 contract gap，并以最高思考预算结尾：
...
每个 PR 必须完成仓库当前明确指定的 review gate 后才能进入下一 PR。pending 时使用 `ScheduleWakeup delaySeconds=1200`，禁止 sleep/busy-poll，最多三轮无结果才停交人工；有修改意见时由新的独立返工上下文修复、重跑对应栈门禁并重新等待 review，不能自行判定“应该通过”。review 触发动作遵循用户当时指令，本 skeleton 不授权自动发送触发评论。
