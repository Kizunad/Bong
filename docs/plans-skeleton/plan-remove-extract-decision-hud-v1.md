# plan-remove-extract-decision-hud-v1（骨架）

> **骨架（skeleton / 草案）**。一句话主题：移除「搜打撤五维撤退决策 HUD」(`ExtractDecisionHud` / 出行脉象 / "该看看这些了")——用户决定不要这个常驻决策面板。
...
| 阶段 | 主题 | 状态 |
|------|------|------|
| P0 | 摘除 HUD 渲染（不再常驻显示，最小可逆） | ⬜ |
| P1 | （可选）彻底清理 state store + server 喂数据 + 测试 + plan 状态回退 | ⬜ |
