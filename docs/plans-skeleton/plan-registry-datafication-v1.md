...
> **一句话主题**：把三处最挡"横向扩内容"的硬编码注册表迁成扫盘数据文件——craft 手搓/制作台配方（~104 条 Rust 元组表）、功法元数据（49 条 const 数组）、terrain 方块名映射（`blocks.rs` ~392 行 match）——**零语义变化、零新系统、零 wire 改动**，只搬装载来源不动消费方，让"加一条内容 = 加一个数据条目"的覆盖面从物品/丹方/锻造蓝图扩到配方/功法/地形材质。

**状态**：骨架（skeleton）。升 active 前按 docs/CLAUDE.md §五 收口 §8。

| 阶段 | 主题 | 状态 |
|------|------|------|
| P0 | craft 配方数据化——workbench ~99 条 + legacy 5 条 → `assets/craft/recipes/*.toml` 扫盘 + 对拍回归门 | ⬜ |
| P1 | 功法元数据数据化——`TECHNIQUE_DEFINITIONS` 49 条 → TOML + 双向 wiring 启动校验 | ⬜ |
| P2 | `blocks.rs` 方块名查表化 + manifest 引用启动期 fail-fast（替代静默丢材质） | ⬜ |
...
1. **craft 配方**：`craft/workbench_recipes.rs:79` `register_workbench_recipes` 十组子函数 Rust 元组表（`:78` 注释自报 98 workbench/coffin + 1 台自身 = 99 条），另有 `craft/mod.rs` `register_examples` 5 条 legacy 手搓（eclipse_needle_iron / poison_decoction_fan / fake_skin_light / zhenfa_trap_iron / herb_knife_iron）。明明 forge / alchemy / processing 三兄弟全是扫盘数据，唯独手搓是代码，风格孤例。
...
3. **方块名映射**：`world/terrain/blocks.rs` `block_from_name` ~392 行硬编码 match。worldgen surface_palette / DecorationSpec.blocks 引入新方块名不在表内 = 运行时静默返回 `None` 丢材质，跨 Python→Rust 的隐性契约，历史上最易漏的扩内容卡点。
...
- **出料**：运行时消费方**零接口变化**——`CraftRegistry::grouped_for_ui`、`SkillRegistry` resolver 查找（`cultivation/skill_registry.rs:79`、`:104` `init_registry`）、`terrain/column.rs` 方块解析，全部不动。
...
- **对拍回归门（本 plan 核心测试策略）**：迁移 commit 前先落一个 test fixture——dump 现 Rust 表全量快照（id → CraftRecipe 序列化）；迁移后断言 TOML 加载结果与快照**逐条相等** + 数量 pin。既有 session / unlock / reclaim / UI 分组测试全绿不动（尤其 `session.rs:1744` 手搓无台可做 pin）。
- 饱和测试：坏 TOML 拒载（未知字段 / 重复 id / 引用不存在 item / 负数 qi / 零产出）；`CraftCategory` / `UnlockSource` / `CraftStationKind` 每 serde 变体正反 sample pin。
...
- **静默 `None` → 启动期 fail fast**：`TerrainProvider::load` 时把 manifest 携带的 surface_palette / decoration blocks 全量预解析，未知方块名启动即报错并列全清单（替代运行时静默丢材质）。
- 饱和测试：现 match 覆盖的**全部名字新旧解析结果对拍**（一致性快照）；未知名报错路径；manifest 校验命中 / 漏配用例；`raster_check` 后验流程不受影响（`bash scripts/dev-reload.sh` 全绿）。
...
- NPC 原型默认掉落：`npc/loot.rs:24` `default_loot_for_archetype` 静态 match 表（世界/TSY/宗门掉落均已 JSON，唯此硬编）。
- 丹道 6 方包装：`dandao/recipes.rs:41` const 数组（引用 alchemy JSON 但包装层硬编）。
...
4. **blocks.rs 方案选型实证**：valence `BlockKind::from_str` 可用性（392 行里有多少带 props 的非直映射条目必须保留特例）；生成查表脚本是否值得。
...
scope 预估 3-4 PR（P0 / P1 / P2 各一，P3 视裁决追加）。纯 server 基建 plan，无视听规格要求（docs/CLAUDE.md §四 豁免条款）；每 PR 以对拍回归门为 merge 前置；本地门禁 server 栈全跑 + P2 追加 `scripts/dev-reload.sh` 全链。
