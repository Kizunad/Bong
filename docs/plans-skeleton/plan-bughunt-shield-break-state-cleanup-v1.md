...
- **出料**：`ShieldBroken` payload/AV 保持；下一次攻击不再进入盾格分支。
...
- `server/src/combat/resolve.rs:1262` 在 offhand 缺失时仍 fallback `wooden_shield`。
- `:1348` 耐久归零路径消费物品并 emit `ShieldBroken`，但未同步移除 `ShieldBlock`、`ShieldBlocking`、`ShieldDrainOverride` 或恢复 stamina state。
- `server/src/combat/lifecycle.rs:295` 仍按 `StaminaState::ShieldBlocking` 持续 drain。

## 验收

1. 正面命中恰好破盾后，同 tick 清空 offhand 与全部举盾状态，恰好发一条 `ShieldBroken`。
2. 下一次攻击零盾减伤；stamina 不再 drain，也不因旧状态进入 Exhausted/ParryRecovery。
3. 未破盾、主动放盾、耐力耗尽、重复破盾事件、盾实例在结算前被移走等边界全覆盖。
4. 既有 client 破盾反馈只发一次；不新增 DefenseKind 或第二 AV 入口。
5. 完整 server gate + bot e2e。
