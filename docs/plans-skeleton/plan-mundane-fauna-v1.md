...
**tier 之间构成食物链，且与既有妖兽层打通成一张食物网（活的生态，非各自为政）**——统一进 `preys_on` 关系图（扩展现有 `is_prey_of` 谓词到跨层）：

```
妖兽层（HybridBeast/AshSpider…）──猎──→ 凡兽全档（凡兽=妖兽的口粮，不只盯玩家）
狼(T2.5) ──猎──→ 兔/鸡/羊(T0/T1) + 噬元鼠     狐(T2) ──猎──→ 兔/鸡/蛙(T0) + 噬元鼠
噬元鼠 ──腐食──→ 凡兽尸体（正典口径见 §8 #8）   T0 ──避──→ 上表一切
```

捕食是**真消费**：猎杀成功回补捕食者 `Hunger`（觅食环闭合），留下的尸体可被鼠类/狐腐食或被玩家屠宰——先到先得。T0~T1 不进 ambient-threat 的 danger 威胁预算统计（威胁贡献≈0 但行为上有牙）；T2+ 是否计入 danger 密度口径见 §8 #3。
...
- **thinker**：commoner 裁剪版——`FleeThreatScorer→FleeAction`（泛化避险：按 `preys_on` 图 + 威胁标记对捕食者/妖兽/修士/敌对 NPC 无差别避让，T0 最敏感，取代仅逃修士的 `FearCultivatorScorer` 单一用法）+ `HungerScorer→FarmAction`（觅食）+ `WanderScorer→WanderAction`；羊/鸡加 `GroupCohesionScorer→RegroupAction`（复用 rat 群聚件）；**每个物种按 ThreatTier 挂对应反抗/反击件**（T0 `CorneredScorer`、T1 `RetaliateScorer`，攻击动作全部复用妖兽 `MeleeAttackAction`）——没有纯挨打的物种。
...
- **跨层食物网落地**（设计原则节的 `preys_on` 图从这里接线）：
  - 狼/狐挂 `PredatorScorer→HuntAction`（复用妖兽领地 Hunt 件），猎物集含 T0/T1 凡兽**和 `BeastKind::Rat` 噬元鼠**——鼠患的天敌来自凡兽层；
  - **妖兽猎凡兽**：既有 `HybridBeast`/`AshSpider` 等的 Hunt 目标集扩进凡兽（`is_prey_of` 跨层扩展）——妖兽不再只盯玩家，凡兽是它们的口粮，玩家能撞见"妖兽扑杀羊群"的生态事件；
  - **捕食闭环**：猎杀成功回补捕食者 `Hunger`（复用 `FarmAction` 回补路径换触发源）；被猎倒的凡兽留 3~5s 尸体窗口，可被鼠/狐腐食（消失）或被玩家抢先 `ButcherRequest` 屠宰——先到先得；
  - **鼠类接入**：噬元鼠腐食凡兽尸体（行为复用其 qi 源趋近件换目标，正典口径 §8 #8）；鼠群袭扰（ambient-threat P2）与凡兽共存时，狼狐的捕食客观上压制鼠患密度——生态自平衡。
  **狼群按威胁谱系 T2.5 落地**：饥饿加权攻击性（`Hunger` 越低越敢），群体成型（≥3）时主动猎杀醒灵/引气期玩家，对高境界忌惮回避（读 realm 的忌惮判定复用 `FearCultivatorScorer` 境界衰减逻辑反向用）。
- **季节权重**：接 §十七 季节生态——夏耐热物种权重升、冬耐寒（兔/山羊）升，读现有季节源（`season_success_modifier` 同源）。
- **测试**：死域/负灵域/塌缩 zone 三门槛；枯萎计时→despawn；`preys_on` 图全边 pin（含跨层：妖兽→凡兽、狼狐→鼠）；捕食闭环（猎杀回补 Hunger、尸体窗口先到先得、腐食 vs 屠宰互斥）；Scorer 优先级（逃跑压过觅食、反抗压过逃跑的触发边界）；季节权重分布 pin。
