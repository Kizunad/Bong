...
**qi_physics 锚点**：
- 时代切换时 `spirit_qi_delta`（agent era_decree 里的字段）必须走 `qi_physics::ledger::WorldQiTransfer { reason: EraShift, delta }`，不能直接加减 WorldQiAccount，守恒律不可绕过
...
- **qi_physics 锚点**：时代切换 spirit_qi_delta → `WorldQiTransfer(EraShift)` → ledger
...
| **P1** | ⬜ | 下游系统读取 EraModifiers：渡劫阈值 × mul / 异变兽 spawn weight × mul / 派系 AI loyalty drift | 三系统分别加 ERA 系数测试；守恒律单测（spirit_qi_delta via ledger）|
...
- [ ] `EraDecreeSystem`：监听 `bong:agent_cmd` 队列里 `style == "era_decree"` 的 NarrationCommand，解析 `params.era_name / spirit_qi_delta / danger_level_delta` → 写 `WorldEraState`；spirit_qi_delta 走 `qi_physics::ledger::WorldQiTransfer { reason: EraShift }`
- [ ] `EraChangedEvent { old_era, new_era, onset_tick }` Bevy event，写入系统 emit
- [ ] ≥ 8 单测（era_decree 解析 / WorldEraState 更新 / EraModifiers 三型正确 / spirit_qi_delta 守恒律）
