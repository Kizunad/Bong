...
截脉·震爆功法**五招完整包**：动画 / 特效 / 音效 / 伤害 / 真元消耗 / 反噬 / 客户端 UI 全流程。承接 `plan-zhenmai-v1` ✅ finished（PR #122 commit 1ae7fd88 归档；P0 极限弹反已实装于 plan-combat-no_ui）—— v2 引入**音论物理**（worldview §P 定律 5 + cultivation-0002 §音论）+ **血肉反应装甲**（worldview §五:432-436 用血条保真元）+ **化虚专属绝脉断链**（worldview §四:319 主动 SEVERED 一条经脉换 60s 选择性免疫）+ **5 招完整规格**（极限弹反 / 局部中和 / 多点反震 / 护脉 / 绝脉断链），无境界 gate 只有威力门坎。
...
- **出料**：5 招 `ZhenmaiSkillId` enum 注册到 SkillRegistry / `JiemaiParryEvent`（扩展 v1 实装含 7 档威力）/ `LocalNeutralizeEvent` 🆕 / `MultiPointBackfireEvent` 🆕 / `MeridianHardenEvent` 🆕 / `MeridianSeveredVoluntaryEvent` 🆕（绝脉断链）/ `SelectiveImmunityActiveEvent` 🆕（60s 选择性免疫激活）/ `JiemaiBackfireBloodSpray` 🆕（皮下震爆痕迹）
- **共享类型**：`StyleDefense` trait（qi_physics::traits）/ `MeridianSeveredVoluntary` component（永久标记 + 持有此 component 的玩家不能再 cast 与该经脉关联的招式）/ `SelectiveImmunity` component（60s 选择性免疫的攻击类型 + 剩余 ticks）
- **跨仓库契约**：
...
- [ ] **化虚级绝脉断链（worldview §四:319 + §五:432）**：截脉的"质变"在化虚级专属——主动 SEVERED 一条经脉换 60s 选择性免疫。这是血肉派的极限代价物理化身：
  - 普通流派化虚级（涡流涡心 / 毒蛊倒蚀）= 主动战斗手段
  - **截脉化虚级 = 主动残废自己**（worldview §四:319 SEVERED 永久残废）→ 换 60s 内对特定攻击类型绝对免疫
  - 哲学：截脉师在生死关头**用经脉换命**，跟血肉派定调一致——身体损伤换战场存活
...
- [ ] **无境界 gate，只有威力门坎**（worldview §五:537）：5 招都允许任何境界 cast，三层物理自然惩罚：
  ...
  - 弹反窗口 200ms 极限判断（境界不影响窗口，但 K_drain 反吸比例分级）
  - **绝脉断链化虚专属** → 通灵以下 cast 仍 SEVERED 一条经脉但**没有免疫效果**（HUD「断脉无应」）—— 自然惩罚，物理代价已付但收益没有
...
| **P1** ⬜ | server `combat::zhenmai_v2::*` 5 招 logic + v1 极限弹反迁入 + 音论物理（R_jiemai / C_contact）+ 多点反震分散 + 护脉硬化 + 绝脉断链永久 SEVERED 写入 MeridianSystem + 60s 选择性免疫判定 + qi_physics 算子调用 + ≥100 单测 | `cargo test combat::zhenmai_v2` 全过 / 守恒断言（弹反 K_drain 走 ledger）/ SEVERED 跨 server restart 持久化测试 / `grep -rcE '#\[test\]' server/src/combat/zhenmai_v2/` ≥ 100 |
...
| **P4** ⬜ | PVP telemetry 校准 / 暴烈色 hook（PracticeLog → QiColor 暴烈色累积演化）/ 跟 baomai-v2 越级+全力一击 vs 截脉弹反实战测试（化虚体修 vs 化虚截脉互克验证）/ 跟 dugu-v2 化虚倒蚀的反制路径（化虚截脉 ⑤ 绝脉断链选"脏真元类"免疫 vs dugu 倒蚀） | 7 流派 4×3 攻防对位截脉通过 / 化虚级 hard counter dugu 实战测试 / 暴烈色长期累积演化测试 |
...
### ① 极限弹反（已 v1 ✅ P0，v2 校准 7 档威力）

worldview §五:434 + §P 定律 5。200ms 接触瞬间触发，必须接触（区别 woliu 瞬涡负压式）。

| 境界 | 弹反窗口 | K_drain（反吸） | 自伤 HP | 适用攻击 |
|---|---|---|---|---|
| 醒灵 | 200ms | 0.05 | 8 | 体修近战可弹 |
| 引气 | 200ms | 0.15 | 8 | + 毒蛊蚀针（W=0.0 仅弹但反吸 0） |
| 凝脉 | 200ms | 0.30 | 6 | + 暗器载体 |
| 固元 | 200ms | 0.40 | 5 | + 阵法触发 |
| 通灵 | 200ms | 0.50（clamp） | 4 | 全面 |
| 半步化虚 | 220ms | 0.50 | 4 | 全面 |
| 化虚 | 250ms | 0.50 | 3 | 全面 |
...
**机制**：主动选择 1 条经脉（inventory inspect UI 内点击）+ qi 消耗 → 该经脉 contam 减少。**专属：高境截脉可中和 dugu 短期 qi_max 衰减**（固元注入的 24h 标记），但**不能中和永久标记**（那需替尸 ③ 上古伪皮 hard counter 或本 plan ⑤ 绝脉断链选"脏真元类"免疫）
**冷却**：3s
...
### ⑤ 绝脉断链 — 化虚专属（主动 SEVERED 换 60s 选择性免疫）

worldview §四:319 SEVERED 永久残废物理 + §五:432 血肉反应装甲极限化身。**化虚专属**——通灵以下 cast 仍 SEVERED 一条经脉但没有免疫效果（HUD「断脉无应」）。

| 境界 | 选定经脉 | 60s 内免疫攻击类型 | qi 启动 |
|---|---|---|---|
| 醒灵-通灵 | 任选 1 条 | **无免疫**，仅 SEVERED | — / **HUD「断脉无应」** |
| 半步化虚 | 任选 1 条 | 选 1/4 类型 | 60 |
| 化虚 | 任选 1 条 | 选 1/4 类型 | 50 |

**4 类免疫攻击类型**（cast 时弹窗选 1）：

1. **真元类**：体修注入 / 涡流抽取 / 截脉反震（vs zhenmai 同源攻击免疫）
2. **物理载体类**：暗器骨刺 / 凡器砍劈
3. **脏真元类**：dugu 蚀针 / 阴诡色侵染（**反制化虚倒蚀的关键路径**）
4. **阵法类**：地师诡雷 / 缚阵触发

**机制**：
- 主动 cast → 弹窗选 1 条经脉（手三阴/三阳/足三阴/三阳/任督共 20 条）→ 弹窗选 1 类攻击免疫
- 选定经脉立即 **永久 SEVERED**（不可逆，除非上古"接经术"残卷 worldview §十六.三 一次性脆化）
- 60s 内对选定攻击类型**绝对免疫**（W=1.0，不进入伤害判定）
- 60s 后免疫消失，但 SEVERED 永久
...
**反制化虚毒蛊师的关键路径**（与替尸 ③ 上古伪皮 hard counter 并列）：
- 化虚毒蛊师倒蚀引爆永久 qi_max 衰减标记
- 化虚截脉师 ⑤ 绝脉断链选"脏真元类"免疫 → 60s 内 dugu 倒蚀对其完全失效
- 代价：永久 SEVERED 一条经脉
- 跟替尸 ③ 上古伪皮的 trade-off：替尸烧物资 vs 截脉烧身体
...
│                                                            severed_at_tick) +
│                        SelectiveImmunity component (60s 选择性免疫攻击类型 +
│                                                     剩余 ticks)
...
server/src/schema/zhenmai_v2.rs  — IPC schema 5 招 + 永久 SEVERED + 选择性免疫 payload
...
| `cast_sever_chain` | 化虚专属判定 + 通灵以下「断脉无应」+ 4 类免疫选定 + 永久 SEVERED 持久化（跨 server restart）+ 60s 选择性免疫倒计 + 反制 dugu 倒蚀实战测试 | 22 |
...
- **2026-05-06** 骨架立项，承接 plan-zhenmai-v1 ✅ finished（PR #122 commit 1ae7fd88 归档；P0 极限弹反已实装于 plan-combat-no_ui）。
  - 设计轴心：血肉派定调（HP / 经脉换 qi 免伤）+ 音论物理（worldview §P 定律 5 + cultivation-0002 §音论）+ 5 招完整规格 + **化虚专属绝脉断链**（worldview §四:319 主动 SEVERED 换 60s 选择性免疫）+ 反噬阶梯含 SEVERED + 瞬时痕迹（区别于其他流派的长期场）
  - 五招完整规格 7 档威力表锁定（极限弹反 / 局部中和 / 多点反震 / 护脉 / 绝脉断链）
  - **化虚做新招**（区别于 tuike 物资派"无新招仅钱包深"）—— 截脉是血肉派，化虚级用经脉换免疫是物理一致
  - 反制化虚毒蛊师两路径：
    1. 替尸 ③ 上古伪皮 hard counter（烧物资）
    2. 截脉 ⑤ 绝脉断链选"脏真元类"免疫（烧身体）
  ...
  - 待补：与 plan-style-balance-v1 W/β 矩阵对齐 / 暴烈色 hook（zhenmai-v2 P1 自行查询 PracticeLog）/ plan-tribulation-v1 化虚断脉触发天道注视 / plan-multi-life-v1 SEVERED 跨周目处理 / plan-narrative-political-v1 化虚断脉求生江湖传闻
...
- **遗留 / 后续**：暴烈色 passive_buff 通用化（其他流派 vN+1 也需类似 hook 时提取）/ telemetry 校准（plan-style-balance-v1）/ 化虚断脉的反制路径（化虚毒蛊师反制 zhenmai 选择性免疫，留 dugu vN+1）/ 上古接经术残卷恢复 SEVERED 路径（plan-tsy-loot-v1 + 多周目策略）
