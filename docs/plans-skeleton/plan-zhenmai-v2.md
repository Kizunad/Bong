...
- `plan-skill-v1` ✅ + `plan-hotbar-modify-v1` ✅ + `plan-hotbar-modify-v2` ⏳ active（P1/P2 SkillConfig 通用底盘 + floating window；⑤ 绝脉断链是其首个强消费者）+ `plan-multi-style-v1` ✅
...
**4 类反震加成攻击类型**（InspectScreen「功法」tab 该招详情卡 → 齿轮 → floating window 预配置 `SkillConfig.backfire_kind` 选 1，详见下方"操作"段；底盘归 plan-hotbar-modify-v2 active P1/P2 交付）：
...
- **InspectScreen 预配置（接 plan-hotbar-modify-v2 active §3 SkillConfig Floating Window）**：玩家在 InspectScreen (I 键)「功法」tab → 该招行 → 详情卡右下齿轮 → 弹出 floating window 浮窗，dropdown 选 `(meridian_id, backfire_kind)`——20 条经脉 × 4 类攻击 = 80 种变种 → 保存按钮走既有 `bong:client_request` 通道发送 `type="skill_config_intent"`，server 校验后写入 `SkillConfigStore[skill_id]`
