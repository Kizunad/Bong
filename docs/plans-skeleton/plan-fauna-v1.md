...
**世界观锚点**：`worldview.md §七 动态生物生态`（噬元鼠 / 灰烬蛛 / 缝合兽行为逻辑）· `worldview.md §十 封灵骨币`（异兽骨骼 + 阵法封死真元 = 通货）· `worldview.md §四 暗器流载体`（"异变兽骨/灵木：飞 50 格保留 80% 真元"——高级载体）· `worldview.md §九 资源与匮乏`（异变兽核 = 固元→通灵突破必需，极稀）· `worldview.md §七 负压畸变体`（负压改造兽 = 变异核心 + 异兽骨骼特殊掉落）
