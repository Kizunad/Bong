...
NPC 系统**三个独立正确性 bug 集中修复**（非性能，纯行为正确性）：① idle NPC 重力失效永远悬空 / ② A* 路径返回空时 NPC 永远 idle（spawn position 在空中或 chunk 没生成时）/ ③ MineSkin fallback 退化女巫 entity（破坏修仙观感）。三个 bug 由 plan-npc-perf-v1 5 路探查 + 主链路源码读取确认（非派生 plan，独立 PR 走 fastlane）。**本 plan 是 plan-npc-perf-v1 P0 baseline 录档与 plan-npc-virtualize-v1 P1 hydrate 路径的隐式前提**——baseline 录档前应已修 #1 #2（避免"NPC 不动"污染性能数据）/ hydrate 后 NPC 必须直接落地（依赖 #1 重力修复），详见反向被依赖。每个 bug 一个独立 PR，互不耦合，配饱和回归测试 pin 行为。**P3 阶段预留 sonnet Explore 探查输出的剩余 bug**（异步进行中，结果回来后追加 §2）。
...
- **进料**：
  ...
  - `spawn_rogue_npc_at`（server/src/npc/spawn.rs:657-694） + `spawn_rogue_commoner_base`（server/src/npc/spawn.rs:821-879） + 各 archetype 的 spawn 入口（commoner / disciple / scattered_cultivator / beast）
...
跟本 plan（v1）关系：**v1 修 navigator/spawn 物理层 → v2 修 ECS lifecycle / state machine 层**，物理层 vs ECS 层正交，可并行 P0 决策门收口；但 **v2 P1 #3 chase/flee Failure 不停 navigator 的稳定测试要求 v1 P0 重力修复完成**（v2 plan 前置依赖已明列），实际排期 v1 → v2 按序。
