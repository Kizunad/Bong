...
- `plan-npc-ai-v1` ✅ — FactionStore / FactionRelation（Neutral / Allied / Hostile）/ `faction::assign_hostile_encounters` scorer
...
- **进料**：`NpcDormantStore`（char_id → NpcDormantSnapshot）+ `FactionStore`（faction 归属）+ `FactionRelation`（Hostile 关系表）+ 敌对 NPC 空间分布（按 zone 聚合）
...
- **共享类型**：复用 `FactionRelation::Hostile` / `QiTransfer` / `bong:npc/death` schema；新增 `DormantCombatOutcome` event
