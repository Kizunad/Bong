...
- [ ] `quota_origin_juebi_retains_then_settles_marker_once`：前置为最终 wave 的 marker-bearing over-quota DuXu；断言 `DuXu + marker → JueBi + identical marker + VoidQuotaExceeded runtime`，之后 `JueBi settle → marker absent`。重复 cleanup/settlement 不得生成额外事件或副作用。正常 settlement 之外，另以 quota-origin JueBi 进入 fled/disconnect/escape 的子例断言 `JueBi + marker → terminal without marker`；该 cleanup 必须发生在 shared `settle_fled_tribulation` 移除 `TribulationState` 的同一终止路径中，从而不能把 marker 留给后续 independent JueBi。对 standalone/non-quota-origin JueBi 不增加 marker 断言。
