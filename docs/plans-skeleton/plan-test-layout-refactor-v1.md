# plan-test-layout-refactor-v1 — 三栈测试布局盘点、所有权冻结与统一入口设计

> **一句话主题**：在不搬迁任何现有测试文件的前提下，冻结 Bong 内 server/client/agent 三栈的测试目录、执行命令、CI job 与报告产物所有权，设计后续可实现的 `scripts/test-all.sh` 统一入口。
>
