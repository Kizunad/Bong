...
- **进料**：`ScatterBeadUseRequest` 经 `server/src/network/client_request_handler.rs:1687-1715` 进入 `server/src/zhenfa/mod.rs`；`handle_scatter_bead_use` 产生即时 source 或 `ScatterBeadBurials`，`handle_scatter_bead_trigger_requests` 处理 owner trigger，`tick_scatter_bead_excretion` 处理自然逸散与 early-depleted。
...
- **共享类型 / event**：复用 `WorldQiAccount`、`QiAccountId`、`QiTransfer`、`QiTransferReason::ReleaseToZone`、`QI_EPSILON`、`QI_SCATTER_BEAD_CAPACITY`、`ScatterBeadBurials`、`ScatterBeadUseRequest`、`ScatterBeadTriggerRequest` 与既有 `CombatClock`/`ZhenfaSystemSet::Runtime`；不新增 ledger、epsilon、transfer event、tick resource、receipt、diagnostic bus 或平行账户表。
- **跨仓库契约**：纯 server runtime lifecycle cleanup；不改 client 请求形状、proto/TypeBox、agent Redis、VFX/audio/narration。
...
4. `WorldQiAccount::iter_balances` / `total`（`ledger.rs:457-480`）与 `build_qi_ledger_hash_fields`（`zhenfa/mod.rs:650-705`）保留并发布每个 zero source key；item instance / bead ID 单调增长，故临时 source cardinality 无界增长。
...
- [ ] 增加 focused `retire_scatter_source_account`（名称可等义）：仅当 `ledger.has_account(source)` 且 `ledger.balance(source) == 0.0` 时调用 `remove_balance`；missing 或任意非零余额均保持 account map 不变并拒绝 retirement。`WorldQiAccount` 公共写 API 已拒绝非有限与负数（`ledger.rs:372-400,438-454`），本 plan 不为不能由该 production API 形成的损坏 map 另造状态机。
...
- [ ] `scatter_source_retirement_contract_is_pinned`（名称可等义）覆盖两种 typed constructor 的 canonical key、exact-zero 删除、missing 保留、正余额（含 `next_down(QI_EPSILON)`、`QI_EPSILON`、`next_up(QI_EPSILON)`）保留，以及 `set_balance(source, 0.0)` 不是合法 retirement 替代。每条拒绝只断言可观察账户、burial、zone、overflow 与 transfer history 不变，不要求新的跨路径 diagnostic 协议。
- [ ] `scatter_terminal_preflight_is_atomic`（名称可等义）只在第一个写操作**之前**注入 source missing、zone missing、余额不足、transfer 构造拒绝等真实 preflight failure；每个 case 断言 source、burial、zone、overflow、完整 transfer history 均零差分。该测试不得要求 accepted/overflow 已写入后的失败回滚；另以 accepted-only、overflow-only、split 三种成功 case 锁定 P0 的精确 transfer/audit 数量与 exact-zero retirement。
...
- [ ] `handle_scatter_bead_trigger_requests` 在既有 `ScatterBeadBurials::trigger_buried` Entity owner gate 成功后，从取出的 burial 的 canonical owner / bead ID 构造 typed buried source；只在 P0 terminal helper 成功 retirement 后删除 burial。zone/source preflight 拒绝时必须把 burial 放回，保留 source 与既有 owner gate 语义。
...
- [ ] production App 从真实 `zhenfa::register` 驱动 `Update`，覆盖 active-use、owner-trigger、natural final 与 early-depleted 四个成功 terminal branch；每个 branch 断言 `!ledger.has_account(source)`、burial 仅在其对应成功路径删除、zone/overflow 与 `WorldQiAccount::transfers` 保留，且 source 生命周期守恒式闭合。
...
- [ ] 覆盖 source absence、exact zero、`next_down(QI_EPSILON)`、`QI_EPSILON`、`next_up(QI_EPSILON)`、partial residual 与 full capacity；任意正额先 canonical transfer 后删除 key，直接 retirement 一律保留 key/余额。
...
- [ ] telemetry pin：`build_qi_ledger_hash_fields` 不再含已 retire source 的 `account:container:qi_scatter*` 字段，活跃 source 与 overflow 字段仍存在；只删除 zero key 前后 `WorldQiAccount::total()` 精确不变。
