...
- **共享类型 / event**：复用 `WorldQiAccount`、`QiAccountId`、`QiTransfer`、`QiTransferReason::ReleaseToZone`、`QI_EPSILON`、`QI_SCATTER_BEAD_CAPACITY`、`ScatterBeadBurials`、`ScatterBeadUseRequest`、`ScatterBeadTriggerRequest` 与既有 `CombatClock`/`ZhenfaSystemSet::Runtime`；P1-B 只补 owner-trigger 所需的真实 client/protocol producer 与 dispatch，不新增 ledger、epsilon、transfer event、tick resource、receipt、diagnostic bus 或平行账户表。owner-trigger 的 wire 字段是客户端准星目标 `x/y/z`，服务端在 owner gate 后解析对应 burial 的内部 `bead_id`。
- **跨仓库契约**：以 server runtime lifecycle cleanup 为主；P1-B 的唯一跨层例外限于 owner-trigger 的 C2S 坐标 producer：client 从现有 `crosshairTarget` 读取被埋方块坐标，`ClientRequestProtocol` / `ClientRequestV1` / `proto` 携带 `x/y/z`，server handler 发送坐标事件，`zhenfa::register` 在 owner gate 后按 canonical owner + exact position 解析 `bead_id`。不把服务端自增 ID 下发给 client，不改 agent Redis、VFX/audio/narration 或无关请求形状。
...
- [ ] 增加 focused `retire_scatter_source_account`（名称可等义）：仅当 `ledger.has_account(source)` 且余额为可接受的 exact `0.0` 时调用 `remove_balance`；missing 或任意正余额均保持 account map 不变并拒绝 retirement。`set_balance` 与 `transfer` 的入口均拒绝非有限余额/金额，故 `NaN`、`+∞`、`-∞` 不是本生命周期可由生产 API 形成的 source 状态；实现可保留防御性 fail-closed 分支，但不得把目标账户 `to_balance + amount` 的潜在溢出描述为 source 边界或列为生产可达验收。
...
- [ ] `scatter_source_retirement_contract_is_pinned`（名称可等义）覆盖两种 typed constructor 的 canonical key、exact-zero 删除、missing 保留、正余额（含 `next_down(QI_EPSILON)`、`QI_EPSILON`、`next_up(QI_EPSILON)`）保留；非有限余额仅作为防御性 fail-closed 不变量（不把目标账户溢出误记为 source 的 production-reachable 状态），同时锁定 `set_balance(source, 0.0)` 不是合法 retirement 替代。每条拒绝只断言可观察账户、burial、zone、overflow 与 transfer history 不变，不要求新的跨路径 diagnostic 协议。
- [ ] `scatter_terminal_preflight_is_atomic`（名称可等义）只在第一个写操作**之前**注入 source missing、非有限 source（若保留防御性分支）、zone missing、余额不足、transfer 构造拒绝等真实 preflight failure；每个 case 断言 source、burial、zone、overflow、完整 transfer history 均零差分。该测试不得要求 accepted/overflow 已写入后的失败回滚；另以 accepted-only、overflow-only、split 三种成功 case 锁定 P0 的精确 transfer/audit 数量与 exact-zero retirement。
...
- [ ] 先补齐真实 owner-trigger producer：为现有 client/protocol 请求链新增一个携带客户端准星命中方块坐标 `x/y/z` 的明确 dispatch、wire/schema 映射和 server `ClientRequestV1` handler；server 在 owner gate 后按 `(canonical owner, exact burial pos)` 唯一解析服务端自增 `bead_id`，零匹配或多匹配均拒绝，再构造 `ScatterBeadTriggerRequest`。不能继续把 `server/src/zhenfa/mod.rs:5562,5576` 的测试注入当作 production entry point，也不向 client 发布 `bead_id`。保持既有请求校验、Entity owner gate 与无裸字符串 source 构造约束，不扩展到无关协议。
- [ ] 真实 producer 接线后，`handle_scatter_bead_trigger_requests` 在既有 owner gate 成功后使用服务端解析出的 burial；从取出的 burial 的 canonical owner / bead ID 构造 typed buried source；只在 P0 terminal helper 成功 retirement 后删除 burial。client/protocol producer 缺失、坐标未命中、zone/source preflight 拒绝或 terminal commit 失败时必须保留 burial/source，不能把测试 fixture 视为生产成功。
...
- [ ] production App 从真实 `zhenfa::register` 驱动 `Update`，覆盖 active-use、真实 client/protocol owner-trigger、natural final 与 early-depleted 四个成功 terminal branch；owner-trigger 测试必须经过生产 producer/dispatch（client 准星坐标 → server 按 owner + exact burial pos 唯一解析 `bead_id`，零/多匹配拒绝 → `ScatterBeadTriggerRequest`），而非直接注入 `ScatterBeadTriggerRequest`。每个 branch 断言 `!ledger.has_account(source)`、burial 仅在其对应成功路径删除、zone/overflow 与 `WorldQiAccount::transfers` 保留，且 source 生命周期守恒式闭合。
...
- [ ] 覆盖 source absence、exact zero、`next_down(QI_EPSILON)`、`QI_EPSILON`、`next_up(QI_EPSILON)`、partial residual、full capacity；任意正额或非有限额均 fail closed，不删除 key、不伪造 zero transfer，任意正额先 canonical transfer 后删除 key，直接 retirement 一律保留 key/余额。非有限 source 只作为入口拒绝/防御性不变量验证，不声称由 `transfer` 目标账户溢出在生产路径中形成。
...
- P1-B 是本 plan 唯一允许改变的 client/proto surface，且仅为 owner-trigger 的 C2S producer 闭环：`client/src/main/java/com/bong/client/mixin/MixinClientPlayerInteractionManagerAlchemy.java` 或现有统一交互键路由读取 `crosshairTarget` 的 burial 方块坐标，`client/src/main/java/com/bong/client/network/ClientRequestProtocol.java` 的编码、`ClientRequestSender.java` 的发送、`proto/bong/envelope.proto` 的 request envelope/message、`server/src/schema/client_request.rs` 的 `ClientRequestV1` variant、`server/src/schema/proto_convert.rs` 的双向 payload conversion，以及 `server/src/network/client_request_handler.rs` 的 `DispatchResources` event writer 与 dispatch arm。实现必须让该 C2S 链携带坐标；server 在 owner gate 后按 canonical owner + exact burial pos 解析内部 `bead_id`，再发给 `ScatterBeadTriggerRequest`，并以 client/protocol → handler → `zhenfa::register` Runtime 的真实路径覆盖 P2；不得向 client 同步服务端 `bead_id`，也不得借此改其他请求、agent Redis、VFX/audio/narration。
...
2. **exact-zero 是唯一删除门**：`WorldQiAccount::transfer` 只写 zero（`server/src/qi_physics/ledger.rs:448-453`），`remove_balance` 是独立删除 API（`:404-405`）；对应 P0，任何非零 residual 先 transfer，不能 epsilon 擦账。`set_balance` 与 `transfer` 入口拒绝非有限余额/金额；目标账户加法的潜在溢出不构成本 source 的 production-reachable retirement 边界。
