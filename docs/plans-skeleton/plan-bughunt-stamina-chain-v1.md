...
> **骨架（2026-07-26）**。一句话主题：体力（Stamina）全链路审计（组件 → stamina_tick → 12+ 扣费点 → emit → client HUD）确认的 **3 个高危**（ShieldBlocking 态被战斗结算覆写 / recover_per_sec 双写打架 / change-detection 被击穿成 5Hz 伪心跳且重连补水恰靠它兜底）+ 中危与附带项，每项发现均有「阶段 → 处置 → 验收锚点」映射（见 §处置总表 与 §扣费路径清单）；正文中的 `file:line` 均为**只读调查取证结果**（promotion 时复核有效性），真正待钉的实现级决议集中登记在 §转 active 前收口清单。

> 立项动机：与同日血条审计同系列的系统性链路审计。3 路只读调查（server 核心 / emit 契约 / client HUD）+ 主循环亲读决定性代码行复核全部高危。已对 bughunt r1-r10 与 bughunt-20260726 ×20 批次（#1280）去重：唯一邻近项 `plan-bughunt-r10-findings-v1` P0 是**破盾（耐久归零）路径**的状态清理缺失，本 plan P0 是**正常格挡受击/出手路径**的 StaminaState 覆写，点位与修法均不同，不重复；`plan-satiety-hydration-v1` 计划向体力恢复注入乘数——**建议**在本 plan P1 收敛单写者后接入（乘数作为 P1 聚合函数的一个输入位）；若其先落地，需自行处理与现存双写点的冲突。
...
主项（3 高危 + 3 中危）分别是 P0-P5 的主题本身；下表收录全部**附带发现**（已尽力逐项映射；promotion 时交叉核验锚点存在性）：

| 发现 | 阶段 | 处置 | 验收锚点 |
|---|---|---|---|
| 剑基础 `spend_stamina` 无余额预检（1 点体力打出 8 耗 Cleave，`sword_basics.rs:877-889`，调用点 `:562/:709/:781`） | P3 | **本 plan 修复**（统一到 `skill_register.rs:846-853` 预检模式） | 验收 P3-c 守恒测试（try_spend 3 + force_drain 2 + 共同 1） |
...
| HuGuSan 直写 `stamina.max`（`client_request_handler.rs:17364-17369`）4 tick 内被 `status.rs:383-385` 不同公式覆盖 | P5 | **本 plan 修复**（删除直写，max 归一走 status 聚合链） | 验收 P5-c |
...
| DerivedAttrsHandler 把 EMPTY 复活成 active+全满（`DerivedAttrsHandler.java:48-53` + `CombatHudState.java:13,39-52`） | P2 | **本 plan 修复**（非权威回写者继承 active；权威 create 路径保持置 true——详见 P2-d） | 验收 P2-d 乱序 e2e |
...
| 明确排除：`status_snapshot` 的 **schema/proto 化改造**、`ThroughputPeakHudPlanner` 层归类、幻觉 overlay 独立偏移 | — | **不在本 plan 范围**（低危 UI/管道债，留待独立 skeleton；注意 `status_snapshot` 的 join resync 触发接入**属 P2 范围**，此处只排除其 wire 格式改造） | status_snapshot 首帧接入见验收 §P2（P2-c 断言③） |
...
体力流出分两类；另有**两个显式豁免类**——零消耗状态 gate（清单 #8：不扣费，仅按 StaminaState 拒绝，不适用余额契约）与 dev 写入（清单 #12：`/stamina set` 为 dev-only，验收豁免，**不得出现在生产路径**）。两类的余额不足语义不同且互不冲突：
...
- **`force_drain`（被动/持续，不可拒绝）**：受击扣费（`resolve.rs:1062`）、`stamina_tick` 各状态 drain。契约：无预检、clamp 到 0（伤害/持续效果照常结算），扣至 `<= 0` 走 Exhausted 转换（见仲裁表）。**与 P3-a 的条件式耦合**：受击扣费的存废与数值以 P3-a 决议为准。若拍板移除：该路径退化为纯状态转换（受击仍进 Combat，不扣费），且 **P0 的耗尽来源由三个降为两个**（tick drain / 出手）——仲裁表「受击 × <=0」半格、拆盾排序契约与 P0 验收中的受击耗尽断言**须同步核销**（回改义务与 P3-b 对 P0 的扩展义务对称，登记在 §转 active 前收口清单）。P0 锁状态机语义、P3-a 锁数值/存废，决议先于 P0 fix 动工。

### 扣费路径清单（12 条归类路径：9 条真实扣费 + 1 条零消耗 gate + 1 条生产不可达 + 1 条豁免；招式级独立扣费点在 #3/#4 内各聚合 4/5 招）
...
| 3 | 剑基础四招（`sword_basics.rs:877-889`，调用点 `:562/:709/:781`） | Cleave 8→5 / Thrust 4→2 / Parry 6→4 / Infuse 3 | try_spend | **缺**（本 plan 修复） | P3-c |
| 4 | 剑道五招（`sword_path/skill_register.rs:932`） | 8/12/15/20/0 | try_spend | 有（`:846-853`，基准范本） | P3-c 回归 |
| 5 | 气针（`needle.rs:138`） | 2.0 | try_spend | 有（`can_shoot_needle`） | P3-c 回归 |
| 6 | 暗器投掷（`carrier.rs` 预检 `:855` / 扣费 `:858`） | 5.0 | try_spend | 有 | P3-c 回归 |
| 7 | 冲刺 dash（`movement/mod.rs:414,719`） | 15−prof×6 | try_spend | 有（reject_reason 回包） | P3-c（clamp 统一） |
| 8 | 举盾开始（`shield_block.rs:282-291`） | 0（零消耗状态 gate，豁免类） | 状态 gate | 有（拒 Exhausted） | P0 矩阵 |
...
| 12 | `/stamina set`（`debug.rs:90-104`）与 HuGuSan max 抬升 | — | 豁免类（dev-only / 增益非扣费） | — | `/stamina set` 验收豁免（dev-only，禁入生产路径）；HuGuSan 由 P5-c 覆盖 |

### P0 状态仲裁表（修复的唯一权威语义，fix PR 不得偏离）

保护 guard 的准确形状：**仅当扣费后 `current > 0` 时保护 ShieldBlocking/Exhausted 不被覆写成 Combat；扣费后 `current <= 0` 时照常写入 Exhausted**（即耗尽转换永不被保护阻断）。ShieldBlock/ShieldDrainOverride 组件的移除**唯一责任方**保持既有 `force_lower_shield_on_stamina_exhausted`（`shield_block.rs:455-495`），扣费点不自行拆盾。**拆盾排序契约**：force_lower 现仅 `.after(stamina_tick)`（注册在 `combat/mod.rs:459-464`，Physics set 内），覆盖不到 Resolve set 内 `resolve.rs:1062` 与 `lifecycle` 出手扣费的耗尽写入——fix PR 须把 force_lower 注册到**全部耗尽写入方之后**（同 tick 内完成拆盾；具体注册位置的 symbol 钉定见 §转 active 前收口清单），验收断言三个耗尽来源（tick drain / 受击 / 出手；**若 P3-a 拍板移除受击扣费则为两个**）各自触发的拆盾都发生在写入同 tick。
...
| Idle/Walking/Jogging/Combat 出手/受击 | <= 0 | Exhausted | 扣费点就地写入（现状保留，`current <= 0` 分支） |
...
- 击穿点：`combat/lifecycle.rs:282-283` `stamina_tick` 每次对每实体无条件可变解引用 `max`/`recover_per_sec` → 每 4 tick（200ms，`components.rs:20`）把所有 `Stamina` 标 Changed，下游 `Changed<Stamina>` 门禁（`movement/mod.rs:523-531`、`network/combat_hud_state_emit.rs:31-39`）退化为 5Hz 全量推送。
- **捆绑关系**：体力下发无显式 join/重连 resync（对比 `network/mod.rs:463-468` 的 inventory/alchemy/skill `emit_join_*` 系列与 `:1014` dropped_loot join hydration），客户端断线清空（`CombatHudBootstrap.java:97-125`）后全靠 200ms 伪心跳补首帧。单独加守卫会让「重连后体力 HUD 长期空白」显形——同 PR 必须补显式 join 首帧。

### P2 通道枚举（三条体力通道 + 1 个旁路回写者）

| 通道 | server producer | payload/schema | client consumer | 携带的体力信息 |
|---|---|---|---|---|
| `movement_state` | `movement/mod.rs:533-566`（filter `:523-531`） | proto `envelope.proto:3166-3178`（`stamina_current/max/low_stamina/stamina_cost_active`，绝对值） | `MovementStateHandler.java` → `MovementStateStore` | 数值（当前零渲染消费，去留由 P4 拍板） |
| `combat_hud_state` | `combat_hud_state_emit.rs:41-89`（filter `:31-39`） | proto `envelope.proto:760-765`（`stamina_percent` 0..1；帧内嵌套 `derived` 对象，server 侧恒 `Some`，`proto_convert.rs:2186-2190`） | `CombatHudStateHandler.java` → `CombatHudStateStore` → `StaminaBarHudPlanner`/`MiniBodyHudPlanner` | **唯一渲染数值源** |
| `status_snapshot` | `status_snapshot_emit.rs:15-67`（`Changed<StatusEffects>`，裸 JSON） | 无 proto/schema（遗民，wire 改造不在本 plan，join 触发接入在） | `StatusEffectStore` → `ExhaustedGreyOverlay` 等 | 仅状态名（Exhausted/StaminaCrash 等），无数值 |
| `derived_attrs_sync`（旁路回写者，非体力通道） | `derived_attrs_emit.rs:29-32`（`Changed` 驱动） | derived flags（无体力字段） | `DerivedAttrsHandler.java:48-53` → 回写 `CombatHudStateStore` | 无数值，但会重建 CombatHudState——P2-d 的治理对象 |

### P2 join 首帧权威契约

- **P2-a 伪心跳修复本体**：`lifecycle.rs:282-283` 加 epsilon 守卫（对齐同文件 `status.rs:355-361` 风格），`Changed<Stamina>` 回归真变化驱动。
- **P2-b 触发源（显式，不隐含在 `Added<T>` 上）**：新增单一 join resync 系统，消费与 `network/mod.rs:463-468` / `:1014` 既有 `emit_join_*` 系列**同一个 join 检测源**（现状范本为 `inventory_snapshot_emit.rs:45-46` 的 `Added<PlayerInventory>`/`Added<Client>` 模式）——该源在 valence 连接模型下对首连与重连一视同仁（每次连接都是新 client 实体），具体事件/组件 symbol 于 promotion 时钉进本节（§收口清单 #1）。该系统**一次性显式推送全部三条体力通道**的权威首帧，均不经 `Changed` filter。
- **P2-c 三通道首帧语义 = at-least-once + 幂等（不是 exactly-once）**：client store 对数值字段为无条件覆盖、对 `active` 走 P2-d 继承规则，两者均幂等——故验收锁「每通道**至少一帧**权威真值 + 重复到达幂等」，不锁帧数。`movement_state` 既有 emit filter 的 `Added<MovementState>` 条件（`movement/mod.rs:526`；组件由 `attach_movement_state_to_joined_clients`（`:170-177`，`Added<Client>` 触发）插入）与新 join resync 可能双发，**允许**；fix PR 可顺手收敛 producer 但不作为验收项。三通道断言：① `combat_hud_state` 全量帧（`hp/qi/stamina_percent + 内嵌 derived`，权威数值源）；② `movement_state` 全量帧（字段集与 P4-b 决议一致：若 P4-b 删除 stamina 字段，则该帧仅断言到达性与存留字段）；③ `status_snapshot` 当前 status 全集（并入 join resync 显式触发，不依赖 `Changed<StatusEffects>` 碰巧命中）。
- **P2-d active 的权威激活方（false→true 有且仅有一条路）**：`CombatHudStateHandler` 收到 `combat_hud_state` 权威帧时置 `active=true`（现状 `create()` 即如此，该权威路径保留）；**其余所有回写者**（`DerivedAttrsHandler` 等）改为继承当前 snapshot 的 active（`CombatHudState.java:39-52` 的硬编码只对非权威通道收回）。断线清空 → inactive；权威首帧到达 → active；`derived_attrs` 先到 → 保持 inactive 不复活、后到 → 保持 active 不降级。顺序无关。
- **P2-e e2e 场景钉死**：客户端断线重连 → 断言三通道各收到至少一帧权威首帧、HUD 在 200ms 内渲染 **server 权威真值**并 active。「权威真值」的定义**参数化挂 P5-e 决议**：当前语义（重连重建实体）= `Stamina::default()` 满值；若 P5-e 拍板持久化 = 持久化值——e2e 断言「HUD 值 == server 组件实际值」而非任何固定数，两种决议下测试形状不变。该场景在伪心跳守卫（P2-a）**开启**状态下运行，证明首帧不再寄生伪心跳。

## P3 — 防御扣费拍板 + 三态拍板 + 扣费守恒（中）

- **P3-a 拍板**：被击方无条件扣 `DEBUG_ATTACK_STAMINA_COST(12.0) * decay`（`resolve.rs:154` 定义、`:1062` 主路径消费、`:4894-4897` 有测试断言该行为存在——定性断言「伤害应耗体力」，非 12.0 数值 pin），近身满额为攻方 `ATTACK_STAMINA_COST` 3.0 的 4 倍且带 `DEBUG_` 前缀。拍板：是设计（受击掉体力）→ 常量改名 + 数值依据入 plan；是调试遗留 → 下调/移除并更新 pin 测试（失败信息写明设计依据）。
...
- **P3-c 修复（扣费守恒，非拍板）**：按「扣费两类契约」（见 P0 前置节）逐路径归类并修复：剑基础 `spend_stamina`（`sword_basics.rs:877-889`，调用点 `:562/:709/:781`）属 `try_spend` 却无预检 → 统一到 `skill_register.rs:846-853` 模式（余额不足 → 拒绝进 InRecovery，零效果/事件/扣费）；dash clamp（`movement/mod.rs:720`）顺手统一为双向。**守恒测试按两类分别断言**：`try_spend` 类——余额恰好等于成本成功且扣至 0、低于成本拒绝且无伤害/事件/扣费、成功恰好扣一次；`force_drain` 类——不足时 clamp 至 0 且伤害/持续效果照常、归零触发 Exhausted 转换（对齐 P0 仲裁表）；两类共同——`current` 永不为负。

## P4 — 阈值语义拍板 + movement 死负载字段拍板（中，decision 先行）

- **P4-a 决策交付物（先于任何代码；表内 0.30 行的结论依赖 P4-b，故 P4-b 先于该行产出，其余三行与 P4-b 无依赖可并行拍板）**：四个「低体力」相关值逐项命名业务语义并拍板归属，**不预设合一**：
...
- **P4-b 决策交付物**：movement 通道 `stamina_current/max/low_stamina/stamina_cost_active` 四字段（server `movement/mod.rs:589-606` 计算编码，client `MovementStateHandler.java:50-52` 入库后零消费）**接消费者 or 全链删除**二选一拍板后才进 fix：删 → proto/schema/sample/client 全链连动 + 反向 sample 锁定；留 → 指定唯一消费者与渲染行为 + server→client 契约测试。两个方向不得同时留在验收里。
- **P4-c 修复**：组件缺失 fallback 统一（`movement/mod.rs:237` vs `:590-592` 方向相反），统一后的方向按 P4-b 决议确定，补缺失组件契约测试。**与 P4-b 的条件式耦合**：若 P4-b 拍板全链删除，`:590-592` 的 payload 编码点随之消失，P4-c 退化为 `:237` 单点缺失契约 pin（速度算子对缺失组件的行为显式钉死），验收同步降维。
...
- **P5-b**：死字段三项逐项拍板（接线 or 删除，各自独立 commit）：`last_drain_tick`（6 写 0 读：`lifecycle.rs:150`、`movement/mod.rs:721`、`sword_basics.rs:888`、`carrier.rs:859`、`resolve.rs:1064`、`skill_register.rs:938`）；`ShieldSpec.stamina_drain_per_s`（`inventory/mod.rs:221-231` 注释自认 P4 承诺未兑现——接进 `shield_block_profile` or 删配置字段）；`technique_proficiency.stamina_cost_mult`（`technique_proficiency.rs:102`）。
- **P5-c 修复**：HuGuSan 删除对 `stamina.max` 的直写（`client_request_handler.rs:17364-17369`），max 计算归一到 status 聚合链单一口径（producer：服药 push StatusEffect → **max 的唯一运行期写入者 = `combat/status.rs` 的 max 聚合点（`status.rs:383-385` 所在链），与 P1 的 `aggregate_stamina_recovery` 是两个各自独立的单写者**——recover 归 P1、max 归本项，互不越界）。测试**同时证明不回跳与药效未丢**：服药后 max 达聚合口径预期值、跨多个 status tick 保持、与 `StaminaCrash` 叠加口径唯一、效果过期回 baseline 100——单锁「不回跳」会让「把药效整个删掉」也通过，不允许。
- **P5-d 修复**：`StaminaRecovBoost` magnitude 双语义拆解（`status.rs:365-397`）。**方案收窄为唯一方向：不改 wire 状态名**——`status_snapshot` 是裸 JSON、client 侧 `ExhaustedGreyOverlay` 等硬依赖 id 字符串（`status_snapshot_emit.rs:83` 兜底生成），拆新 StatusEffectKind = 跨端 wire 迁移，风险/成本不成比例，**否决**；改为 `ActiveStatusEffect` 加显式语义字段（如 `mode: MaxBoost | RecovMult`）在 server 端分流，wire 名与 client 消费零变更。`alchemy/pill.rs:685-707` 配方连动；正反 pin 测试锁两种效果不再靠数值大小分流。
...
1. **join resync 触发源 symbol**：与 `network/mod.rs:463-468` / `:1014` 既有 `emit_join_*` 系列同源的具体事件/组件名（现状范本 `inventory_snapshot_emit.rs:45-46`）（P2-b）。
2. **force_lower 注册位置**：排到全部耗尽写入方（tick drain / resolve 受击 / lifecycle 出手；受击项随 P3-a 决议）之后的具体 system set 与 `.after` 链（P0）。
...
5. **六个 decision 项的拍板动作**（P1-d NPC baseline、P3-a 受击扣费存废、P4-a 阈值语义表、P4-b 字段去留、P5-b 三死字段、P5-e 持久化）：各阶段路由中的「decision 步」**在 promotion 收口（或消费期设计收口）时执行**，本清单登记的是「决议未出前对应 fix 不动工」的门——不是把结论移出阶段，阶段路由 `decision + fix_pr` 中的 decision 即指此拍板动作。
6. **P3-a 移除分支的 P0 回改义务**：若拍板移除受击扣费，P0 仲裁表「受击 × <=0」半格、拆盾排序契约与验收中的受击耗尽断言同步核销（与 #3 的扩展义务对称）。
...
- **P0**：状态仲裁矩阵按 try_spend/force_drain 两组分列全绿（走真实调度；三态格在 P3-b 拍板前 N/A）；格挡中连续受击 state 恒 ShieldBlocking 且 drain 恒走 override（2~3/s）；全部耗尽来源（三个；若 P3-a 移除受击扣费则两个）各自触发的拆盾均发生在耗尽写入同 tick（排序契约）；普通非格挡攻防双方仍进 Combat（反误伤回归）；放盾回 Idle；低体力叙事在格挡受击中正常触发。
...
- **P5**：P5-a 四件套齐且负样本在；P5-b 三项各有着落 commit；P5-c 不回跳 + 药效未丢双向断言（预期值/跨 tick 保持/叠加口径/过期回归）+ max 运行期写入点唯一（grep 口径，与 P1 的 recover 单写者各自独立）；P5-d 显式字段方案 pin 且 wire 名零变更（client 零改动可验）；P5-e 两分支均有测试（持久化→往返 + 无增生；有意重置→重置契约 pin），决策记录落档。
