...
| 阶段 | 内容 | 状态 |
|------|------|------|
| P0 | 人剑共生核心 + 剑品阶 + 5 招定义注册 + 经脉依赖 + 基础 server 逻辑 + 视听 | ⬜ |
| P1 | 铸剑炼器（灵剑锻造 + 原材料 + 品阶升级 + 锻造台扩展）+ 视听 | ⬜ |
| P2 | 巨剑沧海地形（worldgen zone + terrain profile + decorations + 古遗迹 POI）| ⬜ |
| P3 | 黑武士 BOSS（heiwushi 模型接入 + AI 行为 + 叙事 + 战利品）+ 视听 | ⬜ |
| P4 | 化虚·一剑开天门（终极机制 + 天道盲区 + 全套视觉流程 + agent 集成）| ⬜ |
| P5 | 剑道 HUD 完整包 + 饱和测试 + 集成联调 | ⬜ |
