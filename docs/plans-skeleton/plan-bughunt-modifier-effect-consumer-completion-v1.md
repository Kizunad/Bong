...
- 11 条 mapped finding（含 r8 #5 冻结的 13 个子字段）全部转为 `MappedGameplayClosed`，或按 §8.1 删除/禁用 producer、迁移状态并以机械证据转为 `MappedProducerRetired`；`MappedPendingClosure` 必须为 0，Member subject 的 writer/consumer edge 必须不可达，ProducerBinding subject 仅要求该精确 producer site 不可达；共享 scheduler 若仍服务其他 member/site 可以保留，迁移证据必须完整。不得以 `UnmappedObservation`、HUD/storage read 或 planned 状态结案。
