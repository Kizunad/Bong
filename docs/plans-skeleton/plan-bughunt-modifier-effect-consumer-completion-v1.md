...
preflight 不得创建 worktree、branch、commit 或 PR。它有两个机器可读 readiness 结果，但不新增上述六个 protocol stage：**Decision 前**，针对 exact skeleton/active-plan HEAD 只验证 `protocol`、`plan_head`、`workflow_head`、六阶段精确列表与 capability-test run ID，返回 `DECISION_CAPABILITY_READY`；该结果只允许创建 Decision PR，明确不得声称 P0 authority 已就绪。**Decision merge 后、P0 派发前**，必须针对当前 protected base 重跑同一只读入口并取得 `P0_AUTHORITY_READY`：它必须证明该 `protected_base_sha` 是 `decision_merge_sha` 的后继，且 active-plan 的 `decision_plan_blob_sha` 仍等于 Decision PR 冻结的含 §8.1 决议 blob；随后只读验证与该决议一致的 P0 authority，并在输出冻结 `protected_ref`、`protected_base_sha`、`decision_merge_sha`、`decision_plan_blob_sha`、baseline bootstrap merge SHA 与 baseline blob/object SHA、archived canonical source tuple、derivation version、tuple digest、trusted verifier SHA、trusted workflow SHA、baseline schema version 及 verifier/schema/derivation compatibility 结果。baseline/blob/tuple 缺失或不匹配、protected base 非 Decision 后继、决议 plan blob 漂移、trusted verifier/workflow 不在 protected authority、verifier 旧于 baseline schema/derivation version、无法读取 protected base object，或 capability-test evidence 不完整，一律返回 `WORKFLOW_CAPABILITY_BLOCKED`：不得开 P0/P1，也不得把 workflow 缺口计入 modifier manifest。bootstrap/trusted-gate 修复必须由独立 workflow blocker/PR 完成；其 merge 后在保留该 `decision_plan_blob_sha` 的 current protected base 重新执行 P0 authority preflight。

`consume-plan-serial-multi-pr-v1` 的外部 capability suite 必须机械证明：

1. 同一时刻至多一个阶段 PR；前一 PR 未 merge 不得派发下一阶段。
2. Decision 前 preflight 只可返回 `DECISION_CAPABILITY_READY`，不得创建或声称已验证 P0 authority；Decision merge 后、P0 PR 创建前必须在其后继 protected base 上重跑并取得 `P0_AUTHORITY_READY`，冻结 `decision_merge_sha`、未漂移的 `decision_plan_blob_sha`、`CanonicalLifecycleBaseline` authority tuple/digest、bootstrap merge/blob SHA、trusted verifier/workflow SHA 与 baseline schema/derivation compatibility。任一 prerequisite 缺失、非 protected authority、不兼容、protected base 非 Decision 后继或决议 plan blob 漂移即 `WORKFLOW_CAPABILITY_BLOCKED`，不得创建 P0 PR。Decision 不创建/替代 bootstrap 或 trusted-gate PR；它只作本 plan §8.1 决议。
...
5. 确定性故障注入覆盖重复命令、双 runner、派发前后崩溃、PR 创建后未登记、merge 后游标未更新、证据写入失败、陈旧开放 PR、review/e2e 失败、SHA 漂移与 merge conflict，以及 baseline 缺失、authority tuple/blob 不匹配、trusted verifier/workflow 非 protected authority、verifier/schema/derivation 不兼容；断言不越级、不产生两个有效后继 PR、已 merge 阶段不重复执行且 Closure 只归档一次。
...
仅在 Decision 前取得 `DECISION_CAPABILITY_READY` 后，才可创建 Decision PR；其 merge 后还必须在保留该 `decision_plan_blob_sha` 的后继 protected base 取得 `P0_AUTHORITY_READY`，用户的一次 `/consume-plan` 才可从 P0 起按 P0 → P1-A → P1-B → P1-C → Closure 串行运行。每个阶段使用 fresh `origin/main`、独立实施/返工 agent 与独立 PR；前一 PR merge 后才创建下一阶段分支。任一 blocker/major、测试失败、SHA 漂移或 merge conflict 均暂停在当前阶段，修复后以同一 operation key 对账远端状态并从该阶段 exact HEAD 续跑，不跳阶段、不重复派发、不并行消费附录域。Finish Evidence 记录 preflight 的 protected authority tuple、bootstrap/trusted verifier/workflow SHA、compatibility verdict、run ID、每个阶段 PR 与 merge SHA，以及真实运行中的暂停/恢复事件；不得用一次无故障 run 冒充 workflow 故障分支证明。全部实现 PR 收敛后，真实归档入口必须从 Closure PR 的 base tree authority 派生精确 11-ID 与每 finding 完整 expected subject set，确认全部 subject 已有效 Closed/Retired、集合内 `MappedPendingClosure=0`、Retired 证据有效、每个 Closed subject 的 current-HEAD trusted `DifferentialEvidenceReceipt` 可解析且唯一归属，并且 r8 #5 满足 26-subject predicate，Closure PR 才可归档，merge 后流程结束。
