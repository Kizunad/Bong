...
- [ ] P0 同时冻结 `MODIFIER_EFFECT_COMPLETION_CANONICAL_FINDINGS`（名称可等义）为 §Canonical Finding Mapping 的精确 11 个 `CanonicalFindingId`，并冻结 keyed-by-stable-ID 的 `CanonicalLifecycleBaseline`。每个 P1 HEAD 必须与其前一已 merge 阶段 baseline 做机械 transition diff：本集合 subject 只允许同一 stable ID 从 `MappedPendingClosure → MappedGameplayClosed|MappedProducerRetired`，禁止删除/重建、改名换 ID、把 finding 移到新 ID 或以“旧 ID 删除 + 新 ID 新增”绕过；集合外 subject 不得计入这 11 条的完成数。`canonical_lifecycle_transitions_from_baseline_are_pinned` 与 `canonical_stable_id_rekey_cannot_bypass_transition_gate`（名称可等义）分别 pin 合法逐阶段迁移及 rename/rekey/delete+add 失败。
- [ ] 使用互斥生命周期分类：`MappedPendingClosure`、`MappedGameplayClosed`、`MappedProducerRetired`、`ExistingGameplay`、`DormantNoProducer`、`UnmappedObservation`。P0 时本 plan 未闭环的 member/binding subject 合法登记为 Pending，只要求 canonical finding、稳定 subject/producer reachability、owner、目标 P1、预期 consumer domain 与专属测试；不要求尚不存在的 consumer/schedule/differential 先通过。Mapped subject 只能在 Pending、Closed、Retired 间转换，不得改标其他分类消项。Retired 证据按 subject 判定：Member subject 必须无任何 production-reachable writer 与 typed consumer edge，因而无 member→schedule 路径；ProducerBinding subject 只要求该精确 site 已删除/禁用且 site→production 路径为空，不要求删除实际 member 的其他合法 site、consumer 或共享 registration。两者均须绑定删除/禁用证据、迁移版本及持久状态迁移/水合测试 ID（无持久状态时绑定机械证明）。
...
- [ ] **client-hook + server-validation 路线**：从 `server/src/network/derived_attrs_emit.rs` 的 `DerivedAttrs` query、payload 写入与 production send schedule 开始，贯通 `DerivedAttrsSyncV1`、proto/generated、handler/store、非 mixin helper、jump hook 与 disconnect reset；同时必须新增并实际加载 server movement validation consumer（例如 `server/src/movement/jump_validation.rs` 的 `validate_player_jump_motion`，名称可等义），在每个受影响 movement tick 读取 server-side authoritative `(session, revision, effective_tick, multiplier)` 与当前位置/速度/`OnGround` 状态，按 §8.1 锚定 MC 1.20.1 离散物理 envelope/tolerance，对超出 envelope、旧状态或缺失状态执行 reject/correct/fail-closed，并以 observable server outcome 而非只看 payload/store 证明生效。消息采用 session generation + 单调 revision/effective tick 的权威全量状态；同一 session 激活→停用/到期时 server 必须预发布显式 `jump_height_multiplier=1.0` 的更高 revision。§8.1 只能选择以下可收敛交接之一：**ACK 路线**中 server 在收到精确 tuple ACK 前继续按旧 active authority 验证并重传 neutral，超时则主动纠正/断开而不是单方面切 neutral；**future-effective-tick 路线**中 neutral revision 在 effective tick 前按有界 cadence 重传，client 在每次 movement/ACK 上回报当前 tuple，server 到点若未观察到 neutral tuple 则进入 `NeutralReconciliation`，暂停接受新 jump、持续重传并按旧 active envelope 校验/纠正，直到精确 neutral ACK 后双方同 tick 提交 neutral，或超时后 fail-closed 纠正/断开。任一路线都不得在丢包时让 server 已 neutral 而 client 仍 active；字段缺失、旧 revision 或旧 session 不得清除/复活状态。本子合同仅在 §8.1 选择 client 路线时生效：§8.1 还必须冻结 session/revision/effective-tick 的 wire 类型、位宽和比较算法，定义 server/client 进程重启后的 session 唯一性来源，并禁止 revision 静默回绕。revision 耗尽时只能在上述显式 neutral 交接下切换到不可复用的新 session，或停止发送并 fail-closed；ACK/tuple report 仅在 `(session generation, revision, effective tick)` 全部精确匹配时有效，跨 session 的旧 active、neutral、ACK 或 report 均不得改变当前状态。
- [ ] 两路线共同测试 multiplier 1.0/中间/上限与非法输入，并以真实 velocity/apex 而不是 payload/store 值验收；共同覆盖 active→neutral、到期/reset、断线/实体重建/重登、水合后旧状态不复活，以及转换边界前后起跳。server 路线另证明 production schedule 每次读取当前权威倍率且下一次起跳恢复 1.0；client 路线另覆盖旧 active 与 neutral 乱序、重复 neutral、旧 session neutral、neutral 延迟跨 tick、丢失/重传、ACK 迟到/丢失、future-effective tick 到点但 neutral 全丢失、`NeutralReconciliation` 禁止新 jump/旧 active envelope 校验、对账后同 tick neutral commit、reconciliation 超时纠正/断开、effective tick 前后起跳交错、revision max-1/max/耗尽与回绕尝试、server/client 分别重启、session 标识冲突、旧 active/neutral 跨 session 到达及旧 ACK/report 抵达新 session。选择 server-authoritative 路线时，client wire 测试明确记为 N/A，且不得遗留 proto/store 字段。
...
`modifier_archive_rejects_plan_canonical_pending_closure` 必须接入实际 Active→Finished 迁档入口及对应 CI（可扩展 `scripts/plans_progress.py --check` 或使用等价唯一入口），不得只存在为可直接调用的 helper 单测。归档检查的 `MappedPendingClosure=0` 只量化 `MODIFIER_EFFECT_COMPLETION_CANONICAL_FINDINGS` 冻结的精确 11 个 ID；不能用全仓 pending 数替代，也不能让集合外条目抵消/掩盖集合内 pending。集成测试必须证明：11-ID 集合中任一 pending、stable-ID rekey/delete+add、无效 Retired 证据或被擅自扩大的 observation allowlist 均使真实迁档失败；集合外无关 pending 不改变本 plan 判定；仅在这 11 个 ID 全部 Closed 或具备完整 Retired 证据时成功。
...
5. Closure 只能在五个前序 PR 已 merge、`MODIFIER_EFFECT_COMPLETION_CANONICAL_FINDINGS` 的精确 11 个 ID 内 `MappedPendingClosure=0` 且 Retired 证据有效后启动；集合外 pending 不计入也不得掩盖本集合状态。
...
仅在 §10.1 PASS 后，用户的一次 `/consume-plan` 才可按 Decision → P0 → P1-A → P1-B → P1-C → Closure 串行运行。每个阶段使用 fresh `origin/main`、独立实施/返工 agent 与独立 PR；前一 PR merge 后才创建下一阶段分支。任一 blocker/major、测试失败、SHA 漂移或 merge conflict 均暂停在当前阶段，修复后以同一 operation key 对账远端状态并从该阶段 exact HEAD 续跑，不跳阶段、不重复派发、不并行消费附录域。Finish Evidence 记录 preflight 的 `workflow_head`/run ID、每个阶段 PR 与 merge SHA，以及真实运行中的暂停/恢复事件；不得用一次无故障 run 冒充 workflow 故障分支证明。全部实现 PR 收敛且真实归档入口确认冻结 11-ID 集合内 pending 为 0、Retired 证据有效后，Closure PR 才可归档，merge 后流程结束。

## 归档门

- 11 条 mapped finding（含 r8 #5 冻结的 13 个子字段）全部转为 `MappedGameplayClosed`，或按 §8.1 删除/禁用 producer、迁移状态并以机械证据转为 `MappedProducerRetired`；只对 `MODIFIER_EFFECT_COMPLETION_CANONICAL_FINDINGS` 冻结的精确 11 个 stable ID 要求 `MappedPendingClosure=0`，集合外条目不计入也不能掩盖。Member subject 的 writer/consumer edge 必须不可达，ProducerBinding subject 仅要求该精确 producer site 不可达；共享 scheduler 若仍服务其他 member/site 可以保留，迁移证据必须完整。不得以 rekey/delete+add、`UnmappedObservation`、HUD/storage read 或 planned 状态结案。
