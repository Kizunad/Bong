...
| 阶段 | 交付物 | 状态 |
|---|---|---|
| P0 | 11 条 exact finding inventory 与 production-path closure gate | ⬜ |
...
- [ ] 建立可 grep 的 `MODIFIER_EFFECT_COMPLETION_CANONICAL_FINDINGS` exact inventory，逐项列出 11 条 finding、r8 #5 的 13 个冻结子字段、对应 production writer site、唯一 gameplay consumer、真实 scheduler registration 与 differential test selector。该 inventory 只覆盖本表 mapped subjects；新增/删除/改绑 mapped writer 或 consumer 必须由门禁拒绝，未映射字段/variant 仅登记为 `UnmappedObservation`，不得扩成 repository-wide migration。
- [ ] 对每个 mapped subject 建立最小 producer→consumer→schedule contract；允许多个字段共享同一个真实 consumer/system registration，但禁止用 helper、HUD、schema、persistence、client store 或测试读取冒充 gameplay consumer。每个 subject 的状态由其 exact evidence 派生：`MappedPendingClosure` → `MappedGameplayClosed` 或精确 producer 删除后的 `MappedProducerRetired`；不新增通用 subject 生命周期、稳定图节点 rekey/history、全仓 producer/consumer 图或 allowlist 治理子系统。
- [ ] r6 #1 精确登记 JinZhongDan negative branch 的 actual/expected member 与 branch site，P1-A 只改绑该 branch 或删除/禁用该 branch；其他合法 `QiRegenBoost` producer site 不受影响。r8 #5 exact child set 仍需 13 个 member 与 13 个 producer binding 一一对账，`composure_recover_mul` 与动态 marker 只保留为 unmapped observations。
- [ ] manifest 必须把本 plan 的 11 条 canonical finding 及各自 exact mapped subject set 固化为稳定 inventory；父级完成数只能由这些已列 subject 的 evidence 派生。P0 guard 拒绝 mapped subject 缺失、重复、额外或 actual-member 改绑；P1 只可把既有 `MappedPendingClosure` 转为 `MappedGameplayClosed` 或精确 producer 删除后的 `MappedProducerRetired`，集合外 observation 不得计入 11 条完成数。
- [ ] `MappedGameplayClosed` 与 `MappedProducerRetired` 必须按 exact mapped subject evidence 派生。每个 mapped subject 都要保留精确 producer site、consumer、真实 scheduler registration 与专属 observable differential test selector；共享 registration 允许多个字段复用，但不能伪造逐字段 registration。Retired 只证明该精确 producer site 已删除/禁用且必要状态已迁移，不得退役同 member 的其他合法 site 或共享 schedule。HUD、schema、persistence、client store、测试或无 production caller helper 均不满足 closure。
- [ ] r8 #5 只能由 `r8_5_closed`（名称可等义）从 exact inventory 派生：expected set 含 13 个 `InsightModifiers` Member 与 13 个 ProducerBinding（共 26 个），每个 child 必须是有效 Closed 或有效 Retired，Member/Binding 一对一 pairing 成立。13 Binding 只可来自 `apply_choice` 的三个指定 effect arm 与 `apply_tradeoff_cost` 的十个指定 scalar-writing arm，排除 practices marker、r7/阵法字段和 `composure_recover_mul`。任一 child 缺失、重复、额外、Pending 或改绑均由 guard 拒绝。
- [ ] P1 在同一最终 PR 中按 Alchemy/Insight、Iron Cocoon、jump 三组完成每个 exact mapped subject 的 production writer/caller、consumer、schedule 与 observable differential，并把对应 subject 原子转为 `MappedGameplayClosed`；若删除语义，则删除/禁用精确 producer subject、迁移持久状态后转为 `MappedProducerRetired`。Closed/Existing 必须引用权威 producer site、consumer、真实 schedule site 与逐项 differential test ID；未映射 observation 不得计入 11 条完成数。
- [ ] P0 门禁测试放入 `server/src/test_coverage_guards.rs`（由 `server/src/lib.rs` 的 `#[cfg(test)] mod test_coverage_guards` 加载）并由普通 `cargo test` 执行；negative fixture 至少覆盖 mapped writer/consumer/schedule 缺失、actual-member 改绑、r8 #5 child 缺失/额外/Pending，以及普通 inventory 编辑不得接纳新 orphan。只接受标准、非特权 `pull_request` CI 语境；普通编译或无关测试失败不计门禁命中。
- [ ] 可核验 symbol：`MODIFIER_EFFECT_COMPLETION_CANONICAL_FINDINGS`、`MappedModifierSubject`、`ModifierProducerSiteId`、`ModifierConsumerSiteId`、`ModifierScheduleSiteId`、`DifferentialEvidence`、`modifier_consumer_manifest_stays_current`、`modifier_producer_sites_stay_current`、`modifier_schedule_sites_match_production_app`、`jinzhongdan_negative_binding_tracks_actual_and_expected_member`、`r8_5_subject_set_is_exact`、`mapped_pending_requires_closure_target`、`gameplay_closed_requires_scheduled_consumer`、`unmapped_observation_is_not_consumed_by_this_plan`（名称可等义）。
...
- [ ] 实现逐字段冻结单位、neutral、finite 区间、add/mul 顺序、累计上限、消费时点、持久化/水合/reset。表驱动 pin 覆盖合法下界/等号/上界、越界、NaN/±Infinity、组合顺序/cap、持久化往返、水合与 reset/到期回 neutral；r7 五字段与 r8 #5 十三字段各自绑定 exact gameplay consumer 与 differential test ID，多个字段可共享同一真实 scheduler registration，但每个字段都须证明该 production read 已由 app 装载。逐字段以相同 gameplay 输入、仅该 modifier 在 neutral/非 neutral 间变化，断言目标 domain 可观察差分且非目标后果不变，测试表与 18 字段精确集合对账。qi gain/drain 继续断言 `WorldQiAccount` / `QiTransfer` 守恒。
...
- [ ] 选择 **client-hook + server-validation** 时，从 `server/src/network/derived_attrs_emit.rs` 的 `DerivedAttrs` query、payload 写入与 production send schedule 开始，贯通 `DerivedAttrsSyncV1`、proto/generated、`DerivedAttrsHandler`/version-aware jump store、非 mixin helper、jump hook 与 disconnect reset；必须有实际加载的 server movement validation consumer，在每个受影响 movement tick 读取 authoritative tuple 与当前位置/速度/`OnGround`，按冻结的 MC 物理 envelope/tolerance reject/correct/fail-closed。消息采用 session generation、单调 revision/effective tick 的全量 authority state；active→neutral、到期或断线时必须发布更高 revision 的 `jump_height_multiplier=1.0`。`NeutralAppliedAck` 只能证明完整 authority tuple 已安装，`JumpAuthorityTupleReport` 只能报告观测 tuple；ACK/report 两个 C2S variant 必须先加入 `agent/packages/schema/src/client-request.ts` 的 TypeBox source-of-truth union，再同步 `schema-registry.ts`、`generate.ts` 生成的 client-request JSON Schema、命名 shared samples 与重建后的 `@bong/schema` dist，并逐项与 Java encoder、Rust `ClientRequestV1`、proto/generated artifact 对拍。ACK 经命名 `ClientRequestProtocol` encoder、`ClientRequestSender` send site、Rust `ClientRequestV1` decoder、`CustomPayloadEvent` dispatch、typed ACK/report resource/event 与真实注册 server consumer 到达验证链，report 不得替代 ACK。ACK/report 是可选的 client-hook 反馈：vanilla/headless bot 不发送它们时必须保持连接与其它请求可用，超时只使 jump authority fail-closed；malformed、错 session/revision、重复/乱序或未知 ACK/report 必须被安全忽略或拒绝，不得断开连接。丢包、错 tuple、旧 revision/session、重复/乱序、超时或断线都不得错误放行 active jump 或复活旧状态。
...
- [ ] client-hook 路线的真实 client/bot E2E 必须包含 vanilla/headless bot：不发送 ACK/report、延迟 ACK、丢失 ACK，以及 malformed/unknown ACK/report 时连接保持可用、其它合法请求继续处理，而 jump authority 超时/校验失败只 fail-closed；custom client 另覆盖 ACK/report tuple 顺序、重传与 apex 行为。
...
7. P0 只需从 mapped finding rows 生成 exact writer/consumer/schedule evidence；实施者可用声明表、AST 或同等机械检查，但不建立通用 repository-wide graph、稳定节点历史或独立 lifecycle/governance subsystem。
