...
6. **Closure/Archive PR**：从该 PR 的 immutable base tree authority 派生精确 11 个 finding 及各自完整 expected subject set，机械拒绝任一 child 残留 `MappedPendingClosure`、缺失/重复/额外/改绑或手写 parent Closed；每个 Closed subject 还必须从 protected trusted run 取得同时绑定该 Closure `base_sha`/`head_sha`、subject/edge/domain、target/selector 与成功结果的唯一 `DifferentialEvidenceReceipt`；r8 #5 另验证 13 Member + 13 ProducerBinding（26 subjects）的 `r8_5_closed(base,current)` predicate。随后汇总各实现 PR exact HEAD、当前 central-review gate 与 GitHub e2e run URL/ID/result，填写 `## Finish Evidence` 并迁入 `docs/finished_plans/`；不得首次新增 production wiring。

每个实现 PR 都必须：fresh `origin/main` 复验 → production wiring + 饱和测试 → fresh-context exact-HEAD validator → 按所触栈完整 gate → `git diff --check` + `python3 scripts/plans_progress.py --check` → 紧邻 `git fetch origin && git merge origin/main` → HEAD 变化后对新 SHA 重跑 validator 与受影响 gate → push → local/remote/PR HEAD SHA 三方对拍 → 当前 central-review gate 与 CodeRabbit 收敛。任何 review 修改都必须对更新后的 exact HEAD 重走上述闭环。本地严禁运行 `scripts/test-tmux-shutdown-order.sh` 或任何调用它的 suite；该覆盖只留给 GitHub e2e。

- [ ] `modifier_archive_rejects_plan_canonical_pending_closure` 必须升级为真实 PR archive lifecycle gate：它必须在 active→finished rename 的 PR 上运行，取得该 PR 的 base SHA 与 HEAD，读取 base tree authority 并验证 derived `all_11_closed` / `r8_5_closed(base,current)`，以及每个 Closed subject 的 `DifferentialEvidenceReceipt` 在当前 Closure base/HEAD、protected verifier/workflow、target/selector、subject/edge/domain 和成功结果上精确对拍。不得仅挂在可绕过的 helper、`plan-finish.sh`、`plans_progress.py --check` 或 source-path e2e 中。工作流必须监听 active/finished plan rename、guard 实现及 workflow 本身，且 checkout/取 object 的方式必须可读取 base SHA 与对应 blob。集成测试必须证明直接 `git mv`、/consume-plan 内联 `git mv`、helper 外部调用和 docs-only Closure PR 都不能绕过 lifecycle gate。归档检查只量化 base authority 冻结的精确 11 个 finding ID；每个 finding 的完整 expected subject set 必须逐项闭合，r8 #5 额外必须满足 13 Member + 13 ProducerBinding 的 26-subject predicate；集合外 pending 不计入也不能抵消/掩盖 mandatory subject。
...
骨架 → active promotion 后、Decision PR 创建前，runner 必须在该 active plan 的 exact HEAD 上完成并记录以下**人工只读 checklist**；它不创建 worktree、branch、commit 或 PR，不是新的 protocol stage，也不调用 `/consume-plan`：

1. 核对机器阶段合同仍精确为 `Decision → P0 → P1-A → P1-B → P1-C → Closure`，并记录 active plan HEAD 与当前 workflow HEAD；该记录只允许创建 Decision PR，不得声称 P0 authority 已就绪。
2. Decision PR merge 后、P0 PR 创建前，在当前 protected base 上重新完成 checklist：证明该 base 是 `decision_merge_sha` 的后继，且 active-plan 的 `decision_plan_blob_sha` 仍等于 Decision PR 冻结的含 §8.1 决议 blob；随后读取并记录 `protected_ref`、`protected_base_sha`、`decision_merge_sha`、`decision_plan_blob_sha`、baseline bootstrap merge SHA 与 baseline blob/object SHA、archived canonical source tuple、derivation version、tuple digest、trusted verifier SHA、trusted workflow SHA、baseline schema version，以及 verifier/schema/derivation compatibility verdict。
3. baseline/blob/tuple 缺失或不匹配、protected base 非 Decision 后继、决议 plan blob 漂移、trusted verifier/workflow 不在 protected authority、verifier 旧于 baseline schema/derivation version、无法读取 protected base object，或 checklist evidence 不完整时，runner 必须停止，不得创建 P0/P1 PR，也不得把 workflow 缺口计入 modifier manifest。bootstrap/trusted-gate 修复必须由独立 workflow blocker/PR 完成；其 merge 后在保留该 `decision_plan_blob_sha` 的 current protected base 重新执行本 checklist。

`consume-plan-serial-multi-pr-v1` 的外部 capability suite 必须机械证明：

1. 同一时刻至多一个阶段 PR；前一 PR 未 merge 不得派发下一阶段。
2. Decision 前只可完成第 1 项 checklist，不得创建或声称已验证 P0 authority；Decision merge 后、P0 PR 创建前必须完成第 2 项 checklist，冻结 `decision_merge_sha`、未漂移的 `decision_plan_blob_sha`、`CanonicalLifecycleBaseline` authority tuple/digest、bootstrap merge/blob SHA、trusted verifier/workflow SHA 与 baseline schema/derivation compatibility。任一 prerequisite 缺失、非 protected authority、不兼容、protected base 非 Decision 后继或决议 plan blob 漂移即阻止 P0 PR 创建。Decision 不创建/替代 bootstrap 或 trusted-gate PR；它只作本 plan §8.1 决议。
3. 持久 execution ID、单 owner lease/CAS、逐阶段幂等 operation key 与明确 linearization point。
4. 外部副作用采用“先写 intent → 以 operation key 创建/发现 PR → 对账 GitHub 状态 → CAS 提交阶段”的恢复协议，不宣称 PR 创建、merge 与本地游标具有不可能实现的跨系统原子事务。
5. 确定性故障注入覆盖重复命令、双 runner、派发前后崩溃、PR 创建后未登记、merge 后游标未更新、证据写入失败、陈旧开放 PR、central-review/e2e 失败、SHA 漂移与 merge conflict，以及 baseline 缺失、authority tuple/blob 不匹配、trusted verifier/workflow 非 protected authority、verifier/schema/derivation 不兼容；断言不越级、不产生两个有效后继 PR、已 merge 阶段不重复执行且 Closure 只归档一次。
6. Closure 只能在五个前序 PR 已 merge、`MODIFIER_EFFECT_COMPLETION_CANONICAL_FINDINGS` 的精确 11 个 ID 内，每个 finding 的完整 base-derived expected subject set 均逐项闭合，且 `MappedPendingClosure=0`、Retired 证据有效、每个 Closed subject 的 `DifferentialEvidenceReceipt` 对应当前 Closure base/HEAD 与 trusted workflow 成功结果后启动；r8 #5 还必须满足 `r8_5_closed(base,current)` 的 13 Member + 13 ProducerBinding（26 subjects）精确集合 predicate。集合外 pending 不计入也不得掩盖本集合状态。

正常的真实 `/consume-plan <plan-name>` run 只在骨架 promotion 为 `docs/plan-<name>.md` 后作为 happy-path smoke evidence；暂停/恢复与并发安全由上述 workflow-owned deterministic fixture 证明，不要求本 gameplay plan 制造故障。

### §10.2 checklist 完成后的单次消费

仅在 promotion 后完成第 1 项 checklist，才可创建 Decision PR；其 merge 后还必须在保留该 `decision_plan_blob_sha` 的后继 protected base 完成第 2 项 checklist，用户的一次普通 `/consume-plan <plan-name>` 才可从 P0 起按 P0 → P1-A → P1-B → P1-C → Closure 串行运行。每个阶段使用 fresh `origin/main`、独立实施/返工 agent 与独立 PR；前一 PR merge 后才创建下一阶段分支。任一 blocker/major、测试失败、SHA 漂移或 merge conflict 均暂停在当前阶段，修复后以同一 operation key 对账远端状态并从该阶段 exact HEAD 续跑，不跳阶段、不重复派发、不并行消费附录域。Finish Evidence 记录 checklist 的 protected authority tuple、bootstrap/trusted verifier/workflow SHA、compatibility verdict、run ID、每个阶段 PR 与 merge SHA，以及真实运行中的暂停/恢复事件；不得用一次无故障 run 冒充 workflow 故障分支证明。全部实现 PR 收敛后，真实归档入口必须从 Closure PR 的 base tree authority 派生精确 11-ID 与每 finding 完整 expected subject set，确认全部 subject 已有效 Closed/Retired、集合内 `MappedPendingClosure=0`、Retired 证据有效、每个 Closed subject 的 current-HEAD trusted `DifferentialEvidenceReceipt` 可解析且唯一归属，并且 r8 #5 满足 26-subject predicate，Closure PR 才可归档，merge 后流程结束。
...
- Finish Evidence 只在所有实现 PR 的 validator、按栈 gate、`git diff --check`、plan 进度检查、local/remote/PR SHA 对拍、更新后 exact-HEAD central-review gate、CodeRabbit 与 GitHub e2e 全部收敛后填写；记录 commit SHA、validator verdict、命令结果、run URL/ID 与遗留 successor。
