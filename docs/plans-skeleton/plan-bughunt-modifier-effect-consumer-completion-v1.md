...
11. P0 manifest 采用哪一个可机械穷举的权威源（Rust AST、唯一声明宏或同等方案）；如何先生成完整 production writer call-site 集合并排除 test/read，再以全局唯一 typed producer-site ID 对完整字段/variant/key inventory 精确集合对账，区分同函数同 variant 的不同 match arm/branch（例如 `combat_pill_status_intents` 的 `XuGuGao` / `DuanXuSan`），强制唯一 typed constructor，并避免手摘 fixture、文本 grep、函数级 identity、单个合法 writer 或 caller 冒充整项 reachability？
