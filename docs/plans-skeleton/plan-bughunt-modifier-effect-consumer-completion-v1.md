...
- `modifier_consumer_manifest_stays_current` / `modifier_producer_sites_stay_current` 必须从机械权威源穷举最新完整 `DerivedAttrs`、`InsightModifiers`、`StatusEffectKind`、`InsightEffect`、`InsightCost`、typed practices key/prefix，以及各自每个带全局唯一 typed producer-site ID 的 production writer/offer/config/conversion/apply-arm call site，与 manifest 精确一一对账；同函数同 variant 的不同 arm 不得折叠，每个可达 gain/cost 与 writer 绑定 `Gameplay` production caller/schedule + differential test，未知字段/variant/key、no-op/placeholder apply arm、direct string marker、重复 ID、绕过 typed constructor 与新增/遗漏/折叠 site 一律 fail-closed。
