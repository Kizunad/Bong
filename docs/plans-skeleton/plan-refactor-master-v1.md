...
- **UI 语义与呈现必须完全分离**：server/agent 只产生 semantic surface、immutable view data、typed allowed actions 和 authoritative receipts；client 的 owo 主 adapter 与 vanilla 兼容 adapter 只选择本地模板并呈现。bot 复用同一 action registry 和 wire，不通过像素点击、截图、XML/HTML/JS 或 Java 私有 Store 测试 UI 功能；视觉几何另走 viewport matrix。
...
| R7 | `plan-refactor-client-ui-base-v1` | semantic UI surface/headless driver + 库无关 UI contract + owo-first/vanilla-compatible adapters + Screen/Store/Intent/Bootstrap 分层 + InspectScreen 拆解 + responsive viewport | `client/ui/contract/**`、headless driver、owo/vanilla adapters、Screen/HUD/keybind、BongClient UI bootstrap、viewport seam | ~17 |
...
- client：Store 生命周期+`clearClientStateOnDisconnect` 区段=R2；generated bridge+channel/`ServerDataRouter` registration plumbing=R6（与 R2 同文件不同区段，merge 前互 fetch）；R7 独占 `client/ui/contract`、headless projection/driver、owo-first/vanilla-compatible UI adapters、Screen/HUD/keybind/InspectScreen、`UiViewport`/layout policy，以及 `BongClient` 的 UI/HUD/keybind bootstrap call set；第三方宿主和原生 provider 不属于 R7 交付物；cast reducer/store 与具体 domain consumers=R9。semantic wire shape 的 TypeBox/server/agent producer 仍归 R6 及对应 domain owner，R7 不越权改其文件。
...
7. R7 semantic UI surface 不含 XML/HTML/JS/DOM/像素坐标；同一 action registry 可由 Java adapter 与 bot 消费；`UiViewport` 在最低、odd、超宽/超窄/竖屏矩阵下通过 geometry/input contract，且 resize 不重复组件或订阅。
