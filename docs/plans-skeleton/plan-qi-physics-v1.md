...
- [ ] **守恒律（第一公理）**：worldview §二/§十 正典「全服灵气总量 `SPIRIT_QI_TOTAL` 恒定且缓慢衰减；修炼消耗 = 别人少掉」。**`SPIRIT_QI_TOTAL` 是暂定可配置值（当前 const 100.0 是 placeholder）**——所有守恒断言取 const 引用，不写字面 100。代码里所有真元/灵气流动都是**质量流向**，不是数值生成或消失：
...
- [ ] **坍缩渊是质量中转站，不是终点**（worldview §十一 line 789「任由活坍缩渊自然塌缩为死坍缩渊，**高效回收灵气**」）：
  - 活坍缩渊吸进的真元（修士死亡 / 骨币归零 / 入口剥离 §十六.三）暂存在坍缩渊内部账本，**不离开 `SPIRIT_QI_TOTAL` 守恒域**
...
- [ ] **既有代码迁移而非并行**：本 plan P2 阶段把现有散常数全部迁过来调用 `qi_physics`，**不允许"加一个新底盘但旧代码不动"留双轨**
...
**P1 验收**：`grep -rcE '#\[test\]' server/src/qi_physics/` ≥ 40。守恒不变量：跨 24h tick fixture 模拟（不依赖既有代码，纯 qi_physics 内 fixture），`Σall ∈ [SPIRIT_QI_TOTAL - era_decay_accum, SPIRIT_QI_TOTAL]`。
...
pub mod constants {
    ...
    // §十 全服灵气守恒上限(已是双端 const,re-export 不重定义)
    pub use crate::schema::common::SPIRIT_QI_TOTAL;  // 当前 const 100.0,**暂定可配置**(见 §5)
...
- **坍缩渊吸入的真元不消失** —— 暂存于活坍缩渊内部账本，塌缩瞬间 `collapse_redistribute_qi(rift, surrounding_zones)` 按压强法则分发回周围 zone（low-pressure 方向多分），守恒等式 `Σ ≡ SPIRIT_QI_TOTAL`（const 引用，不字面化）在跨坍缩渊事件链上仍闭合
...
- [ ] **`SPIRIT_QI_TOTAL` 是否要从 const 改为 Resource / config**：当前是 server+agent 双端 const 100.0。问题：(1) 运维想调整（小服 50 / 大服 200）需要改源码重编 (2) 未来天道时代衰减实装后，"总量"是缓慢变化的，已经不是 const 语义 (3) 测试 fixture 用 50 跑更快，生产用 100。三选一：**A.** 保持 const + 加 build-time feature flag 切换；**B.** 改成 `Resource<WorldQiBudget>` 由配置文件初始化，agent 端通过 IPC 同步；**C.** 暂保留 const 但 P3 时代衰减实装时再迁。**默认推荐 B**——既支持运维调整又承担时代衰减语义；P0 留接口，P1 落地
