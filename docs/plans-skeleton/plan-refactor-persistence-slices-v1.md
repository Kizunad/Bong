...
7. `session_delivery_outbox_atomicity`：分别在 outbox insert 前、insert 后 checkpoint terminalize 前、事务 commit 后 ack 前强杀重启；断言 checkpoint/outbox 恰有一个权威 owner，完整 payload 不丢且 stable `delivery_id` 不变。
