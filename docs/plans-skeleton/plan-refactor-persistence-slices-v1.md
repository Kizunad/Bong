...
7. `session_delivery_outbox_atomicity`：执行 R1 O-01..O-27 的 durable traces；覆盖双 connection quota race、busy loser 的 O-25（O-05 标记未提交）或 O-05→O-07/O-06（标记已提交）、restore O-04、O-01 固定 1 MiB reservation、payload exact-max/+1 且无 resize、S-07 checkpoint/state commit 前后 crash、handoff crash、attempts 7/8 与 retry age 前一 tick/精确边界/后一 tick、payload digest mismatch、history quota O-26、O-16/O-19 atomic release、O-17 stale replay 不二次 release，以及 O-21→O-24 bounded retention/GC。每步断言 `quota_used = sum(active Q)`。
