...
- **已交付**：heaven_gate charge 前兆 = 复用 release 签名 ogg 的压调层（pitch 0.72/vol 0.4），已接进 charge 招式实际消费的专属 server recipe `heaven_gate_charge`（`sword_path_recipe_for_skill(HeavenGateCharge)` 指向它 + 运行时消费 pin 覆盖），未单独取音。
