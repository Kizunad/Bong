...
- **P5 emit 架构统一（zhenmai/dugu/tuike-active 签名 emit-path 补测）✅ 2026-07-25**：三处内联 emit（Pattern B）已重构为 Pattern A 独立系统 + 补齐 emit-path 集成测试，**9 招签名 emit-firing 全覆盖**：
  ...
  - **音源时序契约**：zhenmai 音源**无条件取事件 `center`（cast-time 位置）**，刻意不查消费时实时 `Position`——重构前内联 emit 锁的就是施法当时位置，读实时位置会随跨帧消费 / 玩家移动漂移且依赖未声明的系统顺序（PR #1262 review 指出）。dugu 倒蚀同理锚 `event.center`（爆发中心）。
