...
- **跨端契约测试**：server `audio::signature_recipes_reference_registered_bong_events`（recipe `bong:` 引用 ⊆ client sounds.json 键）+ `each_server_signature_recipe_swaps_l0_main_layer_to_its_bong_event`（server 7 招锁 **L0 主层** sound + pitch=1.0，非「任意层」）；client `SignatureAudioContractTest`（13 用例：sounds.json↔ogg 双向 + 8 signature 全注册 + **heaven_gate L0 主层 pin**（`heavenGateRecipesSwapL0MainLayerToTheirBongEvent`：release L0 pitch 1.0/vol 0.9、charge_2s L0 pitch 0.72/vol 0.4）补齐 8/8 主层锁 + **错误分支**：重复事件键 token 级检测 + 非法命名空间前缀拒绝 + **committed manifest 覆盖** + **音频格式机器 pin**：手写 Ogg Vorbis 结构探针逐条断言真资产 codec=Vorbis/channels=1/sample_rate=44100/duration≤3s **且容器完整**（id header 30 字节含 framing bit + 末页 EOS + granule>0，挡截断/无音频伪文件），配合成越界 fixture（stereo/48kHz/超长/缺EOS/granule=0/截断header/伪OGG/非Vorbis）证明错误分支判红）。
