...
- **P5 emit 架构统一（zhenmai/dugu/tuike-active 签名 emit-path 补测）✅ 2026-07-25**：三处内联 emit（Pattern B）已重构为 Pattern A 独立系统 + 补齐 emit-path 集成测试，**9 招签名 emit-firing 全覆盖**：
  ...
  - **tuike 主动**：`cast_shed` 内联 emit 直接删除——主动/被动都走 `shed_outer_layer` 发的 `FalseSkinSheddedEvent`，由既有 `emit_tuike_v2_audio_triggers` 统一发 `SHED_SKIN_BURST_RECIPE`；内联那份实为**同 recipe 重复发声**（变异验证：加回内联即出两条 `shed_skin_burst`）。
  - **接线门禁**（PR #1262 review 补）：audio-trigger 调度提取为**唯一生产注册入口** `network::audio_trigger::register`（`network::register` 只留一行委托），测试跑生产函数而非另抄系统清单——`register_wires_all_audio_trigger_systems`（19 个 emit 系统 + dedup 时钟逐个在调度里）/ `production_module_registers_install_signature_cast_events`（三条链的事件由各自模块生产 `register` 装好，缺 `add_event` 则 `send_event` 静默丢弃）。两门均已变异验证（删注册即撞红）。残余接缝：`network::register` 里那一行委托调用无法在单测覆盖（会拉起 Redis bridge 线程），不宣称覆盖。
  ...
  - **音源 / 路由契约（逐字段对齐重构前，PR #1262 review 两轮抓出）**：
    - zhenmai 音源**无条件取事件 `center`（cast-time 位置）**，刻意不查消费时实时 `Position`——重构前内联 emit 锁的就是施法当时位置，读实时位置会随跨帧消费 / 玩家移动漂移且依赖未声明的系统顺序。
    - dugu 倒蚀签名走 `emit_play_listener_anchored_broadcast`（`pos: None` 听者位置发声 + 以爆发中心为圆心 64 格广播），与重构前 `dugu_v2::skills::emit_audio` 逐字段一致；**不能**走通用 `emit_play`——`dugu_poison_signature` 声明 `MELEE`（8 格，比该招自己 10 格的 `ReverseAftermathCloud` 还小）+ 世界锚点距离衰减 + L0 volume 0.24 = 实机几乎听不见（P4 已吃过两次的「签名进了资产却听不到」）。测试断言 `pos == None` 且 recipient 为 64 格广播，已变异验证（换回 `emit_play` 即撞红）。要不要把倒蚀改成空间化签名（须同步调 recipe attenuation/volume）留 P5 盲听回归再定。
    - 已知且可接受的两处路由差异：① zhenmai 收听半径由内联硬编码 32 格改为 recipe 声明的 `world_3d` 64 格（只放宽「谁收到包」，pos 与距离衰减不变，实际可听距离由 volume 决定，不变）；② 音源取整由 `as i32` 截断改为 `floor()`（负坐标差 1 格，新写法才是正确的 block 坐标语义）。
