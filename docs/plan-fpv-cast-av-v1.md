...
- **门控硬约束**（无论 §8 #3 选哪条数据源）：release juice 必须绑定**已确认 accepted 的 cast identity**——`CastStateStore` 中该次施法处于 accepted 且未被拒绝/打断才触发；server 拒绝（`reject_*` 回执）、打断、取消回执到达时作废对应 pending juice（取消令牌语义）。本地计时器到点但确认缺失/已作废 → 不触发。同一 cast identity 的 release juice 幂等（重复回执/乱序不二次触发）。
  ...
  - 幂等与防复活按 identity 存**有界**终态记录（`CastFovController.Terminal{FIRED, VOIDED}`，LRU 上限 `TERMINAL_MEMORY=16`），先到者胜；teardown 把在飞身份整批记 VOIDED。`IDLE` **不清场**——`CastState.idle()` 是 slot=-1/startedAtMs=0 的无身份单例，无条件清场会让迟到的 IDLE 误杀在飞施法；服务端发 Idle 的 4 个站点（race gate / 经脉门两处 / 统一的 `push_skill_cast_rejected_sync`）里，只有后三站会被 `CastSyncHandler` 合成成 INTERRUPT 走取消令牌；**race gate 那一站确实以 IDLE 形态到达**——client 的 `CastSyncHandler.parseOutcome` 没有 `reject_race_mismatch` 分支（client `CastOutcome` 枚举无 race 变体），落回 `default -> NONE`。**安全性不挂在「它到不了」上，而挂在 no-op 本身**：这几站全是施放前拒绝，client 从未收到该次 cast 的 CASTING → 没有 pending / 令牌可杀；no-op 语义又保证它不会顺手误杀别的在飞施法。（附带发现，不在本 plan 范围：race gate 拒绝因此也拿不到技能警示 HUD 文案，属 plan-skill-warn-hud 的 client 解析缺口。）
