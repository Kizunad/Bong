...
- **P5 emit 架构统一（zhenmai/dugu 签名 emit-path 补测）**：签名音效 emit 现有两套架构——**Pattern A**（独立 `emit_*_audio_triggers` 系统读 cast 事件 → 映射 → 发 `PlaySoundRecipeRequest`：sword_path/anqi/baomai/woliu，均已有 emit-path 集成测试）与 **Pattern B**（emit 内联在 cast 逻辑里：zhenmai `emit_skill_feedback` / dugu `emit_audio(cast_eclipse)`，需全套 combat 环境难驱动，暂无 emit-path 测试）。P5 把 zhenmai/dugu 的签名 emit 重构成 Pattern A 独立系统（既消掉架构不一致、又可补 emit-path 集成测试锁「删掉发声调用撞红」），届时 9 招 emit-path 全覆盖。当前 zhenmai/dugu 的 recipe 内容正确性已由运行时消费 pin（经 registry）守护，仅缺 emit-firing 的独立门。
