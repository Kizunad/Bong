...
- **resourcepack**：`scripts/build-resourcepack.sh` INCLUDE_PREFIXES + audio 子包均纳入 `bong/sounds` + `bong/sounds.json`（`count_files` 支持文件前缀计数，audio 子包 file_count 66）；`scripts/test_build_resourcepack.py` 加 fixture + audio 子包路径/计数断言；`server/src/network/resourcepack.rs` sha1/size 同步为新构建值（sha1 `6931ce03…`，size 72_501_963；`publish-release` job 在 merge 到 main 时自动重建 + 上传 release 资产）。
- **跨端契约测试**：server `audio::signature_recipes_reference_registered_bong_events`（recipe `bong:` 引用 ⊆ client sounds.json 键）+ `each_server_signature_recipe_uses_its_bong_event`（server 7 招逐项 pin）；client `SignatureAudioContractTest`（13 用例：sounds.json↔ogg 双向 + 8 signature 全注册 + **第 8 招 heaven_gate recipe 逐项消费 pin**（release + charge_2s）补齐 8/8 消费锁 + **错误分支**：重复事件键 token 级检测 + 非法命名空间前缀拒绝 + **committed manifest 覆盖**（audio 子包声明须覆盖每个源 ogg 与 sounds.json）+ **音频格式机器 pin**：手写 Ogg Vorbis 探针逐条断言真资产 codec=Vorbis/channels=1/sample_rate=44100/duration≤3s，配合成越界 fixture（stereo/48kHz/超长/伪 OGG/非 Vorbis）证明错误分支判红）。
- **遗留**：音色为程序化能量选段（无法试听），需 P5 盲听把关 + 按需换源；heaven_gate charge 尾程复用 release 签名的压调前兆层（未单独取音），P5 盲听时若前兆感不足再单独取一条 charge 专属音。
