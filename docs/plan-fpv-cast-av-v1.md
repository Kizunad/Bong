...
| P3 | 施法瞬间 juice：重型招释放 shake/FOV 脉冲（按招参数化） | ✅ 2026-07-23（纯 client，零 wire 变更）：`CastJuiceProfiles` 5 重型招注册表 + `CastFovController` 生命周期状态机（cast 转换驱动 arming/幂等 release/取消令牌作废/死亡·断线 teardown）+ 复用 `CameraShakeController.triggerDirect` + `MixinGameRenderer` 加法 FOV 合成；`CastFovControllerTest` 16 饱和用例（真实 CastSyncHandler 入口驱动，全路径断言基准 FOV；含 shake 倍率烘焙 vs FOV 每帧乘算的不对称覆盖） |
