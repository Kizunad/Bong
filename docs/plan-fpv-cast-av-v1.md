...
- **门控硬约束**（无论 §8 #3 选哪条数据源）：release juice 必须绑定**已确认 accepted 的 cast identity**——`CastStateStore` 中该次施法处于 accepted 且未被拒绝/打断才触发；server 拒绝（`reject_*` 回执）、打断、取消回执到达时作废对应 pending juice（取消令牌语义）。本地计时器到点但确认缺失/已作废 → 不触发。同一 cast identity 的 release juice 幂等（重复回执/乱序不二次触发）。
  ...
  - 幂等与防复活按 identity 存**有界**终态记录（`CastFovController.Terminal{FIRED, VOIDED}`，LRU 上限 `TERMINAL_MEMORY=16`），先到者胜；teardown 把在飞身份整批记 VOIDED。`IDLE` **不清场**——`CastState.idle()` 是 slot=-1/startedAtMs=0 的无身份单例，无条件清场会让迟到的 IDLE 误杀在飞施法；判据是 **client 侧能不能解析出 outcome**，不是服务端发了什么：服务端发 Idle 的 4 个站点（race gate / 经脉门两处 / 统一的 `push_skill_cast_rejected_sync`）虽然都带非 NONE outcome，但 `CastSyncHandler.parseOutcome` 解析不出的一律 `default -> NONE` → **以 IDLE 形态到达**。当前唯一解析不出的是 `reject_race_mismatch`（client `CastOutcome` 无 race 变体），它有**两条**来源：race gate 那一站直接发，以及 `reason.to_cast_outcome()` 把 `CastRejectReason::RaceMismatch` 映射成同一个（剑道五招全标 `RaceGate::Humanoid`，heaven_gate 自己就能走到）。**安全性不挂在「它到不了」上，而挂在 no-op 本身**：这几站全是施放前拒绝，client 从未收到该次 cast 的 CASTING → 没有 pending / 令牌可杀；no-op 语义又保证它不会顺手误杀别的在飞施法。**故这条设计不依赖「哪些 outcome 能被解析」这个会漂的事实**——将来给 race 补上解析分支（改走 INTERRUPT）也不影响正确性。（附带发现，不在本 plan 范围：race gate 拒绝因此也拿不到技能警示 HUD 文案，属 plan-skill-warn-hud 的 client 解析缺口。）
...
- 测试（饱和覆盖状态机，**从真实入口驱动，不直接调 controller 方法**）：参数表**测试侧字面量**逐招逐字段 pin（含注册集合精确相等、三档常量、动画段两组参数与包络）+ 状态转换全路径——正常 release 完整脉冲后自然回基准、**仅本地预测（缺权威 CASTING）零 juice**、server 拒绝不触发、晚到打断作废 pending juice、回执乱序（打断先于 started 到达 / `INTERRUPT(A)→INTERRUPT(B)→CASTING(A)` 不复活 / 迟到 `IDLE(A)` 不杀 B / 同身份跨 IDLE 重放只触发一次）、同 cast identity 重复 release 幂等、**supersession / TTL 过期 / 容量淘汰后旧身份落 VOIDED 且迟到 CASTING 不得复活**、连续施法（前一脉冲 decay 中开新 cast）、重叠触发合成、倍率 0 进行中立即复位、**倍率 0 期间的动画事件仍被一次性消费（恢复后重放不诈尸，charge/release 各一条）**、死亡/切世界/断线复位 + teardown 后旧身份不可重新武装、动画事件路径的令牌门（无 accepted 零 juice / 重复 charge·release 各一次 / 取消·teardown 后迟到动画不触发 / **teardown 后同身份迟到 CASTING 不得重开令牌** / **打断一次在飞施法不牵连队列里另一次的令牌** / TTL 两侧边界 / **连续同招施法各拿各的令牌不串用（release 与 charge 两段各一条）** / **release 动画条数不得超过 accepted 施法数** / **队列有界、消费即出队、容量淘汰的身份不得被迟到 CASTING 重新武装**）、**共享 shake 通道所有权**（关施法震感不误清命中抖动 / teardown 无差别清场 / 命中 juice 不受本倍率门控）、**非 SKILL_BAR 来源（QUICK_SLOT）的权威 CASTING 不得武装同 slot 重型招的 juice**、**契约常量字面量 pin**（`HEAVEN_GATE_SKILL_ID` 逐字对齐 server `SWORD_PATH_HEAVEN_GATE_ID` / 有界性上限 `TERMINAL_MEMORY`、`ANIM_TOKEN_CAPACITY`）、**同身份重复权威 CASTING 不得多压一枚令牌**、**时钟回跳令牌失效并落 VOIDED**、**施法抖动的偏航与俯仰分量必须同时非零**（取对角方向的意义；任一分量写成 0 会退化成纯左右/上下晃而幅度类断言全绿）、真实 `VfxEventRouter` 路由接线——每条路径末尾断言 FOV == 基准值。
...
> **⚠️ 本节三条已被 §P3 实施修订（PR #1249 二/三轮返工），原文按 `docs/CLAUDE.md §5.1`「原表保留作历史回溯」留存，但读到这里请以 §P3 为准**——照原文实施会把已修掉的两个 bug 重新引回来：
>
> | 本节原文 | 现行口径（§P3） | 为什么改 |
> |---|---|---|
> | 决议 1「`phase==CASTING` 即已 accepted」 | **只认 `CastStateStore.Origin.SERVER_AUTHORITATIVE` 的 CASTING** | `SkillBarKeyRouter` 在按键那一刻就写一条本地乐观预测 CASTING（服务端还没回话），`CastStateStore.tick` 还会按本地计时把它推到 COMPLETE。只看 phase 会把「服务端压根没接受的施法」当成 accepted，本地计时到点就放 juice |
> | 决议 3「身份 = `(source, slot, startedAtMs)` **三元组**」 | **`(slot, startedAtMs)` 二元组，刻意不含 `source`** | `source` 不是 wire 字段，`CastSyncHandler.sourceFor` 靠「当前快照是否正 CASTING 在同一 slot」推断，任何非 CASTING 快照落地后即退化成 QUICK_SLOT；写进身份会让「A 被 B 取代 → 迟到 INTERRUPT(A) → COMPLETE(B)」里的 B 认不出自己的 pending。`source` 的门控作用保留在 **arming 时刻**（`resolveSkillId` 只给 SKILL_BAR 解析 profile） |
> | 决议 3「pending juice 必须绑定当前快照身份并**每 tick 从 store 重解算**」 | **事件驱动**：`CastStateStore` 的带来源转换回调武装 / 触发，client tick 只推进 decay 与死亡 teardown | 每 tick 重解算读的是「当前快照」，迟到 / 乱序回执一旦改写快照就会误判归属；按 identity 记 pending + 有界终态记录（`Terminal{FIRED,VOIDED}`）才能同时做到幂等与防复活 |
>
> 决议 1 的其余部分（**采纳纯 client、拒绝 server juice hint、wire 零变更**）与决议 2 的逐路径到达保证核验**仍然有效**，未被修订。
