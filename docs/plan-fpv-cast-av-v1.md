...
- **P5 emit 架构统一（zhenmai/dugu/tuike-active 签名 emit-path 补测）✅ 2026-07-25**：三处内联 emit（Pattern B）已重构为 Pattern A 独立系统 + 补齐 emit-path 集成测试，**9 招签名 emit-firing 全覆盖**：
  ...
  - **emit-path 测试**：`network::audio_trigger` 的 `zhenmai_skills_emit_their_mapped_recipes`（五招逐招；注：multipoint 与 harden 在既有映射里共用 `zhenmai_shield_hum`，本测试锁的是「每招发它映射到的那条」，不宣称五条两两不同）/ `zhenmai_audio_falls_back_to_event_center_without_position` / `dugu_reverse_emits_signature_recipe` / `dugu_basic_skills_and_reverse_signature_do_not_cross_talk`；**端到端**（真跑一次施法 + 真跑音效系统）`combat::zhenmai_v2::tests::sever_chain_cast_emits_signature_recipe_end_to_end`、`combat::dugu_v2::tests::reverse_cast_emits_signature_recipe_end_to_end`、`combat::tuike_v2::tests::active_cast_shed_emits_signature_recipe_exactly_once_end_to_end`。
