...
- **HUD 现状（2026-09-05）**：生产链路由 `HudRenderCallback.EVENT` → `BongHud.render` → `BongHudOrchestrator.buildCommands` → `BongHud.renderCommands(DrawContext)` 提交；`HudRenderRegistry` 当前登记 59 个 `HudRenderLayer` 与 4 个无 layer 的直接 overlay，共 63 个登记 surface，其中 `MINI_BODY`、`QUICK_BAR` 两层已接入生产 SVG 命令（视觉终验待人工确认）。幻觉 overlay 自身已有 layer，不重复计数；登记数量不代表全部仍有活跃调用，例如涡流 charge/cooldown 待条件 HUD 批次复核。全 client 现有 49 个 `*HudPlanner`；测试专用 `renderSurface` 已删除。SVG 基础设施和显式示例预览已实现，client 尚无 NanoSVG/JNI/native 依赖或跨平台 native 打包体系。

## Pre-P0 Decisions（2026-09-02）

### HUD 统一 SVG 表现后端

**范围裁剪（2026-09-05，用户决议）**：基于代码探索决定移除 `QI_RADAR` 整项功能（删除前提交 `474d9d93a^` 的历史入口为 `client/src/main/java/com/bong/client/BongHud.java:80-86`，语义规划器为同提交中的 `client/src/main/java/com/bong/client/hud/QiDensityRadarHudPlanner.java:27-67`，登记/资源入口为同提交中的 `client/src/main/java/com/bong/client/hud/HudRenderRegistry.java:21-32`）；详见本计划 §9.2 决议索引。裁剪内容包括旧 `QiDensityRadarHudPlanner` 的浓度标记、周边气息白点、TSY 假信号，SVG 阵盘资源与专用 `SvgHudFrame`/`SvgHudFramePlanner`，以及 layer、布局、沉浸模式和 registry 登记。同步删除恢复雷达主路径的骨架，不再把恢复该 HUD 作为 P6 交付物。共用 `ZoneState`、境界门、`PerceptionEdgeState` 和威胁指示继续服务现有功能；方向条在下述常驻裁剪决议中移除，server/schema/wire 不变。
...
**常驻 HUD 裁剪（2026-09-05，用户决议）**：常驻只保留 `MINI_BODY` 与 `QUICK_BAR`。删除连接常驻文字、身份角标、区域常驻文字/灵压数值、顶部方向条和连接色点，不留兼容绘制壳。原 `BASELINE` 中的超载告警独立登记为 `OVERWEIGHT`；原 `ZONE` 中的切区标题/跨维度黑场独立为 `ZONE_TRANSITION`。条件反馈、领域 Store、身份面板、网络连接诊断和断线 Toast 保留；server/schema/wire 不变。

**当前 SVG 进度（⏳，视觉终验待人工）**：`MiniBodyHudPlanner`、`QuickBarHudPlanner`、`WeaponHotbarHudPlanner` 输出库无关 `VECTOR` 命令，registry 将逻辑 key 绑定到 `fill.svg` 和 `mini-body.svg`；`SvgHudBackend.renderVector` 在原命令顺序提交 Minecraft GUI mesh。动态变化包括真元/体力高度、伤势/护甲/部位效果、季节颜色、当前槽位、武器耐久、冷却遮罩；独立的 `CAST_BAR` 暂仍使用 GUI 矩形。帧间只改变实例尺寸/颜色/可见性，资源重载预热解析和三角化，成功/失败结果均缓存。SVG 首轮保留原方块人体造型，布局尺寸仍由 planner 计算，不能宣称所有 HUD 的几何/布局已经数据化。窄屏人体抬到双排快捷栏和武器槽上方。

**预览与证据范围**：`example.svg` 仍只在 `BONG_SVG_HUD_PREVIEW=1` 时加载和绘制。新 `HudCorePreviewScene` 仅由 `BONG_UI_PREVIEW_HARNESS=1` + 显式配置激活，固定快照驱动真实生产 planner 和 GUI 提交器，不写领域 Store。它不是新的生产 Screen，不计入 XML Screen 数量，也不是服务端联机验证。
`client/hud-core-preview.json` 定义满值/受伤低值/恢复三态，在最小、奇数、宽屏、竖屏、GUI scale 1/2/4 共 18 张真实 framebuffer 截图上验证；等待加载遮罩退出及实际绘制后才采集。`client/tools/hud_core_contact_sheet.py` 检查条高、伤势清理、槽位和冷却像素差分，拒绝空白/冻结/状态残留注入，生成同取景三态接触表。截图是本地生成物，不纳入生产资源。当前只到人工视觉审阅门，尚未 Round 3、提交或开新 PR。

**本轮测试调整**：删除随退役方向条、身份角标、旧区域 renderer 和测试专用 `renderSurface` 失去消费者的测试；一并删除已下线的 dropped loot HUD planner（世界空间掉落物仍保留）。连接状态/Toast、切区淡出/黑场、超载和威胁契约保留；布局/沉浸代表改为保留的 BOTANY 或 MINI_BODY。人体/盾牌测试从矩形数量改为动态填充几何，新增 `HudCoreDynamicTest` 锁住冷却到期、施法进度、选中槽位、透明度和窄屏不重叠；资产加载/缓存测试继续保护 reload。

**本地验证（2026-09-05）**：Java 17 下 `bash scripts/build-token.sh gradle test build --console=plain` 通过：5100 单测、0 失败/错误/跳过，3 GameTest 全通过。`runClientUiPreview` 两轮各 18 张通过；资源预热调整前后 18/18 整图像素一致，差分检测及缺陷注入均通过。Round 1 产物在 `client/run/hud-core-screenshots/`，Round 2 在 `client/run/hud-core-screenshots-round2/`，`contact-sheet.png` 含上一轮同取景对照，人工审阅未完成，不计为最终视觉验收。

复现本轮截图：Java 17 环境下设置 `BONG_UI_PREVIEW_CONFIG` 为 `client/hud-core-preview.json` 的绝对路径，运行 `xvfb-run -a --server-args="-screen 0 1600x1200x24" bash scripts/build-token.sh gradle runClientUiPreview --console=plain`；该 Gradle 任务显式启用 UI preview。随后运行 `python3 client/tools/hud_core_contact_sheet.py --config client/hud-core-preview.json --shots client/run/hud-core-screenshots-round2 --output client/run/hud-core-screenshots-round2/contact-sheet.png`；若有 Round 1 产物，加 `--previous client/run/hud-core-screenshots` 生成前后对照。工具仅拼接真实截图，不生成 HUD 替代图。

**测试调整记录**：删除退役功能专用的 `QiDensityRadarHudPlannerTest`、`SvgHudFramePlannerTest` 和 orchestrator 的雷达停用断言；删除雷达资源登记的重复断言与易漂移的 surface/三角形数量断言。完整登记仍由 `HudRenderRegistryTest` 对拍枚举、直接 overlay 和 TSV；布局、沉浸渐隐/Alt 预览测试改用保留的 BOTANY 或人体 HUD；资源加载与缓存测试合并到 `example.svg`，继续覆盖失败缓存和资源重载。测试范围随功能裁剪收窄，不保留失去生产消费者的旧契约。
...
- `ui-svg-hud-inventory.tsv` 逐行登记全部 59 个 `HudRenderLayer`、49 个 `*HudPlanner`、直接 HUD overlay（爆脉、虚蚀、幻觉、共鸣锁等）、对应 SVG asset、dynamic binding、文字/物品 GUI 例外和测试 owner；允许多个 layer 复用同一 SVG asset，但每个 layer 必须有独立可核验 binding。
...
- **测试**：source gate 禁止 raw network/sender imports、vanilla Screen/widget imports 和 owo CODE root；keybind physical duplicate/vanilla reservation/UNKNOWN；thread already-on/off-thread/null executor；open policy 35 条 vectors；fill geometry 与 clearChildren identity tests；resolution matrix 全尺寸/GUI scale 的 no-overlap/no-clipping/in-bounds/text-fit/hit-test；resize 不重复组件或订阅；`NanoSvgParserTest`、`SvgTessellatorTest`、`MinecraftGuiMeshEmitterTest` 覆盖白名单、malformed/unsupported/parser failure、图元/opacity、NaN/Infinity/预算拒绝和 triangle→degenerate-quad；Java 17 full gate。
...
- **交付**：`offer_id` 不丢失；distinct offer + reused trigger 不合并；exact offerId claim/compare-and-clear exactly-once；social invite 读取 server-authoritative combat snapshot；UI registry 清单与 BongClient call set 收口；59 个 layer 和直接 overlay 统一经过同一个 `HudRenderBackend`，动态文字/物品图标保留明确的 Minecraft GUI 例外。
...
纯 client 逻辑与 UI adapter 重构，不产出 NBT、worldgen layout、模型或贴图；允许新增 runtime SVG 资源、受限 Java parser adapter、Java tessellator、GUI emitter 及截图 fixture。后续 native provider 需另立交付并单独完成平台打包门禁。SVG HUD 资源按视觉资产纪律执行 3 轮 screenshot review，终轮 commit 需带 `<PROMISE>`。每个逻辑单元使用中文 atomic commit，并带真实执行模型 `Model:` trailer；不改 `docs/worldview.md`、`docs/library/`、schema shape 或依赖版本。
...
- 59 个 `HudRenderLayer`、49 个 planner 和全部直接 HUD overlay 均经同一 `HudRenderBackend`；奇怪分辨率 `320x240`、`401x241`、`683x384`、超宽、竖屏及 GUI scale 1-4 的 screenshot harness 证明 framebuffer 非空、mesh bounds 不越界、alpha/layer order 正确；旧 primitive DrawContext 分支、`renderSurface` 与生产 fallback 已删除。
