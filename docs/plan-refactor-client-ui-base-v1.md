...
- ⬜ **P4 Screen 迁移批次 B + R7-owned UI thread/open-policy 强制 + 删旧**：验真四个 `client.execute` consumer；先在第一个真实 R7-owned UI/HUD consumer 接入 `ClientThreadMarshal`，再在第一个真实 insight/open-policy consumer 接入 `ScreenOpenPolicy`；协调 `InsightOfferHandler`/`HeartDemonOfferHandler` → `InsightOfferViewModel.offerId` 接缝并把 Insight settlement 接 `ScreenTransitionController`；`BongHudOrchestrator` 恢复 qi radar main path。
