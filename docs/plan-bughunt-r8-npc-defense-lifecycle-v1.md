...
1. `LifecycleState::Alive` 是 NPC 主动战斗行为的前置条件；`NearDeath`、`Dead`、`Reviving` 等非活体状态不能参与战斗决策。
...
1. NPC 进入 `NearDeath` 后仍可能保留或获得 `NpcDefenseAction` 的 `Requested` / `Executing` 状态。
2. 当前 `npc_defense_action_system` 查询不到 `Lifecycle`，无法区分 `Alive` 与 `NearDeath`。
3. `Requested` 分支只检查境界、真元、`ParryRecovery`，非活体 NPC 仍可进入 `Executing`。
4. `Executing` 分支按间隔直接发送 `DefenseIntent { defender, issued_at_tick }`。
5. 下游防御解析会把该 `DefenseIntent` 当成真实 parry 窗口处理，导致 NearDeath NPC 仍能格挡。
