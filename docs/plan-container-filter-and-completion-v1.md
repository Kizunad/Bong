...
7. **station lifecycle**：专用 forge break 路径先于 `world::block_break::apply_default_block_break` AIR 写入；被 reservation 或 `active_attempt_id` 占用一律 typed busy/reseved reject。空闲 station 才在 transaction 写 Removed+revision 后删 ECS/block。startup 从 Active rows重建 durable index/ECS；Prepared attempt 从 escrow hydration，terminal settlement 只能由 `InventorySettlementRecovery` 消费 WAL，二者不得相互替代。
