...
- **进料**：
  ...
  - `server/src/inventory/mod.rs::{apply_death_drop_on_revive, apply_death_drop_to_inventory, transfer_remains_to_looter, apply_treasure_activate}`、`server/src/inventory/tsy_death_drop.rs::apply_tsy_death_drop`、`server/src/combat/resolve.rs`、`server/src/forge/artifact_meridian.rs`、`server/src/network/client_request_handler.rs::handle_external_container_move`、`server/src/mineral/inventory_grant.rs::consume_mineral_drops_into_inventory` 与 `server/src/forge/inventory_bridge.rs::forge_outcome_to_inventory`：死亡、遗骸、法宝停用、装备破损、player↔world ExternalContainer、矿物掉落与锻造产物也必须通过 P1 typed prepared ingress，不得保留 first-fit/live-write 旁路。`break_handler.rs` 的 `MineralDropEvent` 是矿物生产 caller（dev `/give` 复用同入口）；TSY search `world/tsy_container_search.rs` 已经走 `add_existing_item_to_player_inventory`，只列为已覆盖，禁止另造路径；spiritwood/botany/craft_emit 的 `_or_ground` 路径仍必须共用 prepared authority，NPC/world 自产不计玩家 ingress。
...
6. `resolve_item_category(item: &ItemInstance, item_registry: &ItemRegistry, mineral_registry: &MineralRegistry) -> Result<ItemCategory, ItemCategoryResolveError>` 是唯一类别权威：已注册 template 始终取 `ItemRegistry` canonical category；未注册 template 仅在 `mineral_id=Some(canonical)`、`MineralRegistry` 命中该 canonical 且 `template_id == format!("mineral_{canonical}")` 三者同时成立时返回 `Mineral`。prefix spoof、unknown mineral、template/mineral_id mismatch 都返回 typed invalid，绝不 fallback `Misc`。`item_passes_filter`、move/swap、所有 ingress（尤其 mineral/forge/external）和 P4 `InventoryItemViewV1.category` 都必须调用它；client 不得凭 prefix 猜测。
7. `commit_prepared_inventory_mutation(self)` 消费 prepared self 且不可失败：rebuild/spill 只在 staged inventory 上完成，prepare 预校验 dimension/world position/spawn context，任何 prepare 失败均零 live/drop/outcome 副作用；commit 一次替换库存、为每个 overflow remainder（包括 merge 后余量）保留原 `ItemInstance.instance_id` 并恰好 enqueue 一次 drop、发布一次 outcome，最后 revision +1。commit 内不得有可失败操作、重试或重复 spawn。
8. P2 的 `reconcile_container_integrity_freshness` 只能经唯一 reconciliation 调度执行；它与业务 prepare 的 revision 边界为：同一 prepared commit 已纳入 integrity/freshness 修复时只 +1，绝不由 reconcile 再 bump；独立首次 lock、reason change 或 repair 才独立 +1，same reason/no-op=0。即时 corrective snapshot 仍在同 tick 输出，但只能读取 Reconcile/Sweep 后的最终 inventory，绝不从任意业务 caller 旁路 emit。
9. `water_skin` 已移出本 plan scope：不为它定义 grid、filter、`ContainerSpec`、随身包验收或任何剩余 PR 交付物。
10. `trade_crate` 与 `dead_drop_box` 属 placeable container 计划，不计入本 plan 的九个随身容器，也不在本 plan 实装方块放置、打开或持久化。`herb_crate_placed` 是 `herb_crate` 的独立 placeable twin：P3 不修改它，也不实现 portable↔placed 转换。
...
- ingress、跨库存与全入口 grep 门：四个既有自动 ingress `add_item_to_player_inventory_inner`、`add_existing_item_to_player_inventory`、`pickup_dropped_loot_instance`、`force_attach_item_to_inventory`，以及三个遗漏生产 writer `handle_external_container_move` / `ExternalContainerMove`、`consume_mineral_drops_into_inventory`（`break_handler.rs` → `MineralDropEvent`，dev `/give` 复用）与 `forge_outcome_to_inventory`（`ForgeOutcomeEvent`）及其所有 production caller 逐一覆盖。既有 ingress 覆盖合法收纳、filter/structural/category-invalid reject 后继续检查候选、full 或 `_or_ground` fallback、输入/地面物保留、零副作用 reject 与成功 revision +1；`force_attach_item_to_inventory` 仅全量转移/骨币制作两个 caller，均为 typed prepared placement，永无 `(0,0)` 或假 `main_pack` fallback。ExternalContainer 双方向覆盖 session/entity/opened_by/authority、endpoint、position/dimension/supply-coffin、player owner/filter/category/bounds/collision/weight 预检，成功 external+player 原子替换、player +1、`LootContainerUpdate`/outcome/final snapshot 各一次；目标缺失、overflow、filter、drop context 任一失败双方深比较不变、revision=0、typed reject/resync，禁止 remove-then-restore；外部→玩家 full 仅预校验 ground policy/`DropContext`/registry 后 `DroppedToGround`，否则 source 保留。矿物/forge 分别 pin canonical mineral_id/freshness/merge/XP 与 quality/color/side-effects/tier/artifact state；prepare failure 零 live/drop/event/revision，full 仅预校验 drop 后 `DroppedToGround` 否则原 event/outcome 保留，Stored/Dropped outcome、snapshot、矿物 XP（仅成功）各一次、revision +1。TSY `world/tsy_container_search.rs` 已走 `add_existing_item_to_player_inventory`，grep gate 仅列已覆盖；spiritwood/botany/craft_emit `_or_ground` 仍测 prepared authority，NPC/world 自产 registry insert 明确不计入。死亡门覆盖 `prepare_death_drop(Overworld)` deterministic 50%、`prepare_death_drop(Tsy)` 入场物 50%+秘境所得 100%、零选中无 revision/event、被选中 owner item 的 staged rebuild/spill、registry/drop-context/position/dimension/corpse-context 各失败零副作用，以及主世界/TSY caller grep gate。broken recovery 覆盖 combat 与 forge 两 caller、Stored/DroppedToGround、durability终止态/`WeaponBroken` exactly once、drop context失败全回滚，并静态断言 `move_equipped_item_to_first_container_slot` 无 production caller。remains 覆盖 source record/骨币与 looter 双资源成功、非法候选继续、最终无 fit typed reject 与 source/player/revision/outcome深比较不变；treasure deactivate 覆盖 active/equipped source、非法候选继续、无 fit 保留原物、owner-loss rebuild 与一次 revision/outcome。四组入口连同既有 ingress 的全入口 grep 门不得漏；NPC/world 自产 `DroppedLootRegistry.entries.insert` 明确不计入。
...
- classification / resolver：四个静态 ID 分别 ownerless 全收；静态带 owner、未知 ownerless、伪 `pack_` ownerless、placeholder ownerless 各自 typed reject；合法 owner、eligible surface 缺失、template 缺失、无 `ContainerSpec`、ID-owner mismatch 各一条专属 case。`resolve_item_category` 专测合法 `template_id="mineral_fan_tie"` + canonical `mineral_id=Some("fan_tie")` 且 `MineralRegistry` 命中可进入 `ore_sack`，herb 拒绝；template/mineral_id mismatch、fake `mineral_` prefix、unknown mineral 都 typed invalid、P1 fail-closed 零副作用且绝不 fallback `Misc`。
...
4. `InventoryItemViewV1.category` 是 server 通过 P1 唯一 `resolve_item_category(item, item_registry, mineral_registry)` 派生的 required lower-snake canonical 字符串，恰为 `pill, herb, recipe_fragment, recipe_hint, weapon, armor, treasure, bone_coin, tool, scroll, misc, block, mineral, anqi, liquid, container, food, shield`；已注册 template 取 `ItemRegistry` canonical category，synthetic mineral 必须同时满足 canonical `mineral_id`、`MineralRegistry` 命中和精确 `mineral_{canonical}` template。prefix spoof、unknown mineral、template/mineral_id mismatch 都是 typed invalid；P4 对其整份 snapshot fail-closed、保留旧 state，client 禁止维护 template→category 映射或猜 prefix。TOML accept 的 `bonecoin` 与 wire `bone_coin` 是两个不同且已冻结的语法。
...
- category wire：18 个 canonical 值 `pill, herb, recipe_fragment, recipe_hint, weapon, armor, treasure, bone_coin, tool, scroll, misc, block, mineral, anqi, liquid, container, food, shield` 逐一覆盖 serde/proto/TypeBox/JsonFormat/sample/client parser 的正反测试；P1 resolver 的合法 synthetic `mineral_fan_tie` + canonical `fan_tie` 输出 wire `"mineral"`，而 template/mineral_id mismatch、fake prefix、unknown mineral 的整份 snapshot 必须 fail-closed no-op 并保留旧 revision/state。缺/null/unknown category 或 extra key 同样整份 snapshot no-op，server registry + `MineralRegistry` 是唯一派生源，TOML `bonecoin` 不可替换 wire `bone_coin`。
...
1. **PR-1（P1）migration、全入口/跨库存事务、预提交 owner/filter/category 与 live rebuild**：从真实 `inventories.schema_version=2` 的一次性 persisted owner migration（严格 canonical `pack_<u64>`、仅 eligible unique owner、save-success 才升版、失败重试）、private non-Clone `#[must_use]` `PreparedInventoryMutation<'a>` 独占 `&mut PlayerInventory`、`prepare_inventory_mutation_with` / consume-self `commit_prepared_inventory_mutation`、identity `InventoryItemPrepareFn`、non-Clone prepared drops/outcome、静态 allowlist/唯一 `find_live_container_owner`、所有 Container from/to 结构门、双向 swap filter 与 self-containment reject。冻结唯一 `resolve_item_category(item: &ItemInstance, item_registry: &ItemRegistry, mineral_registry: &MineralRegistry) -> Result<ItemCategory, ItemCategoryResolveError>`：registered template 取 canonical registry category；未注册仅 canonical mineral_id+MineralRegistry exact hit+精确 `mineral_{canonical}` template 才 Mineral，prefix spoof/unknown/mismatch typed invalid、不 fallback Misc；`item_passes_filter`、move/swap/all ingress 与后续 P4 wire 共用。四个自动 ingress `add_item_to_player_inventory_inner` / `add_existing_item_to_player_inventory` / `pickup_dropped_loot_instance` / `force_attach_item_to_inventory` 的全 caller grep 门；后者仅全量转移/骨币制作两个 typed prepared placement caller，禁止 `(0,0)`/假包 bypass。`handle_external_container_move` / `ExternalContainerMove` 纳入 player+world ExternalContainer prepared 双资源 transaction：双向预检 session/entity/opened_by/authority、endpoint、position/dimension/supply-coffin、player owner/category/filter/bounds/collision/weight，成功原子替换双方、player +1、LootContainerUpdate/outcome/final snapshot各一次；owner 移出 player 在 staged rebuild/spill，目标缺失/overflow/filter/drop context 失败双方原样、revision=0、typed reject/resync，禁止 remove-then-restore；external→player full 仅预校验 ground policy/DropContext/registry 后 DroppedToGround 否则 source保留。`consume_mineral_drops_into_inventory`（`break_handler.rs`→`MineralDropEvent`，dev give复用）与 `forge_outcome_to_inventory`（`ForgeOutcomeEvent`）改为统一 prepared typed wrappers，禁止 first-fit/push：共享 resolver/filter/fit/drop/commit，矿物保留 canonical mineral_id/freshness/merge/XP，forge 保留 quality/color/side-effects/tier/artifact state；prepare failure零 live/drop/event/revision，full 仅预校验 drop 后 DroppedToGround否则输入 event/outcome保留，Stored/Dropped、snapshot、矿物XP（仅成功）各一次且 +1。writer grep gate 明列此三入口及 production callers；TSY `world/tsy_container_search.rs` 已走 `add_existing_item_to_player_inventory` 仅标已覆盖、不另造路径；spiritwood/botany/craft_emit `_or_ground` 仍共用 prepared authority，NPC/world自产豁免。主世界与 TSY 死亡掉落共用唯一 `prepare_death_drop(DeathDropMode::{Overworld,Tsy}) -> PreparedDeathDrop`，保留各自 50%/TSY所得选择策略，在一笔 transaction stage detach、owner rebuild/spill、remaining placement、registry/drop/event 与 TSY corpse/outcome，并预校验 DropContext/position/dimension/registry/corpse context；零选中无 revision/event、任一失败全资源不变。删除 `move_equipped_item_to_first_container_slot`，combat/forge 两 caller只走 `prepare_broken_equipped_item_recovery`，同笔完成 durability终止、resolver-aware收纳或预校验地面掉落、rebuild/spill 和 `WeaponBroken` outcome，revision/drop/event/outcome all once。`transfer_remains_to_looter` stage 源遗骸/骨币与 looter，`apply_treasure_activate(..., false)` stage active/equipped source 与 player inventory；非法候选继续、无 fit source 不变、成功 only-once。四组入口和既有 ingress全入口 grep 门，明确豁免 NPC/world自产 registry insert。普通 move 同一 prepare 计算 targeted wear、`SlotMove` attrition、pack rebuild、freshness/layout/drop/outcome，删/收所有 commit 后 live writer，组合操作总 revision+1、最终 snapshot/outcome exactly once。`exchange_inventory_items` owner 禁交易 typed reject 或非 owner staged 双库存；`transfer_all_inventory_contents` 允许 owner 的 source+target+drop registry/context 多资源原子全量转移、骨币 JS-safe room 全额或整笔 reject；`apply_bone_coin_craft_session` 的 materials+Crafting ledger+Stored/DroppedToGround+event 单事务。仅 server 栈；完整 gate 为 `cargo fmt --check && cargo clippy --all-targets -- -D warnings && cargo test`.
