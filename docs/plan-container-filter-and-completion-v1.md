...
1. dispatcher 对 ordinary Terminal 只构造 private、`#[must_use]`、non-Clone 的 `PreparedOrdinaryBreak`；它持 immutable `BreakAttemptSnapshot`、original dimension/position/full BlockState、预先完成 template/tool/fit/allocator/drop-policy 失败检查后的 ordinary drop candidate 与 `OrdinaryBreakReceipt` key。AIR CAS 与 drop **不得**分别消费同一 terminal authorization。
2. 唯一 `commit_prepared_ordinary_break(self, &mut ChunkLayer, &mut PlayerInventory | &mut DroppedLootRegistry, &mut InventoryInstanceIdAllocator, &mut FurnitureRegistry, &mut OrdinaryBreakReceiptStore, &mut EventWriter<OrdinaryBreakApplied>)` 同时持有所有实际参与可变资源；其**第一步**比较 live dimension/position state 与 immutable original snapshot，并检查 receipt。receipt 已存、layer/dimension 不符或 state 变化，立即 typed `DuplicateOrdinaryBreak`/`StaleAuthorizedBreak`，零 AIR、零 inventory/drop、零 allocator、零 furniture/container side effect。
3. prepare 完成后 installation 不可失败：同一 exclusive system 依次写 receipt、AIR、inventory 或 ground-drop、已预留 allocator 结果及 furniture/container side effect，最后 emit `OrdinaryBreakApplied`。这是 process-local、单 tick atomic；不声称 SQLite/crash atomic，绝不拆成两个 consumers。后续观察者只读 `OrdinaryBreakApplied`，不得给予掉落。
4. 清除 `apply_default_block_break` 与 `apply_block_drops` 并列提交、以 `.before(...)` 维持正确性的旧设计；二者迁为 prepare/纯 drop mapping 或删除。container/workbench 的返还、container items drain、`FurnitureRegistry` 移除也必须由该唯一 commit owner 作为同一候选安装面处理，prepare 失败时不留下 AIR 后补救。
...
5. ack 后立即释放经济 reservation：chunk 永不加载、player 永不重连、ground entity 未 spawn 均不能滞留。Projection worker 用 stable projection ID 重放；block/source ownership DB finality 与 world projection 分离，hash 不同/缺失进入 `ProjectionQuarantine`，不回滚、不重奖、不重新占 reservation。ground entity id=`hash(settlement_id,payload_ordinal)`，已存在即 no-op。
6. startup 先 hydrate `DurableBlockOwnershipIndex`、canonical receipt/outbox，再 recovery；recovery 对 Prepared 重做 terminal，对 pending 只完成 live/canonical receipt 或 ack，对已 ack 只补 projection。`InventorySettlementRecovery` 早于 player hydration/block writers；ProjectionOutbox 可以无限期等待 chunk/player 可用而不阻塞 finality。
...
1. `PreparedOrdinaryBreak` 覆盖成功 exactly once、重复 terminal、snapshot state/dimension/layer 变化、prepare 后 template/allocator/drop policy 故障、consumer registration 顺序扰动；每个 reject 断言 AIR、inventory、dropped registry、allocator、furniture/container 均零变化，成功时 receipt/AIR/drop-or-inventory/furniture 同一 Update 安装且观察者不发第二掉落。
