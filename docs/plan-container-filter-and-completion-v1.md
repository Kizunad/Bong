...
2. **terminal/recovery transaction**：仅 settlement ID owner 可在单 connection `BEGIN IMMEDIATE` 中读取并 SHA-256/schema 校验 `Prepared` WAL、CAS `Prepared`、写 WAL 的 candidate inventory/skill/drop/source/station after-state、写 terminal 状态和 audit、删除该 ID 全部 reservations，再 commit。矿物按 WAL source-after 推进 `mineral_sources` 和投影；forge 按 WAL attempt/station-after 清 `active_attempt_id`，Perfect/Good/Flawed 写 Stored/Dropped、Waste/Explode 写 NoItem，且仅 Explode 写 WAL 中的 integrity/wear。`Committed*` 重放只读既有 terminal，不二写；`QuarantinedCorrupt` 是 payload hash/schema/row invariant 唯一异常终态，保留原 WAL+reservation 供人工 quarantine，禁止自动重试或重算。reservation 正确时 revision/active-lock conflict 不得出现，因此不得定义 stale retry 分支。
3. **escrow cancel/failure**：Prepared attempt escrow 始终归该 attempt；restart 只重建 attempt session，普通 move/trade/death/disconnect 不得移动已 escrow 材料。若产品允许取消，typed cancel 必须先生成完整 immutable settlement candidate，再走第 1–2 项相同 WAL/reservation/terminal 流程返还 candidate inventory 或 durable drop；其它失败不改变 Prepared。
4. **rollback 边界**：Prepared transaction 在写 WAL/reservation 前任一 SQL/serialize/revision 校验失败则 rollback，DB/ECS/source/drop/XP/outcome 均不变；terminal transaction 失败 rollback 且保留原 Prepared+reservation，由 recovery 继续。普通 writers 绝不以 live ECS 覆写已 reservation 的数据库。
5. **统一恢复与调度**：`durable_block_ownership` migration 后，`durable_block_ownership_hydrate_startup` 必须先加载 `DurableBlockOwnershipIndex`，再由 `InventorySettlementRecovery` 扫描 mineral 与 forge `inventory_settlements` 的 `Prepared`；forge attempt escrow hydration 与 terminal settlement recovery 严格分流。真实 `main.rs::build_server_app` 的 `inventory::register`、`mineral::register`、`forge::register`、`world::register` 共同接线：Startup manifest hydration 严格 `.after(crate::persistence::PersistenceBootstrapSet).after(crate::world::setup_world).before(InventorySettlementRecovery)`，并早于 settlement recovery、玩家 hydration 和所有 block-break writer；首个 Update 仍由 `InventorySettlementSet::Recovery` 兜底。`world::register` 固定 `BlockBreakSet::{Authorize,Ordinary,DurablePrepare,DurableTerminal,Projection}`，以 Recovery → Authorize → Ordinary → DurablePrepare → DurableTerminal → Projection 链保证可观测顺序，但 authority 只来自 manifest+dispatcher。矿物 recovery 只按 WAL candidate 投影 AIR/exhausted/index/despawn；forge terminal recovery 只按 WAL candidate 安装产物/skill/drop/station，不重建或重跑 runtime forge session。chunk 未加载则保留已 terminal DB/index 并延期幂等 block/ECS projection。
6. **install/event**：terminal DB commit 后主线程只安装 transaction 返回的同一 committed snapshots 至 `PlayerInventory`、`SkillSet`、`DroppedLootRegistry`、矿物 node/index/block 或 forge station/session ECS；不得先改 ECS。只在 install 后发 keyed `InventorySettlementCommittedEvent`。经济 source/item/XP/drop/terminal row exactly-once；network/audio/Redis 仅 keyed best-effort wake，重复按 ID 去重，断线重启依赖 DB hydration/resync，不承诺端到端投递。
7. **station lifecycle**：`ForgeStationId` 的 `Active|Removed` durable row 是生命周期真源；`active_attempt_id` 或任一 `InventorySettlementReservation` 存在时，专用 forge break 只返回 typed busy/reserved reject，不得写方块。仅空闲 station 的 `Removed` terminal transaction 可以删除其 durable row 投影。startup 从 Active rows 重建 durable index/ECS；Prepared attempt 从 escrow hydration，terminal settlement 只能由 `InventorySettlementRecovery` 消费 WAL，二者不得相互替代。
...
9. **单一 raw break dispatcher（冻结，不依赖 EventReader 顺序）**：仅 `world::block_break::authorize_block_break_intents` 可在生产 `server/src` 声明 `EventReader<DiggingEvent>`。它为每个可破坏 raw event 生成稳定 `BreakAttemptId`，按 `DurableBlockOwnershipIndex` 发唯一 typed event：`AuthorizedOrdinaryBreak`、`AuthorizedMineralBreak`、`AuthorizedForgeBreak`、`RejectedDurableBreak` 或 `DurableBlockIntegrityFault`。`apply_default_block_break` 与 `apply_block_drops` 都只能消费**同一个** `AuthorizedOrdinaryBreak`；mineral extraction 只消费 `AuthorizedMineralBreak`，forge reserved/active/remove 只消费 `AuthorizedForgeBreak`；fault/reject 只写 typed audit/UX，绝不产生 block、drop、inventory、instance allocator、WAL 或任何经济 side effect。不得以任一 reader 是否先 `read()`、`.before/.after` 或 event 被多个 reader 看见作为授权或 AIR/drop gate。
10. **唯一 mutation/economic owner 与真实 schedule**：`world::block_break::BlockBreakSet::{Authorize,Ordinary,DurablePrepare,DurableTerminal,Projection}` 由 `world::register` 在 `Update` 以 `configure_sets(Update, (Authorize, Ordinary, DurablePrepare, DurableTerminal, Projection).chain())` 注册，并整体置于 `InventorySettlementSet::Recovery` 之后；完整顺序固定为 Recovery → Authorize → Ordinary → DurablePrepare → DurableTerminal → Projection。`apply_default_block_break` 是 ordinary block/AIR+`FurnitureRegistry` 的唯一 owner，`apply_block_drops` 是 ordinary inventory/drop/`InventoryInstanceIdAllocator` 的唯一 owner；mineral/forge 的唯一经济 owner 分别是各自 `Prepared`→terminal WAL transaction，且只由 terminal snapshot 投影 block/index/ECS。`DurableTerminal` 的 mineral only-Exhausted 与 forge only-Removed 才可写 AIR；`Projection` 仅安装上述已 commit snapshot，不能重算或授权。现有 raw reader 必须逐一迁移：`world/block_break.rs::apply_default_block_break`、`world/block_drop.rs::apply_block_drops`、`mineral/break_handler.rs::handle_block_break_for_mineral`、`world/container_block.rs`、`spiritwood/mod.rs`、`social/mod.rs`、`craft/workbench.rs` 均改读相应 typed authorization（ordinary consumer 或 mineral/forge consumer），不得保留 `EventReader<DiggingEvent>`。加 compile/static pin 扫描 production `server/src`：除 dispatcher 外出现 `EventReader<DiggingEvent>` 即失败。
11. **测试与顺序扰动**：真实 `main::build_server_app` SQLite/World/ChunkLayer/client 集成矩阵必须覆盖 ordinary terrain `IRON_ORE`（普通掉落恰一次）、manifest durable FanTie 同 state、未耗尽/耗尽 mineral、reserved forge、transaction failure、manifest/source/index/ECS/block mismatch、repeated dig；每例同时断言 block、玩家 inventory、drop event、instance allocator、SQLite/WAL/manifest/source/station、index/ECS。fault/reject 的上述经济与投影 side effect 全为零。另反转 ordinary/durable consumer registration 或将 projection set 放在可观测的不同合法位置后复跑，所有结果相同，证明 authority 来自 dispatcher+manifest 而非注册顺序。单独覆盖 ordinary `IRON_ORE`、manifest durable 同 block state、partial projection loss、manifest hit+full secondary loss，以及 manifest missing 的上述诚实边界。


### 生产切流与饱和测试

`mineral/break_handler.rs` 的 extraction 只消费 `AuthorizedMineralBreak`，再只通过 `mineral_sources`+manifest 写 Prepared/terminal transaction；`apply_default_block_break` 与 `apply_block_drops` 只消费同一个 `AuthorizedOrdinaryBreak`，后者不得再以 `MineralOreIndex` miss 作为普通掉落授权。terminal projection **只**读取已校验的 WAL `source_after` snapshot，`remaining > 0` 保留矿块与 manifest/DB/index/ECS 一致，只有 `Exhausted` 才在 terminal transaction 删除/更新 manifest、commit 后在 terminal projection 写 AIR、移 index、despawn node。任一 transaction/CAS/serialize/recovery/quarantine/rollback 失败均保留 block，且 DB/manifest/index/ECS 不变或只按同一 WAL 幂等重放；`MineralDropEvent` 最多为含 ID wake/notification，不能作为恢复输入。旧 exhausted JSON 一次性导入/退役后断开 anchor/respawn authority。`InventorySettlementRecovery` 在 normal writers 前完成无客户端 Prepared。forge station placement 走 `ForgeStationPlacementId` 的 inventory+station+manifest 单事务后才投影；专用 forge break 只消费 `AuthorizedForgeBreak`，reserved 或 active attempt 的 reject 保留 ANVIL，空闲成功也仅在 `Removed` transaction 同步删除/更新 manifest 后，由 terminal projection **恰一次**写 AIR 并删 ECS/index。inputs lock 把精确材料转入 `ForgeAttemptId` escrow；canonical XP pure function 仅针对 escrow 产 candidate `SkillSet`，五档终局同 transaction；禁止先 `SkillXpGain` 再 `ForgeOutcomeEvent`。inventory bridge/S2C/Redis/audio/source readers 改读 committed-ID wake 并去重，P1B 不接 P2 snapshot routing，也不扩展其它 placeable。

测试必须覆盖：`mineral_sources` 初始 anchor/legacy JSON 一次导入、维度/generation/extraction_seq/revision、同坐标再生、每次 extraction/exhaustion/respawn CAS、JSON 退役后不能决定 spawn；ordinary terrain `IRON_ORE` 与 manifest durable FanTie 的相同 `BlockState::IRON_ORE` 必须分流正确；manifest hit 的 partial projection loss、source/index/ECS/block 全部 secondary loss 均 fault，manifest missing 则明确 ordinary（不得声称 raw state 自动发现损坏 manifest）。矿物及 forge 五档各自覆盖 Prepared 后立即崩溃、玩家永久离线、registry/code `wal_schema_version` 变化、DB commit 后 install 前崩溃，startup `InventorySettlementRecovery` 都只从 WAL snapshots 得出唯一 terminal，不重算 item/quality/XP/source/station 经济；reservation 后普通 inventory/skill/source/station writer 被 typed `SettlementReserved` 拒绝、重启后恢复、terminal 后 reservation 恰清一次；payload hash/schema 破坏只到 `QuarantinedCorrupt` 且不释放成普通 writer。矿物必须覆盖 `AuthorizedMineralBreak` 命中后的 SQL/CAS/rollback/recovery/quarantine failure 均保块，且 WAL/DB/manifest/source index/node ECS 不变或按 WAL 重放；每次 non-exhausted terminal 保块，只有 Exhausted terminal 才原子删除/更新 manifest 后 AIR+移 index+despawn。station 必须覆盖同址 generation、placement item 精确扣除/重复 placement 幂等/SQL 失败零扣物零 manifest 零投影/commit-install crash reload、reserved 与 `active_attempt_id` busy break 均 typed reject 并保 ANVIL、空闲 break 仅 Removed terminal 原子删除/更新 manifest 后恰一次 AIR+删 ECS/index、chunk unloaded deferred ANVIL projection；forge input exact instance/slice escrow、duplicate Start、lock 前后 crash、重连/死亡/交易/断线不能移动 escrow、Prepared attempt hydration 与 terminal settlement recovery 分流、typed cancel 同一 WAL 流程、五档全部释放 active lock、Perfect/Good/Flawed Stored/Dropped、Waste/Explode NoItem、Explode station wear exactly once、非 Explode 不改 integrity；满包不可drop、首次无 inventory row、duplicate source/wake/retry/reconnect/reload；每个 SQL write 失败、autosave/disconnect/shutdown 不回退；经济物/XP/source/drop/outcome row 恰一次。

**强制真实 App block-authority 集成矩阵**：不得只调用 handler helper。每例均以 `main::build_server_app` 建最小 SQLite/World/ChunkLayer/client，发送真实 `DiggingEvent` 并运行完整 Update，且同时断言 `ChunkLayer` block、player inventory、drop event、`InventoryInstanceIdAllocator`、SQLite row/WAL/manifest、durable projection index、相关 `MineralOreNode`/`WeaponForgeStation` ECS：① ordinary terrain `IRON_ORE` 只掉普通物恰一次；② manifest durable FanTie 同 state 只走 mineral；③ reserved forge 维持 ANVIL、station row/revision/active lock、manifest 与 ECS/index 不变且只产 typed reject；④ mineral transaction failure 维持矿块，source/WAL/manifest/index/node 均不变；⑤ mineral WAL terminal 未耗尽维持矿块，source remaining/revision 与 manifest/node/index 同步；⑥ mineral Exhausted terminal 才 AIR，source Exhausted、manifest 终态、index 移除、node despawn、WAL terminal 一致；⑦ 成功 forge remove 仅一次 Removed terminal 后 AIR，manifest/station ECS/index 移除且 repeated dig 不二写；⑧ manifest hit 的 source/index/ECS/block mismatch 均保留原块、发 `DurableBlockIntegrityFault`，并且 block/drop/inventory/allocator/DB/WAL/manifest/index/ECS 均零副作用。另以 ordinary/durable consumer registration 顺序扰动复跑①–⑧，结果必须相同，证明安全性来自 dispatcher+manifest 而不是 `EventReader` 读取顺序；并加 static/compile pin 保证生产 `server/src` 仅 dispatcher 声明 `EventReader<DiggingEvent>`。
...
**决议**：P1A 交付 process-local prepared core；borrowed mutation/ordinary commit 永不跨 DB、不作 crash atomic。P1B 交付 `PreparedInventorySettlement`、`InventorySettlementId`、SQLite `mineral_sources`（legacy exhausted JSON 一次导入后退役）、`durable_block_ownership`/`DurableBlockOwnershipIndex`（唯一物理 durable ownership）、mineral generation/sequence/recovery、`ForgeStationId`/SQLite `forge_stations`、`ForgeStationPlacementId` 原子扣 station item、escrow `ForgeAttemptId`、Prepared→Committed source/WAL 和单事务 inventory/skill/drop/source/station/manifest；station placement 先 durable commit 后投影。固定仅 `authorize_block_break_intents(DiggingEvent)` 读取 raw dig 并按 manifest 发 `BreakAttemptId` 绑定的 `AuthorizedOrdinaryBreak`/`AuthorizedMineralBreak`/`AuthorizedForgeBreak` 或 reject/fault；manifest miss 一律 Ordinary，即使 state 为 `IRON_ORE`/ANVIL，manifest hit 后才校验 source/index/ECS/block projection。ordinary default AIR/drop 只消费同一 `AuthorizedOrdinaryBreak`；mineral/forge 只消费各自 typed authorization，矿物只依 WAL source-after（未耗尽保块、Exhausted 原子 manifest terminal 后才 AIR），锻炉 reserved/active/busy 保 ANVIL、仅 Removed 原子 manifest terminal 后恰一次 AIR。manifest 尚存时任一 secondary projection loss 是 typed integrity fault；manifest 与全部 secondary metadata 同时丢失时 raw state 无法区分 ordinary，交 migration/integrity sweep/backup/manual quarantine，禁止 DiggingEvent 猜测。Active row/Prepared attempt 支持 deferred projection 与 hydration。矿物 Prepared 无客户端也由 startup recovery 决定性 commit；commit 后安装同 snapshots ECS、再发 committed-ID best-effort wake；现有 mineral direct consumer/forge direct grant 只为待替基线；禁止 consumer-first pending、source-event 重发、runtime session key、generic durable commit 和端到端一次通知承诺。

**落点**：`server/src/mineral/anchors.rs`、`persistence.rs`、`respawn.rs`、`break_handler.rs:170-260`（迁为 `AuthorizedMineralBreak` consumer）/ `server/src/mineral/inventory_grant.rs:38-138`（direct consumer）/ `server/src/forge/station.rs:26-165`（现有 ECS placement projection）/ `server/src/forge/mod.rs:182-384`、`server/src/forge/session.rs:81-222`（runtime attempt baseline）/ `server/src/world/block_break.rs:34-59`（替为 `authorize_block_break_intents` 与 typed consumer）/ `server/src/world/block_drop.rs:161-277`（替为 `AuthorizedOrdinaryBreak` consumer，移除 index-miss authority）/ `server/src/persistence/mod.rs:1083-2351`（`durable_block_ownership`、`mineral_sources`/`forge_stations` migration）/ `server/src/player/state.rs:1313-1375,2342-2367`（load/transaction helper）+ 本 plan「P1A」「P1B」。
