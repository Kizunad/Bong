...
2. `DeadDropWardTriggered` 只可由 `ContainerBlockKind::DeadDrop` 的 active ward 且 breaker != owner 产生；owner、inactive ward、expired/absent ward 均是 `OrdinaryReturnAndDrain`。此分支冻结 `server/src/world/container_block.rs:235-253` 的现状：先向已打开者发送 `LootContainerCloseReasonV1::ContainerDestroyed`，移除 `ExternalContainerRegistry` session，再将 visual container entity 标记 `Despawned`；**不**写 AIR、**不**返还 dead_drop_box、**不** drain contents/写 ground drop，物理 block 保留。它唯一拥有一次 `CombatEvent` + `ApplyStatusEffectIntent` 半径反噬、`bong:dead_drop_ward_break` VFX 与 `dead_drop_ward_break` audio，且须以 receipt 防止 terminal replay、entity 已 despawn 或 session 已移除时重复的 combat/AV/close side effect；不得向 breaker 或旁观者泄露 contents。现状中 entity despawn 而 block 留存不是本 plan 要修正的内容，P1B 必须显式保持并以测试锁定。
...
6. runtime `DroppedLootRegistry.entries` 与 client `instance_id` 只能使用独立 `GroundProjectionSurrogateId(u64)`，范围固定为 `1..=JS_SAFE_INTEGER_MAX` 即 `1..=9_007_199_254_740_991`（生产 `InventoryInstanceIdAllocator::next_id`、Rust schema 与 TypeBox `Number.MAX_SAFE_INTEGER` 的现有界限）。在创建 canonical outbox row 的同一 SQLite transaction，使用 durable global monotonic allocator（与 `InventoryInstanceIdAllocator` 同一 collision-free 序列语义、CAS/UNIQUE 推进）分配并持久映射 `(settlement_id,payload_ordinal) → surrogate_id`，`surrogate_id UNIQUE`。若唯一冲突所属 composite key 不同，事务 fail-closed 并重试取得新序号，绝不覆盖或 no-op；worker 以 surrogate 投影 entity/registry 前后均核验 composite key + payload hash。entity 已存在但其 persisted composite key 或 hash 不同进入 quarantine，不得借 instance ID 吞掉 payload。两 settlement 同 ordinal、allocator collision、重复 replay、ack 后 crash 与 entity 已存在的 same/different-key 都必须证明每 payload 最多投影一次；block/source ownership DB finality 与 world projection 分离，缺失/不一致进入 quarantine，不回滚、不重奖、不重新占 reservation。
7. startup 先 hydrate `DurableBlockOwnershipIndex`、canonical receipt/outbox，再 recovery；recovery 对 Prepared 重做 terminal，对 pending 只完成 live/canonical receipt 或 ack，对已 ack 只补 projection。`InventorySettlementRecovery` 早于 player hydration/block writers；ProjectionOutbox 可以无限期等待 chunk/player 可用而不阻塞 finality。
...
1. `PreparedOrdinaryBreak` 覆盖 ordinary 成功 exactly once、重复 terminal、snapshot state/dimension/layer 变化、prepare 后 template/allocator/drop policy 故障、consumer registration 顺序扰动；每个 reject 断言 AIR、inventory、dropped registry、allocator、furniture/container 均零变化，成功时 receipt/AIR/drop-or-inventory/furniture 同一 Update 安装且观察者不发第二掉落。另对 `DeadDropWardTriggered` 建 effect matrix：owner、non-owner active、inactive/expired 三列分别断言 block、entity、session、container return/contents drain/ground drop、combat/VFX/SFX/audio/receipt；non-owner active 必须保 block、Despawn entity、清 session、零 return/drain/drop，combat/AV/close 各一次且无 contents payload，重复 terminal 零第二 side effect；prepare 任一失败零 mutation。
