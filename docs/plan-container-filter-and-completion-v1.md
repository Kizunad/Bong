...
5. **统一恢复与调度**：`InventorySettlementRecovery` 是唯一 terminal settlement scanner，同时扫描 mineral 与 forge `inventory_settlements` 的 `Prepared`；与 forge attempt escrow hydration 严格分流。它在 `inventory::register` 新增 `InventorySettlementSet::{Recovery,NormalWriters}`；与方块投影的完整唯一链由 `configure_sets(Update, (Recovery, NormalWriters, DurableTerminalProjection, DefaultProjection).chain())` 注册，其中后两项是 `world::block_break::BlockBreakProjectionSet`。`player::register` 的 `attach_inventory_to_joined_clients` 必须 `in_set(NormalWriters)`，`mineral::register` 的 `handle_block_break_for_mineral`/`consume_mineral_drops_into_inventory` 和 `forge::register` 的 placement/start/step/outcome/break systems 也必须 `in_set(NormalWriters)`，recovery `in_set(Recovery)`。`main.rs::build_server_app` 已按 `player::register → inventory::register → forge::register → mineral::register` 建 app；P1B 在这四个既有 `register` 入口接线，Startup 必须以 `inventory_settlement_recover_startup.after(crate::persistence::PersistenceBootstrapSet).after(crate::world::setup_world)` 处理离线 WAL，首个 Update 仍以 Recovery set 兜底，均早于 player hydration、normal writers、terminal projection 与 default projection。矿物 recovery 只按 WAL candidate 投影 AIR/exhausted/index/despawn；forge terminal recovery 只按 WAL candidate 安装产物/skill/drop/station，不重建或重跑 runtime forge session。chunk 未加载则保留已 terminal DB/index 并延期幂等 block/ECS projection。
...
8. **方块写入唯一权威（冻结，非 EventReader 顺序）**：新增共享纯 preflight `world::block_break::classify_durable_break_target`，输入固定为 `DurableBlockKey { dimension: DimensionKind, pos: BlockPos }`、当前 `ChunkLayer` block state、`MineralOreIndex` + 对应 `MineralOreNode`、`MineralRegistry`、由 `mineral_sources` hydration 建立的 `MineralSourceProjectionIndex`、由 `forge_stations` hydration 建立的 `ForgeStationProjectionIndex` + 对应 `WeaponForgeStation`，输出固定为 `Ordinary`、`Mineral { source_id }`、`Forge { station_id }` 或 typed `DurableBlockIntegrityFault`。矿物 projection 必须同时核对 `(dimension,pos)`、node position、canonical mineral_id、generation/revision、source state 与 source 的 expected block state；锻炉 projection 必须同时核对 `(dimension,pos)`、`ForgeStationId`、ECS station 的位置/tier/revision/state 与 ANVIL expected state。任何 durable row/index 命中而实际 state 不符、实际 state 表明矿物/ANVIL durable target 而任一 index/registry/ECS/source row 缺失或不一致，均发带 key 与 kind 的 `DurableBlockIntegrityFault` 并返回 fault；不得降级为 `Ordinary` 或清 stale index。
9. **唯一 block mutation owner**：`apply_default_block_break` 在每个可破坏 `DiggingEvent` 内先取得 client 的 `CurrentDimension`，以同一 `DurableBlockKey` 调上述 preflight；仅 `Ordinary` 才可 `ChunkLayer::set_block(pos, AIR)`（及移除普通 `FurnitureRegistry` 项）。`Mineral`、`Forge`、fault 均跳过默认 AIR。矿物/锻炉专用路径各自也调用同一 preflight，只能在自身 WAL terminal transaction commit 后做 terminal projection：矿物仅 WAL `source_after.remaining > 0` 时保留 expected block，`Exhausted` 时才写 AIR；锻炉仅 WAL/transaction 已写 `Removed` 时写 AIR。独立 `EventReader<DiggingEvent>` 都读取同一事件，`continue`、先注册、`.before/.after` 或 system set 均不具有消费/排他语义，绝不可作为 AIR gate。
10. **调度仅保证可观测投影次序，不承担安全性**：`inventory::register` 固定 `InventorySettlementSet::{Recovery,NormalWriters}`，并新增 `world::block_break::BlockBreakProjectionSet::{DurableTerminalProjection,DefaultProjection}`；`configure_sets(Update, (Recovery, NormalWriters, DurableTerminalProjection, DefaultProjection).chain())`。`InventorySettlementRecovery` 在 Recovery；矿物 extraction/forge placement/start/step/break preflight 在 NormalWriters；矿物 exhaustion 与 forge Removed 的 WAL terminal block projection 在 DurableTerminalProjection；`apply_default_block_break` 在 DefaultProjection。`main.rs::build_server_app` 的现有 `player::register → inventory::register → forge::register → mineral::register` 保持不变，且 startup recovery 仍严格 `.after(crate::persistence::PersistenceBootstrapSet).after(crate::world::setup_world)`；无论该顺序是否被未来改动，默认 writer 的 shared preflight 仍必须 fail-closed。


### 生产切流与饱和测试

`mineral/break_handler.rs` 的 extraction 先以共享 preflight 确认 `Mineral`，再只通过 `mineral_sources` 写 Prepared/terminal transaction；terminal projection **只**读取已校验的 WAL `source_after` snapshot，`remaining > 0` 保留矿块与 DB/index/ECS 一致，只有 `Exhausted` 才在 terminal projection 写 AIR、移 index、despawn node。任一 transaction/CAS/serialize/recovery/quarantine/rollback 失败均保留 block，且 DB/index/ECS 不变或只按同一 WAL 幂等重放；`MineralDropEvent` 最多为含 ID wake/notification，不能作为恢复输入。旧 exhausted JSON 一次性导入/退役后断开 anchor/respawn authority。`InventorySettlementRecovery` 在 normal writers 前完成无客户端 Prepared。forge station placement 走 `ForgeStationPlacementId` 的 inventory+station 单事务后才投影；专用 forge break 先以共享 preflight 确认 `Forge`，reserved 或 active attempt 的 reject 保留 ANVIL，空闲成功也仅在 `Removed` transaction commit 后由 terminal projection **恰一次**写 AIR 并删 ECS/index。inputs lock 把精确材料转入 `ForgeAttemptId` escrow；canonical XP pure function 仅针对 escrow 产 candidate `SkillSet`，五档终局同 transaction；禁止先 `SkillXpGain` 再 `ForgeOutcomeEvent`。inventory bridge/S2C/Redis/audio/source readers 改读 committed-ID wake 并去重，P1B 不接 P2 snapshot routing，也不扩展其它 placeable。

测试必须覆盖：`mineral_sources` 初始 anchor/legacy JSON 一次导入、维度/generation/extraction_seq/revision、同坐标再生、每次 extraction/exhaustion/respawn CAS、JSON 退役后不能决定 spawn；矿物及 forge 五档各自覆盖 Prepared 后立即崩溃、玩家永久离线、registry/code `wal_schema_version` 变化、DB commit 后 install 前崩溃，startup `InventorySettlementRecovery` 都只从 WAL snapshots 得出唯一 terminal，不重算 item/quality/XP/source/station 经济；reservation 后普通 inventory/skill/source/station writer 被 typed `SettlementReserved` 拒绝、重启后恢复、terminal 后 reservation 恰清一次；payload hash/schema 破坏只到 `QuarantinedCorrupt` 且不释放成普通 writer。矿物必须覆盖 shared preflight 命中后的 SQL/CAS/rollback/recovery/quarantine failure 均保块，且 WAL/DB/source index/node ECS 不变或按 WAL 重放；每次 non-exhausted terminal 保块，只有 Exhausted terminal 才 AIR+移 index+despawn。station 必须覆盖同址 generation、placement item 精确扣除/重复 placement 幂等/SQL 失败零扣物零投影/commit-install crash reload、reserved 与 `active_attempt_id` busy break 均 typed reject 并保 ANVIL、空闲 break 仅 Removed terminal 后恰一次 AIR+删 ECS/index、chunk unloaded deferred ANVIL projection；forge input exact instance/slice escrow、duplicate Start、lock 前后 crash、重连/死亡/交易/断线不能移动 escrow、Prepared attempt hydration 与 terminal settlement recovery 分流、typed cancel 同一 WAL 流程、五档全部释放 active lock、Perfect/Good/Flawed Stored/Dropped、Waste/Explode NoItem、Explode station wear exactly once、非 Explode 不改 integrity；满包不可drop、首次无 inventory row、duplicate source/wake/retry/reconnect/reload；每个 SQL write 失败、autosave/disconnect/shutdown 不回退；经济物/XP/source/drop/outcome row 恰一次。

**强制真实 App block-authority 集成矩阵**：不得只调用 handler helper。每例均以 `main::build_server_app` 建最小 SQLite/World/ChunkLayer/client，发送真实 `DiggingEvent` 并运行完整 Update，且同时断言 `ChunkLayer` block、SQLite row/WAL、durable projection index、相关 `MineralOreNode`/`WeaponForgeStation` ECS：① reserved forge 维持 ANVIL、station row/revision/active lock 与 ECS/index 不变且只产 typed reject；② mineral transaction failure 维持矿块，source row/WAL/index/node 均不变；③ mineral WAL terminal 未耗尽维持矿块，source remaining/revision 与 node/index 同步；④ mineral Exhausted terminal 才 AIR，source Exhausted、index 移除、node despawn、WAL terminal 一致；⑤ 成功 forge remove 仅一次 Removed terminal 后 AIR，station ECS/index 移除且重复 dig 不二写；⑥ 任一 `MineralOreIndex`、`MineralSourceProjectionIndex`、`ForgeStationProjectionIndex`、registry/ECS/expected block state 不一致均保留原块、发 typed `DurableBlockIntegrityFault`，并不得改变 DB/WAL/index/ECS。另以系统顺序扰动（令 default projection 早于 specialized reader）复跑至少①–⑥，结果必须相同，证明安全性来自 shared preflight 而不是 EventReader 读取顺序。

以真实 `main::build_server_app` 的最小 App/SQLite 集成测试断言 `InventorySettlementRecovery` 已由 `inventory::register` 加载，且对同一份待迁移 SQLite 数据库的可执行 Startup 观测证明 `crate::persistence::PersistenceBootstrapSet` migration、`crate::world::setup_world` 均先于 recovery（删除或反转任一 `.after(...)` 依赖必须使该顺序断言失败）；Update Recovery 仍早于 `player::attach_inventory_to_joined_clients`、mineral normal writer、forge normal writer、durable terminal projection 和 default projection；漏 `configure_sets`/`add_systems` 或顺序反转必须红。P1A 未 merge不得开 P1B；P1B 完整 crash matrix、block-authority matrix 与注册顺序 gate 未过不得开 P2。
...
**决议**：P1A 交付 process-local prepared core；borrowed mutation/ordinary commit 永不跨 DB、不作 crash atomic。P1B 交付 `PreparedInventorySettlement`、`InventorySettlementId`、SQLite `mineral_sources`（legacy exhausted JSON 一次导入后退役）、mineral generation/sequence/recovery、`ForgeStationId`/SQLite `forge_stations`、`ForgeStationPlacementId` 原子扣 station item、escrow `ForgeAttemptId`、Prepared→Committed source/WAL 和单事务 inventory/skill/drop/source/station；station placement 先 durable commit 后投影。固定由 `classify_durable_break_target(DurableBlockKey { dimension, pos })` 在**默认** `apply_default_block_break` 内执行矿物/锻炉共享 preflight：durable 命中或 index/registry/ECS/block state 不一致均跳过 AIR，后者发 typed integrity fault；矿物只依 WAL source-after（未耗尽保块、Exhausted 才 AIR），锻炉 reserved/active/busy 保 ANVIL、仅 Removed terminal 恰一次 AIR。`EventReader<DiggingEvent>` 不会互相消费，系统先后不是安全机制。Active row/Prepared attempt 支持 deferred projection 与 hydration。矿物 Prepared 无客户端也由 startup recovery 决定性 commit；commit 后安装同 snapshots ECS、再发 committed-ID best-effort wake；现有 mineral direct consumer/forge direct grant 只为待替基线；禁止 consumer-first pending、source-event 重发、runtime session key、generic durable commit 和端到端一次通知承诺。
