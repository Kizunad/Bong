...
- **技术零门槛**：Valence 2b705351 生成代码含全部 1.20.1 被动生物 bundle（`CowEntityBundle`/`PigEntityBundle`/... 共 248 个，`valence_entity` OUT_DIR entity.rs）；zombie NPC 已走同款 Rail A（`npc/spawn/zombie.rs:52` `ZombieEntityBundle { kind: EntityKind::ZOMBIE, .. }`），**协议层原生渲染，无需 fauna/visual.rs raw_id 对齐、无需客户端 GeckoLib 注册**——本 plan P0 照抄这个头部（**不照 `beast.rs:64` 的 `MarkerEntityBundle` custom visual 路数**（`beast.rs:51` 是 `spawn_beast_npc_at` 函数签名，`MarkerEntityBundle` 实际 spawn 在 `:64`），那条是妖兽专属视觉 shell）。当前全库零处使用 `EntityKind::COW/PIG/SHEEP`——全新接入面。
...
- `enum MundaneFaunaKind { Cow, Pig, Sheep, Chicken, Rabbit, Goat, Frog, Fox, Wolf }`（9 变体终表，§8.1 #1 锁定不增删）+ `fn bundle_for(kind)` 映射 Valence 原生 `<X>EntityBundle` + `EntityKind::COW/...`——**照 `npc/spawn/zombie.rs:52` 的 `ZombieEntityBundle { kind: EntityKind::ZOMBIE, .. }` Rail A 头部，不照 `beast.rs:64` 的 `MarkerEntityBundle` custom visual 路数**（`beast.rs:51` 是函数签名，非 bundle spawn 行）。
...
- `spawn_mundane_fauna_at(kind, pos, zone)`：Rail A bundle 头部 + 新 `MundaneFaunaMarker { kind, spawned_at, home_zone }`；位置过 `snap_spawn_y_to_surface`（`spawn/common.rs:219`）。**组件清单系统性列全（Debate 第三轮修正——上轮只补了死链侧，本轮补全反抗链侧）**：以 `beast.rs:51-120`（`spawn_beast_npc_at` 全函数，非仅 `MarkerEntityBundle` 那一行）+ `commoner.rs:44-90`（`spawn_commoner_npc_at`）+ `spawn/common.rs:288-358`（`spawn_rogue_commoner_base`，commoner 复用的公共骨架）三份先例为准，`spawn_mundane_fauna_at` 产出实体必须同时满足**通用底盘**和**四条 thinker 分支各自的硬性依赖**，缺一处 = 对应链路静默孤岛（query 不满足只会让 Action 恒 `ActionState::Failure`，不 panic，肉眼从游戏内看不出来）：

  **通用底盘（不分分支，spawn 时必挂一次）**：
  - Rail A `<X>EntityBundle { kind: EntityKind::COW/PIG/..., layer: EntityLayerId(layer), position: Position::new(...), ..Default::default() }`——`EntityKind` 随 bundle 天然带出，`NpcGoalQueryFilter`/`NpcFleeQueryItem`（`actions_combat.rs:30-43`）的 filter 硬性要求 `With<EntityKind>`，Rail A 免费满足（`beast.rs` 的 `MarkerEntityBundle` 才需要额外关心这一点）。
  - `Transform`/`GlobalTransform`（Position↔Transform 同步桥，`beast.rs:71-76`）+ `NpcMarker`（`npc/spawn/common.rs`，上轮已补）——`update_npc_blackboard`（`brain/mod.rs:291`）、全部 Scorer/Action 系统、`fauna_drop_system` 查询均硬编 `With<NpcMarker>`。
  - `NpcBlackboard::default()`（`spawn/common.rs:181-198`）——`nearest_player`/`player_distance`/`target_position`/`retaliation_target` 感知缓存，由 `update_npc_blackboard`（`brain/mod.rs:290-385`）逐 tick 扫描 `With<NpcMarker>` 实体回填；`FleeThreatScorer`/`CorneredScorer`/`MeleeAttackAction` 全部读写它——漏挂 = 这三者永远读不到目标、score/attack 恒空转。
  - `Navigator::new()` + `MovementController::new()` + `loadout.movement_capabilities` + `MovementCooldowns::default()`（`beast.rs:86-89`）——`FleeAction`/`MeleeAttackAction`/`WanderAction`/`GoToPoiAction` 全部要 `&mut Navigator`。
  - `NpcPatrol::new(home_zone, ...)`（`beast.rs:90`）——`NpcFleeQueryItem`（`FleeAction` 用）、`WanderAction`（`&NpcPatrol`）、`GoToPoiAction`（`&NpcPatrol`）均硬性要求。
  - `loadout`/`loadout.melee_archetype`/`loadout.melee_profile()`（`beast.rs:79-81`；轻量物种可选偏弱档，具体数值 P0 实施时定）——`MeleeAttackAction`（`actions_combat.rs:245-317`）硬性读 `&NpcMeleeProfile`（`.reach`/`.disengage_distance`/`.wound_kind`）。
  - `NpcArchetype::Mundane`（**新增变体，见 §8.1 #10**）+ `fauna_tag`/`MundaneFaunaMarker`。
  - `npc_runtime_bundle_with_age(entity, NpcArchetype::Mundane, initial_age_ticks)`（`lifecycle.rs:596`）——内含 `Wounds`/`CombatState`/`Lifecycle`（后者被 `MeleeAttackAction` 读 `Option<&Lifecycle>` 判活）——`fauna_drop_system`（`drop.rs:269`）查询硬要求 `With<NpcMarker>` 且触发源是 `DeathEvent`（依赖 `Wounds` 归零判定），漏挂 = 凡兽物理上打不死、`DeathEvent` 永不触发、资源链孤岛（死链侧，上轮已补，本轮维持）。
  - `NpcDailySchedule::for_archetype(NpcArchetype::Mundane, ...)` + `home_base_for_archetype(...)` + `GoToPoiState::default()`（`spawn/common.rs:349-354` 先例）+ `NpcLodTier::Dormant`（`beast.rs:100`）。
  - despawn 一律 `insert(Despawned)`（[[feedback_valence_despawn_layer_entity]]）——超距回收天然满足：调度核 alive query 硬编 `Option<&RatBlackboard>`（`ambient_scheduler.rs:613`）/`Option<&MimicSpiderBlackboard>`（`:614`），凡兽不挂这些组件恒为 `None`，无需改调度核。

  **四条 thinker 分支各自的硬性组件依赖（穷举，本轮重点——上轮遗漏项）**：

  1. `FleeThreatScorer → FleeAction`（逃跑链）：Scorer（净新增，泛化版 `fear_cultivator_scorer_system`，`scorers_survival.rs:330-347`）读 `&NpcBlackboard`（`With<NpcMarker>`）；Action 复用现成 `FleeAction`（**不是** `FleeCultivatorAction`——`FleeThreatScorer` 泛化到"一切威胁"而非"仅怕修士"，语义对应 `disciple.rs`/`rogue.rs` 已在用的裸 `FleeAction`）——`flee_action_system`（`actions_combat.rs:62-105`）读 `NpcFleeQueryItem = (&Position, &NpcBlackboard, &NpcPatrol, &mut Navigator)` + `NpcGoalQueryFilter = (With<NpcMarker>, With<EntityKind>, Without<ClientMarker>)`。**已知感知缺口（P0 范围限定，须在 plan 里写明，不能含糊 implies 已对"一切威胁"生效）**：`update_npc_blackboard` 的 `nearest_player` 只从 `player_query: Query<..., With<ClientMarker>>`（+ decoy/retaliation/duel 覆盖）取值，不扫描非玩家的捕食者 NPC——P0 的 `FleeThreatScorer` 实际只对"玩家/修士"生效（对齐 `FearCultivatorScorer` 现状），对"妖兽"的泛化避险要等 P2 `preys_on` 表落地时扩展感知源，§8.1 #3 已定案 P0 是最小反抗档。
  2. `CorneredScorer → MeleeAttackAction`（反击链，**上轮遗漏，本轮补全**）：Scorer（净新增）读 `NpcBlackboard.player_distance`（判定"被追打/逼入死角"，可选参考 `Navigator` 连续寻路失败信号，阈值 P0 实施时定）；Action 复用 `melee_attack_action_system`（`actions_combat.rs:245-317`）硬性读 `(&Position, &mut NpcBlackboard, &NpcMeleeProfile, &mut Navigator, Option<&Lifecycle>)`（`With<NpcMarker>`）+ 写 `EventWriter<AttackIntent>` + 读 `Option<Res<GameTick>>`——这是 big-brain `Scorer→Action` 强绑定（`Thinker::build().when(Scorer, Action)`），"复用 `MeleeAttackAction` 兜底"这句话本身不会让任何物种反击：不挂 `CorneredScorer` 就永远不会被 picker 选中，产出"全程只逃不反抗"的无害背景板，直接违反 [[feedback_threat_spectrum]]。**已知感知缺口（跟 #1 同源）**：`retaliation_target` 只在攻击者是玩家时回填（`interaction_memory.rs:153-181` 的 `record_attack_memories` 对非玩家攻击者 `continue`）——凡兽被"妖兽"打时 `retaliation_target` 不会置位；P0 最小闭环走 `player_distance` 阈值路线即可（不依赖 `retaliation_target`），跨物种反击感知归 P2 扩展。命中/脱险后评分回落，thinker 转回 `FleeThreatScorer` 继续逃（T0 惊扰反抗语义：逃为主，逼急了咬一口）。威胁谱系其余更复杂件（T1 `RetaliateScorer` 受击记仇窗口、T1.5 山羊冲撞、T2.5 狼群饥饿加权）仍推迟 P2（§8.1 #3 已定案），但**最低一档反抗必须在 P0 就有真实 Scorer 接线**，不能只写"复用 Action"这句话空转。
  3. `HungerScorer → FarmAction`（进食链，沿用 commoner 现成件）：Scorer 读 `(&Hunger, Option<&NpcDailySchedule>, Option<&NpcLodTier>)`（`With<NpcMarker>`，`scorers_survival.rs:88-120`）；Action `farm_action_system`（`actions_life.rs:229-268`）读 `(&mut Navigator, &mut Hunger)`（`With<NpcMarker>`）。硬性组件 `Hunger::default()`（已在通用底盘列出）。
  4. `WanderScorer → GoToPoiAction::default()`（游荡链，照 `commoner.rs:32` 的 `WanderScorer→GoToPoiAction` 用法，**不是** `beast.rs` 的 `WanderScorer→WanderAction`，见 §8.1 #2）：Scorer 读 `(Option<&PendingRetirement>, Option<&NpcDailySchedule>, Option<&NpcLodTier>)`（`With<NpcMarker>`，`scorers_survival.rs:140-183`）；Action `go_to_poi_action_system`（`actions_life.rs:354-410`）硬性读 `(&Position, &NpcPatrol, &mut Navigator, &mut GoToPoiState, Option<&NpcDailySchedule>, Option<&mut Hunger>, Option<&mut Cultivation>)`（`With<NpcMarker>`）——`&mut GoToPoiState` 是**非 `Option` 硬依赖**（跟 `beast.rs` 的 `WanderAction`→`&mut WanderState` 不是同一个组件，两者不可混用/漏挂其一），漏挂 = query 不满足、游荡链整体孤岛。

- **thinker 组合**（`FirstToScore` 按注册顺序取第一个过阈值的分支，顺序即优先级——反抗必须排在逃跑前，否则"被逼急了咬一口"永远轮不到）：`Thinker::build().picker(FirstToScore{..}).when(CorneredScorer, MeleeAttackAction).when(FleeThreatScorer, FleeAction).when(HungerScorer, FarmAction).when(WanderScorer, GoToPoiAction::default())`。**威胁谱系硬约束**（[[feedback_threat_spectrum]]）：即便 P0 最小实现，也不允许任何物种是无害背景板。
...
- **测试**：kind→bundle 映射全 9 variant pin；`AmbientMarkerData` 两函数契约；分池按 biome 命中专属 case（4 biome × 命中物种集）；spawn 落地吸附；despawn 走 `Despawned`；`counts_against_threat_budget=false` 下与 `AmbientThreatMarker` 预算互不干扰（仿 `TestFaunaMarker` 回归模式，`ambient_scheduler.rs:1578-1967`）；密度上限/回收边界；组件清单 pin（`spawn_mundane_fauna_at` 产出的实体带 `NpcMarker`/`NpcBlackboard`/`Navigator`/`MovementController`/`NpcPatrol`/`NpcMeleeProfile`/`Wounds`/`CombatState`/`Lifecycle`/`GoToPoiState`，9 variant 全覆盖）；死链闭环 pin（凡兽被打至 `Wounds` 归零 → 触发 `DeathEvent` → `fauna_drop_system` 掉落分支被调用，非静默孤岛）。**四条 thinker 分支各自命中一条真实触发 pin（本轮新增，Debate 第三轮修正——只写"复用 Action"不算数，必须验证 Scorer 真的能把 picker 带到对应 Action）**：
  1. **逃跑**：`FleeThreatScorer` 在玩家/修士靠近（`NpcBlackboard.player_distance` 在阈值内）时评分超过 `HungerScorer`/`WanderScorer`，thinker 选中 `FleeAction`；
  2. **被逼反击**：模拟"被追打至逼入死角"状态后，`CorneredScorer` 评分超过 `FleeThreatScorer`，thinker 选中 `MeleeAttackAction`（而非继续 `FleeAction`），命中/脱险后评分回落、thinker 转回 `FleeThreatScorer`；
...
4. 超距回收零改造：调度核 alive query 硬编 `Option<&RatBlackboard>`（`ambient_scheduler.rs:613`）/`Option<&MimicSpiderBlackboard>`（`:614`，函数签名 `ambient_scheduler_system` 在 `:604`），凡兽实体不挂这两个组件，恒为 `None`，超距即走 `insert(Despawned)`，正是凡兽期望语义。
...
2. 幼崽 `initial_age_ticks=0` 接口虽已在 `spawn_beast_npc_at` 存在（`beast.rs:57` 参数），但本 plan 的 `spawn_mundane_fauna_at` 不复用此参数做繁殖用途——繁殖判定/幼崽成长曲线归 `plan-husbandry-v1`（若立项）。
...
### #10 NpcArchetype 新增 Mundane 变体（Debate 第三轮补充，2026-07-04）

> 本条不在原 §8 九条之列——第三轮对抗审查核实代码后发现的系统性缺口：P0 组件清单里反复引用的 `NpcArchetype::Mundane`（喂给 `npc_runtime_bundle_with_age`/`NpcDailySchedule::for_archetype`/`home_base_for_archetype`）在当前 `NpcArchetype` enum 里**不存在**，必须在 P0 决策门内定案，否则实施阶段第一步就编译不过。

**决议**：
1. `NpcArchetype`（`server/src/npc/lifecycle.rs:90-108`）现 11 变体，均为穷尽 `match`（无 wildcard `_`）。P0 新增 `NpcArchetype::Mundane` 第 12 个变体，并同步补全该 enum 自身全部穷尽分支：`ALL`（`:142-154`，数组扩到 12）、`as_str`/`from_str`（`:156-194`，字符串 `"mundane"`，二者严格对称）、`budget_bucket()`（`:197-208`）、`default_max_age_ticks()`（`:210-226`）、`uses_lifespan_aging()`（`:228-233`）。
2. `budget_bucket()` 归类到 `NpcBudgetBucket::Beast` 档（`lifecycle.rs:205` 现有 `Self::Beast | Self::Fuya => NpcBudgetBucket::Beast` 追加 `Self::Mundane`）——不新开 bucket：语义上凡兽和妖兽都是"野生生物"预算池，且 `NpcBudgetBucket::ALL`/`default_cap()`（`:124-136`）不需要改动；拒绝"新开 Mundane bucket"路线的理由：三桶系统（Humanoid/Beast/Special）是 `plan-npc-overhaul-v1` 定的封闭分类，新增第四桶要求同步改 `NpcRegistry`/`reserve_bucket_spawn` 等多处消费方，超出本 plan 边界且无实际收益（凡兽走独立的 `AmbientSchedulerState<MundaneFaunaMarker>` 被动池自行限流，`NpcBudgetBucket` 只是 `NpcRegistry` 全局计数副产物，不是本 plan 真正的密度控制入口——真正上限是 §8.1 #4 的 `mundane_passive_budget_fn.max_alive`）。
3. **穷尽 match 的连锁范围**：`server/src/npc/` 下至少 `equipment.rs:287`（`match archetype`，11 支穷尽无 wildcard，需补 `NpcArchetype::Mundane => NpcEquipment::default()`——凡兽天生武器无装备，同 `Beast`/`Zombie` 档）是另一处编译期会报错的非通配符 match；`schedule.rs:100`/`seasonal_behavior.rs:61,95,121,146` 等已有 `_ => fallback` 通配符分支的不会编译报错，但需要人工确认 fallback 语义（如 `fallback_schedule_weights()`）是否适合凡兽日程——不能假设"编译过了=行为正确"。P0 实施时以 `cargo build` 报错列表为唯一权威缺口清单逐一补分支，本决议只锁定"必须做"和"两处已知的具体点"，其余以编译器输出为准。

**落点**：`server/src/npc/lifecycle.rs:90-233`（enum 变体 + 6 处自身穷尽 match）/ `server/src/npc/equipment.rs:287`（`NpcEquipment::default()` 分支）/ plan §P0「组件清单」通用底盘段 + §8.1 #2（`NpcBudgetBucket` 不变）。
