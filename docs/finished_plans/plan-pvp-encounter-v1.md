...
- [ ] **每次遭遇都是一场没有对话的谈判**：两个无名修士在荒野相遇——眼神（实际是移动方向/停顿/距离）是唯一的沟通方式。系统应提供足够的非语言信号让双方做出有意义的判断
- [ ] **三幕遭遇结构**：遭遇前（远距离发现→评估→决策）→ 遭遇中（近距离交互→试探→爆发 or 和平）→ 遭遇后（幸存者处理信息→改变行为→影响下一次遭遇）
- [ ] **不鼓励无差别 PvP**：战斗是真元汇率兑换——打赢了也可能耗尽真元死在野兽手里。遭遇的设计应让玩家自然倾向于"不打"除非有明确收益
- [ ] **信息差是遭遇的核心引擎**：你不知道对方是谁/什么境界/什么流派/有什么后招/是不是诱饵/身后有没有队友。这 6 个"不知道"驱动每一次遭遇的紧张感
- [ ] **从默认敌对到临时合作到背叛——让故事自然发生**：系统不应强制合作或强制敌对，但应提供让合作→背叛叙事自然涌现的条件
