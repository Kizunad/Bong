...
- **关键 commit**
  ...
  - `7e3b3d15c` · 2026-05-11T11:45:02+12:00 · `plan-poison-trait-v1: 收紧毒丹 review 回归测试`
...
- **测试结果**
  ...
  - `CARGO_PROFILE_TEST_DEBUG=0 CARGO_INCREMENTAL=0 cargo test poison_trait -j 1`（server）→ 119 passed / 3979 filtered。
  - `CARGO_PROFILE_TEST_DEBUG=0 CARGO_INCREMENTAL=0 cargo test cast_ -j 1`（server）→ 53 passed / 4044 filtered。
  - `CARGO_PROFILE_TEST_DEBUG=0 CARGO_INCREMENTAL=0 cargo test parse_item_effect -j 1`（server）→ 3 passed / 4095 filtered。
  ...
  - `cd agent/packages/tiandao && npm test -- poison-trait-runtime` → 1 file / 6 tests passed。
  - `cd agent/packages/tiandao && npm test` → 50 files / 344 tests passed。
