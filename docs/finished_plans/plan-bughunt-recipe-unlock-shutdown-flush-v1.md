...
- `server/src/shutdown.rs`、`server/src/main.rs`、`server/src/server_readiness.rs`：命名 listener thread 在 Unix 注册 `SIGINT`/`SIGTERM`，通过容量为 1 的 channel 在 `PreUpdate` 仅发送一次 `AppExit::Success`；生产 `build_server_app()` 安装 bridge，并在完整首次 Startup frame 后以同目录 0600 临时文件 + hard-link no-replace 原子发布精确 PID readiness，防止陈旧或并发发布者覆盖权威。
- `server/tests/shutdown_signal.rs`、`server/tests/full_app_startup.rs`：真实子进程经完整 Startup 后发布 readiness，再接收 `kill -INT` / `kill -TERM`，验证正常退出、`AppExit → Last` 落盘、JSON hydrate、无 `.tmp` 残留，以及 production PostStartup readiness 的 no-replace 契约。
- `scripts/lib/bong-server-lifecycle.sh`、`scripts/{start,stop,dev-reload}.sh`：完整生命周期事务由同一 `flock` 串行；ownership record 以 PID、`/proc/<pid>/stat` starttime、canonical executable 和 `/proc/<pid>/exe` device/inode image identity 验证。标准停服按 TERM→有界等待→身份复核→KILL 处理，并保留 `0=graceful`、`3=identity-safe forced cleanup`、`1/2=unsafe or uncertain` 三态：start/reload 对 3 明确告警后才允许替换，stop 对 3 完成 tmux/Agent/Redis teardown 后仍返回 3，1/2 全部 fail closed；startup readiness 失败 cleanup 的 3 仍保留 tmux 诊断现场。
- `scripts/lib/bong-pidfd-signal.py`、`scripts/lib/bong-listener-owner.py`：signal 前通过 pidfd 重验 starttime、executable identity 与 expected PGID；readiness 只有在目标 PID 的 `/proc/<pid>/fd/socket:[inode]` 与其 `/proc/<pid>/net/tcp` 对应 25565 LISTEN inode 相交且前后身份稳定时成立，generic TCP connect 不再允许 foreign listener 冒充目标 server。
- `scripts/lib/bong-process-group-supervisor.py`、`scripts/e2e-redis.sh`：preview server 运行于 persistent 私有 session/process group，父进程按 `READY pid=… → C → COMMITTED` 握手，在 commit ACK 与 owner identity 重验完成前不发布 authority；cargo/server 不继承控制 FD，未 commit、坏命令或 EOF 都回滚完整 group。
- `scripts/test-server-lifecycle.sh`、`scripts/test-dev-reload-disown.sh`、`scripts/test-{listener-owner,supervisor-protocol,tmux-shutdown-order,process-group-snapshot}.sh`、`scripts/smoke-test-e2e.sh`：锁住优雅 TERM、KILL 升级、production caller 0/3/1/2 传播、malformed/foreign record fail-closed、精确 listener ownership、supervisor commit ACK、陈旧 TERM/KILL 成员快照、真实 tmux 的 `AppExit/Last` 先于 teardown、跨锁串行、可执行文件置换与 detached launch record 契约。
