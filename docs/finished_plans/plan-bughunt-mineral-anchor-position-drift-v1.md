...
> 状态：Finished（✅ 2026-07-19）。本文件恢复自 `353225a4` 的父提交 `c3018995`；PR #1187 合并时删除了 skeleton，却没有留下 active / finished plan。P0/P1 保留 2026-07-18 的历史验真日期；P2/P3 于 replacement 分支从最新 `origin/main` 干净拓扑重建，并补齐真实生产 Startup 排序、最终候选整批 fail-closed 与非空 fossil 原子性回归。
...
| P2 | 锁定 runtime manifest 契约与锚点物化边界 | ✅ 2026-07-19 |
| P3 | 当前主线验真、测试证据与归档收口 | ✅ 2026-07-19 |
...
- `startup_fails_closed_before_materializing_invalid_anchor` 锁定中心点级错误 manifest 下索引与 `MineralOreNode` 均为 0；`startup_keeps_ordinary_and_fossil_materialization_atomic_when_later_candidate_fails` 锁定“前序合法 anchor + 后序最终候选被嵌套 zone 抢夺”时仍零物化；`startup_spawns_index_entries_and_skips_exhausted_positions` 锁定有效启动、耗尽过滤和 `Gatherable` 挂载。
...
- [x] 最终候选 preflight：`final_candidates_accept_radius_exactly_on_declared_aabb_boundary`、`final_candidates_reject_radius_one_block_outside_declared_aabb`、`final_candidates_reject_surface_snap_below_declared_aabb`、`final_candidates_reject_more_specific_zone_capture_at_radius_edge`、`startup_keeps_ordinary_and_fossil_materialization_atomic_when_later_candidate_fails`；targeted `mineral::anchors::tests` 21/21 PASS。
...
- P2：`server/src/mineral/anchors.rs` — `prepare_mineral_anchor_positions` / `validate_final_anchor_positions` 在任何 `Commands::spawn`、`MineralOreIndex` 写入、化石物化之前整批校验最终候选；`production_registration_runs_mineral_startup_after_zone_registry` 通过真实 `mineral::register` 与真实 `zone::register` 锁住生产 Startup 排序；`startup_fails_closed_before_materializing_invalid_anchor`、`startup_keeps_ordinary_and_fossil_materialization_atomic_when_later_candidate_fails`、`startup_spawns_index_entries_and_skips_exhausted_positions` 及 4 条 final-candidate 边界/嵌套-zone 测试；`server/src/world/terrain/raster.rs` — `TerrainProvider::with_fossil_for_tests` 提供最小 mmap-backed 非零 fossil mask fixture。
