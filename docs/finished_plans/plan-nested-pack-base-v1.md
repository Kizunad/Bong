...
每 PR 走完整等待：`gh pr checks <PR>` `pending` → `ScheduleWakeup delaySeconds=1200`，最多 3 回合（60 min）；`fail` 按 skills/consume-plan/SKILL.md step 7 严重性桶处理；修完 review **必重等 CR re-review**，不自判通过；前一个 PR 未 APPROVED/收敛不开下一个。
