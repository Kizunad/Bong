...
3. **`water_skin` 灌装实装**——已移出套包 scope，归属 `docs/plan-satiety-hydration-v1.md:114`（饱食度/水分双轴生存系统骨架，2026-07-18 已立骨架；截至本次归档在 origin/main 上仍为 skeleton，其升 active + P0 实装由在跑的 PR #1259（`auto/plan-satiety-hydration-v1-pr1`）承接）。该骨架文档已明确记录 `water_skin_filled` 需要从零接线（TOML 注册 + 消费链 + icon），不存在捷径。
...
- **遗留 / 后续**：
  ...
  3. `water_skin` 灌装实装 → 归 `docs/plan-satiety-hydration-v1.md:114`（归档时在 origin/main 上仍为骨架；升 active + P0 由在跑的 PR #1259 承接）。
