...
- **落地清单**：P0 实地追踪 `agent/packages/tiandao/src/main.ts` 启动链与 `validate*Contract` imports，确认原 5 项并补发现 live 的 `CraftOutcomeV1`、`RecipeUnlockedV1`、`SkillLvUpPayloadV1`；排除未实例化的 `FactionCensusStore` / `FactionStateV1`。P1 在 `agent/packages/schema/src/schema-registry.ts` 注册 8 项契约并生成 `agent/packages/schema/generated/{meridian-severed-event,tuike-ash-decay,elder-encounter-event,faction-war-event,named-faction-state,craft-outcome,recipe-unlocked,skill-lv-up-payload}-v1.json`。P2 在 `agent/packages/schema/tests/generated-artifacts.test.ts` 通过 TypeScript AST 从 Tiandao 非测试生产源码发现实际 import 且调用的 `validate*V1Contract`，逐项对拍 TypeBox schema identity 与 `GENERATED_SCHEMA_FILES`；非 server→agent public wire validator 进入带 `reason`/`expiresOn` 的集中显式豁免（严格 ISO 日期且不得过期）；AST 同时覆盖 namespace member call、named import 一跳 alias，并排除 test/spec/`__tests__`；另以独立 filename→`SCHEMA_REGISTRY` 命名规则覆盖全部 405 项生成物。P3 对抗追踪所有实际启动 Tiandao validator 后 clean。
- **关键 commit**：`4a1f7474`（2026-07-13）纳入首批 5 项天道运行时契约；`909edf4c`（2026-07-13）对抗复核后补齐 craft/recipe/skill-lv-up 三项相邻 live 契约；本次返工 commit（2026-07-13）消除手写 8 项同源白名单与局部 identity 自证；本次二轮返工补齐 namespace/alias/测试目录发现边界、结构化有期限豁免及合法 schema 互换 identity 回归（SHA 见本条提交）。
- **测试结果**：`cd agent/packages/schema && npm test`（29 files / 853 tests PASS，含 happy/missing/wrong-mapping/豁免边界与 test-file 排除）；`npm run generate:check`（405 generated schemas fresh）；`cd agent && npm run build -w @bong/schema` PASS；`cd agent/packages/tiandao && npm test` PASS；`cd agent && npm run build` PASS。合并最新 `origin/main` 后再次执行 schema 850 tests 与 schema build 均 PASS。
