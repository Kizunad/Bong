...
**对标 AlchemyScreen**：`BaseOwoScreen<FlowLayout>` / PANEL_W=640 PANEL_H=340 / `Surface.flat(#0D0D15)+outline(#4A4050)`

**关键约束**：CraftScreen 必须与 InventoryScreen / AlchemyScreen / ForgeScreen **tab 页高度对齐**（340px 固定面板高度），切换时无跳动。

```
┌─────────────────────────── 640×340 ──────────────────────────────────┐
│ header: "手搓台"                          "C 关闭 · 双击快速制作" 20px│
├────────┬──────────────────────────────────┬──────────────────────────┤
│160px   │           flex (≈280px)          │        200px             │
│        │                                  │                          │
│[全 甲  │    ── 铁甲·胸甲 ──              │    ┌──────┐              │
│ 兵 具  │                                  │    │产物图│              │
│ 丹 杂] │    ┌──┐ ┌──┐ ┌──┐ (44×44px)   │    └──────┘              │
...
│        │    ═══[进度条 65%]═══            │                          │
├────────┴──────────────────────────────────┴──────────────────────────┤
│ bottom 32px: [一键填充]  [- 1 +]                    [材料不足] │
└──────────────────────────────────────────────────────────────────────┘
```

**配色方案**（对标 AlchemyScreen 实际代码）：
- 面板底色：`Surface.flat(0xFF0D0D15).and(Surface.outline(0xFF4A4050))`
- 左栏（配方列表）：`#1A1814` 底 + `#4A4030` 描边（暖棕调，同 Alchemy scrollColumn）
- 中栏（材料）：无独立底色，跟面板
- 右栏（产物）：`#14141C` 底 + `#3A3A50` 描边（冷色偏蓝，区分左栏暖棕）
- 材料格子：`GridSlotComponent` 风格 `#1E1E1E` + 绿边 `#3A6A3A`（充足）/ 红边 `#6A3A3A`（缺失）
- 底部杠：`#12121C` + top border `#3A3040`

**Tab 对齐要求**：
- CraftScreen 与其他 screen 共用 tab 系统时，面板 top-y 和 bottom-y 必须完全一致
- 如果游戏内有统一 tab bar（背包/锻造/炼丹/手搓 横向 tab），CraftScreen 只替换内容区，tab bar 高度不变
- 参考 mockup HTML：`docs/mockup-craft-ui.html`
...
- **协议 / 运行时**：`craft_start` 支持 `quantity`（缺省 1，拒绝 0）；server `CraftSession` 支持批量预扣材料和 qi、逐件完成、`completed_count/total_count` 进度同步、取消时仅按未完成件数 70% 返还材料；agent schema 同步 `CraftStartReqV1.quantity` 与 `CraftSessionStateV1.completed_count/total_count`。
- **关键 commit**：`a3564a4ac` 实现手搓界面三栏交互；`a3885a98d` 支持手搓批量制作会话；`904ade38b` 同步手搓批量契约 schema；`355115917` 关闭手搓界面时取消会话。
- **验证**：`client`: `JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64 PATH=/usr/lib/jvm/java-17-openjdk-amd64/bin:$PATH ./gradlew test build`；`agent`: `npm run build`；`agent/packages/schema`: `npm test && npm run generate:check`；`server`: `cargo fmt --check`、`CARGO_BUILD_JOBS=1 cargo check`、`CARGO_BUILD_JOBS=1 cargo clippy --all-targets -- -D warnings`、`CARGO_BUILD_JOBS=1 cargo test craft_start -- --nocapture`、`CARGO_BUILD_JOBS=1 cargo test batch -- --nocapture`、`CARGO_BUILD_JOBS=1 cargo test`（3977 passed）。
