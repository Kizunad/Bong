...
4. 集成 pin `rat_bite_and_death_cycle_preserves_world_qi_total`（新增 `server/src/qi_physics/ledger.rs` 测试模块或 `combat` 集成测试）：模拟"咬 3 次 qi_steal=2 + 鼠死亡"完整链路（复用 `apply_rat_bite_qi_drain` + `release_drained_qi_on_death_system` 两系统跑同一个 `App`），断言链路头尾 `summarize_world_qi(...)` 的 `player_qi + zone_qi + ledger_qi` 总和严格相等（float epsilon 容差），failure message 引用 worldview §二守恒律。**本 pin 必须把 `world::heartbeat::zone_qi_inflow_tick`（生产 `register()` 里注册进 `Update` 的常驻系统，heartbeat.rs:454-468）一并注册进测试 `App` 并在鼠死亡结算后额外推进至少一个 tick**（例：`app.add_systems(Update, (apply_rat_bite_qi_drain, release_drained_qi_on_death_system, zone_qi_inflow_tick).chain())`）——如果 `transfer_rat_drained_qi_to_zone` 只改了 `zone:<name>` 账户余额、没有同步写回 `zone.spirit_qi` 字段，`zone_qi_inflow_tick` 的覆盖式 `set_balance`（heartbeat.rs:2131-2138）会把刚转入的账户余额用未变的字段值重新覆写清零，本 pin 必须能撞红这一二次蒸发回归；此前版本的 pin 不含 `zone_qi_inflow_tick` 视角，看不到这层碰撞，是本次 promote 博弈 blocker 指出的核心风险点。
   ...
   **★第二轮 blocker 修正——只说"注册 + 推进一 tick"规格不足，必须显式点名 4 道门，否则对"写回字段"和"不写回字段"两版实现都会通过（false-green）**。`zone_qi_inflow_tick`（heartbeat.rs:2077-2160）的覆盖式 `set_balance`（:2134-2138）挂在循环体内、连续 4 道 `continue` 门之后，必须同时满足才会执行到该行；本 pin 的测试夹具必须**逐条显式配置**，照抄 `heartbeat.rs:3091-3106` 现有 `inflow_test_app` 模板（不是重新发明一套 fixture）：
   - **门①（`CultivationClock` 在场且 `elapsed_ticks > 0`）**：`insert_resource(CultivationClock { tick: <非零起始值> })` 起步就用非零 tick（对齐 `inflow_test_app(zones, pending_pool_balance, start_tick)` 的 `start_tick` 参数），而不是默认 0——若从 0 起步、且死亡结算与 `zone_qi_inflow_tick` 挤在同一次 `app.update()` 里推进，`elapsed_ticks = current_tick.saturating_sub(clock_state.last_tick)` 算出的是 `0`，门①直接 `return`（heartbeat.rs:2096-2098），覆盖式 clobber 永远够不到，pin 对"漏写字段"这条回归视而不见。死亡结算跑完后，必须照抄 `heartbeat.rs:3108-3112` 的 `advance_ticks` helper 再显式推进至少 1 tick，让 `zone_qi_inflow_tick` 在死亡结算**之后**单独多跑一次。
   - **门②（`zone.qi_equilibrium > 0.0 && zone.qi_inflow_per_min > 0.0`）**：测试用 `Zone` 禁止用 `Zone::default()`（两字段均为 `0.0`，会在门②直接 `continue`，heartbeat.rs:2102-2104）。必须显式设为非零，照抄 `zones.json` `spawn` 区真实配置 `qi_equilibrium = 0.35` / `qi_inflow_per_min = 0.4`（`heartbeat.rs` 既有 pin 如 `:3163-3164` 同款写法）。
   - **门③（`zone.spirit_qi < zone.qi_equilibrium`，使 `desired_absolute > 0`）**：测试 zone 的 `spirit_qi` 必须在鼠死亡结算前后都严格小于 `qi_equilibrium`（例如 `spirit_qi = 0.1 < 0.35`），否则 `zone_equilibrium_inflow` 算出 `desired_absolute <= 0.0`，门③在 :2120-2122 直接 `continue`。
   - **门④（`pending_inflow_account()` 池有余额，使 `actual_absolute > 0`）**：必须给 `pending_inflow_account()` 注资（例如 `ledger.set_balance(pending_inflow_account(), 1000.0)`，照抄 `heartbeat.rs:3097-3102` `inflow_test_app` 的 seeding 写法）——池余额为 0 时 `actual_absolute` 恒为 `0.0`，门④在 :2127-2129 `continue`，同样够不到覆盖式 `set_balance`。
   - **四道门必须同时点名**，任何一道漏配置都会让 `zone_qi_inflow_tick` 在循环体内提前 `continue`，本 pin 就退化成"注册了系统但从未真正跑到 clobber 那一行"的形式通过。
   - **实现顺序强制要求（红→绿两态验证）**：实施时**先**起一版"`transfer_rat_drained_qi_to_zone` 只 `transfer` 账户余额、不写回 `zone.spirit_qi` 字段"的 stub 实现，跑一次本 pin **必须撞红**（证明四道门确实全部触达、`zone_qi_inflow_tick` 的覆盖式 `set_balance` 真的把刚转入的余额清零、本 pin 真的在盯这条二次蒸发回归，而不是摆设）；确认红了之后，**再**接上 §8.1 #3 决议的完整"同步→transfer→写回字段"三段式实现，本 pin 才应该转绿。红版验证不是可选步骤——PR 描述或 commit message 里必须留痕"先跑 stub 撞红、再跑真实 impl 转绿"这一验证过程，仅提交一版直接绿的实现不足以证明 pin 真的守住了这条回归（对应 CLAUDE.md「测试要把目标行为完全锁住」的饱和化测试要求）。
...
6. **promote 博弈守恒 blocker 三次收口（2026-07-04，第二轮）**：反方进一步指出 §8.1 #3 决议虽然定案了实现方案（三段式写回字段），但 P0 验收抓手 #4 的 pin 规格本身不足以撞穿"实现方没写回字段"这条回归——`zone_qi_inflow_tick` 的覆盖式 `set_balance` 挂在 4 道连续 `continue` 门之后，pin 只说"注册系统 + 推进 tick"而不点名门②（`qi_equilibrium`/`qi_inflow_per_min` 非零）、门③（`spirit_qi < qi_equilibrium`）、门④（`pending_inflow_account()` 池有余额），测试 zone 若用 `Zone::default()`（两字段皆 0.0）或不注资 pending 池，`zone_qi_inflow_tick` 会在门②/④提前 `continue`，永远碰不到 clobber 那一行，对"写回字段"和"不写回字段"两版实现都通过（false-green）。人工复核 `heartbeat.rs:2077-2160` 循环体逐门核实 4 道 `continue` 门的触发条件，并核对 `heartbeat.rs:3091-3106` 现有 `inflow_test_app` 测试模板确认可复用的 fixture 写法（`zones.json` `spawn` 区 `qi_equilibrium=0.35`/`qi_inflow_per_min=0.4` 为真实非零值范本），采纳修正：P0 验收抓手 #4 改写为逐条点名全部 4 道门的显式夹具规格 + 强制"先跑 stub 撞红、再跑真实 impl 转绿"两态验证流程，见 §P0 验收抓手 #4 与 §8.1 #3 决议第 5 点。
...
5. **★第二轮 blocker 修正（2026-07-04 三次收口）**：#4 只写"注册 `zone_qi_inflow_tick` + 推进 tick"仍规格不足——`zone_qi_inflow_tick` 循环体内的覆盖式 `set_balance`（heartbeat.rs:2131-2138）挂在 4 道连续 `continue` 门之后（`CultivationClock` 在场且 `elapsed_ticks>0` / `zone.qi_equilibrium>0 && qi_inflow_per_min>0` / `spirit_qi<qi_equilibrium` / `pending_inflow_account()` 池有余额），4 道门任一没点名，测试对"写回字段"和"不写回字段"两版实现都会通过（false-green），守不住二次蒸发回归。P0 验收抓手 #4 已改写为显式点名全部 4 道门的夹具规格（照抄 `heartbeat.rs:3091-3106` `inflow_test_app` 模板），并要求实现时先跑"只改账户不写回字段"的 stub 确认 pin 撞红、再接真实三段式实现确认转绿，两态验证过程需在 PR/commit 留痕。
