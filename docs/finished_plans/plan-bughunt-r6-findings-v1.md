...
|---|---|---|---|---|
| #8 mundane armor duplicate 早退 | `server/src/armor/mundane.rs:249-270` 当前 duplicate 走 skip-on-duplicate；`:332-373` 覆盖非空 registry | `already-fixed/invalid`（already-fixed） | `e42092c11` / PR #1068 | 仅归档 |
| #0 `ContaminationBoost` 无 consumer | `server/src/alchemy/side_effect_apply.rs:25` 仍生产；`server/src/cultivation/contamination.rs` tick 不读 `StatusEffects` | `independent-domain-fix` | `docs/plans-skeleton/plan-bughunt-modifier-effect-consumer-completion-v1.md` P1 | 统一 alchemy effect owner |
