...
> **一句话主题**：补齐 `vortex_spiral` 粒子贴图的 Minecraft 1.20.1 atlas `single` source，并以通用对账测试与涡流专属测试锁住 descriptor → atlas → PNG 的完整资产契约。
...
| 阶段 | 状态 | 可核验结果 |
|---|---|---|
| P0 atlas / 资产链修复 | ✅ 2026-07-03 | `vortex_spiral.json` 引用 `bong:particle/vortex_spiral`，32×32 RGBA PNG 存在，`particles.json` 以 `single` source 登记同一资源 |
| P1 防回归测试 | ✅ 2026-07-07 | `ParticleAtlasReconciliationTest` 双向对账全部粒子资产；`VortexSpiralParticleAssetTest` 专门 pin 涡流 descriptor、PNG 与 atlas source |
...
2. `assets/bong/textures/particle/vortex_spiral.png` 是真实存在且含可见像素的 PNG；
...
缺少第 3 项时，上游 event、registry、factory 与粒子参数都可正常执行，但 sprite 不会被烘焙进 atlas，玩家看到的是静默空白。PR #838 的 `754b8c3f` 已修复该根因并加入全局对账；PR #1079 的 `3cb86210` 再以涡流专属测试锁住此用户可见路径。
...
- `client/src/main/resources/assets/bong/textures/particle/vortex_spiral.png` 存在；当前文件为 32×32、8-bit RGBA、851 bytes，含 250 个 alpha > 0 的像素，不是空白占位。
- `client/src/main/resources/assets/minecraft/atlases/particles.json` 以 exact `single` source 登记 `bong:particle/vortex_spiral`。

### P1 — 防回归测试 — ✅ 2026-07-07

- `VortexSpiralParticleAssetTest::vortexSpiralDescriptorReferencesCommittedTexture` 断言 descriptor 与 PNG 均为普通文件，并 pin exact texture ID。
- `VortexSpiralParticleAssetTest::vortexSpiralTextureIsRegisteredAsParticleAtlasSingleSource` 解析 atlas，要求 exact `type=single` + exact resource。
- `ParticleAtlasReconciliationTest::everyReferencedParticleTextureIsWhitelistedInAtlasAndHasPng` 对所有粒子 descriptor 正向检查 atlas 与 PNG。
- `ParticleAtlasReconciliationTest::atlasWhitelistHasNoDanglingEntries` 反向拒绝 atlas 中没有 PNG 的悬空条目。
...
- exact-head Actions run `28849850549` / job `85562238335` 的唯一 GitHub Actions check `e2e` 为 `SUCCESS`。原始 job log 明确记录 `java-version: 17`、执行 `./gradlew test`，并以 `BUILD SUCCESSFUL in 1m 3s` 收口；因此本 PR 新增的专属测试与既有通用对账测试均在 final head 的完整 client test stage 内执行成功。
...
- 本 plan 不要求新增游戏内截图；其验收对象是 atlas 资产契约、生产引用链与防回归测试。

## Finish Evidence

### 落地清单

- **资源契约**：`client/src/main/resources/assets/bong/particles/vortex_spiral.json`、`client/src/main/resources/assets/bong/textures/particle/vortex_spiral.png`、`client/src/main/resources/assets/minecraft/atlases/particles.json`。
- **通用防线**：`client/src/test/java/com/bong/client/visual/particle/ParticleAtlasReconciliationTest.java`。
- **专属防线**：`client/src/test/java/com/bong/client/visual/particle/VortexSpiralParticleAssetTest.java`。
...
- `3cb86210e0f8c02053e627b59d1d53fec499b896`（2026-07-07，PR #1079 final head）— 新增 `VortexSpiralParticleAssetTest` 专属 pin。
...
- 当前树静态复核：descriptor JSON 与 atlas JSON 可解析且 exact ID 一致；PNG signature、32×32 RGBA 元数据和非透明像素均有效；两个测试类的 4 个契约 case 仍在现行源码中。
...
- 本 plan 定义的 atlas source、descriptor、PNG、专属 pin 与通用对账均已交付，无独立未完成项。
- `ash_spider_disguised.png` 与技能图标 backlog 仍由各自 plan / 美术流程负责，不阻塞本 plan 归档。
- 历史 PR #1079 没有成功启动 CodeRabbit review，也没有粒子画面 artifact；本归档只声称 exact-head CI、资产契约与生产引用链已核验，不将额度失败或通用 e2e artifact包装成历史 review / 视觉证明。
- 当前逐招专属 event ID 继续共享 `vortex_spiral` sprite 是有意的资源复用；后续若重命名 descriptor、atlas entry 或 PNG，两个 asset test 必须同步撞红并阻止静默隐形回归。
