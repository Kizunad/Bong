...
> **一句话主题**：补齐 `vortex_spiral` 粒子贴图的 Minecraft 1.20.1 atlas `single` source，并以通用对账测试与涡流 exact-ID pin 锁住 descriptor → atlas → PNG 文件存在性/引用一致性（不做 PNG 解码）。
...
| P1 防回归测试 | ✅ 2026-07-07 | `ParticleAtlasReconciliationTest` 双向对账 descriptor↔atlas↔PNG 文件存在；`VortexSpiralParticleAssetTest` exact pin 涡流 texture ID / 路径与 atlas `single` source |
...
缺少第 3 项时，上游 event、registry、factory 与粒子参数都可正常执行，但 sprite 不会被烘焙进 atlas，玩家看到的是静默空白。PR #838 的 `754b8c3f` 已修复该根因并加入全局对账；PR #1079 的 `3cb86210` 再以涡流 exact-ID pin 防止 `vortex_spiral` 被单独改名或漏登记 atlas。

**自动测试契约边界（与生产可见性要求区分）**：现有 4 个 asset test **只**断言普通文件存在、descriptor exact texture ID、atlas `single` source 对账；**不解码 PNG**，不锁 32×32 / RGBA / 非全透明，也不保证运行时画面非空白。32×32 RGBA 与非透明像素属于归档时的**静态人工复核事实**，不是 JUnit 契约。
...
- `ParticleAtlasReconciliationTest::everyReferencedParticleTextureIsWhitelistedInAtlasAndHasPng`：对所有粒子 descriptor 正向检查：引用的 `bong:particle/<name>` 已在 atlas 登记，且 `textures/particle/<name>.png` 存在（`Files.exists`，不解码）。
- `ParticleAtlasReconciliationTest::atlasWhitelistHasNoDanglingEntries`：反向拒绝 atlas 中没有对应 PNG 文件的悬空 `bong:particle/*` 条目。

**重命名边界**：

- **exact-ID pin**（`VortexSpiralParticleAssetTest`）：硬编码路径与 `TEXTURE_ID`，单独把 `vortex_spiral` 改名会撞红。
- **通用对账**（`ParticleAtlasReconciliationTest`）：只保证 descriptor ↔ atlas ↔ PNG **彼此一致**；若三者整体同步重命名为同一新 ID，通用对账仍可通过，**不会**因「离开 `vortex_spiral` 这个名字」而失败。
...
- exact-head Actions run `28849850549` / job `85562238335` 的唯一 GitHub Actions check `e2e` 为 `SUCCESS`。原始 job log 明确记录 `java-version: 17`、执行 `./gradlew test`，并以 `BUILD SUCCESSFUL in 1m 3s` 收口；因此本 PR 新增的专属 pin 与既有通用对账测试均在 final head 的完整 client test stage 内执行成功。
...
- 本 plan 不要求新增游戏内截图；不新增 PNG 解码/像素级测试。验收对象是 atlas 接线、文件存在性对账、exact-ID pin，以及生产引用链（像素可见性仅静态人工复核）。
...
- **通用对账（文件存在 + 引用一致）**：`client/src/test/java/com/bong/client/visual/particle/ParticleAtlasReconciliationTest.java`。
- **exact-ID pin（涡流专属路径/ID）**：`client/src/test/java/com/bong/client/visual/particle/VortexSpiralParticleAssetTest.java`。
...
- 当前树复核：
  - **自动测试契约（4 case）**：descriptor/PNG 普通文件存在 + exact texture ID + atlas single-source 双向对账；**不做 PNG 解码**。
...
- 本 plan 定义的 atlas source、descriptor、PNG 文件存在、exact-ID pin 与通用对账均已交付；PNG 像素/格式可见性仅静态复核，无未完成的自动测试项，也不在本 plan 范围扩 PNG 解码测试。
- `ash_spider_disguised.png` 与技能图标 backlog 仍由各自 plan / 美术流程负责，不阻塞本 plan 归档。
- 历史 PR #1079 没有成功启动 CodeRabbit review，也没有粒子画面 artifact；本归档只声称 exact-head CI、文件存在性/引用一致性对账、exact-ID pin 与生产引用链已核验，不将额度失败或通用 e2e artifact 包装成历史 review / 视觉证明。
- 当前逐招专属 event ID 继续共享 `vortex_spiral` sprite 是有意的资源复用。防回归边界：exact-ID pin 阻止 `vortex_spiral` 被单独改名/漏 atlas；通用对账阻止 descriptor↔atlas↔PNG **不一致**；二者都不能单独阻止「descriptor + atlas + PNG 整体一致重命名」。
