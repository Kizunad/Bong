...
- **落地清单**：`PseudoVeinRuntime` component + 5 阶段生命周期 / 聚集加速衰减公式 / 消散→负灵风暴 / `ZoneQiCriticalEvent` / `MigrationTarget` component + 迁徙→兽潮联动 / 伪灵脉光柱 VFX / 大迁徙震动+音效 / 兽潮方向 HUD / agent ecology skill / fallback 自动触发 / 完整反馈环 E2E
- **关键 commit**：P0-P4 各自 hash
- **遗留 / 后续**：
  - 伪灵脉地面装饰（灵草瞬间生长→枯萎的视觉 cycle）→ plan-botany-visual-v2
  - 兽潮内战（resident vs migrant 领地冲突细化）→ plan-fauna-v2
  - 域崩后地形永久变化（馈赠区→死域地形替换）→ plan-terrain-collapse-v1
  - 玩家预测伪灵脉（高境修士感知天道意图 → perception 扩展）
  - NPC 对伪灵脉的反应（散修也会去蹭灵气 → NPC daily-life 日程注入）
