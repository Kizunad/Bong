...
- `076835927b0e8c80d4a35642106a45f07dcbeebb`（2026-07-22）：修复 `UiResponseConsumer` 清理与 pending subscribe 的生命周期竞态；cleanup 开始后不再安装或处理迟到 listener，factory 500ms 超时后的迟到 subscribe completion 同样保持 inert。
...
- **本轮三文件返工（Ajv 备选根 + 所有权注释 + 元数据证据纠偏，代码 HEAD `839a596a7c960a093a4ec3e7ed84849ae83886d9`）**：`TRUSTED_AJV_NODE_MODULES_ROOTS` 明确作为合法备选安装布局处理；候选根仅在 `lstat` 明确返回 `ENOENT` / `ENOTDIR` 时视为缺失并跳过，任何已存在但无法 `realpathSync.native()` canonicalize 的根均 fail-closed。Ajv package metadata 与 runtime 路径自身始终先 canonicalize，且必须共同处于同一个实际存在的 canonical trusted root；自动化反例覆盖 workspace-only、package-local-only、两根均缺失、external path、metadata/runtime 跨根分裂、dangling trusted-root symlink 与 in-root Ajv symlink escape。`npm ci` 按既有根 lockfile 安装 113 packages 并退出 0（仅报告 6 个 audit 项，未运行 audit fix、未改依赖）；`npm run build -w @bong/schema` 退出 0；定向 `npx vitest run tests/generated-artifacts.test.ts` 为 1 file / 26 tests passed（Ajv `8.12.0`、`assertionCount=157`、22 boundary cases × 6 ID fields）；`npm run check` 退出 0 且 406 generated artifacts fresh；完整 schema 为 29 files / 906 tests passed；根 `npm run build` 退出 0；完整 Tiandao 为 72 files / 839 tests passed；`python3 -m unittest scripts.bot.test_protocol` 为 126/126 passed（仍报告与 `origin/main` 同现的 unclosed-socket `ResourceWarning`，不宣称 warning-free）；Java `17.0.19` 的 `./gradlew test build` 为 4,222 tests / 0 failures / 0 errors / 0 skipped；server `cargo fmt --check && cargo clippy --all-targets -- -D warnings && cargo test` 整体退出 0，suite 汇总 11,865 + 11 + 1 + 4 = 11,881 passed / 0 failed / 6 ignored。两份受保护 untracked snapshot 哈希分别保持 `c6ee23bbe36c6fde5f2c5c2d470359c89b0a64174226ab2691a2afe32f82d0ab`、`eb0b116cad4cd803e5548e0faaf5f9d7f77766310304c7e7acf1c964e9d416e8`，均未删除、stage 或覆盖。针对该 SHA 的 bounded validator 已逐字对拍 HEAD，但四个独立 inspect agent 均超时并返回 `null`，最终裁决按缺失报告 fail-closed 为 `passed=false`；这不是代码 finding，也绝不是 PASS，且不得外推到后续 evidence commit。
- **cleanup/subscribe 生命周期返工（代码提交 `076835927b0e8c80d4a35642106a45f07dcbeebb`）**：`UiResponseConsumer` 改为 one-shot `idle → connecting → connected → disconnecting → disconnected` 生命周期；并发 connect 共享一个 subscribe，disconnect 同步关闭消息门、只执行一次 unsubscribe，subscribe 的迟到 resolve/reject 均不得重新挂 listener，终态 reconnect fail-closed，physical Redis disconnect 所有权仍只在 factory。定向 TypeScript 检查与 `agent-ui.test.ts` + `main.test.ts` 为 2 files / 117 tests passed；完整 `cd agent/packages/tiandao && npm test` 退出 0，为 72 files / 846 tests passed。factory 回归覆盖 cleanup 早于 subscribe completion、500ms physical-disconnect timeout 后迟到 completion、exactly-once 三客户端 disconnect 与零 narration/callback/stats 副作用。两份受保护 untracked snapshot 仍未删除、stage 或覆盖，哈希逐字保持 `c6ee23bbe36c6fde5f2c5c2d470359c89b0a64174226ab2691a2afe32f82d0ab`、`eb0b116cad4cd803e5548e0faaf5f9d7f77766310304c7e7acf1c964e9d416e8`。该代码提交后的 evidence commit 与 fresh exact-HEAD validator 尚待完成，不能把旧 validator 结果外推为 PASS。
