...
- `agent/packages/schema/src/schema-registry.ts`、`agent/packages/schema/generated/agent-ui-response-payload-v1.json`、`agent/packages/schema/generated/client-request-v1.json`、`agent/packages/schema/generated/server-data-v1.json`、`agent/packages/schema/tests/generated-artifacts.test.ts`、`agent/packages/schema/package.json`、`agent/package-lock.json`：真实 server→Tiandao response 保持独立 generated artifact；三份生成物按精确 JSON path 锁定 2/1/3 共六个 Agent UI ID 字段，避免字段等量替换假绿，禁止恢复 `minLength/maxLength`，并分别用无 flag 与 Unicode-aware `u` 的实际 artifact pattern 对拍。当时 `generated-artifacts.test.ts` 仅用 JavaScript RegExp（无 flag + `u`）对六个字段 pattern 对拍，没有自动化 Ajv compile/validate 门禁；历史「宿主 Ajv 8.12.0 78/78」属 overclaim，不得再当作当前真相。后续曾短暂用 `createRequire` 解析本机 `/usr/share/nodejs/ajv`（不改 `package.json`）做 host Ajv 门禁，但 GitHub clean runner 无法解析该 host 包（e2e `29712024499` 失败），该 host-only 取径现已**撤回**。当前真相：`@bong/schema` 以可复现 workspace `devDependency` 声明 `ajv@8.12.0`（锁在 `agent/package-lock.json`），测试 `import Ajv from "ajv"` + `loadWorkspaceAjv()` 对三份生成物六个 ID 字段执行 22 边界用例 × 6 = 132 次根 schema validate，另加字段结构 pin 与 root 契约（legacy 缺 `target` / `additionalProperties` / `null` / C2S 禁字段等）；并拒绝解析到 `/usr/share/nodejs/`。`discover` 与 workspace Ajv 用例超时放宽到 60s。
...
- （2026-07-20，本 commit）：撤回 host-only Ajv 取径。改为 `@bong/schema` workspace `devDependency ajv@8.12.0` + lockfile，测试 `import Ajv from "ajv"` / `loadWorkspaceAjv()`，保证 clean CI 可安装复现。
...
- Schema（本轮 CI 可移植修复，基于 `f4f0c92d`）：`@bong/schema` 增加可安装 `devDependency ajv@8.12.0`（`agent/package-lock.json` 锁定），`generated-artifacts.test.ts` 改为 `import Ajv from "ajv"` + `loadWorkspaceAjv()`，保留完整 22 边界 × 6 字段矩阵、RegExp 双模式对拍与 root 契约；resolved path 必须落在 workspace `node_modules` 且拒绝 `/usr/share/nodejs/`。门禁命令与退出码见下方「本轮 gate 实测」。**不预写尚未发生的 final exact-HEAD validator PASS**。
...
- **本轮 gate 实测（CI 可移植 workspace Ajv 修复，起始 HEAD `f4f0c92d`）**：
  - 起始 HEAD 核验：`git rev-parse HEAD` = `f4f0c92d45a097043e8cb84ce3f0fd64d3599127`；分支 `tmp/pr1217-review-fix`。
  - snapshot 保全（未改、未删除）：`agent/packages/tiandao/data/tiandao-snapshot-200.json` / `tiandao-snapshot-300.json` 仍在；哈希见下方终态抓取。
  - 历史 overclaim 继续作废：不得引用「宿主 Ajv 8.12.0 78/78」。
  - host-only 中间态撤回：`createRequire` → `/usr/share/nodejs/ajv`、不改 `package.json` 的写法在 clean CI 失败（e2e `29712024499`），不得再写为可复现依赖。
  - 可复现依赖：`@bong/schema` `devDependencies.ajv = ^8.12.0`，`agent/package-lock.json` 锁定 `node_modules/ajv@8.12.0`；测试 `import Ajv from "ajv"` / `loadWorkspaceAjv()`。
  - 真 Ajv gate：自动化 workspace Ajv compile/validate；基线 22 边界 × 6 字段 = 132 次根 validate，另加字段结构 pin 与 root 契约；实测 stdout `{"ajvVersion":"8.12.0","ajvResolvedPath":".../agent/node_modules/ajv/dist/ajv.js","assertionCount":157,"boundaryCases":22,"idFields":6}`。
