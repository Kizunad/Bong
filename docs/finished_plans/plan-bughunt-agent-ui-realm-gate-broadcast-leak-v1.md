...
- `agent/packages/schema/src/schema-registry.ts`、`agent/packages/schema/generated/agent-ui-response-payload-v1.json`、`agent/packages/schema/generated/client-request-v1.json`、`agent/packages/schema/generated/server-data-v1.json`、`agent/packages/schema/tests/generated-artifacts.test.ts`：真实 server→Tiandao response 保持独立 generated artifact；三份生成物按精确 JSON path 锁定 2/1/3 共六个 Agent UI ID 字段，避免字段等量替换假绿，禁止恢复 `minLength/maxLength`，并分别用无 flag 与 Unicode-aware `u` 的实际 artifact pattern 对拍。当时 `generated-artifacts.test.ts` 仅用 JavaScript RegExp（无 flag + `u`）对六个字段 pattern 对拍，没有自动化宿主 Ajv compile/validate 门禁；历史「宿主 Ajv 8.12.0 78/78」属 overclaim，不得再当作当前真相。自 baseline `5a019eeeea5d05ea1cf23c37ad09d662d91c82c7` 起，同文件新增 `loadHostAjv()` + 根 schema compile 矩阵：宿主 Ajv 8.12.0（`createRequire` 解析到 `/usr/share/nodejs/ajv/dist/ajv.js`，package 路径 `/usr/share/nodejs/ajv@8.12.0`，不改 `package.json`）对三份生成物六个 ID 字段执行 22 边界用例 × 6 = 132 次根 schema validate，另加字段结构 pin 与 root 契约（legacy 缺 `target` / `additionalProperties` / `null` / C2S 禁字段等）。本轮实测 `assertionCount=157`（≥132 下界仍锁住全矩阵），`discover` 与 host Ajv 用例超时放宽到 60s。
...
- （2026-07-20，本 commit）：纠正 Finish Evidence 的宿主 Ajv 78/78 overclaim，回填真实 gate 数字。
...
- Schema（本轮 baseline `5a019eeeea5d05ea1cf23c37ad09d662d91c82c7` → test commit `bdc37989b`）：同文件新增 `loadHostAjv()` + 根 schema compile 矩阵，host Ajv gate 已自动化——宿主 Ajv 8.12.0（`createRequire` → `/usr/share/nodejs/ajv/dist/ajv.js`，不改 `package.json`）对三份生成物六个 ID 字段执行 22 边界用例 × 6 = 132 次根 schema validate，另加字段结构 pin 与 root 契约；本轮 `npm test` 实测 stdout `assertionCount=157`（boundaryCases=22, idFields=6）。门禁命令与退出码：`npm run build -w @bong/schema` EXIT 0；`cd agent/packages/schema && npm test` EXIT 0（29 files / 893 tests passed）；`npm run check` EXIT 0（406 generated artifacts fresh）；`cd agent && npm run build` EXIT 0。**不预写尚未发生的 final exact-HEAD validator PASS**。
...
- **本轮 gate 实测（test SHA `bdc37989b`，baseline `5a019eeee`）**：
  - 起始 HEAD 核验：`git rev-parse HEAD` 曾为 `5a019eeeea5d05ea1cf23c37ad09d662d91c82c7`；test 门禁 commit = `30a71c767f4e58d8254d18fd12d68bbd344e4892` + timeout/print 收口 `bdc37989b6de5d2dee2dfb4ddf28aa90304b294a`，分支 `tmp/pr1217-review-fix`。
  - snapshot 保全（未改）：`tiandao-snapshot-200.json` sha256=`c6ee23bbe36c6fde5f2c5c2d470359c89b0a64174226ab2691a2afe32f82d0ab`；`tiandao-snapshot-300.json` sha256=`eb0b116cad4cd803e5548e0faaf5f9d7f77766310304c7e7acf1c964e9d416e8`。
  - 宿主 Ajv：version `8.12.0`；resolvedPath `/usr/share/nodejs/ajv/dist/ajv.js`（`createRequire`，不改 `package.json`）。
  - 真 Ajv gate：自动化 host Ajv compile/validate；基线 22 边界 × 6 字段 = 132 次根 validate，另加字段结构 pin 与 root 契约；实测 stdout `{"hostAjvVersion":"8.12.0","hostAjvResolvedPath":"/usr/share/nodejs/ajv/dist/ajv.js","assertionCount":157,"boundaryCases":22,"idFields":6}`。
  - 命令退出码：`SCHEMA_BUILD_EXIT=0` / `SCHEMA_TEST_EXIT=0`（29 files / 893 tests） / `SCHEMA_CHECK_EXIT=0`（406 fresh） / `AGENT_BUILD_EXIT=0`。
  - 本轮**未**对最终 SHA 另开无上下文 validator；**不预写尚未发生的 final exact-HEAD validator PASS**。
