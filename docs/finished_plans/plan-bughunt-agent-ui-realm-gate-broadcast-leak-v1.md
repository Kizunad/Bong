...
- Schema：受影响树 `8b319420` 执行 `cd agent && npm run build -w @bong/schema` 退出 0；合并主线并补精确 path pin 后，最终测试树 `3a8dd33d` 执行 `cd agent/packages/schema && npm test` 退出 0，29 files / 892 tests passed，其中 `agent-ui.test.ts` 54/54、`generated-artifacts.test.ts` 12/12 passed。旧构建日志位于 `.sisyphus/evidence/pr1217-validator-fixes-8b319420/01-schema-build.log`；最终测试再次证明生成物与 source freshness 一致。
...
- 主线同步：在 `0b75c630` 的 server/schema/Tiandao 复验与证据提交后，紧邻执行 `git fetch origin && git merge origin/main`，合并 commit 为 `37ce9c97`，`origin/main=c5e45f526b4d7ae2410d41710546feffbd07dd43` 是 HEAD 祖先。该 merge 只带入十份 `docs/plans-skeleton/` 文档变更，不触及本 PR 的 server/schema/agent/client/worldgen 路径；随后 `3a8dd33d` 仅收紧 schema 测试自身的精确 path 断言，并在新 HEAD 完整复跑 Schema 29 files / 892 tests。`496c6985` / `0b75c630` 实际触及的 Tiandao 与 server serde 已分别由 831 个 Tiandao 测试和 11,797 个 server 测试复验，未沿用旧的“server 未触及”判断。
