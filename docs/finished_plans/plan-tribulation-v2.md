...
- Review blocker 收敛：`server/src/combat/resolve.rs` 消费 `JueBiLawDisruption`，将绝壁劫法则紊乱接入实战攻击距离、运气通量与反噬伤害；`server/src/cultivation/tribulation.rs` 的地形 overlay 不再为未加载 chunk 记录 `AIR` restore，并固定为触发 tick + 5 分钟恢复。
- Review 后续收敛 v21（`9419060c1` / `3bd927a03`）：`tribulations_active` 持久化 `kind/source/epicenter/intensity`，JueBi 恢复不再伪装成 DuXu，runtime intensity 进入三相伤害/抽压/寂灭衰减；结算按 kind/source 发事件，所有 JueBi 结算都会清 active row；zone/terrain 处理按 dimension 过滤，非 Overworld 不写 Overworld terrain，重叠地形 op 只记录一次原始方块，并修复 partial JueBi epicenter migration。
- Review 后续收敛 v22（`a65c61e14` / `7beeb3a69`）：`tribulations_active` 补 `origin_dimension`，恢复只按持久化起劫维度，不再用重连时 `CurrentDimension` 误绑定；独立 JueBi 存活为 `half_step` 且不占 quota slot，void-quota 追加的 JueBi 成功仍按渡虚占位；缺 active row 的 `complete_tribulation_ascension` 幂等不增 quota，active count 只统计 `du_xu` 与 `jue_bi + void_quota_exceeded`。
...
- P1 强度修正：`world::karma::KarmaWeightStore` 作为可选资源参与 `juebi_intensity_for_source()`；未来触发源统一走 `JueBiTriggerSource`。
...
- `67a53201a9` · 2026-05-10 · `plan-tribulation-v2: 修复绝壁劫 review 阻塞`
- `9419060c1` · 2026-05-10 · `plan-tribulation-v2: 收敛绝壁劫 review 后续`
- `3bd927a03` · 2026-05-10 · `plan-tribulation-v2: 修复绝壁劫迁移后续`
- `a65c61e14` · 2026-05-10 · `plan-tribulation-v2: 修复绝壁劫持久化维度`
- `7beeb3a69` · 2026-05-10 · `plan-tribulation-v2: 收紧绝壁劫配额占位`

### 测试结果

- `cd server && cargo fmt --check` → passed。
- `cd server && cargo clippy --all-targets -- -D warnings` → passed。
- `cd server && cargo test` → 3592 passed。
- `cd server && cargo test juebi -- --nocapture` → 15 passed。
- `cd server && cargo test joined_clients_ -- --nocapture` → 7 passed。
- `cd server && cargo test complete_tribulation_ascension -- --nocapture` → 2 passed。
- `cd server && cargo test ascension_quota -- --nocapture` → 13 passed。
- `cd server && cargo test juebi_law_disruption_reduces_hit_and_backfires_attacker -- --nocapture` → 1 passed。
- `cd server && cargo test juebi_terrain_tick_skips_unloaded_chunks_without_air_restore -- --nocapture` → 1 passed。
- `cd server && cargo test juebi_terrain_tick_records_original_once_for_overlapping_ops -- --nocapture` → covered by `cargo test juebi`。
- `cd server && cargo test juebi_settlement_treats_zero_health_as_killed_even_with_qi -- --nocapture` → covered by `cargo test juebi`。
- `cd server && cargo test juebi_settlement_clears_independent_active_row -- --nocapture` → 1 passed。
- `cd server && cargo test joined_clients_restore_juebi_active_tribulation_kind -- --nocapture` → covered by `cargo test juebi`。
- `cd server && cargo test joined_clients_do_not_bind_missing_tribulation_origin_to_current_dimension -- --nocapture` → covered by `cargo test joined_clients_`。
- `cd server && cargo test active_tribulation_roundtrip_and_delete -- --nocapture` → covered by `cargo test tribulation`。
- `cd server && cargo test joined_client_snapshot_counts_in_flight_tribulation_slots -- --nocapture` → covered by `cargo test tribulation`。
- `cd server && cargo test task13_migration_backfills_legacy_player_cultivation -- --nocapture` → 1 passed。
- `cd server && cargo test phase7_migration_drill_upgrades_legacy_v12_fixture_to_current_schema -- --nocapture` → 1 passed。
- `cd server && cargo test v21_migration_backfills_partial_juebi_epicenter_columns -- --nocapture` → 1 passed。
- `cd server && cargo test tribulation -- --nocapture` → 121 passed。
