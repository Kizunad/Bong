...
- Review 后续收敛：`tribulations_active` 升到 v21/v22，持久化 `kind/source/epicenter/intensity/origin_dimension`，JueBi 恢复不再伪装成 DuXu，且只按持久化起劫维度恢复，不再用重连时 `CurrentDimension` 误绑定；JueBi runtime intensity 进入三相伤害/抽压/寂灭衰减；结算按 kind/source 发事件，独立 JueBi 存活为 `half_step` 且不占 DuXu quota slot，void-quota 追加的 JueBi 仍按渡虚成功占位；所有 JueBi 结算都会清 active row；zone/terrain 处理按 dimension 过滤，非 Overworld 不写 Overworld terrain，重叠地形 op 只记录一次原始方块。
...
- `cd server && cargo test tribulation -- --nocapture` → 120 passed。
