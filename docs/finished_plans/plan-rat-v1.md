...
- `16a374fd`（2026-05-07）`plan-rat-v1: 增加噬元鼠测试指令`
- `d2f59f7b`（2026-05-07）`plan-rat-v1: 修复灵蝗潮 review 问题`
- `ffef9e6f`（2026-05-07）`plan-rat-v1: 修复噬元鼠 review P1 问题`
- `20b2760f`（2026-05-07）`plan-rat-v1: 修复灵蝗潮跨维 review 问题`

### 测试结果

- `server/`: `cargo fmt --check && cargo clippy --all-targets -- -D warnings && cargo test` ✅，`cargo test` 2510 passed。
...
- `server/`: `cargo test pressure_sensor_density_threshold_triggers_transition` ✅，1 passed；`cargo test seek_qi_source_action_filters_targets_by_dimension` ✅，1 passed（Codex P1 review 回归）。
- `server/`: `cargo test locust_swarm_advance_drains_qi_loot_and_cultivator_qi_pressure` ✅，1 passed（Codex 跨维掉落物 review 回归）。
- `agent/`: `npm run build && npm test -w @bong/schema && npm test -w @bong/tiandao` ✅，schema 283 passed，tiandao 256 passed。
- `agent/packages/tiandao`: `npm test -- locust-swarm-narration.test.ts runtime.test.ts` ✅，85 passed（PR review 回归）。
- `agent/packages/tiandao`: `npm test -- locust-swarm-narration.test.ts` ✅，6 passed（Codex target zone review 回归）。
