...
- `3693f5a6d`（2026-05-10）`test(plan-npc-visual-v1): 对齐高境界光环测试构造`
...
- 当前 `FactionRank` 只有 `Leader` / `Disciple` / `Ally` 三档；真传弟子独立背后光环与按 `QiColor` 着色的粒子细分，需要等 faction rank / qi color plumbing 扩展后补。
