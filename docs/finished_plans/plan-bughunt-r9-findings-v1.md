...
|---|---|---|---|---|
| #6 `spirit_qi_max` 未下发 | `server/src/player/state.rs:265` 当前 emit `cultivation.qi_max`，`:5907-5918` 有 payload pin | `already-fixed/invalid`（already-fixed） | `9cfdfe0f7` / PR #700 | 仅归档 |
| #2 vortex particle atlas | `client/.../atlases/particles.json:209` 已含 `bong:particle/vortex_spiral`，并有 asset/reconciliation tests | `already-fixed/invalid`（already-fixed） | `754b8c3fa`/#838；test hardening `1294cfc0b`/#1079 | 仅归档 |
| #3 ash spider disguise texture | `client/.../textures/entity/fauna/ash_spider_disguised.png` 当前存在；`FaunaModelDisguiseTest.java:48` pin 路径 | `already-fixed/invalid`（already-fixed） | `1285f79a0` / PR #674 | 仅归档 |
| #1 scatter-bead ledger zombie | `server/src/zhenfa/mod.rs:215-229,2589-2647` 主动成功链移除 burial 后不删 account；`:2726-2732` 自然耗尽同样只删 burial；canonical API 在 `qi_physics/ledger.rs:404-405` | `independent-domain-fix` | `docs/plans-skeleton/plan-bughunt-scatter-bead-ledger-account-cleanup-v1.md` | 新建唯一 runtime cleanup owner；`:2531` 仅是 `set_balance` 失败 rollback，不算成功终止 |
