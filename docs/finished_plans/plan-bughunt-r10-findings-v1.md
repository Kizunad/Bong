...
| #1 mineral exhausted shutdown flush | `server/src/mineral/mod.rs:93-102` 仍只在 Update 注册；节流 persistence 无 Last/AppExit 强刷 | `absorbed-by-track` | `docs/plan-refactor-persistence-slices-v1.md`（R3 shutdown-flush 域） | 最小 absorb-list 行补录 mineral finding |
