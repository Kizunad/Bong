...
- `e4da30aa2`（2026-07-30）：补充真实 `BONG_ASSETS_DIR` 覆盖下仅向 TOML 追加任意 `direct_generic` 功法仍可完成完整 App 启动的一体化回归。
...
- GitHub e2e run `30532612538` 在 `b279791e` 全绿，证明上述 Bot schema 漂移已从真实集成路径消失；该 SHA 随后的 fresh-context validator 仍发现 `server/src/main.rs::assert_full_app_core_resources` 把完整启动 smoke 钉在历史 49 条及固定首尾 ID，合法 TOML 扩展会在 App 已动态装载后被探针误拒绝。`e88200146` 改为对当前非空、唯一集合的动态断言；`e4da30aa2` 用复制后的完整 assets 根仅追加 `test.data_only_startup_smoke` 一条任意 `direct_generic` TOML 元数据，真实启动路径 `3 passed / 0 failed`。
- 启动探针修复后的完整 server 门（`e4da30aa2`）：同一 flock 命令全绿——library `12191 passed / 0 failed / 2 ignored`（共发现 `12193`）；CLI `12 passed`；full-app startup `3 passed`；shutdown-signal Rust integration `2 passed`；Tarkov backpack e2e `4 passed`；doc tests `5 ignored`。
