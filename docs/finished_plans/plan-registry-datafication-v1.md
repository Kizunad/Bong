...
- GitHub e2e run `30532612538` 在 `b279791e` 全绿，证明上述 Bot schema 漂移已从真实集成路径消失；该 SHA 随后的 fresh-context validator 仍发现 `server/src/main.rs::assert_full_app_core_resources` 把完整启动 smoke 钉在历史 49 条及固定首尾 ID，合法 TOML 扩展会在 App 已动态装载后被探针误拒绝。`e88200146` 改为对当前非空、唯一集合的动态断言；`e4da30aa2` 用复制后的完整 assets 根仅追加 `test.data_only_startup_smoke` 一条任意 `direct_generic` TOML 元数据，真实启动路径 `3 passed / 0 failed`；最终边界以 `6e3e5ccb0` 的 consumerless rejection 与 resolver-backed acceptance 为准。
- 启动探针修复后的完整 server 门（`e4da30aa2`）：同一 flock 命令全绿——library `12191 passed / 0 failed / 2 ignored`（共发现 `12193`）；CLI `12 passed`；full-app startup `3 passed`；shutdown-signal Rust integration `2 passed`；Tarkov backpack e2e `4 passed`；doc tests `5 ignored`。其中历史 data-only 正例现由最终 boundary 的 resolver-backed 正例替代，consumerless direct_generic 另有拒绝正例。
...
- `6e3e5ccb0` final boundary targeted gate：`scripts/build-token.sh cargo fmt --check`、`scripts/build-token.sh cargo test cultivation::known_techniques::tests -- --nocapture`、`scripts/build-token.sh cargo test network::race_gate_meta_emit::tests -- --nocapture`、`scripts/build-token.sh cargo test --test full_app_startup -- --nocapture` 均通过后，以本提交 exact HEAD 作为新鲜只读 validator 的绑定 SHA；执行结果与 validator PASS SHA 在本条证据中补记。
