...
- **关键 commit**：
  - `42363edec`（2026-05-11）`docs(plan-hud-immersion-v2): 升级为 active plan`
  - `96ad0277e`（2026-05-11）`docs(plan-hud-immersion-v2): 补充实地核验边界`
  - `33956ec8a`（2026-05-11）`feat(plan-hud-immersion-v2): 增加 HUD 沉浸感知组件`
  - `88e8f2ee9`（2026-05-11）`test(plan-hud-immersion-v2): 覆盖 HUD 沉浸矩阵`
- **测试结果**：
  ...
  - `JAVA_HOME=$HOME/.sdkman/candidates/java/17.0.18-amzn ./gradlew --no-daemon test build`：通过，`client/build/test-results/test` 汇总 1148 tests / 0 failure / 0 error / 0 skipped。
