...
- **Server / IPC**：`server/src/schema/yidao.rs`、`server/src/schema/channels.rs`、`server/src/network/redis_bridge.rs` 新增 `YidaoEventV1` 与 `bong:yidao/event`；`server/src/network/yidao_state_emit.rs` 下发 `HealerNpcAiStateV1` / `YidaoHudStateV1`，并在 review 修复中补齐迟到客户端稳定快照、HUD 空投影清理路径、医者 AI stale clear payload，以及医道 cast 完成结算后的 HUD / AI state emit 顺序。
...
- **Client / HUD 与视觉**：`client/src/main/java/com/bong/client/yidao/*`、`YidaoHudPlanner.java`、`BongHudOrchestrator.java`、`ServerDataRouter.java` 接入医道 HUD / 医者 NPC AI 状态；`YidaoPeacePulsePlayer.java` 与 `VfxBootstrap.java` 注册 5 个平和色 VFX；review 修复确认空 HUD 投影会清除旧面板、负信誉不会被客户端钳成 0、医者 AI state 按 `healer_id` 缓存并支持 clear、异常粒子原点/时长不会污染渲染。
...
- `614c89d3a` · 2026-05-09 · `fix(yidao): 清理医者 AI 状态缓存`
...
- `cd server && CARGO_BUILD_JOBS=1 cargo test yidao_state_emit` ✅ `6 passed; 0 failed`
...
- `cd client && JAVA_HOME="/usr/lib/jvm/java-17-openjdk-amd64" PATH="/usr/lib/jvm/java-17-openjdk-amd64/bin:$PATH" ./gradlew test --tests com.bong.client.yidao.YidaoClientTest` ✅ `BUILD SUCCESSFUL`
- `git diff --check` ✅
