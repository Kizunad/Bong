...
- `8963fbe2` · 2026-05-04 · `feat(tuike): 接入伪皮战斗与制作闭环`
- `c1e49109` · 2026-05-04 · `feat(tuike): 对齐 schema 与天道蜕壳叙事`
- `471e8215` · 2026-05-04 · `feat(tuike): 接入客户端伪皮装备与 HUD`
- `de7dde4e` · 2026-05-04 · `feat(tuike): 记录蜕壳生平战绩`
- `db8a5bd2` · 2026-05-04 · `fix(tuike): 收敛防御查询类型`
- `0b9dee06` · 2026-05-04 · `fix(tuike): 对齐生平记录格式化`

### 测试结果

- `cd server && cargo test tuike -- --nocapture`：通过；13 tests passed。
- `cd server && cargo fmt --check && cargo clippy --all-targets -- -D warnings && cargo test`：通过；2249 Rust tests passed。
- `cd agent && npm run build && (cd "packages/tiandao" && npm test) && (cd "packages/schema" && npm test)`：通过；tiandao 34 files / 232 tests，schema 9 files / 267 tests。
- `cd client && JAVA_HOME="/usr/lib/jvm/java-17-openjdk-amd64" PATH="/usr/lib/jvm/java-17-openjdk-amd64/bin:$PATH" ./gradlew test build`：通过；Gradle BUILD SUCCESSFUL，JUnit report 808 tests / 0 failures / 0 errors。
