...
- 2026-04-25：代码核对——P1–P5 全部完成；`InventoryEventV1`（Moved/Dropped/StackChanged/DurabilityChanged）在 schema/server/client 三端对齐，`ServerDataRouter` 已注册 `inventory_snapshot` + `inventory_event` 双通道；剩余 `[ ]` 项（`ItemRegistryStore` / `ItemRegistrySnapshotHandlerTest` / `InventoryItemTest` / §7.5 Manual QA）均为已知非主线或待实机记录，不影响 v1 闭环。
...
- `server/assets/items/*.toml` + `server/src/inventory/mod.rs`（`ItemRegistry` / `ItemTemplate` / `attach_inventory_to_joined_clients`）+ `server/src/network/inventory_snapshot_emit.rs`（`emit_join_inventory_snapshots` / `emit_changed_inventory_snapshots` / `emit_revive_inventory_resyncs`）
- 客户端：`client/src/main/java/com/bong/client/inventory/state/InventoryStateStore.java` + `client/src/main/java/com/bong/client/network/InventorySnapshotHandler.java`

**P2 — 移动 + 装备 + 堆叠**
- Server：`apply_inventory_move` + `handle_inventory_move` / `send_moved_event` + `InventoryEventV1::Moved` / `StackChanged` 分支（`server/src/schema/inventory.rs:221`）
- 客户端：`InspectScreenMoveIntentTest.java` + `InventoryEventHandler` 增量合并

**P3 — 丹药效果联动**
- Server：`handle_apply_pill` → `handle_alchemy_take_pill` + `ApplyPillRequestV1` + `ApplyPillTargetV1`（`agent/packages/schema/src/client-request.ts`）
- 三分支：`BreakthroughBonus` / `MeridianHeal` / `ContaminationCleanse`（`server/src/inventory/mod.rs` + cultivation/meridian 桥）
- 客户端：`InspectScreenApplyPillTest.java`（5 测）

**P4 — 死亡掉落 + dropped-loot 最小闭环**
- Server：`server/src/inventory/mod.rs:1550` `DeathDropOutcome` / `:1556` `DroppedLootEntry` / `:1566` `DroppedLootRegistry` / `:1976` `apply_death_drop_to_inventory`
- TSY 联动：`server/src/inventory/tsy_death_drop.rs:54` `apply_tsy_death_drop`
- S2C：`server/src/schema/server_data.rs:554` `DroppedLootEntryV1`
- 客户端：`client/src/main/java/com/bong/client/inventory/render/DroppedItemWorldRenderer.java`（world-space billboard）+ `state/DroppedItemStore.java`（含 insertionOrder tie-breaker）+ `DroppedItemPickupBootstrap.java`（G 键）
- HUD marker 已下线：`BongHudOrchestrator.buildCommands` 不再调用 `DroppedItemHudPlanner`，类作为规格保留

**P5 — Weight penalty**
- Server：`sync_overloaded_marker` + `OverloadedMarker`
- 客户端：`OverweightHudPlanner` + `max_weight` 字段贯穿 snapshot

### 关键 commit

| hash | 说明 |
|---|---|
| `54345dc4` | server inventory v1 主链 — discard/pickup/dropped-loot/apply-pill/death-drop |
| `a1177a3f` | schema inventory v1 IPC 契约 — discard/pickup/dropped-loot-sync/apply-pill |
| `c1420cd3` | client inventory v1 主链 — InspectScreen discard/apply-pill + dropped-loot + overweight |
| `ae3740fb` | DroppedItemStore + nearestTo tie-breaker（4 单测） |
| `4bd2d63b` | 地面 dropped loot 世界空间 billboard 渲染 |
| `c9c54a4b` | 切回 world billboard，再次下线 HUD marker |
| `0a24146a` | ItemTooltipPanel 左上角加物品 icon |
| `0529e175` | tooltip 高度按描述长度自适应 |
| `9bf2b6e1` | tooltip description 改 TextRenderer.wrapLines 真 word-wrap |
| `601120a9` | tooltip DEFAULT_HEIGHT 72→112 保底够长 description |
| `422de31e` | DroppedLootRegistry 改 instance_id 全局寻址 |
| `2815df99` | TSY 死亡分流复用主世界武器保护（codex review #1） |
| `701310db` | TSY spawn 成功后才 mark family 为 spawned（codex review #2） |
| `c5c37b78` | charges 字段替代 durability 越界（codex review #3） |

### 测试结果

| 命令 | 通过数 |
|---|---|
| `cd server && cargo test inventory` | **191 passed**（覆盖 `inventory::*` / `schema::inventory::*` / `network::inventory_snapshot_emit::*` / `world::tsy_container_search::*` / `world::tsy_filter::*`；2026-05-08 复测） |
| `cd agent/packages/schema && npm test` | **283 passed**（含 inventory snapshot/event roundtrip + client-payload + tsy + 后续 plan 接入；2026-05-08 复测） |
| `cd client && ./gradlew test --tests '*inventory*' --tests '*Inventory*' --tests '*DroppedItem*' --tests '*InspectScreen*'` | **BUILD SUCCESSFUL**（72 tests / 0 failures，覆盖 `InventoryStateStoreTest` / `DroppedItemStoreTest` / `InspectScreenApplyPillTest` / `InspectScreenBootstrapTest` / `InspectScreenMoveIntentTest` / `InventoryEquipRulesTest` 等 15 个测试套） |

### 跨仓库核验

| 层 | 命中 symbol |
|---|---|
| **server/** (Rust) | `InventorySnapshotV1` `InventoryEventV1`（Moved/Dropped/StackChanged/DurabilityChanged/Added/Removed/ApplyRejected）`DroppedLootRegistry` `DroppedLootEntry` `DeathDropOutcome` `apply_death_drop_on_revive` `apply_death_drop_to_inventory` `apply_tsy_death_drop` `apply_inventory_move` `handle_apply_pill` `handle_alchemy_take_pill` `sync_overloaded_marker` `inventory_snapshot_emit::{emit_join_inventory_snapshots, emit_changed_inventory_snapshots, emit_revive_inventory_resyncs}` |
| **agent/packages/schema** (TS) | `InventorySnapshotV1` `InventoryEventV1` `ItemTemplateV1` `ItemInstanceV1` `ContainerV1` `ItemEffectV1` `InventoryMoveRequestV1` `ApplyPillRequestV1` `ApplyPillTargetV1` `InventoryDiscardItemRequestV1` `PickupDroppedItemRequestV1` |
| **client/** (Java) | `InventoryStateStore` `InventorySnapshotHandler` `InventoryEventHandler` `DroppedItemStore`（含 insertionOrder tie-breaker）`DroppedLootSyncHandler` `DroppedItemPickupBootstrap` `DroppedItemWorldRenderer` `InspectScreenBootstrap` `InspectScreen`（discard / apply-pill / equip）`InventoryEquipRules` `OverweightHudPlanner` `ItemTooltipPanel` |
| **协议契约对齐** | `agent/packages/schema/samples/inventory_snapshot.json` 双端 round-trip 测试通过；`InventoryEventV1` 四个 delta 变体在 schema/server/client 三端命名一致；`ServerDataRouter` 已注册 `inventory_snapshot` + `inventory_event` + `dropped_loot_sync` 三通道 |

### 遗留 / 后续（明确不在本 plan 范围）

**1. 设计上撤回（非缺失）**
- `ItemRegistryStore`（client 侧模板缓存）+ 其 `ItemRegistrySnapshotHandlerTest`：§3.3 已决策「`InventorySnapshot` 自包含 UI 显示字段，模板同步若以后要做另起 plan」，对应 handler 与 store 均无需实装

**2. 小覆盖缺口（非阻塞，记技术债）**
- `InventoryItemTest`（新字段 default / clamp 独立单测）：字段行为已被 `InventorySnapshotHandlerTest` + `InventoryStateStoreTest` 端到端间接覆盖，缺的是直接对字段的 unit test。按饱和化测试原则可补，但不阻塞 v1 闭环

**3. 显式延后到 v2 / 独立 plan**
- **真正世界实体化掉落**（server Item entity + 多人争抢 + 物理 + 自动 proximity pickup）：当前 client billboard 仅视觉 surrogate
- **持久化**（DB / 文件存盘）：offline 重启丢失
- **灵气流失税**（worldview §486）：MVP 显式延后
- **多人交易 / 箱子 / 骨币死信箱**（worldview §526/§528）：独立 plan
- **法器装备加成 / 耐久度消耗体系深化**：装备槽与耐久字段已贯穿，但战斗/移动减益接入由各战斗子 plan 接管

**4. §7.5 Manual QA Checklist hand-off 到 v2**
- 七项实机 checklist（A 背包 UI / B Discard / C Pickup / D ApplyPill / E Overweight / F 死亡掉落 / G 回归）合并到 `plan-inventory-v2`（已 active，格子分配 + 堆叠合并）的实机验收一并跑，避免 v1/v2 各自独立搭机
- 决策依据：`plans-status` 2026-05-08 三端 191 + 283 + 72 单测全绿，IPC 契约对齐 ✅；§7.5 是手测补强而非闭环阻塞，与 v2 共用一次实机即可

— 验收人 Kiz · 2026-05-08（基于 `/plans-status` 实地核验：P1-P5 全部 ✅，文档 ↔ 代码无红旗，唯一遗留 §7.5 实机 hand-off 至 v2）
