...
- **关键 commit**：
  - `b35940c08` · 2026-05-09 · `plan-woliu-v2: 接入 server 五招物理`
  - `8c76d59b5` · 2026-05-09 · `plan-woliu-v2: 接入 agent 契约与叙事`
  - `c12a8787f` · 2026-05-09 · `plan-woliu-v2: 接入 client 视听与 HUD`
  - `169dbf2e5` · 2026-05-09 · `fix(plan-woliu-v2): 接通紊流运行时与 HUD 状态`（PR review 修复：紊流投射、vortex_state HUD 字段、肺经依赖、涡心 30s 绝壁劫延迟触发、真实 zone 账本分发）
  - `a5ebba35a` · 2026-05-09 · `fix(plan-woliu-v2): 收紧涡流状态生命周期`（PR review 修复：搅拌污染持久化、VortexV2State/PassiveVortex 生命周期清理、HUD tick 换算、99/1 守恒 pin 测试）
  - `403cc9806` · 2026-05-09 · `fix(plan-woliu-v2): 落实涡引位移结算`（PR review 修复：`woliu.pull` 结算直接改写目标 `Position`，事件保留为观测/动画信号）
  - `970776de8` · 2026-05-09 · `fix(plan-woliu-v2): 对齐 technique 数量`（rebase 修复：主线暗器技法与涡流技法合并后 `KnownTechniques` 固定数组长度更新为 22）
- **测试结果**：
  - `cd server && cargo fmt --check` → passed
  - `cd server && CARGO_BUILD_JOBS=1 cargo clippy --all-targets -j 1 -- -D warnings` → passed
  - `cd server && CARGO_BUILD_JOBS=1 cargo test -j 1` → 3385 passed
  - `cd server && CARGO_BUILD_JOBS=1 cargo test -j 1 combat::woliu_v2` → 127 passed
  ...
  - `cd server && grep -rcE '#\[test\]' src/combat/woliu_v2` → `tests.rs:127`
  - `cd server && CARGO_BUILD_JOBS=1 cargo test -j 1 qi_physics` → 84 passed
  - `cd agent && npm run build && npm test -w @bong/schema && npm test -w @bong/tiandao` → schema 339 passed，tiandao 315 passed
  - `cd agent && npm run generate:check -w @bong/schema` → generated schema artifacts fresh (314 files)
