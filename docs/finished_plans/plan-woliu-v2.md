...
| **P1** ✅ 2026-05-09 | server `combat::woliu_v2::*` 5 招 logic + 搅拌器物理 + 紊流场 EnvField 写入 + 反噬阶梯 + qi_physics 算子调用 + 化虚双场模型 + ≥100 单测 | `cargo test combat::woliu_v2` 136 passed；`grep -rcE '#\[test\]' server/src/combat/woliu_v2/` = `tests.rs:136` |
...
- **关键 commit**：
  - `28747cc25` · 2026-05-09 · `plan-woliu-v2: 接入 server 五招物理`
  - `dc31cc072` · 2026-05-09 · `plan-woliu-v2: 接入 agent 契约与叙事`
  - `453f0a34d` · 2026-05-09 · `plan-woliu-v2: 接入 client 视听与 HUD`
  - `acb525a7c` · 2026-05-09 · `fix(plan-woliu-v2): 接通紊流运行时与 HUD 状态`（PR review 修复：紊流投射、vortex_state HUD 字段、肺经依赖、涡心 30s 绝壁劫延迟触发、真实 zone 账本分发）
  - `566e4dd62` · 2026-05-09 · `fix(plan-woliu-v2): 收紧涡流状态生命周期`（PR review 修复：搅拌污染持久化、VortexV2State/PassiveVortex 生命周期清理、HUD tick 换算、99/1 守恒 pin 测试）
  - `00fea1f00` · 2026-05-09 · `fix(plan-woliu-v2): 落实涡引位移结算`（PR review 修复：`woliu.pull` 结算直接改写目标 `Position`，事件保留为观测/动画信号）
  - `89ce21c07` · 2026-05-09 · `fix(plan-woliu-v2): 对齐 technique 数量`（rebase 修复：主线暗器/截脉/涡流技法合并后 `KnownTechniques` 固定数组长度更新为 25）
  - `9d249dd27` · 2026-05-09 · `fix(plan-woliu-v2): 对齐 yidao rebase 注册格式`（rebase 修复：主线医道注册与涡流注册共存后保持 server 注册表格式化）
  - `bc80f65d6` · 2026-05-09 · `fix(plan-woliu-v2): 收紧 pull 与紊流边界`（PR review 修复：`woliu.pull` 缺目标拒绝、只在真实位移后发事件、已耗尽紊流不再投影、涡流粒子共享中心点、冷却秒数极端值钳制）
  - `e9cc5f5ad` · 2026-05-09 · `fix(plan-woliu-v2): 对齐 combat-feedback rebase 格式`（rebase 修复：主线战斗反馈 plan 合入后的 registry / HUD / network 格式共存）
  - `2f8c79b41` · 2026-05-09 · `fix(plan-woliu-v2): 保留冷却期 HUD 状态`（Codex review 修复：`VortexV2State` 保留到冷却结束、`charge_progress` 按施法时间线计算）
  - `b659b62f8` · 2026-05-09 · `fix(plan-woliu-v2): 适配最新污染源契约`（rebase 修复：主线 `ContamSource::meridian_id` 新字段接入涡流搅拌污染）
  - `4ab1564d4` · 2026-05-09 · `fix(plan-woliu-v2): 校验涡口与涡引目标边界`（Codex review 修复：`woliu.mouth` 使用目标位置生成涡口；`woliu.pull` 在扣资源/冷却前拒绝缺组件、越界、零真元目标）
- **测试结果**：
  - `cd server && cargo fmt --check` → passed
  - `cd server && CARGO_BUILD_JOBS=1 cargo clippy --all-targets -j 1 -- -D warnings` → passed
  - `cd server && CARGO_BUILD_JOBS=1 cargo test -j 1` → 3472 passed
  - `cd server && CARGO_BUILD_JOBS=1 cargo test -j 1 combat::woliu_v2` → 136 passed
  - `cd server && CARGO_BUILD_JOBS=1 cargo test -j 1 network::woliu_state_emit::tests` → 3 passed
  - `cd server && CARGO_BUILD_JOBS=1 cargo test -j 1 cultivation::tick::tests::turbulence_exposure_blocks_qi_regen` → 1 passed
  - `cd server && grep -rcE '#\[test\]' src/combat/woliu_v2` → `tests.rs:136`
  - `cd server && CARGO_BUILD_JOBS=1 cargo test -j 1 qi_physics` → 91 passed
  - `cd agent && npm run build && npm test -w @bong/schema && npm test -w @bong/tiandao` → schema 344 passed，tiandao 320 passed
  - `cd agent && npm run generate:check -w @bong/schema` → generated schema artifacts fresh (323 files)
