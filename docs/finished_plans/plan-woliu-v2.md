...
- **关键 commit**：
  - `cfbf5b03d` · 2026-05-09 · `plan-woliu-v2: 接入 server 五招物理`
  - `fabb2a1ff` · 2026-05-09 · `plan-woliu-v2: 接入 agent 契约与叙事`
  - `fc0229bf6` · 2026-05-09 · `plan-woliu-v2: 接入 client 视听与 HUD`
  - `377d6698d` · 2026-05-09 · `fix(plan-woliu-v2): 接通紊流运行时与 HUD 状态`（PR review 修复：紊流投射、vortex_state HUD 字段、肺经依赖、涡心 30s 绝壁劫延迟触发、真实 zone 账本分发）
  - `68a268e8d` · 2026-05-09 · `fix(plan-woliu-v2): 收紧涡流状态生命周期`（PR review 修复：搅拌污染持久化、VortexV2State/PassiveVortex 生命周期清理、HUD tick 换算、99/1 守恒 pin 测试）
  - `e7214b46b` · 2026-05-09 · `fix(plan-woliu-v2): 落实涡引位移结算`（PR review 修复：`woliu.pull` 结算直接改写目标 `Position`，事件保留为观测/动画信号）
- **测试结果**：
  - `cd server && cargo fmt --check` → passed
  - `cd server && CARGO_BUILD_JOBS=1 cargo clippy --all-targets -j 1 -- -D warnings` → passed
  - `cd server && CARGO_BUILD_JOBS=1 cargo test -j 1` → 3364 passed
  ...
  - `cd server && cargo test qi_physics` → 75 passed
  - `cd agent && npm run build && npm test -w @bong/schema && npm test -w @bong/tiandao` → schema 336 passed，tiandao 311 passed
