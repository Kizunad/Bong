...
- **关键 commit**：
  - `e48c52918` · 2026-05-09 · `plan-woliu-v2: 接入 server 五招物理`
  - `1f0152043` · 2026-05-09 · `plan-woliu-v2: 接入 agent 契约与叙事`
  - `f0fcfd1dd` · 2026-05-09 · `plan-woliu-v2: 接入 client 视听与 HUD`
  - `cfbe9cec1` · 2026-05-09 · `fix(plan-woliu-v2): 接通紊流运行时与 HUD 状态`（PR review 修复：紊流投射、vortex_state HUD 字段、肺经依赖、涡心 30s 绝壁劫延迟触发、真实 zone 账本分发）
  - `0def6df65` · 2026-05-09 · `fix(plan-woliu-v2): 收紧涡流状态生命周期`（PR review 修复：搅拌污染持久化、VortexV2State/PassiveVortex 生命周期清理、HUD tick 换算、99/1 守恒 pin 测试）
  - `c9cb044f1` · 2026-05-09 · `fix(plan-woliu-v2): 落实涡引位移结算`（PR review 修复：`woliu.pull` 结算直接改写目标 `Position`，事件保留为观测/动画信号）
  - `65f2bd913` · 2026-05-09 · `fix(plan-woliu-v2): 对齐 technique 数量`（rebase 修复：主线暗器技法与涡流技法合并后 `KnownTechniques` 固定数组长度更新为 22）
  - `c95a1b478` · 2026-05-09 · `fix(plan-woliu-v2): 对齐 yidao rebase 注册格式`（rebase 修复：主线医道注册与涡流注册共存后保持 server 注册表格式化）
  - `183558ffb` · 2026-05-09 · `fix(plan-woliu-v2): 收紧 pull 与紊流边界`（PR review 修复：`woliu.pull` 缺目标拒绝、只在真实位移后发事件、已耗尽紊流不再投影、涡流粒子共享中心点、冷却秒数极端值钳制）
  - `721d88741` · 2026-05-09 · `fix(plan-woliu-v2): 对齐 combat-feedback rebase 格式`（rebase 修复：主线战斗反馈 plan 合入后的 registry / HUD / network 格式共存）
  - `16bd246a9` · 2026-05-09 · `fix(plan-woliu-v2): 保留冷却期 HUD 状态`（Codex review 修复：`VortexV2State` 保留到冷却结束、`charge_progress` 按施法时间线计算）
- **测试结果**：
  - `cd server && cargo fmt --check` → passed
  - `cd server && CARGO_BUILD_JOBS=1 cargo clippy --all-targets -j 1 -- -D warnings` → passed
  - `cd server && CARGO_BUILD_JOBS=1 cargo test -j 1` → 3423 passed
