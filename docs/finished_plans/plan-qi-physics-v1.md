...
- **Review 收口**：`qi_collision` 改为接收真实攻防/environment 账户 ID，并修正 ledger 方向；`WorldQiBudget` 注册改走 config/env 初始化；`qi_release_to_zone` 接受负灵气 zone；`EnvField::default()` 改为中性局部灵气，避免默认触发 starvation；批量释放与 `summarize_world_qi` 保留负灵气 zone 的守恒语义；`regen_from_zone` 对齐既有 `compute_regen` 增益公式并封顶 zone drain；`WorldQiAccount::transfer` 拒绝微量 overdraft 铸造真元。

### 关键 commit

- `a190e98b`（2026-05-05）`feat(qi-physics): 建立真元物理底盘`
- `77d3bd07`（2026-05-05）`fix(qi-physics): 收口 review 账本语义`
- `dff957f8`（2026-05-05）`fix(qi-physics): 保留负灵气 zone 守恒语义`
- `9358711c`（2026-05-05）`fix(qi-physics): 对齐修炼吸纳守恒公式`
- `937c54ef`（2026-05-05）`fix(qi-physics): 拒绝微量 overdraft 转账`

### 测试结果

- `cd server && cargo fmt --check` ✅
- `cd server && cargo clippy --all-targets -- -D warnings` ✅
- `cd server && cargo test qi_physics` ✅ 61 passed
- `cd server && cargo test` ✅ 2382 passed
- `grep -R -c -E '#\[test\]' server/src/qi_physics/*.rs` 合计 61，满足 §1.5 ≥ 40 单测下限
