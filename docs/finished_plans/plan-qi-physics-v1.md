...
- **Review 收口**：`qi_collision` 改为接收真实攻防/environment 账户 ID，并修正 ledger 方向；`WorldQiBudget` 注册改走 config/env 初始化；`qi_release_to_zone` 接受负灵气 zone；`EnvField::default()` 改为中性局部灵气，避免默认触发 starvation；批量释放与 `summarize_world_qi` 保留负灵气 zone 的守恒语义。
...
- `cd server && cargo test qi_physics` ✅ 58 passed
- `cd server && cargo test` ✅ 2379 passed
- `grep -R -c -E '#\[test\]' server/src/qi_physics/*.rs` 合计 58，满足 §1.5 ≥ 40 单测下限
