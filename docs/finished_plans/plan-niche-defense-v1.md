...
- P2 ✅ 守宅道伥（运行时契约落地；worldview §七 prose anchor 留给后续 canon PR）
- P3 ✅ 异体真元残留（抄家物品反噬）+ 一生记录联动
- P4 ✅ perception 神识追凶 + 离线场景广播 + 仇家 NPC 抄家行为接入
- P5 ⏳ worldview §六.2 染色谱补"异种染色"小节（后续 canon PR）
...
- [x] **P4**：perception 神识追凶 + 离线广播 + 仇家 NPC 抄家
...
- **P0 基础数据 + 守家傀儡**：
  ...
  - `server/src/social/niche_defense.rs` 新增守家激活、服务端背包材料校验/消耗、同档上限、反击次数、自然朽坏和入侵处理主流程。
  - `server/assets/items/niche/house_puppet.toml` 与 `server/assets/items/fauna.toml` 补守家傀儡材料抓手。
- **P1 阵法陷阱 + 龛侵色**：
  - `server/src/social/niche_defense.rs` 支持 `GuardianKind::ZhenfaTrap`、陷阱 tier 保留、触发扣次数、缜密色加成和疲劳/破损事件。
  - `server/src/cultivation/realm_taint.rs` 新增 `RealmTaintedKind::NicheIntrusion`、缺失 `RealmTaintState` 初始化、`qi_taint_severity` 累积与 8h 洗清规则。
...
- **P4 perception / 离线广播 / NPC 抄家接入**：
  ...
  - `server/src/social/mod.rs` 将非龛主破坏灵龛方块的真实 gameplay 路径接入 `NicheIntrusionAttempt`，保证守家链路可被运行时触发。
...
- `server/`: `cargo test niche_defense` — 7 passed
- `server/`: `cargo test realm_taint` — 4 passed
- `server/`: `cargo test spirit_niche_break_attempt_reveals_and_disables_anchor` — 1 passed
- `server/`: `cargo test load_social_spirit_niche_rejects_invalid_guardians_json` — 1 passed
- `server/`: `cargo test` — 2321 passed
- `agent/`: `npm run build`
