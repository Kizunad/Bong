...
- `plan-tribulation-v2` 🆕 active → 化虚倒蚀触发绝壁劫（天地排异三相）
...
**绝壁劫触发**（plan-tribulation-v2）：化虚级倒蚀 = zone 内灵气大规模异常 → 30s 内触发天地排异三相（灵压坍缩→法则紊乱→寂灭扩散）。非必死但存活率 <10%，zone 内高境者一损俱损。这是化虚毒蛊师最孤注的一击
