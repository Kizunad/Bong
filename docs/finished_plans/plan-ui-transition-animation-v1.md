...
- **关键 commit**：
  - `6a425e332`（2026-05-10）`feat(client): 增加 UI 过渡动画引擎`
  - `4998d07ff`（2026-05-10）`feat(client): 增加 UI loading 过渡层`
  - `19856301e`（2026-05-10）`feat(client): 接入天道连接状态指示`
  - `da9e0da96`（2026-05-10）`fix(client): 收紧 UI 过渡 review 阻塞项`
  - `50615041d`（2026-05-10）`fix(client): 收敛 UI transition 复审建议`
  - `853b5f4c1`（2026-05-10）`fix(client): 修复 UI transition 复审边界`
  - `fa3760e82`（2026-05-10）`fix(client): 统一连接状态单调计时`
  - `44c36a92c`（2026-05-10）`fix(client): 修复 UI transition review 竞态`
- **测试结果**：
