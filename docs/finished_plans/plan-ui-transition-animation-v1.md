...
- **关键 commit**：
  - `f07a8d936`（2026-05-10）`feat(client): 增加 UI 过渡动画引擎`
  - `c377c1cdd`（2026-05-10）`feat(client): 增加 UI loading 过渡层`
  - `5c12e107e`（2026-05-10）`feat(client): 接入天道连接状态指示`
  - `f6dc5f024`（2026-05-10）`fix(client): 收紧 UI 过渡 review 阻塞项`
  - `1262ace34`（2026-05-10）`fix(client): 收敛 UI transition 复审建议`
  - `f50eb8568`（2026-05-10）`fix(client): 修复 UI transition 复审边界`
  - `3f98430ce`（2026-05-10）`fix(client): 统一连接状态单调计时`
  - `2361e75d7`（2026-05-10）`fix(client): 修复 UI transition review 竞态`
- **测试结果**：
  ...
  - `JAVA_HOME=/home/kiz/.sdkman/candidates/java/17.0.18-amzn ./gradlew test --tests com.bong.client.ui.ScreenTransitionTest`（client）✅ `BUILD SUCCESSFUL`
  - `JAVA_HOME=/home/kiz/.sdkman/candidates/java/17.0.18-amzn ./gradlew runClient`（client）✅ `BUILD SUCCESSFUL in 14m 16s`；日志确认 `Bong Client bootstrap ready`、whale/fauna/entity-model raw_id 对齐、资源 atlas 创建与客户端运行态（WSL 音频设备缺失导致声音关闭但不阻断启动；退出阶段有 Minecraft texture shutdown warning，但 Gradle 任务成功）。
