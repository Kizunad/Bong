...
- **关键 commit**：
  - `b6181b087`（2026-05-10）`feat(client): 增加 UI 过渡动画引擎`
  - `a4c9ec0cb`（2026-05-10）`feat(client): 增加 UI loading 过渡层`
  - `8c0f44abb`（2026-05-10）`feat(client): 接入天道连接状态指示`
  - `344613879`（2026-05-10）`fix(client): 收紧 UI 过渡 review 阻塞项`
  - `8f7570cb9`（2026-05-10）`fix(client): 收敛 UI transition 复审建议`
  - `905837973`（2026-05-10）`fix(client): 修复 UI transition 复审边界`
  - `b424b6260`（2026-05-10）`fix(client): 统一连接状态单调计时`
  - `88602d71e`（2026-05-10）`fix(client): 修复 UI transition review 竞态`
- **测试结果**：
