...
- **关键 commit**：
  - `c69a21fb4`（2026-05-10）`feat(client): 增加 UI 过渡动画引擎`
  - `86ee20ac8`（2026-05-10）`feat(client): 增加 UI loading 过渡层`
  - `4c34c0601`（2026-05-10）`feat(client): 接入天道连接状态指示`
  - `fb33366ab`（2026-05-10）`fix(client): 收紧 UI 过渡 review 阻塞项`
  - `c7fc9ff32`（2026-05-10）`fix(client): 收敛 UI transition 复审建议`
  - `63cb09901`（2026-05-10）`fix(client): 修复 UI transition 复审边界`
  - `d6259d5f7`（2026-05-10）`fix(client): 统一连接状态单调计时`
- **测试结果**：
