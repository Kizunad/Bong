...
- **关键 commit**：
  - `b906e763d`（2026-05-10）`feat(client): 增加 UI 过渡动画引擎`
  - `2bcd2ab7c`（2026-05-10）`feat(client): 增加 UI loading 过渡层`
  - `858a57224`（2026-05-10）`feat(client): 接入天道连接状态指示`
  - `b052e05d4`（2026-05-10）`fix(client): 收紧 UI 过渡 review 阻塞项`
  - `3ae3045dd`（2026-05-10）`fix(client): 收敛 UI transition 复审建议`
  - `8e34221c1`（2026-05-10）`fix(client): 修复 UI transition 复审边界`
  - `76d3a7534`（2026-05-10）`fix(client): 统一连接状态单调计时`
- **测试结果**：
  ...
  - `JAVA_HOME=/home/kiz/.sdkman/candidates/java/17.0.18-amzn ./gradlew runClient`（client）✅ `BUILD SUCCESSFUL in 1m 22s`；日志确认 `Bong Client bootstrap ready`、whale/fauna/entity-model raw_id 对齐、资源 atlas 创建与客户端运行态（WSL 音频设备缺失导致声音关闭但不阻断启动）。
