...
- **关键 commit**：
  ...
  - `bc42bfed6`（2026-05-10）`fix(client): 修复 UI transition 复审边界`
- **测试结果**：
  ...
  - `JAVA_HOME=/home/kiz/.sdkman/candidates/java/17.0.18-amzn ./gradlew runClient`（client）✅ 启动到 `Bong Client bootstrap ready`、whale/fauna raw_id 对齐、资源 atlas 创建与客户端运行态；验证后手动 `Ctrl-C` 停止常驻客户端。
