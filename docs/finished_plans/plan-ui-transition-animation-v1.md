...
- **关键 commit**：
  - `8b36b72c9`（2026-05-10）`feat(client): 增加 UI 过渡动画引擎`
  - `46aa4970d`（2026-05-10）`feat(client): 增加 UI loading 过渡层`
  - `bf446bcef`（2026-05-10）`feat(client): 接入天道连接状态指示`
  - `89e8103ae`（2026-05-10）`fix(client): 收紧 UI 过渡 review 阻塞项`
  - `ce209d515`（2026-05-10）`fix(client): 收敛 UI transition 复审建议`
  - `26300a73d`（2026-05-10）`fix(client): 修复 UI transition 复审边界`
  - `a5376b906`（2026-05-10）`fix(client): 统一连接状态单调计时`
  - `11e6deba0`（2026-05-10）`fix(client): 修复 UI transition review 竞态`
- **测试结果**：
