...
- **测试结果**：
  ...
  - `JAVA_HOME=/home/kiz/.sdkman/candidates/java/17.0.18-amzn ./gradlew runClient`（client）✅ `BUILD SUCCESSFUL in 1m 21s`；日志确认 `Bong Client bootstrap ready`、whale/fauna raw_id 对齐、资源 atlas 创建与客户端运行态（WSL 音频设备缺失导致声音关闭但不阻断启动）。
