...
| 18 | `wounds-relog-full-heal` | **无 Wounds durable schema** | **无** | **无** | join/rebirth 均 `Wounds::default()`，重连满血缺口仍在 | `server/src/combat/components.rs:73-102`；`server/src/combat/mod.rs:171-184,197-200`；`server/src/combat/lifecycle.rs:1803-1863` | P2 新建 player Slice/load guard；**不得与 Lifecycle 混记** |
