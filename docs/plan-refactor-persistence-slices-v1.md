...
- 在后续精确 HEAD `659997434777eefd34da5067328718fb8c727c0a4` 的无上下文并发复核中，validator 发现 digest sweep 先读 stale rows、再取锁时可能删除 cooperative capture 已提交的 successor，且按 `char_id` 无条件删除无法构成所有权证明。该结论已第一性复核并在 `46a5d380f` 修复：`persist_npc_capture` 与 sweep 复用同一 per-character lifecycle lock；sweep 持锁重读候选并仅按原候选集合处理，删除使用完整 digest 快照 CAS，`changes != 1` 即 `WouldBlock` 回滚；新增 `npc_digest_sweep_does_not_delete_successor_digest` 回归测试覆盖非协调 successor 更新。该修复保持迁移链、表结构、事务边界和 R3 P0 接入点不变；定向 `persistence::tests::` 已通过 223 passed、0 failed，最终 validator、完整 gate、合主线复验与当前 HEAD Kody 结论待本轮最终 HEAD 确认。
