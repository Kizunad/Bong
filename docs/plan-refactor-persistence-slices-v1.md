...
- 新增的并发复核发现：匹配 orphan 但后续 DB 事务失败时，未发布该文件的调用方不得按 `previous_archive == None` 删除并发发布者的归档；现以发布所有权分支保留该文件，并由 `npc_archive_matching_orphan_db_failure_preserves_archive` 锁定。故本 PR 的返工内容继续诚实记录为拆分加既有缺陷及其后继路径修复，未宣称纯移动或行为不变。
