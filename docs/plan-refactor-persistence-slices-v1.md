...
迁移拆分的基线只允许机械移动并保持版本、顺序、transaction 与失败行为；本 PR 另把 review 验真的既有缺陷作为明确例外逐条修复并在 P1 证据中登记。v13 legacy cultivation backfill、v21/v23 缺表/缺列兼容、v30 faction 数据映射、v34 qi unknown/fail-closed、v35 heartbeat 保守回填、v38 overflow 初始化，以及 v39 Lifecycle rebase 都是 load-bearing migration，禁止未登记的行为级 squash。
...
- 拆分落点与最终行数为 `mod.rs` 214 行、`models.rs` 533 行、`known_techniques.rs` 986 行、`bootstrap.rs` 407 行、`migrations.rs` 1914 行、`void_actions.rs` 131 行、`agent.rs` 442 行、`tribulation.rs` 461 行、`world.rs` 1183 行、`world_qi.rs` 141 行、`player.rs` 373 行、`npc.rs` 1502 行、`life.rs` 1058 行、`social.rs` 301 行、`helpers.rs` 686 行、`epitaph.rs` 80 行，全部小于 3000 行。
...
- 当前 HEAD `7de451365` 的 Kody 后继意见经第一性复核确认是前述修复引入的两个路径缺陷，已纳入本 PR：`helpers.rs` 的最终文件回滚失败现在通过 `combine_persistence_failure` 同时暴露主清理错误与回滚错误，并由 `zstd_bundle_reports_final_cleanup_rollback_failure` 锁定；`npc.rs` 在 no-replace 返回 `AlreadyExists` 时解码并复用内容完全匹配的有效 orphan bundle，继续完成 deceased index/upsert 与 hot-row 删除，冲突或损坏 bundle 仍 fail-closed，并由 `npc_archive_reconciles_matching_orphan_bundle_after_crash` 锁定。本轮不改迁移链、表结构、事务边界或 R3 P0 接入点。
- 新增的并发复核发现：匹配 orphan 但后续 DB 事务失败时，未发布该文件的调用方不得按 `previous_archive == None` 删除并发发布者的归档；现以发布所有权分支保留该文件，并由 `npc_archive_uncoordinated_competing_publish_is_not_reused` 锁定。故本 PR 的返工内容继续诚实记录为拆分加既有缺陷及其后继路径修复，未宣称纯移动或行为不变。
- 当前 HEAD `16a665a38` 的 Kody 后继意见经第一性复核确认仍是本轮原子发布代码的两个并发/错误处理缺陷，已纳入本 PR：`helpers.rs` 不再静默丢弃临时文件清理失败；原始写入错误与清理错误以 `combine_persistence_failure` 结构化聚合，并由 `zstd_bundle_reports_temporary_cleanup_failure_after_write_error` 锁定。`npc.rs` 为 deceased/digest 归档共享一个固定域级 lifecycle lock，覆盖初始观察、no-replace 发布、身份/内容复核、DB transaction 与失败处理；`ArchiveFileIdentity`（Unix device/inode、长度、mtime 等）与解压 payload 双重校验，初始不存在后收到 `AlreadyExists` 必须 fail-closed，不复用未协调 publisher；successor 替换在 DB commit 前被拒绝且不删除 successor，由 `npc_archive_rejects_successor_replacement_before_db_commit` 锁定。digest sweep 同样按稳定 char_id 顺序持有该域级锁至事务完成，并在删除前后复核身份与内容。该轮不改变 R3 P0 canonical registry、`AppExit → Last`、zone-runtime shutdown descriptor、KnownTechniques reconnect/load guard/dirty snapshot/durable fence adapter，不改变迁移链、表结构或事务边界；定向/完整门禁、fresh validator 与新 HEAD Kody 复审结果待最终提交后补记。
- 合并后复验（2026-09-07）：已紧邻执行 `git fetch origin && git merge origin/main`，把 `origin/main=51fcda26e`（#2182 combat/resolve 外置及其既有 Cargo/docs 条目）合入；当前本 PR HEAD 为 `d1c1dcb0473e5201029966a22d80f726f4c4f9c4`，无冲突且保留本计划与主线条目。合并后的 locked server gate `flock /tmp/bong-cargo.lock -c 'cd server && ../scripts/build-token.sh cargo fmt --check && ../scripts/build-token.sh cargo clippy --all-targets -- -D warnings && ../scripts/build-token.sh cargo test'` 全部通过：fmt/clippy 通过，`src/lib.rs` 为 `12187 passed, 0 failed, 1 ignored`，`src/main.rs` 为 `18 passed, 0 failed`，已登记的独立 targets（含 `emit_scope_contract`、`forge_history_unit`、`range_gate_unit`、`rift_portal_unit`、`resolve_unit` 等）及 doc-tests 均通过。无上下文 read-only validator 已针对该精确 HEAD `d1c1dcb0473e5201029966a22d80f726f4c4f9c4` 给出 PASS；随后 plan evidence 的文档变更需在新 HEAD 上再次复验。该证据仍仅表示本 PR 的拆分与 review 修复可验证，P1 总体及 M-04/M-12 仍未完成。
- 在后续精确 HEAD `659997434777eefd34da5067328718fb8c727c0a4` 的无上下文并发复核中，validator 发现 digest sweep 先读 stale rows、再取锁时可能删除 cooperative capture 已提交的 successor，且按 `char_id` 无条件删除无法构成所有权证明。该结论已第一性复核并在 `46a5d380f` 修复：`persist_npc_capture` 与 sweep 复用同一固定域级 lifecycle lock；sweep 持锁重读候选并仅按原候选集合处理，删除使用完整 digest 快照 CAS，`changes != 1` 即 `WouldBlock` 回滚；新增 `npc_digest_sweep_does_not_delete_successor_digest` 回归测试覆盖非协调 successor 更新。该修复保持迁移链、表结构、事务边界和 R3 P0 接入点不变；定向 `persistence::tests::` 已通过 223 passed、0 failed，最终 validator、完整 gate、合主线复验与当前 HEAD Kody 结论待本轮最终 HEAD 确认。
- 精确 HEAD `7d306462e20f57b20f2355799f73f83b98ce7d6e` 的 fresh validator 又发现：混合 digest sweep 批次在后一个快照 CAS miss 时仅记录 `WouldBlock`、仍提交前面已删除的行，违反整批回滚语义。已在 `d12646d93` 修复为 CAS miss 立即返回并由 SQLite transaction 回滚整批，同时新增 `npc_digest_sweep_cas_failure_rolls_back_other_deletions`；相关 `persistence::tests::npc_digest_sweep` 定向测试 4 passed、0 failed。该修复不改变迁移链、表结构、事务边界或 R3 P0 接入点；新精确 HEAD 的 validator、完整 gate、主线复验与 Kody 结论待确认。
- 精确 HEAD `567431dde96e75e3cb7ba02acccdabc763e48aec` 的 fresh validator 复核出两点：其将“单条归档准备失败后继续清理健康 stale row”误判为应整批回滚；该行为是既有契约并由 `npc_digest_sweep_continues_after_one_archive_failure` 锁定，CAS miss 的整批回滚仍已由上一条证据单独锁定。另验出 deceased 归档发布成功后若无法建立文件 identity 会留下未登记目标，已在 `f1492b1d6` 修复：identity 未建立时不盲删未知目标，并合并 primary/ownership 诊断；新增 `npc_archive_post_publish_identity_failure_surfaces_cleanup_context`，与 digest 定向测试分别为 1 passed、4 passed。该修复不改变迁移链、表结构、事务边界或 R3 P0 接入点；新精确 HEAD 的 validator、完整 gate、主线复验与 Kody 结论待确认。
- 精确 HEAD `9160d77bfc6205c92c596fcb277332ecd5de10ff` 的 fresh validator 对代码与契约核验 PASS：确认固定域级 lifecycle lock、no-replace/ownership 身份与 payload 双重校验、prepared CAS miss 整批回滚，以及 preparation error 的健康行隔离语义；计划证据中的行数、SHA 与行为变化描述也已对齐。该 HEAD 的定向 archive identity failure 测试 1 passed、digest sweep 测试 4 passed；新文档提交后的精确 HEAD 仍需再次对拍。
- 精确 HEAD `99dc60d84cbafb6b7a385755990904628b725484` 的 fresh validator 结论为 FAIL，但唯一原因是该 HEAD 的 plan 尚未预先写入它自己的 validator 结论；其同时确认 `helpers.rs`/`npc.rs` 的 lifecycle lock、no-replace、identity/payload、rollback/CAS 实现存在，且 `git diff --check` 通过。该自指性文档缺口不构成代码缺陷；下一轮 validator 将按当前 HEAD 的代码与历史证据一致性重新核验，不把本轮 FAIL 冒充 PASS。
- 精确 HEAD `19aa4a8567b62fbaa265ad9a236074e89278c545` 的 fresh-context read-only validator 已 PASS：确认 helpers 的原始错误与临时/最终清理错误均经 `combine_persistence_failure` 可观察，NPC deceased/digest 归档在固定域级 lifecycle lock 下执行 no-replace、identity/payload ownership 校验与 CAS 失败保护，且对应回归测试覆盖；该 HEAD 的定向归档/ digest 复核为 4 passed、0 failed，随后完整 locked server gate 为 `12194 passed, 0 failed, 1 ignored`（主程序 18 passed，doc-tests 3 passed、5 ignored）。
- 合并后复验（2026-09-08）：已紧邻执行 `git fetch origin && git merge origin/main`，实际合入 `origin/main=eedf9903d8f2d819847d4cf394e43ddf7bb2b0fa`，当前 merge HEAD 为 `7ca76f259a93ae30bcb509fcba544cab6b32e624`；冲突为 0，主线只触及 client/network、bot 与测试清单等文件，未触及本 PR 的 persistence 修复文件。该 merge HEAD 上四条关键回归 `zstd_bundle_reports_temporary_cleanup_failure_after_write_error`、`npc_archive_rejects_successor_replacement_before_db_commit`、`npc_archive_post_publish_identity_failure_surfaces_cleanup_context`、`npc_digest_sweep_cas_failure_rolls_back_other_deletions` 各 1 passed、0 failed，共 4 passed；随后完整 locked server gate 退出码 0：fmt/clippy 通过，`src/lib.rs` 12194 passed、0 failed、1 ignored，`src/main.rs` 18 passed、0 failed，独立 targets 与 doc-tests 全部通过（doc-tests 3 passed、5 ignored）。fresh validator 服务在该 SHA 上两次有界等待均超时，未形成 PASS/FAIL 结论，故此处明确不把超时冒充 PASS；下一轮必须针对证据更新后的精确 HEAD 重新核验。
- 本轮继续修复前先冻结归档发布不变式：①最终归档只能在对应生产调用方持有固定 `.npc.lifecycle.lock`、且 `ensure_archive_identity`（NPC 归档同时要求 payload 校验）仍证明该路径是本次发布者创建的文件时删除；身份不匹配、目标消失或无法证明 ownership 均不删除，并用 `combine_persistence_failure` 保留 ownership 诊断。②批次中途失败时，SQLite transaction 回滚本批次已删除的 hot rows；本次 sweep 自己发布的文件只按保存的 `ArchiveFileIdentity` 回滚，既有文件或 successor 不得触碰；回滚失败与 primary 一并上报。单条准备失败仍按既有契约隔离该条、继续处理健康 stale row，并在最终返回中保留首个准备错误。③同一路径采用同一生命周期锁串行化，锁外/未协调 writer 只能在 `hard_link` no-replace 竞争中由先发布者获胜；失败方看到 `AlreadyExists` 或身份变化即 fail-closed，不覆盖、不删除、不把别人的成功发布当成自己的 orphan。该不变式冻结本 PR 的已有身份/回滚抽象，不再扩展新的 helper 类型或 ownership 概念。
- 精确 HEAD `43974d798238763364c80b85d3eafbd28bceaf62` 的本轮验证：无上下文 read-only validator 对身份/ownership、lifecycle lock、no-replace 与 CAS 整批回滚结论为 PASS；四条关键回归（`zstd_bundle_reports_temporary_cleanup_failure_after_write_error`、`npc_archive_rejects_successor_replacement_before_db_commit`、`npc_archive_post_publish_identity_failure_surfaces_cleanup_context`、`npc_digest_sweep_cas_failure_rolls_back_other_deletions`）各 `1 passed, 0 failed`，persistence 定向集合 `230 passed, 0 failed`。按新约定直接调用 `build-token.sh` 的完整 server gate 真实退出码为 0：fmt/clippy 通过，`src/lib.rs` `12197 passed, 0 failed, 1 ignored`，`src/main.rs` `18 passed, 0 failed`，全部独立 targets/e2e 通过，doc-tests `3 passed, 0 failed, 5 ignored`。该证据仍只表示拆分加 review 修复可验证，P1 总体及 M-04/M-12 仍未完成。
- 当前文档证据提交 `3611a07f72fb10067998ae62cbaee5c7d4ef18f3` 仅追加上述验证记录，未改变 `server/src/persistence/` 代码、测试夹具或已执行命令；因此本轮代码/测试结论仍精确绑定其父代码 HEAD `43974d798238763364c80b85d3eafbd28bceaf62`。随后仅文档提交 `39b12de34b1376da60112fa966abd699841c2203` 追加主线基线记录，未改变 persistence 代码；无上下文 read-only validator 已对该精确 docs-only HEAD 给出 PASS。
- 合并后复验（2026-09-08）：已紧邻执行 `git fetch origin && git merge origin/main`，实际合入 `origin/main=d9e1839e2f759611e181fa9f2c7143091beb6123`，无冲突，当前 merge HEAD 为 `0776783b5c7d25786c8d2b8d511ea4c51a0119ee`。主线带入的变更仅在 combat/tribulation、Cargo、测试清单与其它计划文档，未触及本 PR 的 persistence 代码或本计划；因此保留全部既有 persistence 条目与测试 targets。merge 前 gate 不作为 merge HEAD 证据；merge 后 gate 结果见下一条。
- merge HEAD `0776783b5c7d25786c8d2b8d511ea4c51a0119ee` 上随后实际执行的完整 server gate 退出码为 0：fmt/clippy 通过，`src/lib.rs` `12100 passed, 0 failed, 1 ignored`（listed 12101，1 ignored），`src/main.rs` `18 passed, 0 failed`，全部独立 targets/e2e 通过，doc-tests `3 passed, 0 failed, 5 ignored`；四条本轮归档回归与 persistence 定向集合均在该代码树中通过。该 gate 与上条 merge 基线精确对应；本次文档修订仍不改变 persistence 代码。
