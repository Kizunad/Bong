...
寂照镜的能力不是数值 buff——是**别的系统做不到的信息获取机制**。每个能力都有代价和暴露风险，与 worldview §十五 #2 "信息比装备更值钱"直接对应。
...
> "镜面一闪，对方以为只是光影。但你已经看穿了他的底细。"

**效果**：对视线内一名修士使用，**无视匿名系统**，在你的 inspect 界面中显示对方的：
- 真实境界（精确到经脉数，不是"气息在你之上"的模糊判断）
- 体表伤口分布（16 部位全览）
- 经脉损伤/污染状态（20 条全览）
- 真元百分比
- 是否携带灵宝（只告诉你"有"还是"没有"，不告诉你哪件）

**代价**：
- 使用瞬间镜面闪烁一次青光——**目标和附近所有人看到闪光方向**（暴露你的位置）
- 粒子效果：目标身上短暂浮现青色扫描线（目标知道自己被"照"了，但不知道是谁）
- 好感度 < 0.4 时器灵可能拒绝执行（LLM 判断："我为何要帮你窥探此人？"）

**战术价值**：PVP 前的情报战决定性工具。你花 5 分钟 CD 换到对方的完整三层状态——但对方知道自己被扫了，你也暴露了位置。用完马上跑还是直接动手？

**好感度影响**：
- < 0.4：拒绝率 50%，即使成功也只显示境界和真元
- 0.4-0.7：完整三层信息
- 0.7+：额外显示对方最近 3 次战斗的损耗趋势（"此人近日频繁战斗，底子已薄"）
...
**效果**：激活后 30 秒内，**你的气息从所有感知系统中消失**：
- 其他玩家的神识/感知探测读不到你（`SpiritualSenseTargets` 过滤）
- NPC 的威胁评估失效（你从 `brain.rs` 的 target 列表中移除）
- 匿名系统加强：即使固元+修士也看不到你的境界提示
- 坍缩渊内使用：道伥/执念不会主动攻击你（你从敌对目标列表消失）

**代价**：
- 隐匿期间**器灵沉默**——不能 @寂照镜 对话，明虚残识不触发，好感度冻结
- 隐匿期间**不能攻击/施法**——任何攻击行为立即打破隐匿
- 隐匿结束瞬间镜面**爆闪一次青光**——50 格内所有人看到闪光方向 + 粒子爆发
- 爆闪后 10 秒内你的气息被**反向放大**（感知范围内的人更容易锁定你）

**实现**：
- server：`ApplyStatusEffectIntent { kind: MirrorConcealment, duration: 600 ticks, magnitude: 1.0 }`
- `attribute_aggregate_tick()`：检测 `MirrorConcealment` → 从 `SpiritualSenseTargets` 查询结果中排除
- NPC brain：`score_threat()` 读到目标有 `MirrorConcealment` → 返回 0.0
- 结束时：emit `VfxEvent("bong:mirror_flash", pos, radius=50)` + `ApplyStatusEffectIntent { kind: MirrorExposed, duration: 200 ticks }`

**战术价值**：照破是进攻前的情报，镜隐是撤退时的保命。坍缩渊深层搜刮完用镜隐跑过道伥区；PVP 劣势时隐匿脱战——但 30 秒结束那一闪全场都知道你在哪。照破 + 镜隐 = 先看穿对方再决定打还是跑。

**好感度影响**：
- < 0.4：持续时间缩短到 15 秒
- 0.4-0.7：完整 30 秒
- 0.7+：持续 45 秒 + 结束闪光半径缩小到 30 格（器灵帮你收敛光芒）

---

#### 能力三：明虚残识（被动，器灵自主触发）

> "明虚不会教你功法。但他走过的路，你踩上去时他会告诉你。"

**效果**：**不由玩家主动触发**——器灵明虚在特定位置/情境下**通过聊天栏自动说话**，透露信息：
- 进入清风宗遗迹类坍缩渊时：自动提示内部大致结构（"此地格局……与旧宗相仿。东廊是弟子居所，西厢多有禁制。"）
- 靠近阵法节点时：提示阵法类型和危险等级（"脚下有旧阵，引气以下踩上去会被弹开。"）
- 附近有道伥时：提前 3 秒预警（"有故人……不，是执念。你右后方。"）
- 附近有伪灵脉时：判断真假（"此灵脉太亮。天道不会这么慷慨。"）

**代价**：
- 器灵说话 = zone 广播 = **附近所有人也听到了**
- 你知道这里有阵法——但旁边那个陌生人也知道了
- 器灵偶尔判断错误（LLM 生成，不是 100% 准确——器灵毕竟沉睡了很久，记忆有偏差）

**好感度影响**：
- < 0.3：器灵沉睡，完全不触发
- 0.3-0.5：只在高危情境触发（道伥预警）
- 0.5-0.7：触发频率正常
- 0.7+：额外触发宗门旧事碎片（"此处……我曾在此教弟子剑法。那是三百年前的事了。"——纯叙事，无机制效果，但建立情感连接）
