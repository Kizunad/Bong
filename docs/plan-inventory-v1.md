...
- **HUD 标记简化为"只画 icon"**：原 `DroppedItemHudPlanner` emit 了 background rect / edge accent / icon / directional-prefixed text 一整套，文字标签在屏幕边缘 clamp 和 stabilization lerp 作用下和图标错位"乱飘"（投影不同步 + lerp 滞后 + 屏幕 clamp）。现在 `buildCommands` emit 阶段**只剩 1 个 itemTexture command**；layout 计算（投影 + 边缘 clamp + 位置 stabilize）保留——图标仍浮在物品上方、仍被 clamp 到屏幕内、仍做微动平滑。directional prefix / edge accent / background 的函数保留作 dead-code 规格（便于以后需要回退）。`DroppedItemHudPlannerTest` 从 14 删到 8：保留 empty / projection / clamp / stability / nearest-selection 核心断言；删掉 6 个断言方向前缀文字的测试（没 text 了）。
- **世界空间 billboard 方案试验后放弃**：曾尝试 `DroppedItemWorldRenderer`（WorldRenderEvents.AFTER_ENTITIES + yaw-only semi-billboard）作为 HUD marker 的替代，但实机未观察到渲染（顶点 flush 时机 / renderlayer 细节需深挖），且"单个 HUD 图标"更符合"最小可用"直觉——本轮改回 HUD icon-only 路径，`DroppedItemWorldRenderer` 文件已删除、BongClient 里的 register 已移除。**真正世界实体化**仍按原 plan 延后到 Phase B。
