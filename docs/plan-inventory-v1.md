...
- `server/assets/items/*.toml` + `server/src/inventory/mod.rs`（`ItemRegistry` / `ItemTemplate` / `inventory_init_system` / `inventory_snapshot_emit`）
...
**P2 — 移动 + 装备 + 堆叠**
- Server：`inventory_move_system` + `InventoryEventV1::Moved` / `StackChanged` 分支（`server/src/schema/inventory.rs:221`）
- 客户端：`InspectScreenMoveIntentTest.java` + `InventoryEventHandler` 增量合并

**P3 — 丹药效果联动**
- Server：`apply_pill_system` + `ApplyPillRequestV1` + `ApplyPillTargetV1`（`agent/packages/schema/src/client-request.ts`）
...
**P5 — Weight penalty**
- Server：`weight_penalty_system` + `OverloadedMarker`
...
| 层 | 命中 symbol |
|---|---|
| **server/** (Rust) | `InventorySnapshotV1` `InventoryEventV1`（Moved/Dropped/StackChanged/DurabilityChanged/Added/Removed/ApplyRejected）`DroppedLootRegistry` `DroppedLootEntry` `DeathDropOutcome` `apply_death_drop_to_inventory` `apply_tsy_death_drop` `inventory_move_system` `apply_pill_system` `weight_penalty_system` `inventory_snapshot_emit` |
