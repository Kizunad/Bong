...
| 命令 | 通过数 |
|---|---|
| `cd server && cargo test inventory` | **145 passed**（覆盖 `inventory::*` / `schema::inventory::*` / `network::inventory_snapshot_emit::*` / `world::tsy_container_search::*` / `world::tsy_filter::*`） |
| `cd agent/packages/schema && npm test` | **190 passed**（`schema.test.ts` 101 / `client-payload.test.ts` 16 / `tsy.test.ts` 36 / `extract-v1.test.ts` 5 / `container-interaction.test.ts` 15 / `tsy-hostile-v1.test.ts` 13 / `generated-artifacts.test.ts` 4） |
...
**4. 待实机一轮 QA**
- §7.5 Manual QA Checklist（A 背包 UI / B Discard / C Pickup / D ApplyPill / E Overweight / F 死亡掉落 / G 回归）—— 归档前需补 `QA pass by <人>, <日期>` 一行
