...
- [ ] `DeathRegistry`：`{ char_id, death_count, last_death_tick, last_death_zone }`
- [ ] `RebirthChanceCalc`：纯函数，输入 DeathRegistry + 条件 → 概率 / 运数消耗
- [ ] `LifespanComponent`：`{ born_at_tick, years_lived, cap_by_realm, offline_pause_tick? }`
