...
- [ ] NPC 散修/弟子在 `CultivateAction` 期间消费区域灵气推进经脉（与玩家同规则）
- [ ] **不走突破事件交互**——NPC 满足突破条件自动过（凡是需要 inspect UI 的分支全部自动选默认）
- [ ] NPC 奇经八脉全通 → `TribulationReadyScorer` 触发 → `StartDuXuAction`
- [ ] NPC 渡虚劫走 plan-tribulation 同通道：占名额 / 被截胡（玩家可截胡）/ 失败退境
- [ ] **被截胡的 NPC 掉落规则**：NPC 通常无背包物品；改为按 archetype 的 `LootTable` 掉落（散修掉丹药/法器残片 / 弟子掉派系信物 / 僵尸掉素材），写在 `NpcLootTable` 配置
- [ ] NPC 化虚后进入"隐世"模式（`SeclusionAction`：少活动，agent narration 变稀）

### §3.3 代际更替

- [ ] **凡人**：老死后 spawn 邻居新生儿（同坐标 100 格内，年龄 0-5）
- [ ] **散修**：老死后 **不直接补**；由天道 agent 在"灵脉波动"事件触发时批量 spawn 新散修（避免满地 respawn 的观感）
- [ ] **妖兽**：在领地内繁衍，不需要 agent 介入
- [ ] **NPC 老死事件**：走 `plan-death §4b` 善终路径 + 写 `bong:npc/death` channel（含生平卷 snapshot）；亡者博物馆**不展示 NPC**（只展示玩家，避免被 NPC 刷屏）
- [ ] **NPC 总量上限**：全服 `max_npc_count`（首版 512，config 可调），达到上限后自然老死不再补，直到低于阈值 × 0.9 再恢复补员
...
- [ ] `Lineage { master: CharId, disciples: Vec<CharId> }` Component
- [ ] 师承影响：继承部分功法残篇（NPC 内部，不影响玩家 plan-death "不继承"原则）
- [ ] 师父老死 → 真传弟子继承派系领袖（若掌门位空）

### §4.3 声望

- [ ] NPC 对玩家 / 其他 NPC 的 `Reputation`（-100 ~ +100）
...
- [ ] 派系 × 派系 矩阵（友/敌/中立/血仇）
- [ ] 由 agent 长期推演（灾劫/变化/演绎三 agent 可修改）
- [ ] 关系变更触发事件：开战、结盟、瓦解
...
- [ ] 同派系 NPC 相遇 → `SocializeAction`（寒暄、交换情报）
- [ ] 敌对派系相遇 → Threat Scorer 起跳 → Fight or Flee
- [ ] 交易：散修带的物品 + 货币按简单市场估价
...
- [ ] Agent 可下发 `FactionEvent` 命令（战争宣告、联盟、瓦解）
...
- [ ] `FleeCultivatorAction` / `FarmAction` / `WanderAction`
- [ ] 凡人老死后邻居生子（同坐标 100 格内 spawn 新凡人，年龄 0-5）
- [ ] 凡人被夺舍路径（plan-death §4e）

**Phase 2 — 散修 archetype + 境界推进**
- [ ] `RogueBundle` + `CultivationDriveScorer` + `CuriosityScorer`
- [ ] `CultivateAction`（消费区域灵气推经脉）
- [ ] NPC 境界自动突破（无 UI 交互）
- [ ] 散修流浪 / 寻机缘 / 避世

**Phase 3 — NPC 渡虚劫 + 隐世**
- [ ] `TribulationReadyScorer`（三重门槛：条件满足 + 100 格无敌意 + `CultivationDrive > 0.6` 持续 30 min）+ `StartDuXuAction`
- [ ] 与 plan-tribulation §2 共用流程（全服广播 / 截胡窗口 / 心魔劫）
- [ ] NPC 心魔劫：agent 可生成 NPC 版心魔选项（简化版），或自动走"坚心"默认
- [ ] NPC 化虚名额：占用 `AscensionQuotaStore`
- [ ] NPC 被玩家截胡 → 按 `NpcLootTable` 掉落（非空背包规则）
- [ ] NPC 化虚后 `SeclusionAction`：低频移动 + 偶发 narration

**Phase 4 — 妖兽 archetype**
- [ ] `BeastBundle` + `TerritoryScorer` + `ProtectYoungScorer`
- [ ] 领地巡逻 / 捕食 / 护崽
- [ ] 妖兽繁衍（领地内自然 spawn 幼崽）

**Phase 5 — 派系 / 宗门弟子**
- [ ] `FactionState` / `FactionStore` / 关系矩阵
- [ ] `Lineage` 师承
- [ ] `Reputation` 组件
- [ ] `DiscipleBundle` + `LoyaltyScorer` + `MissionQueueScorer`
- [ ] `MissionExecuteAction`

**Phase 6 — 社交（NPC ↔ NPC）**
- [ ] `SocializeAction`（寒暄 / 情报）
- [ ] NPC 交易市场估价
- [ ] 敌对派系相遇自动交战

**Phase 7 — Agent producer 端**
- [ ] 天道 agent 发 `SpawnNpc` / `DespawnNpc` / `FactionEvent`
- [ ] 灾劫 agent 可触发"灵脉波动批量 spawn 散修"
- [ ] 演绎 agent 控制派系兴衰
- [ ] **NPC 主动截胡玩家渡虚劫**：三重门槛（敌对派系弟子 + 业力 > 玩家 + agent 推演确认）→ 派 NPC 团前往截胡坐标

**Phase 8 — 仙家遗种**
- [ ] `RelicGuardBundle` + `GuardianDuty` + `TrialEval`
- [ ] 绑定 worldgen 遗迹 POI
- [ ] 考验脚本（agent 驱动，Dynamic XML UI）

**Phase 9 — 性能 / LOD**
- [ ] 远离玩家的 NPC brain 降频（每 10 tick 思考一次）
- [ ] 超远 NPC 卸载到 "dormant" 列表（agent 代管）
- [ ] 大服压测：1000 NPC / 50 玩家 同时 tick 的开销
