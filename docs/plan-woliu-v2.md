...
- [ ] **搅拌器物理（区别于黑洞）**：worldview §二 守恒律 + plan-qi-physics-v1 §0 第一公理。涡流不是吸光 zone qi，而是**搅拌**——99% 甩回环境，1% 吸入：
  ```
...
- [ ] **紊流场（涡流流派专属边界）**：99% 甩出 ≠ 静态浓度溢出，是**动态高速漩涡**——其他玩家无法吸收（修炼靠静坐+稳定环境，紊流冲击经脉失败）。紊流场内效果：
...
- [ ] **化虚被动场可关**（worldview §四:506 末土后招原则物理化身）：化虚者可在 inspect 界面手动 toggle 被动场。关闭 = 不再搅拌 + 灵气分布回归正常 + 天道注视消退 / 代价 = 不再有持续 1% 吸入。**默认状态待 P0 决策**（§5 #1）
...
| 阶段 | 内容 | 验收 |
|---|---|---|
| **P0** ⬜ | 决策门：5 招数值表锁定 + 搅拌器 99/1 比例 + 紊流场半径与耗散公式 + §5 七个开放问题决策 + qi_physics 接入面定稿 + 与 plan-style-balance-v1 W/β 矩阵对齐 + ContainerKind/EnvField 紊流态扩展 design | 数值矩阵 + 物理公式落 plan §2 / qi_physics::env 紊流态 design 对齐 |
| **P1** ⬜ | server `combat::woliu_v2::*` 5 招 logic + 搅拌器物理 + 紊流场 EnvField 写入 + 反噬阶梯 + qi_physics 算子调用 + 化虚双场模型 + ≥100 单测（每招 ≥15 测覆盖 happy/边界/反噬/化虚特殊态/守恒断言） | `cargo test combat::woliu_v2` 全过 / 守恒断言每招 + 紊流场 decay 闭合 / `grep -rcE '#\[test\]' server/src/combat/woliu_v2/` ≥ 100 |
| **P2** ⬜ | client 2 动画（vortex_palm_open / vortex_spiral_stance）+ VORTEX_SPIRAL 粒子（继承 BongRibbonParticle + 向心加速度）+ 4 HUD 组件（VortexChargeProgressHud / VortexCooldownOverlay / BackfireWarningHud / TurbulenceFieldVisualizeHud） | render_animation.py headless 验证通过 / WSLg 实跑 5 招视觉确认 / HUD 4 组件渲染无闪烁 |
| **P3** ⬜ | 2 音效 recipe（vortex_low_hum / vortex_qi_siphon）+ agent 5 招 narration template + 反噬叙事（4 级）+ 紊流场叙事 + 化虚被动场天道注视 narration | narration-eval ✅ 5 招 + 反噬 + 紊流 全过古意检测 / 联调实测玩家施法时 narration 出现 |
| **P4** ⬜ | PVP telemetry 校准（5 招 vs 4 攻 3 防对位）+ 缜密色 hook（PracticeLog → QiColor）+ 坍缩渊负灵域内涡流自动反噬规则（worldview §十六.五）+ 化虚绝壁劫触发链（接 plan-tribulation-v1）| 7 流派 5×7 对位矩阵跑通 / 缜密色长期累积演化测试 / 坍缩渊内涡流强制 SEVERED 测试 |
...
**待 P0 决策** —— 需 plan-zhenfa-v1 配合定义阵法的 EnvField 写入边界。本 plan 只声明依赖，落点归 zhenfa-v1 vN+1。
...
- **2026-05-09**：升 active（`git mv docs/plans-skeleton/plan-woliu-v2.md → docs/plan-woliu-v2.md`）。触发条件：
  ...
  - 下一步：进 P0（持涡 v1 校准 + 99/1 搅拌器物理 + 紊流场 EnvField 边界），收口 §5 七决策门（化虚被动场默认开关 / 紊流场对 caster 自身影响 / 紊流 vs 阵法仲裁 / 99/1 比例 telemetry / 干尸涡引 / 被动场 qi 消耗 / 防御招精度衰减）
...
- **测试结果**：`cargo test combat::woliu_v2` + 测试数 / `narration-eval` 5 招通过率 / WSLg 联调实录
- **跨仓库核验**：server 5 招 SkillRegistry 注册 / agent 5 招 narration runtime / client 4 HUD + 1 粒子 + 2 动画 + 2 音效
- **遗留 / 后续**：缜密色加成（plan-color-v1）/ telemetry 校准（plan-style-balance-v1）/ 阵法冲突仲裁（plan-zhenfa-v1 vN+1）
