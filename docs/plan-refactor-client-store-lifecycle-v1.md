...
- ⬜ **P3 全量强制 + 删旧**：统一**生产生命周期入口**为 `clearOnDisconnect()`（不把清 listener / test seam 的 `resetForTests` 当生产清理）；删除手工 Store 清单；源码扫描强制“生产 `*Store.java` 必须恰好有一种分类，session 类必须以强类型 handle 登记”，新增漏分类或漏登记即红。
  ...
  - **门禁能力边界**：P3 使用保守的 source-level 词法 tripwire，而不引入 javac attribution / symbol-resolution 引擎。test-reset 门扫描全部 `src/main/java/com/bong/client/**/*.java` AST：方法声明可保留，但任何 invocation / member reference 的名字命中 `resetForTests`、`resetForTest`、`clearForTests` 即 fail-closed，不区分 receiver 或 overload，故同名但语义无关的调用也会产生刻意接受的假阳性。断线 helper 门以 registry-managed Store 的 FQCN / simple type name / 声明成员名及 import/static-import token 封锁直接或同名委托，只放行 `SessionScopedStoreRegistry.clearAllOnDisconnect()`；刻意改名后再绕行 Store 的 deliberate rename bypass 不在自动词法分析能力内，必须由 code review 捕获。
