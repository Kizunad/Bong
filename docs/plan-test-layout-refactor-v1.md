...
- **B 类与 `cfg(test)` wire 边界**：5 条依赖 `cfg(test)` 下 JSON server-data payload 分支的测试归入 B 类；外置 integration target 编译时会走 protobuf 路径，无法验证这些测试锁定的 JSON 契约，因此不能仅按公开函数调用机械外置。其余 B 类测试直接依赖生命周期私有 helper、ECS 装配或同 crate 私有状态观测；外置会迫使实现细节/测试专用 seam 进入生产 API，故未新增或扩大 `pub`、`pub(crate)`、`#[doc(hidden)]` seam。
