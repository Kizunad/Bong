...
### P4 combat/resolve.rs 测试分类与迁移（✅ 2026-09-05）

- **分类与处置**：按 `docs/inline-test-inventory.tsv` 对当前 HEAD 的 129 条 inline 测试逐条复审；128 条属于伤害结算、部位倍率、命中/权限/状态转换、目标解析、真元副作用与守恒等可观察契约，保留在同级 `server/src/combat/resolve_tests.rs`；`legacy_table_matches_humanoid_json_for_all_eight_parts` 仅复制内部硬编码表与 fixture 的实现镜像，已有行为测试覆盖，按清单删除。数量变化由清单逐条处置解释，不以机械守恒为验收。
- **生产边界与落点**：`server/src/combat/resolve.rs` 仅保留 `#[cfg(test)] #[path = "resolve_tests.rs"] mod tests;`，生产实现由 12,862 行收缩为 2,532 行；测试体落在同级私有测试模块，公开可观察 gate 另落在 `server/tests/unit/combat/resolve_test.rs`，并新增显式 `resolve_unit` Cargo target。未改伤害公式、部位倍率、命中判定、暴击、护甲减免、玩法、schema、wire 或其它迁移范围。
- **私有访问与守恒**：同级测试模块直接验证既有私有 resolver/ECS fixture，无新增 `pub`、`pub(crate)` 或 `#[doc(hidden)]` seam，也未复制生产实现；所有涉及真元的断言及 `qi_physics` 守恒/无副作用检查均保留。
- **迁移对拍**：迁移前 `cargo test combat::resolve::tests --lib` 为 `129 passed / 0 failed / 0 ignored`；迁移后同过滤器为 `128 passed / 0 failed / 0 ignored`，独立 `resolve_unit` 为 `1 passed / 0 failed / 0 ignored`。测试总契约仍为 129 条：128 条同级私有契约加 1 条公开 gate；减少的 1 条由 inventory 中的内部实现镜像处置记录解释。
- **提交前 locked gate**：提升权限串行执行 `flock /tmp/bong-cargo.lock -c 'cd server && ../scripts/build-token.sh cargo fmt --check && ../scripts/build-token.sh cargo clippy --all-targets -- -D warnings && ../scripts/build-token.sh cargo test'` exit `0`；library 为 `12246 passed / 0 failed / 1 ignored`，main binary 为 `18 passed / 0 failed / 0 ignored`，`proto_gen_unit` `55 passed`、`client_payload_unit` `9 passed`、`resolve_unit` `1 passed`、`shader_state_unit` `7 passed`、`skin_packet_unit` `2 passed`，其它 integration targets 与 doc-tests 无失败（doc-tests `3 passed / 0 failed / 5 ignored`）。
- **主线合并后最终 gate**：紧邻执行 `git fetch origin --prune && git merge origin/main`，基于 `origin/main=3ec765b4138f5d82479ef3eaf9c8b5009d2068d3` 无冲突生成 merge commit `a327bcb37`；其后定向 `combat::resolve::tests --lib` 为 `128 passed / 0 failed / 0 ignored`、`resolve_unit` 为 `1 passed / 0 failed / 0 ignored`。完整 locked server gate exit `0`：library `12246 passed / 0 failed / 1 ignored`，main binary `18 passed / 0 failed / 0 ignored`，`proto_gen_unit` `55 passed`、`client_payload_unit` `9 passed`、`resolve_unit` `1 passed`、`shader_state_unit` `7 passed`、`skin_packet_unit` `2 passed`，其它 integration targets 无失败，doc-tests `3 passed / 0 failed / 5 ignored`。
- **最终提交证据**：代码、外置测试与 `resolve_unit` target 对应 `5f2668627`，inventory/active plan evidence 对应 `5307b816e`，主线合并后最终 HEAD 为 `a327bcb37`（2026-09-05）；按当前 Kody-only 流程未启动独立 validator。
