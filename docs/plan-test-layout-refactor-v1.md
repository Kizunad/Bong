...
- **验证与门禁**：`npc_lod_unit` 为 `50 passed / 0 failed / 0 ignored`，同 crate `npc::lod::tests` 为 `10 passed / 0 failed / 0 ignored`。无上下文只读 validator 已绑定迁移代码 HEAD `39c56b634a66aff18bc8b7e51487f501a21495e3` 并 PASS，确认 `50+10=60`、target/挂载路径、无新增 seam 及离屏 qi 释放路径。`scripts/build-token.sh cargo fmt --check`、`scripts/build-token.sh cargo clippy --all-targets -- -D warnings`、`scripts/build-token.sh cargo test` 真实退出码均为 `0`；证据提交后的最终 HEAD 将再次绑定无上下文 validator 验证。
- **提交与状态**：代码迁移/Cargo target 提交为 `39c56b634a66aff18bc8b7e51487f501a21495e3`（2026-09-14，带 `Model: gpt-5-codex`）；本条为独立中文 evidence 提交。按任务卡“merge 不归你做”未执行主线合并，P2 总体、P3、P4 仍未完成，plan 保持 active、不归档。
