...
- **定向对拍与 validator**：迁移前 `flock /tmp/bong-cargo.lock -c 'cd server && ../scripts/build-token.sh cargo test network::client_request_handler::tests --lib'` 为 `72 passed / 0 failed / 0 ignored`；迁移后 `client_request_handler_unit` 为 `198 passed / 0 failed / 0 ignored`；私有 72 + 外置 198 = 270。无上下文只读 validator 已绑定 `93b2b3972567a5731f1803baabe801c6e49fff06` 并 PASS：270 个测试名集合完整、11 个契约断言恢复、无新增 seam、TSV/Cargo target 唯一性通过。
- **完整门禁**：在本 slot 基于当前 HEAD 执行任务卡指定的 `flock /tmp/bong-cargo.lock -c 'cd server && ../scripts/build-token.sh cargo fmt --check && ../scripts/build-token.sh cargo clippy --all-targets -- -D warnings && ../scripts/build-token.sh cargo test'`，exit `0`；library `12009 passed / 0 failed / 1 ignored`，main binary `18 passed / 0 failed / 0 ignored`，`client_request_handler_unit` `198 passed / 0 failed / 0 ignored`，全部既有 integration/独立 Cargo targets 无失败，doc-tests `3 passed / 0 failed / 5 ignored`。
- **提交与主线证据**：代码/target/测试收口对应 `93b2b3972`（2026-09-06），此前主线合并对应 `3ab21f940`；最终提交前仍须紧邻 `git fetch origin && git merge origin/main`，若主线触及 Cargo/plan 或其它受影响文件则保留双方条目并重跑 validator 与完整 server gate。本条 evidence 独立于并保留既有 P2/P3/P4 条目。
