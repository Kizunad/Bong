...
- review 修订 commit：`38daf9dbf`（2026-08-28，修复 preview authority handoff 失败清理）、`6c5bf9e25`/`f9b8cfd39`（2026-08-28，补齐真实 handoff 发布失败/rollback 未确认合同并隔离专用失败码）、`172a61141`/`98de9ed16`/`3f4fbb575`（2026-08-28，固定 identity handoff 对拍与 `renameat2(RENAME_NOREPLACE)` 独占发布）；最终 fresh read-only validator 已携 SHA `cbd945b5eb2449a5de80aede929c1e7c6ec27e10` 对拍 PASS，validator 模型为 `gpt-5.6-luna`。
