...
- **验证与门禁**：迁移后 `npc_trade_unit` 为 `53 passed / 0 failed / 0 ignored`，同 crate B 类定向为 `9 passed / 0 failed / 0 ignored`。无上下文只读 validator 绑定代码 HEAD `67720475a0963d8d69444a433d73c176c3c4b5d4` 并 PASS，确认 53/9 分类、生产边界及无新增 seam；紧邻执行 `git fetch origin && git merge origin/main`，结果 `Already up to date`。三条 server gate 均取 `PIPESTATUS[0]`：`scripts/build-token.sh cargo fmt --check` 为 `fmt_exit=0`，`scripts/build-token.sh cargo clippy --all-targets -- -D warnings` 为 `clippy_exit=0`，`scripts/build-token.sh cargo test` 为 `test_exit=0`。
