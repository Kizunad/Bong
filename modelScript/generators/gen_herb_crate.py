...
少磨损，filter=Herb）。本脚本对齐延寿棺管线（gen_mundane_coffin.py 同款）：
    ...
    modelScript/models/HerbCrate.bbmodel        ← 本脚本产物（Blockbench 源）
...
用法:
    python3 modelScript/generators/gen_herb_crate.py            # 仅刷预览（安全）
    python3 modelScript/generators/gen_herb_crate.py --write    # 覆盖手改 bbmodel（慎用）
"""
...
BBMODEL_OUT = Path(__file__).resolve().parents[1] / "models" / "HerbCrate.bbmodel"
PREVIEW_OUT = Path(__file__).resolve().parents[1] / "out" / "herb_crate_preview.png"
...
def main():
    ...
    ap.add_argument("--write", action="store_true",
                    help="覆盖 modelScript/models/HerbCrate.bbmodel（默认只刷预览，保护用户手改源）")
    args = ap.parse_args()
