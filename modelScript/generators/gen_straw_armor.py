...
#
# 纵向：参考图稻杆从 MC y1.35 到 y11.9。下缘上收到 2.70 —— 草鞋的踝箍在
# y2.18~2.84，照 1.35 会把踝箍整个埋掉。收到 2.70 后底穗正好搭在踝箍上沿，
# 读成「腿绑压着鞋绑」，是对的。
#
# 绳箍位置来自 ortho6 左腿柱的亮度剖面（绳比杆暗 14 以上）：
#     y 613 / 656 / 765 / 880 / 985 px → MC 10.85 / 9.91 / 7.54 / 5.04 / 2.76
# 顶上那两道挨得近（差 0.94），是「捆到髋部时多绕一圈」，保留成双箍。
# 最下一道随下缘一起上抬到 3.78。
#
# 箍厚：参考只有 0.28 单位。做成 0.28 在 MC 里基本看不见（一个 texel 都不到），
# 按兽皮甲绳件同族放到 0.50~0.58。

LEG_BODY_BOTTOM = 3.05
LEG_BODY_TOP = 11.35
LEG_BAND_Y = (10.63, 9.74, 7.20, 4.42)
...
# 底面一律压到 y<0：脚底本身就在 y=0，鞋底停在 0 等于和脚底面共面打架，而且
# "鞋底在脚底之下"本来就是鞋的定义（原版皮靴同理）。round 2 的底只有 0.78 厚，
# 穿上去几乎看不见，读成一圈铁丝；这轮加到 1.15~1.25，才是一叠编出来的草垫。
BOOT_SOLE_SLATS = (
    #  name        inner    w     z0     z1     y0     y1
    ("sole_toe", -1.68, 4.40, -4.62, -3.10, -0.30, 0.86),
    ("sole_fore", -1.86, 4.88, -3.10, -1.62, -0.38, 0.79),
    ("sole_mid", -1.90, 5.00, -1.62, -0.10, -0.29, 0.88),
    ("sole_arch", -1.88, 4.94, -0.10, 1.42, -0.37, 0.81),
    ("sole_heel", -1.74, 4.58, 1.42, 2.88, -0.28, 0.90),
)
...
def _boot_cubes(mount: str, sign: float) -> tuple[Cube, ...]:
    ...
    # ── 编织草底：五道横片，相邻片顶面差 0.07~0.09 ──
    # 交替的高低差在 MC 分轴着色下会把每片的顶面切成一条条 —— 那就是"编"的读法。
    for name, inner, w, z0, z1, y0, y1 in BOOT_SOLE_SLATS:
        out.append(c2(name, (x(inner, w), y0, z0), (w, y1 - y0, z1 - z0), UV_BRAID))

    # ── 底沿的散穗：参考图整圈都有草头炸出来 ──
    # 只做趾/外/跟三面。内侧那面挨着另一只鞋，做了也看不见，而且往里探就越过
    # 中线钻进对侧的鞋里了。
    out.append(c2("welt_toe", (x(-1.52, 4.06), 0.10, -5.02), (4.06, 0.44, 0.42), UV_WORN_A))
    out.append(c2("welt_out_f", (x(3.12, 0.36), 0.04, -3.34), (0.36, 0.44, 2.12), UV_WORN_B))
    out.append(c2("welt_out_b", (x(3.08, 0.32), 0.14, -0.62), (0.32, 0.44, 2.24), UV_WORN_A))
    out.append(c2("welt_heel", (x(-1.58, 4.24), 0.12, 2.90), (4.24, 0.44, 0.38), UV_WORN_B))

    # ── 沿口缠绳：把草底那一叠编片箍成一块 ──
    # round 1 没有这一圈，草底五片各露各的截面，侧视读成「一摞散木板」而不是
    # 一只编出来的鞋底。绳压在片的外缘、比散穗略高，两者不抢。
    out.append(c2("lash_out", (x(2.96, 0.30), 0.58, -3.92), (0.30, 0.36, 6.34), UV_CORD))
    out.append(c2("lash_toe", (x(-1.44, 3.90), 0.62, -4.80), (3.90, 0.34, 0.30), UV_CORD))
    out.append(c2("lash_heel", (x(-1.50, 3.98), 0.53, 2.64), (3.98, 0.34, 0.30), UV_CORD))

    # ── 趾间绳桩 + 前叉 ──
    # 参考图正视能看到一根绳从草底前端立起来穿进趾缝，再往后分成 V。
    out.append(c2("thong_post", (x(-0.26, 0.52), 0.84, -3.86), (0.52, 0.82, 0.48), UV_CORD))
    out.append(c2("thong_yoke", (x(-0.32, 0.64), 1.40, -3.40), (0.64, 0.36, 0.62), UV_CORD))

    # ── 脚背 V 叉：每边三级台阶，压低贴着脚背 ──
    # round 1 放在 y1.58~2.24，和踝箍（y2.2）之间没有空隙，两者糊成一坨；
    # 而且下面没有任何东西接到草底，侧视整条绳是**悬空**的。这轮压到 y1.18~1.82，
    # 并加下面那四根立绳把它锚回底上。
    out.append(c2("vee_out_a", (x(0.18, 0.72), 1.10, -3.02), (0.72, 0.40, 0.56), UV_CORD))
    out.append(c2("vee_out_b", (x(0.86, 0.76), 1.34, -2.72), (0.76, 0.40, 0.60), UV_CORD))
    out.append(c2("vee_out_c", (x(1.56, 0.44), 1.58, -2.34), (0.44, 0.40, 0.64), UV_CORD))
    out.append(c2("vee_in_a", (x(-1.02, 0.74), 1.06, -3.02), (0.74, 0.40, 0.56), UV_CORD))
    out.append(c2("vee_in_b", (x(-1.72, 0.76), 1.30, -2.72), (0.76, 0.40, 0.60), UV_CORD))
    out.append(c2("vee_in_c", (x(-1.88, 0.42), 1.54, -2.34), (0.42, 0.40, 0.64), UV_CORD))

    # ── 四根立绳：草底 → 踝箍 ──
    # 这是 round 1 最大的窟窿。参考图侧视里绳是从鞋底**穿上来**再绕踝的；没有
    # 立绳，踝箍就是一个凭空浮在小腿上的圈，整只鞋读成脚手架。
    #
    # 立绳贴着腿走（局部 x≈±1.95，腿的外表面在 ±1.9），不是从草垫的外沿
    # （3.10）起 —— round 2 摆在 2.42 时侧视是一根斜撑，读成支架而不是绑绳。
    # 真草鞋的绳本来就是从垫子**中间**穿孔上来的。
    for name, x0, w, z0, dz in (
        ("riser_out_f", 1.97, 0.34, -1.78, 0.34),
        ("riser_out_b", 1.96, 0.32, 1.12, 0.32),
        ("riser_in_f", -1.86, 0.32, -1.76, 0.32),
        ("riser_in_b", -1.84, 0.30, 1.14, 0.30),
    ):
        out.append(c2(name, (x(x0, w), 0.80, z0), (w, 1.32, dz), UV_CORD))

    # ── 踝箍：整圈，内缘落世界 0.0 ──
    # 探出量从 round 2 的 0.44 压到 0.30：0.44 在小腿上鼓得像个轴承座圈。
    out.append(c2("ankle_front", (x(-1.90, 4.02), 2.06, -2.32), (4.02, 0.34, 0.30), UV_CORD))
    out.append(c2("ankle_back", (x(-1.86, 3.96), 2.10, 2.04), (3.96, 0.34, 0.30), UV_CORD))
    out.append(c2("ankle_out", (x(1.94, 0.28), 2.04, -2.04), (0.28, 0.34, 4.10), UV_CORD))
    out.append(c2("ankle_in", (x(-1.90, 0.26), 2.12, -2.00), (0.26, 0.34, 4.02), UV_CORD))

    # ── 外侧那个结（参考图侧视最显眼的一处做工）──
    out.append(c2("knot_core", (x(2.18, 0.74), 1.88, -1.02), (0.74, 0.78, 0.74), UV_CORD))
    out.append(c2("knot_loop", (x(2.32, 0.56), 2.56, -1.28), (0.56, 0.30, 0.70), UV_CORD))
    out.append(c2("knot_tail_f", (x(2.34, 0.38), 1.10, -0.84), (0.38, 0.74, 0.34), UV_CORD))
    out.append(c2("knot_tail_b", (x(2.28, 0.34), 1.26, -0.16), (0.34, 0.60, 0.32), UV_CORD))

    # ── 跟后横绳：把草底后端箍在脚跟上 ──
    out.append(c2("heel_strap", (x(-1.88, 4.00), 1.44, 2.16), (4.00, 0.42, 0.30), UV_CORD))
