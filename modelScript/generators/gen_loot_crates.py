...
世界散布战利品容器（loot crate）家族，对齐延寿棺/货箱管线（gen_trade_crate.py 同款）：
    ...
    modelScript/models/LootCrate<Variant>.bbmodel   ← 本脚本产物（Blockbench 源）
...
用法:
    python3 modelScript/generators/gen_loot_crates.py                 # 生成全部 5 个
    python3 modelScript/generators/gen_loot_crates.py --variant ash_urn
    python3 modelScript/generators/gen_loot_crates.py --preview-only
"""
...
LOCAL_MODELS = Path(__file__).resolve().parents[1] / "models" / "lootcrate"
...
def main():
    ...
    if len(previews) > 1:
        ...
        # modelScript/out/render_loot_crates_all.png（plan/PR 引用的是后者）。
