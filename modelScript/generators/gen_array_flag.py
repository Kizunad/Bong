...
产物：
    modelScript/models/ArrayFlagBasic.bbmodel
    modelScript/out/array_flag_basic_preview.png

造型目标：凡阶组网阵的低矮阵旗。不是华丽法宝，而是旧木杆、骨钉压脚、褪色布幡、
青白弱光阵纹。旗面拆成多片薄 cuboid，避免“一块板贴图”偷懒。

用法：
    python3 modelScript/generators/gen_array_flag.py
    python3 modelScript/generators/gen_array_flag.py --preview-only
"""
...
BBMODEL_OUT = Path(__file__).resolve().parents[1] / "models" / "ArrayFlagBasic.bbmodel"
PREVIEW_OUT = Path(__file__).resolve().parents[1] / "out" / "array_flag_basic_preview.png"
