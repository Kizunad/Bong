...
通用仓储 accept=[] 全收）。本脚本对齐延寿棺管线（gen_mundane_coffin.py 同款）：
    ...
    modelScript/models/TradeCrate.bbmodel       ← 本脚本产物（Blockbench 源）
...
用法:
    python3 modelScript/generators/gen_trade_crate.py
    python3 modelScript/generators/gen_trade_crate.py --preview-only
"""
...
BBMODEL_OUT = Path(__file__).resolve().parents[1] / "models" / "TradeCrate.bbmodel"
PREVIEW_OUT = Path(__file__).resolve().parents[1] / "out" / "trade_crate_preview.png"
