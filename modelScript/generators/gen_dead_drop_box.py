...
worldview §九:850 阵法防砸，解锁需走私者师承）。对齐延寿棺管线：
    ...
    modelScript/models/DeadDropBox.bbmodel      ← 本脚本产物（Blockbench 源）
...
用法:
    python3 modelScript/generators/gen_dead_drop_box.py
    python3 modelScript/generators/gen_dead_drop_box.py --preview-only
"""
...
BBMODEL_OUT = Path(__file__).resolve().parents[1] / "models" / "DeadDropBox.bbmodel"
PREVIEW_OUT = Path(__file__).resolve().parents[1] / "out" / "dead_drop_box_preview.png"
