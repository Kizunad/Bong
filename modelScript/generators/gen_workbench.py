...
旧 modelScript/models/Workbench.bbmodel 是个 16³ 实心方块（贴图换皮），对「通往 104
配方手搓树的核心台」太敷衍。本脚本重做成真正的散修工作台，对齐延寿棺/容器管线：

    modelScript/models/Workbench.bbmodel        ← 本脚本产物（覆盖旧方块版）
...
用法:
    python3 modelScript/generators/gen_workbench.py
    python3 modelScript/generators/gen_workbench.py --preview-only
"""
...
BBMODEL_OUT = Path(__file__).resolve().parents[1] / "models" / "Workbench.bbmodel"
PREVIEW_OUT = Path(__file__).resolve().parents[1] / "out" / "workbench_preview.png"
