...
用法:
  python3 modelScript/creatures/horse/gen_skeleton.py                     # 三档全出
  python3 modelScript/creatures/horse/gen_skeleton.py --profile medium    # 只出常马
  python3 modelScript/creatures/horse/gen_skeleton.py --part skull        # 单件预览
  python3 modelScript/creatures/horse/gen_skeleton.py --check             # 只跑结构自检
  python3 modelScript/creatures/horse/gen_skeleton.py --list              # 列出部件 / 档位
"""
...
# 骨架/肌肉是**中间产物**，放 stages/ 单独存；modelScript/models/horse/ 只留最终 9 份皮层。
OUT_DIR = Path(__file__).resolve().parents[2] / "models" / "horse" / "stages"
...
class Rig:
    ...
    与 modelScript/creatures/dainu_lion/gen_skeleton.py 同构（同一套 bbmodel 装配约定），
    但材质表按马换过：无角，多蹄匣。
    """
