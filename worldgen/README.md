...
python3 -m venv .venv
source .venv/bin/activate
pip install mcworldlib nbtlib
```

> **注意**：不使用 amulet-core（其 amulet-rocksdb 依赖需要 CMake 4.1+，大多数系统不满足）。
> mcworldlib 支持 1.20.1 Anvil 格式的 region 读写，但其 block API 不兼容 1.18+ 新格式，
> 因此 `scripts/postprocess.py` 内含自研的 packed block state 编解码器。
