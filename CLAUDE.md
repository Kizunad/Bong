...
- **worldgen/** — Python 地形生成流水线（blueprint 驱动，terrain_gen 模块）
...
- **Worldgen 流水线**：blueprint 定义固定坐标大地图 → terrain_gen 生成区域 field → stitching 负责 zone→wilderness 过渡 → Python 导出 raster / preview → Rust server 运行时按需生成 chunk
...
| Schema | ✅ 双端对齐 |
| Worldgen | Phase A ✅（blueprint + terrain_gen），Phase B 后处理进行中 |
