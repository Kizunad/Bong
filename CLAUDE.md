...
- **默认自动 review**：PR 创建时 Kody 必跑（实测十几秒内出结论），push 新提交后通常也会重跑但**不保证**——别把"没看到新评论"直接当成"还在跑"。要针对最新 HEAD 复审就在 PR 根评论发 `@kody start-review`。
- **判审查归属必须核 SHA，不能只看时间戳**：旧 HEAD 的延迟评论会落在新 push 之后。认行内评论的 `original_commit_id`——**`commit_id` 会被 GitHub 改写成当前 HEAD，用它必得假阳性**（#2058 实测 6 条里 4 条被改写）。同时必须按 `<!-- kody-codereview` 标记认作者（**Kody 用仓库 owner 自己的账号发言，`user.login` 区分不出它和真人**）并带 `--paginate`。Kody 查无问题那一轮不产生 review 对象、**无 SHA 可核**——那条 clean 结论的归属只能靠启发式推定，别说成"已验证通过"；要确定性就推一个新 commit 让它重跑（发 `@kody start-review` 不保证重跑，它常在 push 后十几秒已经自动审完）。详见 `docs/CLAUDE.md §6.5`。
- **`/review-next` 与 CodeRabbit 都不要再等**：`/review-next` 已废弃，评论它不会触发任何 workflow（`review-next.yml` 调的中心引擎早已不出 success），**紧随其后出现的 Kody 结论是它自己按 push 跑的，不是这条评论的结果，别误判成因果**。CodeRabbit 则由 `.coderabbit.yaml` 的 `auto_review.enabled: false` 关掉了自动触发，PR 上没有它的评论是正常状态而不是"尚未跑完"；确实想要第二双眼睛时才评论 `@coderabbitai review` 按需拉起。也不要用 `@pi`/`@hive`/`@claude` mention 陌生账号。
