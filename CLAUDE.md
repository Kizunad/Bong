...
- **判审查归属必须核 SHA，不能只看时间戳**：旧 HEAD 的延迟评论会落在新 push 之后。认 `reviews[].commit_id` 与行内的 `original_commit_id`——**行内的 `commit_id` 会被 GitHub 改写成当前 HEAD，用它必得假阳性**（#2058 实测 6 条里 4 条被改写）。Kody 查无问题那一轮不产生 review 对象、无 SHA 可核，只能靠 push 后显式 `@kody start-review` 把结论绑到已知动作上。详见 `docs/CLAUDE.md §6.5`。
