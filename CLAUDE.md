...
- 编译型任务并发 **≤2**（3 个并行 cargo 编译历史上 OOM + 塞盘）；validator/验证类 agent 并发 **≤3**。实施工作区用**常驻 slot 池**（`.agent-worktrees/slot-<k>`，k=1..N，主干按需 `git worktree add --lock --detach` 创建后永久复用）：**detached HEAD 即空闲**（占用中的 slot 必然检出着任务分支）。进驻契约：① 双门核验 detached + 工作区干净；② 进驻前枚举 ignored，仅 `server/target`/`client/build`/`client/.gradle` 缓存白名单可接受，其它 ignored（`.env` 等）转人工；③ 本地分支不存在时才 `git checkout -B bugfix/plan-X origin/bugfix/plan-X`；本地分支已存在时**禁止 `checkout -B`**，改为直接 `git checkout bugfix/plan-X` 并核验本地 SHA == claim SHA，不一致转人工。释放 = detach 回 origin/main + 删本地分支。BLOCKED 任务现场以 commit 留在任务分支上后同样 detach 释放，脏现场才允许冻结交人工；BLOCKED 恢复进驻同样直接检出既有本地分支并 SHA 对拍。slot 跨任务保温缓存——磁盘上限固定、热编译，**严禁 remove slot 或删其构建缓存**。共享 `CARGO_TARGET_DIR` 时删过 worktree 后若报 `No such file` → `cargo clean -p valence_generated`（故障修复手段，须确认无并行编译在跑；slot 路径恒定后此坑不应再现）
...
1. **Claim + 进驻常驻 slot**：subagent 是 claim ref 的**唯一创建主体**。分支名固定 `bugfix/<plan-basename>`，认领 = create-ref API 原子创建远端分支：`gh api repos/{owner}/{repo}/git/refs -f ref="refs/heads/bugfix/plan-X" -f sha="$(git rev-parse origin/main)"`——**201 = 认领到手**；**422 先甄别再判占用**（查响应体 / `git ls-remote` 确认同名 ref 确实存在才算被占、回报主干换任务；其他原因的 422 = 流程错误，上报诊断而不是换任务）。认领成功后 `git fetch origin bugfix/plan-X` 同步远端引用，再进驻主干分派的常驻 slot：核验 detached + `git status --porcelain=v1 --untracked-files=all` 为空 + ignored 仅缓存白名单 → 本地分支不存在时 `git checkout -B bugfix/plan-X origin/bugfix/plan-X`，本地分支已存在则直接 `git checkout` 并核验 SHA==claim SHA（不一致转人工，**禁 `checkout -B` 覆盖残留提交**）+ 显式设 upstream，核验 slot HEAD = claim SHA 且 locked；slot 不存在时主干先 `git worktree add --lock --detach` 一次性创建。进驻失败 → slot 内 detach + 删本地分支 + 删刚创建的 claim ref 回滚，不留孤儿锁、不 remove slot
