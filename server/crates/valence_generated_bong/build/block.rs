...
pub(crate) fn build() -> anyhow::Result<TokenStream> {
    ...
    println!("cargo:rerun-if-env-changed=BONG_BLOCKS_JSON");
...
fn append_bong_blocks(blocks: &mut Vec<Block>) -> anyhow::Result<()> {
    ...
    let content = fs::read_to_string(&path)
        .with_context(|| format!("failed to read Bong block manifest {}", path.display()))?;
    let manifest: BongBlocksJson = serde_json::from_str(&content)
        .with_context(|| format!("failed to parse Bong block manifest {}", path.display()))?;

    let mut next_block_id = u16::try_from(blocks.len()).context("vanilla block count exceeds u16")?;
    let mut next_state_id = blocks
        .iter()
        .map(Block::max_state_id)
        .max()
        .context("vanilla blocks.json must contain at least one block")?
        .checked_add(1)
    ...
    for bong_block in manifest.blocks {
        ...
        let block = Block {
            id: next_block_id,
            item_id: bong_block.item_id.unwrap_or(0),
            wall_variant_id: None,
            translation_key: bong_block.translation_key,
            name: format!("{}_{}", bong_block.namespace, bong_block.name),
        ...
        next_block_id = next_block_id
            .checked_add(1)
            .context("Bong block id overflowed u16")?;
        next_state_id = next_state_id
            .checked_add(state_count)
            .context("Bong block state id overflowed u16")?;
        blocks.push(block);
    }
...
fn bong_blocks_path() -> PathBuf {
    if let Some(path) = std::env::var_os("BONG_BLOCKS_JSON") {
        return PathBuf::from(path);
    }

    let manifest_dir = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    for candidate in [
        manifest_dir.join("bong_blocks.json"),
        manifest_dir.join("../../../bong_blocks.json"),
    ] {
        if candidate.exists() {
            return candidate;
...
impl BongBlockDef {
    fn expand_states(&self, start_state_id: u16) -> anyhow::Result<Vec<State>> {
        self.validate()?;
        let count = self.state_count()?;
        let mut states = Vec::with_capacity(count);

        for offset in 0..count {
            let offset = u16::try_from(offset).context("Bong state offset exceeds u16")?;
            states.push(State {
                id: start_state_id
                    .checked_add(offset)
                    .context("Bong block state id overflowed u16")?,
                luminance: self.default_state.luminance,
                opaque: self.default_state.opaque,
                replaceable: self.default_state.replaceable,
                blocks_motion: self.default_state.blocks_motion,
                collision_shapes: self.default_state.collision_shapes.clone(),
    ...
    fn validate(&self) -> anyhow::Result<()> {
        if self.namespace != "bong" {
            bail!("Bong custom block `{}` must use namespace `bong`", self.name);
        }

        if self.name.trim().is_empty() {
            bail!("Bong custom block name must not be empty");
        }
        ...
        for property in &self.properties {
            ...
            if property.values.is_empty() {
                bail!(
                    "Bong custom block `{}` property `{}` must define at least one value",
                    self.name,
                    property.name
                );
            }
    ...
    fn state_count(&self) -> anyhow::Result<usize> {
        let mut count = 1usize;
        for property in &self.properties {
            count = count
                .checked_mul(property.values.len())
                .context("Bong block state count overflowed usize")?;
        }
        Ok(count)
    }
}
