...
use bong_server::network::agent_ui::*;
use bong_server::network::redis_bridge::RedisOutbound;
use bong_server::network::RedisBridgeResource;
use bong_server::schema::agent_ui::AgentUiActionType;
// ── Bevy App 级系统测试（驱动真 system，断言 Redis emit payload）───────────

use crossbeam_channel::unbounded;
use valence::prelude::{App, Entity, IntoSystemConfigs, Update};
use valence::testing::create_mock_client;

use bong_server::cultivation::components::{Cultivation, Realm};
use bong_server::schema::agent_ui::AgentUiRequestCommandV1;
...
fn build_agent_ui_app() -> (App, crossbeam_channel::Receiver<RedisOutbound>) {
    ...
    let (_tx_inbound, rx_inbound) = unbounded::<bong_server::network::redis_bridge::RedisInbound>();
