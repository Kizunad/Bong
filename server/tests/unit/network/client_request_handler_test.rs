// client_request_handler.rs 的公开 C2S 契约测试外置入口。
#![allow(clippy::too_many_arguments, clippy::type_complexity)]
#![allow(dead_code, unused_imports)]

// The shared test body keeps crate-relative paths so its fixtures can be
// reviewed alongside the handler's private test registry. This re-export gives
// those paths the same meaning in this integration target without exposing a
...
mod tests {
    ...
    fn decode_quickslot_config_payload(
        ...
        let crate::schema::proto_gen::bong::server_data_envelope::Payload::QuickSlotConfig(data) =
            decode_proto_server_data_payload(bytes)?
        else {
            return None;
        };
    ...
    fn decode_forge_blueprint_book_payload(
        ...
        let crate::schema::proto_gen::bong::server_data_envelope::Payload::ForgeBlueprintBook(data) =
            decode_proto_server_data_payload(bytes)?
        else {
            return None;
        };
    ...
    fn inventory_event_is_durability_changed(bytes: &[u8], instance_id: u64) -> bool {
        ...
        let Some(crate::schema::proto_gen::bong::server_data_envelope::Payload::InventoryEvent(
            data,
        )) = decode_proto_server_data_payload(bytes)
        else {
            return false;
    ...
    fn has_inventory_snapshot_payload(helper: &mut MockClientHelper) -> bool {
        for frame in helper.collect_received().0 {
            ...
            if server_data_payload_type(packet.data.0 .0) == Some("inventory_snapshot".to_owned()) {
                return true;
            }
        }
        false
