// client_request_handler.rs 的契约测试外置入口。
...
use bong_server::alchemy::residue::{residue_alchemy_data, residue_kind_for_recyclable_outcome};
use bong_server::alchemy::{
...
use bong_server::botany::components::HarvestSessionStore;
use bong_server::coffin::{CoffinEnterRequest, CoffinLeaveRequest, CoffinPlaceRequest};
use bong_server::combat::anqi_v2::{cycle_container_slot, switch_container_slot};
use bong_server::combat::carrier::{CarrierSlot, ChargeCarrierIntent, ThrowCarrierIntent};
...
#[cfg(test)]
use bong_server::combat::events::RevivalActionIntent;
...
use bong_server::cultivation::dugu::SelfAntidoteIntent;
use bong_server::cultivation::forging::ForgeRequest;
use bong_server::cultivation::insight::{InsightChosen, InsightRequest};
use bong_server::cultivation::known_techniques::{KnownTechniques, TechniqueRegistry};
use bong_server::cultivation::lifespan::LifespanExtensionIntent;
use bong_server::cultivation::meridian::severed::{
    MeridianSeveredPermanent, SkillMeridianDependencies,
};
use bong_server::cultivation::meridian_open::MeridianTarget;
use bong_server::cultivation::poison_trait::{ConsumePoisonPillIntent, PoisonPillKind};
use bong_server::cultivation::possession::{DuoSheRequestEvent, UseLifeCoreEvent};
use bong_server::cultivation::skill_registry::{CastRejectReason, CastResult, SkillRegistry};
use bong_server::cultivation::technique_scroll::{TechniqueLearnedEvent, TechniqueScrollReadEvent};
use bong_server::cultivation::tribulation::{HeartDemonChoiceSubmitted, StartDuXuRequest};
use bong_server::cultivation::void::actions::VoidActionIntent;
use bong_server::fauna::dying_elder::DyingElderState;
...
use bong_server::forge::session::{ForgeSessionId, ForgeSessions, ForgeStep};
use bong_server::forge::station::{PlaceForgeStationRequest, WeaponForgeStation};
#[cfg(test)]
use bong_server::inventory::add_item_to_player_inventory;
...
use bong_server::lingtian::session::{ReplenishSource, SessionMode};
use bong_server::lingtian::LingtianPlot;
use bong_server::mineral::probe::is_probe_target_in_range;
use bong_server::mineral::MineralProbeIntent;
use bong_server::movement::{MovementAction, MovementActionIntent};
use bong_server::network::agent_bridge::{
    payload_type_label, serialize_server_data_payload, SERVER_DATA_CHANNEL,
};
use bong_server::network::alchemy_snapshot_emit;
use bong_server::network::audio_event_emit::{AudioRecipient, PlaySoundRecipeRequest};
use bong_server::network::cast_emit::{
    current_unix_millis, push_cast_sync, CAST_INTERRUPT_COOLDOWN_TICKS,
};
use bong_server::network::gate::budget::BudgetStore;
use bong_server::network::gate::{GateContext, GateDenialReason};
use bong_server::shelflife::probe::FreshnessProbeIntent;
// dropped_loot_sync is emitted by dropped_loot_sync_emit.
#[cfg(test)]
use bong_server::identity::PlayerIdentities;
use bong_server::network::inventory_move_rejected_emit::emit_inventory_move_rejected;
use bong_server::network::qi_attrition_emit::{
    emit_attrition_applied_if_lost, item_abs_qi_for_attrition, AttritionAppliedEvent,
};
use bong_server::network::qi_color_observed_emit::QiColorInspectRequest;
use bong_server::network::{
    gameplay_vfx, redis_bridge::RedisOutbound, vfx_event_emit::VfxEventRequest, RedisBridgeResource,
};
#[cfg(test)]
use bong_server::npc::faction::FactionMembership;
...
use bong_server::persistence::ZoneRuntimeRecord;
use bong_server::player::gameplay::GameplayActionQueue;
...
use bong_server::qi_physics::attrition::{apply_attrition_checked, is_attrition_exempt};
use bong_server::qi_physics::constants::QI_TARGETED_ITEM_WEAR_WEIGHT_THRESHOLD;
use bong_server::qi_physics::ledger::AttritionOpKind;
use bong_server::qi_physics::qi_targeted_item_wear_fraction;
use bong_server::qi_physics::AnqiContainerKind;
use bong_server::schema::alchemy::{AlchemyInterventionResultV1, AlchemySessionStartV1};
use bong_server::schema::client_request::{ClientRequestV1, SkillBarBindingV1};
use bong_server::schema::combat_hud::{CastOutcomeV1, CastPhaseV1, CastSyncV1};
...
use bong_server::schema::server_data::{PillBuffStatusV1, ServerDataPayloadV1, ServerDataV1};
use bong_server::schema::social::GuardianKindV1;
use bong_server::shelflife::{
...
use bong_server::skill::events::{SkillScrollUsed, SkillXpGain};
#[cfg(test)]
use bong_server::social::components::{FactionReputation, FactionReputationTier};
use bong_server::social::events::{
...
use bong_server::world::block_place::BlockPlaceRequest;
...
use bong_server::world::extract_system::{
...
use bong_server::world::season::{query_season, WorldSeasonState};
use bong_server::world::spawn_tutorial::CoffinOpenRequest;
...
mod tests {
    ...
    use bong_server::botany::components::{
        BotanyHarvestMode, BotanyPhase, HarvestSession, HarvestSessionStore,
    };
    use bong_server::botany::registry::BotanyPlantId;
    use bong_server::combat::components::{Lifecycle, UnlockedStyles, WoundKind, Wounds};
    use bong_server::cultivation::components::{Cultivation, MeridianId, MeridianSystem, Realm};
    use bong_server::cultivation::known_techniques::KnownTechniques;
    use bong_server::cultivation::tribulation::TribulationState;
    use bong_server::forge::session::{ForgeSession, StepState};
    use bong_server::inventory::{
    ...
    use bong_server::lingtian::events::{
    ...
    #[derive(Default)]
    struct CapturedCoffinBreakRequests(Vec<bong_server::coffin::CoffinBreakRequest>);

    impl valence::prelude::Resource for CapturedCoffinBreakRequests {}

    #[derive(Default)]
    struct CapturedCoffinMenuReclaimRequests(Vec<bong_server::coffin::CoffinMenuReclaimRequest>);
    ...
    #[derive(Default)]
    struct CapturedRaiseShieldIntents(Vec<bong_server::combat::shield_block::RaiseShieldIntent>);

    impl valence::prelude::Resource for CapturedRaiseShieldIntents {}

    #[derive(Default)]
    struct CapturedLowerShieldIntents(Vec<bong_server::combat::shield_block::LowerShieldIntent>);
    ...
    fn capture_coffin_break_requests(
        mut events: EventReader<bong_server::coffin::CoffinBreakRequest>,
    ...
    fn capture_coffin_menu_reclaim_requests(
        mut events: EventReader<bong_server::coffin::CoffinMenuReclaimRequest>,
    ...
    fn run_supply_coffin_open_payload_case(player_pos: DVec3) -> (App, Entity, u64, Vec<String>) {
        ...
        app.insert_resource(bong_server::inventory::InventoryInstanceIdAllocator::default());
        app.insert_resource(bong_server::inventory::load_item_registry().expect("item registry"));
        app.add_systems(
            Update,
        ...
        let target = app
            .world_mut()
            .spawn((
                bong_server::world::entity_model::COFFIN_COMMON_ENTITY_KIND,
                EntityId::default(),
        ...
        registry.insert_active(
            ...
            bong_server::supply_coffin::current_wall_clock_secs(),
        );
    ...
    fn supply_coffin_open_payload_wiring_accepts_finite_and_rejects_non_finite_coordinates() {
        ...
        assert!(
            finite
                .world()
                .get::<bong_server::inventory::external_container::ExternalContainer>(target)
                .is_some(),
        ...
        for (label, x) in [
            ...
            assert!(
                app.world()
                    .resource::<bong_server::inventory::external_container::ExternalContainerRegistry>()
                    .sessions
                    .is_empty(),
            ...
            assert_eq!(
                app.world()
                    .resource::<bong_server::supply_coffin::SupplyCoffinRegistry>()
                    .rng_state,
                rng_before,
    ...
    fn run_external_container_move_case(
        ...
        player_pos: DVec3,
        source_kind: bong_server::inventory::external_container::ExternalContainerKind,
        source_active: bool,
    ...
    fn assert_external_move_rejected_without_mutation(app: &App, player: Entity, coffin: Entity) {
        let ext = app
            .world()
            .get::<bong_server::inventory::external_container::ExternalContainer>(coffin)
    ...
    fn supply_coffin_external_move_real_c2s_rejects_cross_dimension_same_xyz_while_session_is_valid_and_resyncs(
        ...
        let (app, player, coffin, payload_types) = run_external_container_move_case(
            ...
            DVec3::new(0.0, 64.0, 0.0),
            bong_server::inventory::external_container::ExternalContainerKind::SupplyCoffin {
                grade: bong_server::supply_coffin::SupplyCoffinGrade::Common,
            },
            true,
            true,
            true,
    ...
    fn external_move_rejects_forged_external_source_coordinates_without_mutation() {
        let (app, player, coffin, payload_types) = run_external_container_move_case_with_source(
            Some(DimensionKind::Overworld),
            DVec3::new(0.0, 64.0, 0.0),
            bong_server::inventory::external_container::ExternalContainerKind::StorageCrate {
                is_herb: false,
            },
            false,
            true,
    ...
    ) -> Vec<bong_server::schema::alchemy::AlchemySessionDataV1> {
        helper
            .collect_received()
            .0
            .into_iter()
    ...
    fn assert_authoritative_alchemy_guidance(
        snapshot: &bong_server::schema::alchemy::AlchemySessionDataV1,
        ...
        assert_eq!(
            snapshot.stages,
            vec![
                bong_server::schema::alchemy::AlchemyStageHintV1 {
                    at_tick: 0,
                    window: 0,
    ...
    fn alchemy_take_back_repushes_finished_guidance_after_furnace_session_is_removed() {
        ...
        app.insert_resource(ItemRegistry::from_map(HashMap::from([(
            bong_server::alchemy::residue::FAILED_PILL_RESIDUE_TEMPLATE_ID.into(),
            ItemTemplate::minimal_for_test(
                bong_server::alchemy::residue::FAILED_PILL_RESIDUE_TEMPLATE_ID,
            ),
        )])));
    ...
    fn alchemy_take_back_missing_allocator_still_pushes_finished_session() {
        ...
        let messages: Vec<String> = frames
            ...
            .collect();
        let snapshots: Vec<bong_server::schema::alchemy::AlchemySessionDataV1> = frames
            .iter()
            .filter_map(|frame| {
                let packet = frame.decode::<CustomPayloadS2c>().ok()?;
                if packet.channel.as_str() != SERVER_DATA_CHANNEL {
        ...
        let outcome_events = app
            .world()
            .resource::<valence::prelude::Events<bong_server::alchemy::AlchemyOutcomeEvent>>();
    ...
    fn collect_skill_config_snapshots(
        helper: &mut MockClientHelper,
    ) -> Vec<bong_server::skill::config::SkillConfigSnapshot> {
        helper
            .collect_received()
            .0
            .into_iter()
    ...
    fn collect_quickslot_configs(
        helper: &mut MockClientHelper,
    ) -> Vec<bong_server::schema::combat_hud::QuickSlotConfigV1> {
        helper
            .collect_received()
            .0
            .into_iter()
    ...
    fn craft_start_live_gate_rejects_missing_inventory_without_emitting_intent() {
        ...
        let accepted = app
            .world_mut()
            .resource_mut::<Events<bong_server::craft::CraftStartIntent>>()
            .drain()
            .collect::<Vec<_>>();
        assert_eq!(
            accepted.len(),
    ...
    fn workbench_open_live_gate_dispatches_only_a_resolved_nearby_workbench() {
        ...
        let workbench = app
            .world_mut()
            .spawn((
                bong_server::world::entity_model::WORKBENCH_ENTITY_KIND,
                EntityId::default(),
        ...
        let requests = app
            .world_mut()
            .resource_mut::<Events<bong_server::craft::WorkbenchOpenRequest>>()
            .drain()
            .collect::<Vec<_>>();
        assert_eq!(
            requests.len(),
    ...
    fn give_dan_live_gate_preserves_inventory_when_elder_state_is_invalid() {
        ...
        let elder = app
            ...
            .spawn((
                EntityKind::new(164),
                EntityId::default(),
                bong_server::npc::lifecycle::NpcArchetype::DyingElder,
        ...
        let accepted = app
            .world_mut()
            .resource_mut::<Events<bong_server::fauna::dying_elder::GiveDanToElderIntent>>()
            .drain()
            .collect::<Vec<_>>();
        assert_eq!(
            accepted.len(),
    ...
    fn register_request_resources(app: &mut App) {
        // The ingress system's queue is intentionally crate-private.  Assemble it through the
        // existing public lingtian registration instead of widening the queue's test surface.
        app.insert_resource(bong_server::botany::PlantKindRegistry::default());
        bong_server::lingtian::register(app);
        app.insert_resource(CombatClock::default());
        app.init_resource::<ClientRequestBudget>();
        app.insert_resource(bong_server::cultivation::skill_registry::init_registry());
        ...
        app.add_event::<bong_server::inventory::RemainsLootIntent>();
        app.insert_resource(ItemRegistry::default());
        ...
        app.add_event::<bong_server::combat::events::AttackIntent>();
        app.add_event::<bong_server::cultivation::burst_meridian::BurstMeridianEvent>();
        app.add_event::<bong_server::network::vfx_event_emit::VfxEventRequest>();
        app.add_event::<bong_server::network::audio_event_emit::PlaySoundRecipeRequest>();
        ...
        app.add_event::<bong_server::alchemy::LearnRecipeFragmentIntent>();
        ...
        app.add_event::<bong_server::coffin::CoffinBreakRequest>();
        app.add_event::<bong_server::coffin::CoffinMenuReclaimRequest>();
        app.add_event::<bong_server::craft::CraftStartIntent>();
        app.add_event::<bong_server::fauna::dying_elder::GiveDanToElderIntent>();
        app.add_event::<bong_server::craft::WorkbenchOpenRequest>();
        app.add_event::<bong_server::world::container_open::ContainerOpenRequest>();
        ...
        app.add_event::<bong_server::alchemy::AlchemyOutcomeEvent>();
        app.add_event::<bong_server::combat::events::CombatEvent>();
        app.add_event::<bong_server::combat::events::DeathEvent>();
        app.add_event::<bong_server::combat::zhenmai_v2::LocalNeutralizeEvent>();
        app.add_event::<bong_server::combat::zhenmai_v2::MultiPointBackfireEvent>();
        app.add_event::<bong_server::combat::zhenmai_v2::MeridianHardenEvent>();
        app.add_event::<bong_server::combat::zhenmai_v2::MeridianSeveredVoluntaryEvent>();
        app.add_event::<bong_server::combat::zhenmai_v2::BackfireAmplificationActiveEvent>();
        app.add_event::<bong_server::cultivation::meridian::severed::MeridianSeveredEvent>();
        app.add_event::<bong_server::cultivation::overload::MeridianOverloadEvent>();
        // plan-shield-block-v1 P1 — 举盾 intent events（ClientRequestDispatchParams 需要）。
        app.add_event::<bong_server::combat::shield_block::RaiseShieldIntent>();
        app.add_event::<bong_server::combat::shield_block::LowerShieldIntent>();
        // plan-agent-ui-data-v1 P0 — 天道 UI 响应 event（ClientRequestDispatchParams 需要）。
        app.add_event::<bong_server::network::agent_ui::AgentUiResponseEvent>();
        // plan-worldgen-v4 P5 §8.1#5 — dev give-block intent（ClientRequestDispatchParams 需要）。
        app.add_event::<bong_server::cmd::dev::block_picker::BlockPickerGiveIntent>();
    }
    ...
    fn register_request_systems(app: &mut App) {
        ...
        app.add_systems(
            Update,
            bong_server::network::inventory_event_emit::emit_durability_changed_inventory_events
                ...
                .after(bong_server::lingtian::LingtianRequestIngressSet),
        );
    }
    ...
    fn make_armor_straw_chestplate_registry(allowed_race: &str) -> ItemRegistry {
        ItemRegistry::from_map(HashMap::from([(
            ...
            ItemTemplate {
                ...
                shelflife_profile: None,
                shelflife_track: None,
                wearer_race: bong_server::body_plan::types::RaceGateOwned::Species {
                    species: vec![bong_server::body_plan::RaceId::new(allowed_race)],
                },
            },
        )]))
    }
    ...
    fn spawn_player_with_armor_straw_chestplate_in_pack(
        ...
        let inventory = PlayerInventory {
            ...
            containers: vec![ContainerState {
                quick_access: false,
                id: bong_server::inventory::MAIN_PACK_CONTAINER_ID.to_string(),
                name: "主背包".to_string(),
                rows: 4,
                cols: 4,
                items: vec![PlacedItemState {
        ...
        app.world_mut()
            .spawn((
                ...
                inventory,
                Cultivation {
                    race: bong_server::body_plan::RaceId::new(intrinsic_race),
                    ..Cultivation::default()
                },
                PlayerState {
                    karma: 0.0,
            ...
            .send(CustomPayloadEvent {
                ...
                data: serde_json::to_vec(&ClientRequestV1::InventoryMoveIntent {
                    ...
                    from: InventoryLocationV1::Container {
                        container_id: bong_server::inventory::MAIN_PACK_CONTAINER_ID.to_string(),
                        row: 0,
                        col: 0,
                    },
    ...
    fn morphed_player_equip_gate_uses_form_race_not_intrinsic_race() {
        ...
        app.insert_resource(make_armor_straw_chestplate_registry(
            bong_server::body_plan::HUMAN_RACE_ID,
        ));

        let client = spawn_player_with_armor_straw_chestplate_in_pack(&mut app, "Morpher", "whale");
        app.world_mut()
            .entity_mut(client)
            .insert(bong_server::body_plan::MorphState::new(
                bong_server::body_plan::RaceId::new(bong_server::body_plan::HUMAN_RACE_ID),
                0,
                0,
            ));
    ...
    fn run_npc_trade_request(
        ...
        trade_inventory: Option<bong_server::npc::trade::NpcTradeInventory>,
    ...
    fn run_npc_trade_request_with_context(
        ...
        app.add_plugins(EntityPlugin);
        register_request_app(&mut app);
        app.insert_resource(bong_server::inventory::load_item_registry().unwrap());
        app.insert_resource(bong_server::inventory::InventoryInstanceIdAllocator::default());
    ...
    ) -> bong_server::npc::trade::TradeOffer {
        bong_server::npc::trade::TradeOffer {
            template_id: template_id.to_string(),
    ...
    fn npc_trade_request_grants_full_bundle_count() {
        ...
        let (app, player, _helper) = run_npc_trade_request(
            inventory,
            Some(bong_server::npc::trade::NpcTradeInventory {
    ...
    fn npc_trade_request_rejects_empty_live_inventory_without_side_effects() {
        ...
        let (app, player, mut helper) = run_npc_trade_request(
            inventory,
            Some(bong_server::npc::trade::NpcTradeInventory { offers: vec![] }),
            "spirit_grass",
        );
        let inventory = app.world().get::<PlayerInventory>(player).unwrap();
        assert_eq!(inventory_item_count(inventory, "spirit_grass"), 0);
    ...
    fn npc_trade_request_partial_bundle_capacity_fails_atomically() {
        let registry = bong_server::inventory::load_item_registry().unwrap();
        let mut allocator = bong_server::inventory::InventoryInstanceIdAllocator::default();
        let mut inventory = empty_inventory();
    ...
    fn set_meridian_target_with_unknown_channel_id_is_handled_safely_not_panicking() {
        ...
        app.world_mut()
            ...
            .send(CustomPayloadEvent {
                ...
                data: serde_json::to_vec(&ClientRequestV1::SetMeridianTarget {
                    v: 1,
                    meridian: bong_server::cultivation::components::MeridianChannelId::new(
    ...
    fn set_meridian_target_with_legacy_pascal_case_lung_string_is_rejected_as_unknown() {
        ...
        app.world_mut()
            ...
            .send(CustomPayloadEvent {
                ...
                data: serde_json::to_vec(&ClientRequestV1::SetMeridianTarget {
                    v: 1,
                    meridian: bong_server::cultivation::components::MeridianChannelId::new("Lung"),
                })
                .expect("set meridian target request should serialize")
                .into_boxed_slice(),
            });
    ...
    fn dispatch_block_picker_give(
        ...
        Vec<bong_server::cmd::dev::block_picker::BlockPickerGiveIntent>,
    ) {
        let mut app = App::new();
        register_request_app(&mut app);
        let (client_bundle, _helper) = create_mock_client("Azure");
    ...
    fn dispatch_remains_loot(
        ...
        valence::prelude::Entity,
        Vec<bong_server::inventory::RemainsLootIntent>,
    ) {
        let mut app = App::new();
        register_request_app(&mut app);
        let (client_bundle, _helper) = create_mock_client("Azure");
        ...
        let events = app
            .world()
            .resource::<valence::prelude::Events<bong_server::inventory::RemainsLootIntent>>();
        let collected = events
            .iter_current_update_events()
            .cloned()
            .collect::<Vec<_>>();
    ...
    fn workbench_open_payload_requires_entity_manager_before_dispatch() {
        ...
        let events = app
            .world()
            .resource::<valence::prelude::Events<bong_server::craft::WorkbenchOpenRequest>>();
        assert_eq!(
    ...
    fn container_open_payload_requires_entity_manager_before_dispatch() {
        ...
        let events = app
            .world()
            .resource::<valence::prelude::Events<bong_server::world::container_open::ContainerOpenRequest>>();
        assert_eq!(
            events.iter_current_update_events().count(),
            0,
    ...
    fn alchemy_explode_take_back_applies_damage_and_meridian_crack() {
        let mut app = App::new();
        register_request_app(&mut app);
        app.insert_resource(bong_server::alchemy::recipe::load_recipe_registry().unwrap());
        app.insert_resource(bong_server::inventory::load_item_registry().unwrap());
        app.insert_resource(bong_server::inventory::InventoryInstanceIdAllocator::default());

        let (client_bundle, _helper) = create_mock_client("Azure");
        let entity = app.world_mut().spawn(client_bundle).id();
        let mut meridians = bong_server::cultivation::components::MeridianSystem::default();
        meridians
            .get_mut(bong_server::cultivation::components::MeridianId::Lung)
            .opened = true;
        app.world_mut().entity_mut(entity).insert((
            bong_server::combat::components::Wounds {
                health_current: 100.0,
                health_max: 100.0,
                entries: Vec::new(),
            },
            meridians,
            bong_server::cultivation::components::Cultivation::default(),
            PlayerState::default(),
        ...
        let overload_events = app
            .world()
            .resource::<valence::prelude::Events<bong_server::cultivation::overload::MeridianOverloadEvent>>();
            ...
            inventory.containers.iter().any(|container| {
                container.items.iter().any(|placed| {
                    placed.instance.template_id
                        == bong_server::alchemy::residue::FLAWED_PILL_RESIDUE_TEMPLATE_ID
                        && matches!(
    ...
    fn alchemy_ignite_rejects_low_zone_qi_on_live_request_path() {
        ...
        app.insert_resource(bong_server::alchemy::recipe::load_recipe_registry().unwrap());
        app.insert_resource(bong_server::inventory::load_item_registry().unwrap());
        app.insert_resource(bong_server::world::zone::ZoneRegistry {
            spatial_revision: 0,
            zones: vec![bong_server::world::zone::Zone {
    ...
    fn p3_take_back_high_tier_furnace_upgrades_bucket_vs_tier0_control() {
        ...
        use bong_server::alchemy::outcome::OutcomeBucket;
    ...
    fn botany_harvest_request_invalid_session_does_not_grant_gather_rewards() {
        ...
        register_request_app(&mut app);
        app.insert_resource(HarvestSessionStore::default());
        app.insert_resource(bong_server::player::gameplay::PendingGameplayNarrations::default());
        app.insert_resource(bong_server::qi_physics::WorldQiAccount::default());
        ...
        assert!(
            app.world()
                .resource::<bong_server::qi_physics::WorldQiAccount>()
                .transfers()
                .is_empty(),
        ...
        let narrations = app
            .world_mut()
            .resource_mut::<bong_server::player::gameplay::PendingGameplayNarrations>()
            .drain();
        assert!(
            narrations.is_empty(),
    ...
    fn quick_slot_bind_persists_atomic_block_mirror_for_reload() {
        ...
        bong_server::persistence::bootstrap_sqlite(&db_path, "quick-bind-test")
    ...
    fn production_scroll_request_app() -> App {
        let mut app = App::new();
        register_request_app(&mut app);
        let item_registry = bong_server::inventory::load_item_registry()
            .expect("production item registry must load for scroll routing tests");
        let mut craft_registry = bong_server::craft::CraftRegistry::new();
        bong_server::craft::load_default_craft_recipes(&mut craft_registry, &item_registry)
        ...
        app.insert_resource(bong_server::craft::RecipeUnlockState::new());
        app.add_event::<bong_server::craft::CraftUnlockIntent>();
    ...
    fn production_technique_scroll_falls_through_craft_routing() {
        ...
        assert!(
            app.world_mut()
                .resource_mut::<valence::prelude::Events<bong_server::craft::RecipeUnlockedEvent>>()
                .drain()
                .next()
                .is_none(),
    ...
    fn technique_scroll_realm_too_low_emits_structured_rejection() {
        ...
        assert_eq!(
            rejected,
            vec![bong_server::schema::server_data::InventoryMoveRejectedV1 {
        ...
        let entity = app
            ...
            .spawn((
                ...
                Cultivation {
                    ...
                    race: bong_server::body_plan::RaceId::new("whale"),
                    ..Default::default()
                },
                MeridianSystem::default(),
                PlayerState::default(),
    ...
    fn learn_skill_scroll_routes_positive_craft_recipe_unlock() {
        ...
        assert!(
            app.world()
                ...
                .is_unlocked(
                    &player_id,
                    &bong_server::craft::RecipeId::new("workbench.shelter.lantern")
                ),
    ...
    fn craft_scroll_unlock_uses_stable_player_id_when_caster_entity_is_gone() {
        ...
        let mut app = production_scroll_request_app();
        let player_id = canonical_player_id("Azure");
        let recipe_id = bong_server::craft::RecipeId::new("workbench.shelter.lantern");
        ...
        // 已解锁 → reserve 返回 false（防止再扣第二张卷）。该行为不变。
        {
            ...
            let re_reserved = app
                .world_mut()
                .resource_mut::<bong_server::craft::RecipeUnlockState>()
    ...
    fn craft_scroll_unlock_with_dead_caster_entity_still_commits_via_player_id() {
        ...
        let another_recipe = bong_server::craft::RecipeId::new("workbench.shelter.torch");
        assert!(
            app.world_mut()
                .resource_mut::<bong_server::craft::RecipeUnlockState>()
    ...
    fn collect_forge_blueprint_books(
        helper: &mut MockClientHelper,
    ) -> Vec<bong_server::schema::forge::ForgeBlueprintBookDataV1> {
        helper
            .collect_received()
            .0
            .into_iter()
    ...
    fn skill_bar_bind_skill_then_cast_starts_skillbar_cast() {
        ...
        let mut ms = bong_server::cultivation::components::MeridianSystem::default();
        for id in [
            bong_server::cultivation::components::MeridianId::LargeIntestine,
            bong_server::cultivation::components::MeridianId::SmallIntestine,
            bong_server::cultivation::components::MeridianId::TripleEnergizer,
        ] {
            let m = ms.get_mut(id);
            m.opened = true;
            m.integrity = 1.0;
        ...
        app.world_mut().entity_mut(entity).insert((
            ...
            bong_server::cultivation::components::Cultivation {
    ...
    fn runtime_only_direct_generic_can_be_learned_bound_and_cast() {
        ...
        scroll_template.technique_scroll_spec = Some(bong_server::inventory::TechniqueScrollSpec {
        ...
        let entity = app
            ...
            .spawn((
                client_bundle,
                bong_server::cultivation::components::Cultivation::default(),
                bong_server::cultivation::components::MeridianSystem::default(),
                SkillBarBindings::default(),
                QuickSlotBindings::default(),
                inventory_with_skill_scroll(skill_scroll_item(
                    SCROLL_INSTANCE_ID,
...
"#
        ...
        let registry = TechniqueRegistry::load_from_path(
            &path,
            &bong_server::body_plan::RaceRegistry::default(),
        )
    ...
    fn beng_quan_ready_meridians() -> bong_server::cultivation::components::MeridianSystem {
        let mut ms = bong_server::cultivation::components::MeridianSystem::default();
        for id in [
            bong_server::cultivation::components::MeridianId::LargeIntestine,
            bong_server::cultivation::components::MeridianId::SmallIntestine,
    ...
    fn user_cancel_by_slot_switch_stops_looping_charge_anim() {
        ...
        app.world_mut().entity_mut(entity).insert((
            ...
            yidao_charge_casting(bong_server::combat::yidao::MERIDIAN_REPAIR_SKILL_ID),
        ));
        ...
        assert_eq!(
            stop_anims,
            vec![(
                bong_server::combat::yidao::ANIM_YIDAO_MERIDIAN_REPAIR_LOOP.to_string(),
                Some(3),
            )],
    ...
    fn skill_bar_cast_defined_skill_without_resolver_uses_generic_cast_path() {
        ...
        app.world_mut()
            ...
            .set_config(
                "offline:Azure",
                "body.guangbo_ticao",
                bong_server::skill::config::SkillConfig::new(std::collections::BTreeMap::from([(
                    "stance".to_string(),
    ...
    fn known(ids: &[&str]) -> KnownTechniques {
        use bong_server::cultivation::known_techniques::KnownTechnique;
        KnownTechniques {
            entries: ids
                .iter()
    ...
    fn skill_bar_cast_meridian_gate_passes_when_all_deps_satisfied_resolver_path() {
        ...
        {
            let stomach = ms.get_mut(bong_server::cultivation::components::MeridianId::Stomach);
            stomach.opened = true;
        ...
        app.world_mut().entity_mut(entity).insert((
            ...
            empty_inventory(),
            ms,
            bong_server::cultivation::meridian::severed::MeridianSeveredPermanent::default(),
            // resolver 前置：realm Condense + qi ≥ 35。
            bong_server::cultivation::components::Cultivation {
                realm: bong_server::cultivation::components::Realm::Condense,
                qi_current: 100.0,
                qi_max: 100.0,
                ..Default::default()
            },
    ...
    fn skill_bar_cast_meridian_gate_rejects_when_required_meridian_integrity_too_low() {
        ...
        ms.get_mut(bong_server::cultivation::components::MeridianId::LargeIntestine)
            .integrity = 0.0;
        ms.get_mut(bong_server::cultivation::components::MeridianId::SmallIntestine)
            .integrity = 0.5;
        ms.get_mut(bong_server::cultivation::components::MeridianId::TripleEnergizer)
            .integrity = 0.5;
        app.world_mut().entity_mut(entity).insert((
            Position::new([0.0, 0.0, 0.0]),
            skill_bar,
    ...
    fn skill_bar_cast_meridian_gate_rejects_via_deps_table_when_severed() {
        ...
        app.world_mut()
            ...
            .declare(
                "sword.cleave",
                vec![bong_server::cultivation::components::MeridianId::LargeIntestine],
            );
        ...
        let entity = app.world_mut().spawn(client_bundle).id();
        let ms = bong_server::cultivation::components::MeridianSystem::default(); // LargeIntestine integrity 默认 1.0
        let mut severed =
            bong_server::cultivation::meridian::severed::MeridianSeveredPermanent::default();
        severed.insert(
            bong_server::cultivation::components::MeridianId::LargeIntestine,
            bong_server::cultivation::meridian::severed::SeveredSource::TribulationFail,
            100,
        );
        app.world_mut().entity_mut(entity).insert((
            Position::new([0.0, 0.0, 0.0]),
    ...
    fn skill_bar_cast_meridian_gate_passes_for_skill_with_no_deps() {
        ...
        let entity = app.world_mut().spawn(client_bundle).id();
        let ms = bong_server::cultivation::components::MeridianSystem::default();
        // 设置一条无关经脉 SEVERED，验证不会误伤
        let mut severed =
            bong_server::cultivation::meridian::severed::MeridianSeveredPermanent::default();
        severed.insert(
            bong_server::cultivation::components::MeridianId::Gallbladder, // 非依赖
            bong_server::cultivation::meridian::severed::SeveredSource::CombatWound,
            1,
        );
        app.world_mut().entity_mut(entity).insert((
    ...
    fn skill_bar_cast_meridian_gate_covers_resolver_path_via_deps_table() {
        ...
        severed.insert(
            bong_server::cultivation::components::MeridianId::LargeIntestine,
            bong_server::cultivation::meridian::severed::SeveredSource::BackfireOverload,
            200,
        );
        app.world_mut().entity_mut(entity).insert((
            Position::new([0.0, 0.0, 0.0]),
    ...
    fn skill_bar_cast_meridian_gate_rejects_when_integrity_just_below_min_health() {
        ...
        let entity = app.world_mut().spawn(client_bundle).id();
        let mut ms = bong_server::cultivation::components::MeridianSystem::default();
        ms.get_mut(bong_server::cultivation::components::MeridianId::Stomach)
    ...
    fn skill_bar_cast_resolver_reject_pushes_cast_sync_with_reason() {
        ...
        app.world_mut().entity_mut(entity).insert((
            ...
            bong_server::cultivation::components::Cultivation {
                realm: bong_server::cultivation::components::Realm::Awaken,
                qi_current: 100.0,
                qi_max: 100.0,
                ..Default::default()
            },
        ...
        assert!(
            syncs.iter().any(|s| s.outcome
                == bong_server::schema::combat_hud::CastOutcomeV1::RejectRealmTooLow
    ...
    fn non_humanoid_race_fixture() -> (
        bong_server::body_plan::BodyPlanRegistry,
        bong_server::body_plan::RaceRegistry,
    ) {
        let assets_root = bong_server::body_plan::resolve_assets_root();
        let body_plans = bong_server::body_plan::BodyPlanRegistry::load_dir(
            assets_root.join(bong_server::body_plan::registry::DEFAULT_BODY_PLANS_DIR),
        )
        .expect("checked-in body-plan assets must load");
        let races = bong_server::body_plan::RaceRegistry::load_file(
            assets_root.join(bong_server::body_plan::race_registry::DEFAULT_RACES_PATH),
            &body_plans,
        )
    ...
    fn setup_sword_cleave_caster(
        ...
        let cultivation = bong_server::cultivation::components::Cultivation {
            ...
            qi_max: 100.0,
            race: match race {
                Some(race_id) => bong_server::body_plan::RaceId::new(race_id),
                None => bong_server::body_plan::RaceId::new(bong_server::body_plan::HUMAN_RACE_ID),
            },
            ..Default::default()
        };
        app.world_mut().entity_mut(entity).insert((
            ...
            QuickSlotBindings::default(),
            empty_inventory(),
            bong_server::combat::weapon::Weapon {
                slot: bong_server::combat::weapon::EquipSlot::MainHand,
                instance_id: 1,
                template_id: "test_sword".to_string(),
                weapon_kind: bong_server::combat::weapon::WeaponKind::Sword,
                base_attack: 10.0,
                quality_tier: 0,
                durability: 100.0,
    ...
    fn skill_bar_cast_race_gate_rejects_non_humanoid_caster_before_resolver_qi_untouched() {
        ...
        let attack_intents = app
            .world()
            .resource::<valence::prelude::Events<bong_server::combat::events::AttackIntent>>();
        assert!(
        ...
        let qi_current = app
            .world()
            .get::<bong_server::cultivation::components::Cultivation>(entity)
    ...
    fn equip_false_skin_realm_too_low_emits_structured_rejection() {
        use bong_server::combat::tuike::FAKE_SPIRIT_HIDE_ITEM_ID;
        ...
        app.world_mut()
            ...
            .send(CustomPayloadEvent {
                ...
                data: serde_json::to_vec(&ClientRequestV1::EquipFalseSkin {
                    v: 1,
                    slot: bong_server::schema::inventory::EquipSlotV1::Chest,
    ...
    use bong_server::network::scroll_open_emit::ScrollReading;
    ...
    fn scroll_anim_drain_vfx(
        app: &mut App,
    ) -> Vec<bong_server::network::vfx_event_emit::VfxEventRequest> {
        app.world_mut()
            .resource_mut::<valence::prelude::Events<bong_server::network::vfx_event_emit::VfxEventRequest>>()
            .drain()
            .collect()
    }

    fn scroll_anim_find_play<'a>(
        reqs: &'a [bong_server::network::vfx_event_emit::VfxEventRequest],
        anim_id: &str,
    ) -> Option<&'a bong_server::network::vfx_event_emit::VfxEventRequest> {
        reqs.iter().find(|r| {
...
mod freshness_probe_handler_tests {
    ...
    fn freshness_probe_request_finds_item_in_equipped() {
        ...
        inv.equipped.insert(
            "chest".to_string(),
            bong_server::inventory::SlotContents::worn_single(item),
        );
        app.world_mut().entity_mut(entity).insert(inv);
    ...
    fn capture_raise_shield_intents(
        mut events: EventReader<bong_server::combat::shield_block::RaiseShieldIntent>,
    ...
    fn capture_lower_shield_intents(
        mut events: EventReader<bong_server::combat::shield_block::LowerShieldIntent>,
