...
fn scatter_bead_active_use_consumes_item_and_applies_ledger_transfer() {
    ...
    assert!(
        (zone.spirit_qi - (0.2 + QI_SCATTER_BEAD_CAPACITY / QI_ZONE_UNIT_CAPACITY)).abs() < 1e-9,
    ...
    assert!(narrations
        .iter()
        .all(|narration| matches!(narration.scope, crate::schema::common::NarrationScope::Zone)));
}
    ...
    let seal_transfer = transfers
        ...
        .find(|transfer| {
            transfer.reason == QiTransferReason::Channeling
                && transfer.to == zhenfa_sealed_qi_account(&instance.owner_player_id, instance.id)
        })
...
fn deceive_heaven_exposure_emits_dedicated_event() {
    ...
    let exposed_id = (1..=1_000)
        .find(|id| {
            deceive_heaven_reveal_tick(*id, 1, 1 + DECEIVE_HEAVEN_DURATION_TICKS, Realm::Solidify)
                .is_some()
        })
...
fn ward_expiry_qi_routes_to_overflow_when_zone_full() {
    ...
    let overflow_total: f64 = transfers
        .iter_current_update_events()
        .filter(|t| t.from == container && t.to.kind == crate::qi_physics::QiAccountKind::Overflow)
        .map(|t| t.amount)
        .sum();
    assert!(
