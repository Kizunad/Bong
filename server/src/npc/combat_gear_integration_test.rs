...
mod tests {
    ...
    fn integration_protocol_full_metadata_roundtrip() {
        let equipment = assign_npc_equipment(NpcArchetype::Rogue, Realm::Condense, None, 42);
        let meridian_sys = npc_meridian_system_for_realm(Realm::Condense, crate::body_plan::humanoid_plan_static());
        let deps = empty_deps();
    ...
    fn integration_meridian_system_realm_count_alignment() {
        for realm in [
            ...
            let sys = npc_meridian_system_for_realm(realm, crate::body_plan::humanoid_plan_static());
    ...
    fn integration_protocol_max_payload_not_exceeded() {
        ...
        let meridian_sys = npc_meridian_system_for_realm(Realm::Void, crate::body_plan::humanoid_plan_static());
        let deps = empty_deps();
        let techniques = assign_npc_techniques(
            NpcArchetype::GuardianRelic,
            Realm::Void,
