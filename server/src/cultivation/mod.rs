    ...
    for (entity, username, player_state, restored_lifespan) in &joined_clients {
        ...
        let persisted_bundle = persisted_bundle.and_then(|bundle| {
            ...
            if unknown_persisted_race == Some(true) {
                ...
                let Some(value) = bundle.get("life_record") else {
                    tracing::error!(
                        "[bong][cultivation] rejecting persisted cultivation bundle for `{}`: \
                         required life_record identity anchor is absent; falling back to the \
                ...
                match serde_json::from_value::<LifeRecord>(value.clone()) {
                    Ok(decoded) if decoded.character_id == canonical_id => Some(bundle),
                    Ok(decoded) => {
                        tracing::error!(
                            "[bong][cultivation] rejecting persisted cultivation bundle for `{}`: \
                    ...
                    Err(error) => {
                        tracing::error!(
                            "[bong][cultivation] rejecting persisted cultivation bundle for `{}`: \
                             required life_record identity anchor is invalid: {error}; \
                             falling back to canonical live record {}",
                            username.0,
                            canonical_id,
                        );
                        None
        ...
        if let Some(persisted_bundle) = persisted_bundle.as_ref() {
            ...
            if let Some(value) = persisted_bundle.get("cultivation") {
                ...
                match components::decode_persisted_cultivation(value.clone()) {
...
fn warn_cultivation_decode(username: &str, slice: &str, error: impl std::fmt::Display) {
    tracing::warn!(
...
mod tests {
    ...
    use crate::cultivation::components::{ActorQiIdentity, ActorQiKind, ColorKind, ContamSource};
