...
mod tests {
    ...
    fn home_return_narration_by_changes() {
        ...
        assert_eq!(3, narrations.len());
        assert!(narrations[0].text.contains("42 分钟"));
        assert!(narrations[1].text.contains("龛内镜面"));
        assert!(narrations[2].text.contains("下一趟"));
        assert!(narrations
            .iter()
