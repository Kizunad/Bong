...
use valence::prelude::{
    App, Client, EventReader, IntoSystemConfigs, Query, Res, Resource, Update, Username,
};
...
pub(crate) struct SceneDefinition {
    ...
    effects: &'static [(StatusEffectKind, f32, u64)],
}
...
fn handle_scene(
    ...
    for event in events.read() {
        let Ok((username, mut client, statuses, lifecycle)) = players.get_mut(event.executor)
        else {
            continue;
        };
        ...
        let scene = match &event.result {
            SceneCmd::List => {
                client.send_chat_message(
                    "[scene] 固定测试场景：切换会替换自身状态效果；/scene clear 清空效果。",
                );
                for scene in SCENES {
        ...
        statuses.active = scene.map_or_else(Vec::new, |scene| {
            scene
                .effects
                .iter()
        ...
        match scene {
            ...
            None => client.send_chat_message("[scene] 已清空当前角色的全部状态效果。"),
        }
    }
}
