...
fn assert_spec_case(index: usize) {
    ...
    if !skill.is_erosion_skill() {
        assert!(
            spec.cooldown_ticks > 0,
            "non-erosion skill {skill:?} at {realm:?} should have cooldown > 0, got {}",
            spec.cooldown_ticks
        );
        assert!(
            spec.cast_ticks > 0,
            "non-erosion skill {skill:?} at {realm:?} should have cast_ticks > 0, got {}",
            spec.cast_ticks
        );
        assert!(
            spec.duration_ticks > 0,
            "non-erosion skill {skill:?} at {realm:?} should have duration > 0, got {}",
            spec.duration_ticks
        );
    }
    assert!(
        spec.total_qi_cost().is_finite(),
        "skill {skill:?} at {realm:?} total_qi_cost must be finite, got {}",
        spec.total_qi_cost()
    );
    assert!(
        spec.total_drained().is_finite(),
        "skill {skill:?} at {realm:?} total_drained must be finite, got {}",
        spec.total_drained()
    );
...
fn erosion_skills_practice_xp_values() {
    assert_eq!(
        WoliuSkillId::AmbientVortex.practice_xp(),
        0,
        "AmbientVortex is a toggle, should grant 0 XP"
    );
    assert_eq!(
        WoliuSkillId::VortexEcho.practice_xp(),
        0,
        "VortexEcho is passive, should grant 0 XP"
    );
    for skill in [
        WoliuSkillId::VoidVortex,
        WoliuSkillId::SwallowingVortex,
